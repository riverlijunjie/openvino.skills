...
> **Converging faster / fixing macroscopic descriptors.** This baseline *illuminates* the space but
> is not designed to *converge* to a local optimum, and the static 3×4 descriptor can be too coarse
> (search concentrates in a few cells, drifts off the optimum). Opt-in remedies — measured/CVT
> descriptors, staged coarse→fine activation, surrogate pre-ranking, and trust-region local
> refinement (parameter ④a and edit/structure ④b) — are documented in
> [advanced_convergence.md](advanced_convergence.md) and implemented in the same script
> (`--demo-advanced` for ①②③④a, `--demo-edit-tr` for ④b).
