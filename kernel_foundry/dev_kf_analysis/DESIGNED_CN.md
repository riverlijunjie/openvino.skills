...
7. 在内层循环结束后，将最佳 kernel materialize 成外层 `Candidate`，
8. 仅在外层对 candidate 做 runtime integration 和 E2E benchmark，
...
| Candidate 生命周期不够面向 agent 决策 | 区分 KF 内层 `InnerTrial` 与外层 `Candidate`；只有完成一个 custom task 内层循环后才生成外层 candidate | 4.2、5 |
| 测试/harness 通常随 task 静态定义，难以随 kernel 变体安全演化 | 增加 `UPDATE_TASK_TESTS` 与 `update_task_tests`，将 `TEST_SPEC`、`PERF_SPEC`、`HARNESS` 作为可版本化 blocks | 5.6.1、6.1.1 |
...
| 容易误把一个 KF CustomTask 当成一次 candidate trial | 明确双层循环：KF 内层 evolution loop 负责多轮 kernel 生成/测试；Agent 外层 loop 负责 E2E、promotion 和下一轮 task | 1.2、4.2、5 |
...
1. **外层主循环**：`PARSE_REQUIREMENT -> PLAN_OUTER_ROUND -> RUN_KF_CUSTOM_TASK_EVOLUTION -> MATERIALIZE_CANDIDATE -> INTEGRATE -> BENCHMARK_E2E -> DECIDE_NEXT_ROUND`。
...
该状态机的核心变化是：`BUILD`、`VERIFY`、`BENCHMARK_MICRO` 和可选 inner profile 属于 KF custom task 内层循环；`INTEGRATE` 与 `BENCHMARK_E2E` 属于外层 agent loop。E2E 不应出现在内层 loop 中。
...
该 tool 是双层循环的核心边界：它把 KF 原生 `CustomTask` 内部 evolution loop 作为一个可调用的内层 kernel 生成循环暴露给外层 agent。它返回一个 inner best kernel，而不是直接返回外层 promoted candidate。
...
该 tool 将一个完成内层 evolution 的 KF task run 汇总为外层 `Candidate`。只有 materialized candidate 才进入 integration、E2E benchmark 和 promotion decision。
