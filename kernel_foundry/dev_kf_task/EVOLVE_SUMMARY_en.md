...
## 9. Experiment Reproduction Guide
...
# 3. Enter experiment directory
cd kernelfoundry.internal

# 4. Modify configuration
#    - runme_matmul.sh: job_name, task_name, max_iters, etc.
...
├── config.yaml          # Evolution config (model, temperature, iterations, etc.)
├── matmul_opt.cl        # Seed kernel (EVOLVE_START...EVOLVE_END)
├── matmul_reference.cl  # Reference baseline (for correctness verification)
└── v4_best_11.4ms.cl   # Saved v4 best kernel

runs/b580_matmul_dpas_v{N}/
├── controller.log       # Run log
├── results.json         # Summary results
├── eval_result_*.json   # Per-branch evaluation results
├── generated_kernel_*.py # LLM-generated kernels (with analysis)
└── stdout_*.txt         # Evaluation output
```

### 9.3 Recommended Experiment Sequence

1. **First**: Validate environment (run validate task)
2. **Then**: Establish baseline with v4-style (T=0.0, no seed)
3. **Next**: Use baseline as seed, T=0.25, 12 iterations for breakthrough
4. **Finally**: Use best result as seed, T=0.15–0.2, 16 iterations for fine-tuning

---

## 10. Conclusions

### 10.1 Three Core Conclusions
...
### 10.2 Final Performance Milestones
