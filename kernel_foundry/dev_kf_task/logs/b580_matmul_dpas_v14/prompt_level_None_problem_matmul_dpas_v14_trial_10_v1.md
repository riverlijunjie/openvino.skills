...
### Version 1 (Result: Correctness score: 5 / 5 (compiles and correct), runtime in ms: 1.010):
```OCL
...
### Version 2 (Result: Correctness score: 5 / 5 (compiles and correct), runtime in ms: 1.030):
```OCL
#pragma OPENCL EXTENSION cl_intel_subgroups : enable
#pragma OPENCL EXTENSION cl_intel_subgroup_matrix_multiply_accumulate : enable
#pragma OPENCL EXTENSION cl_khr_fp16 : enable
    ...
    const int N)
{
    ...
    int a_local_off[16];
    int a_global_row_off[16]; // row offset * K (precomputed)
    int a_c[16];
    int a_r[16];
    #pragma unroll
    for (int i = 0; i < 16; i++) {
        ...
        a_c[i] = elem_id & 31;
        a_local_off[i] = a_r[i] * SLM_STRIDE + a_c[i];
        a_global_row_off[i] = a_r[i] * K; // precompute row*K offset
    }
    ...
    // Load first A tile (k=0) into buffer 0
    {
        ...
        if (row_tile_valid) {
            ...
            #pragma unroll
            for (int i = 0; i < 16; i++)
                dst[a_local_off[i]] = A_base[a_global_row_off[i] + a_c[i]];
        } else {
            #pragma unroll
            for (int i = 0; i < 16; i++) {
                int gr = row_base + a_r[i];
    ...
    // Main K-loop: compute from cur_buf, load next into 1-cur_buf
    for (int kt = 0; kt < num_k_tiles; kt++) {
        ...
        if (kt + 1 < num_k_tiles) {
            ...
            if (row_tile_valid) {
                ...
                #pragma unroll
                for (int i = 0; i < 16; i++)
                    next_slm[a_local_off[i]] = A_base[a_global_row_off[i] + a_c[i]];
            } else {
                #pragma unroll
                for (int i = 0; i < 16; i++) {
                    int gr = row_base + a_r[i];
...
self = <task.TestMatmulOCL object at 0x7dfa3ef9aa20>
kernel = <function initialize_matmul_kernel.<locals>.run at 0x7dfa3efa7060>
ocl_queue = <pyopencl._cl.CommandQueue object at 0x7dfa3efadb20>, _run = 0
...
================== 4 failed, 1 deselected, 1 warning in 0.78s ==================
