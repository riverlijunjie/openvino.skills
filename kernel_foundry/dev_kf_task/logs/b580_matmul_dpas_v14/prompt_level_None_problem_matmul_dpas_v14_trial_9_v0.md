...
Here is the last OCL kernel we tested (Result: Correctness score: 5 / 5 (compiles and correct), runtime in ms: 2.990):
```OCL
#pragma OPENCL EXTENSION cl_intel_subgroups : enable
#pragma OPENCL EXTENSION cl_intel_subgroup_matrix_multiply_accumulate : enable
#pragma OPENCL EXTENSION cl_khr_fp16 : enable
...
// SLM stride = 34, two buffers for double-buffering
// K=2048 divides evenly by 32 (64 k-tiles), loop processes 1 tile at a time
// with double-buffered A: prefetch next A while computing current
// GWS = (ceil(N/64)*64, ceil(M/32))  LWS = (64, 1)
//
// Optimizations:
//   - Double-buffered SLM: load A[kt+1] while computing A[kt]
//   - Better interleaving of B loads with DPAS instructions
    ...
    const int N)
{
    ...
    int a_slm_off[16], a_glob_off[16];
    {
        ...
        for (int i = 0; i < 16; i++) {
            int eid = lid + i * 64;
            int ar = eid >> 5;   // row within tile (0..31)
            int ac = eid & 31;   // col within tile (0..31)
            a_slm_off[i] = ar * SLM_STRIDE + ac;
            // Global offset relative to row_base, k_off=0:
            // A_us[row_base * K + ar * K + k_off + ac]
            if (row_tile_valid) {
                a_glob_off[i] = a_row_base_K + ar * K + ac;
            } else {
                int gr = row_base + ar;
                a_glob_off[i] = (gr < M) ? (gr * K + ac) : -1;
            }
        }
    }
    ...
    {
        __local ushort* dst = slm_A;  // buffer 0
        if (row_tile_valid) {
            #pragma unroll
            for (int i = 0; i < 16; i++)
                dst[a_slm_off[i]] = A_us[a_glob_off[i]];
        } else {
            #pragma unroll
            for (int i = 0; i < 16; i++)
                dst[a_slm_off[i]] = (a_glob_off[i] >= 0) ? A_us[a_glob_off[i]] : (ushort)0;
        }
    }
    barrier(CLK_LOCAL_MEM_FENCE);
    ...
    for (int kt = 0; kt < num_k_tiles; kt++) {
        ...
        __local const ushort* src_slm = slm_A + cur_buf * SLM_BUF_SIZE;
        ...
        #pragma unroll
        for (int r = 0; r < 8; r++)
            ((ushort*)&a0)[r] = intel_sub_group_block_read_us(src_slm + r * SLM_STRIDE);

        // Start B load for k16 step 0 - interleave with A reads
        int8 bv0;
        {
            int boff = k_off * N + b_col;
            #pragma unroll
        ...
        #pragma unroll
        for (int r = 0; r < 8; r++)
            ((ushort*)&a1)[r] = intel_sub_group_block_read_us(src_slm + (8 + r) * SLM_STRIDE);

        // Continue B load
        {
            int boff = k_off * N + b_col + (N << 3);
            #pragma unroll
            for (int p = 4; p < 8; p++) {
                ushort s0 = B_us[boff];
                ushort s1 = B_us[boff + N];
        ...
        // DPAS 0,1 interleaved with A[16:32] load
        acc0 = intel_sub_group_f16_f16_matrix_mad_k16(a0, bv0, acc0);

        short8 a2;
        #pragma unroll
        for (int r = 0; r < 8; r++)
            ((ushort*)&a2)[r] = intel_sub_group_block_read_us(src_slm + (16 + r) * SLM_STRIDE);
        ...
        #pragma unroll
        for (int r = 0; r < 8; r++)
            ((ushort*)&a3)[r] = intel_sub_group_block_read_us(src_slm + (24 + r) * SLM_STRIDE);

        acc2 = intel_sub_group_f16_f16_matrix_mad_k16(a2, bv0, acc2);
        acc3 = intel_sub_group_f16_f16_matrix_mad_k16(a3, bv0, acc3);

        // ==== k16 step 1: A cols [16..31], B rows [k_off+16..k_off+31] ====
        __local const ushort* s16 = src_slm + 16;
        ...
        // Start loading next A tile into alternate SLM buffer (overlapped with DPAS)
        // Use SLM fence before writing to next buffer
        if (has_next) {
            int next_k_off = k_off + TILE_K;
            __local ushort* dst_nxt = slm_A + nxt_buf * SLM_BUF_SIZE;
            if (row_tile_valid) {
                #pragma unroll
                for (int i = 0; i < 16; i++)
                    dst_nxt[a_slm_off[i]] = A_us[a_glob_off[i] + next_k_off];
            } else {
                #pragma unroll
                for (int i = 0; i < 16; i++)
                    dst_nxt[a_slm_off[i]] = (a_glob_off[i] >= 0) ? A_us[a_glob_off[i] + next_k_off] : (ushort)0;
            }
        }

        // Remaining A reads for step 1
        short8 a2b;
        #pragma unroll
        for (int r = 0; r < 8; r++)
            ((ushort*)&a2b)[r] = intel_sub_group_block_read_us(s16 + (16 + r) * SLM_STRIDE);

        // Interleave DPAS with A reads
        acc0 = intel_sub_group_f16_f16_matrix_mad_k16(a0b, bv1, acc0);
        ...
        // Boustrophedon: reverse for step 1
        acc3 = intel_sub_group_f16_f16_matrix_mad_k16(a3b, bv1, acc3);
        acc2 = intel_sub_group_f16_f16_matrix_mad_k16(a2b, bv1, acc2);
        acc1 = intel_sub_group_f16_f16_matrix_mad_k16(a1b, bv1, acc1);
