...
The previous kernel (2.280ms) regressed significantly from the best (1.030ms). The key difference: the previous version manually unrolled B loads into individual scalar reads (16 separate loads for each k16 step) instead of using a compact loop. This likely caused instruction cache pressure and register spilling.

The best version (1.030ms) uses a tight `#pragma unroll` loop for B loads. Key optimizations to try now:

1. **K-loop 2x unroll**: Process two K-tiles (64 K elements) per iteration, reducing loop overhead and barrier count by half
2. **Better interleaving**: Start loading A for next iteration while computing current DPAS
3. **Double-buffered SLM**: Load next A tile while computing on current, with 2x K-unroll this means alternating buffers
4. **Tighter B load pattern**: Keep the proven compact loop from the best version

Since K=2048 divides by 64, we can safely unroll 2x. Double buffering with 2x unroll means we load A[kt+1] into buffer B while computing on buffer A, then swap. This halves the number of barriers.
...
// WG tile: 32 rows x 64 cols, K-step: 32, K-loop unrolled 2x
...
// K=2048 divides evenly by 64 (2x32), no remainder path needed.
// GWS = (ceil(N/64)*64, ceil(M/32))  LWS = (64, 1)
//
// Optimizations:
//   - Double-buffered SLM with 2x K-loop unroll: halves barrier count
//   - Load next A tile overlapped with current DPAS compute
//   - Boustrophedon DPAS ordering for register locality
//   - Compact B load loops with #pragma unroll
    ...
    const int N)
{
    ...
    // Double buffer: two SLM A buffers
    ...
    // Preload first A tile into buffer 0
    {
        ...
        if (row_tile_valid) {
            __global const ushort* src = A_us + row_base * K;
            #pragma unroll
            for (int i = 0; i < 16; i++)
            ...
            for (int i = 0; i < 16; i++) {
                ...
                dst[a_slm_off[i]] = (gr < M) ? A_us[gr * K + a_c[i]] : (ushort)0;
            }
        }
    }
    ...
    for (int kt = 0; kt < num_k_tiles; kt++) {
        ...
        __local const ushort* s = slm_A + cur_buf * SLM_BUF_SIZE;
        ...
        // Read A from SLM for rows 0-31, cols 0-15
        short8 a0, a1, a2, a3;
        #pragma unroll
        for (int r = 0; r < 8; r++)
        ...
        // Read A step 1 from SLM (offset +16 for cols 16..31)
        ...
        // Load next A tile into other buffer while DPAS executes
        if (has_next) {
            int next_buf = 1 - cur_buf;
            __local ushort* dst = slm_A + next_buf * SLM_BUF_SIZE;
            int next_k_off = (next_kt) * TILE_K;
            if (row_tile_valid) {
                __global const ushort* src = A_us + row_base * K + next_k_off;
                #pragma unroll
                for (int i = 0; i < 16; i++)
                    dst[a_slm_off[i]] = src[a_r[i] * K + a_c[i]];
            } else {
                ...
                for (int i = 0; i < 16; i++) {
                    int gr = row_base + a_r[i];
                    dst[a_slm_off[i]] = (gr < M) ? A_us[gr * K + next_k_off + a_c[i]] : (ushort)0;
                }
            }
        }

        // DPAS reverse (boustrophedon) for step 1
