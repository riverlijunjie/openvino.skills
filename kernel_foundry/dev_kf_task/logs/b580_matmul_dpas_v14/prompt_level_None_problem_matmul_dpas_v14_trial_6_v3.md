...
### Version 2 (Result: Correctness score: 5 / 5 (compiles and correct), runtime in ms: 1.140):
```OCL
#pragma OPENCL EXTENSION cl_intel_subgroups : enable
#pragma OPENCL EXTENSION cl_intel_subgroup_matrix_multiply_accumulate : enable
#pragma OPENCL EXTENSION cl_khr_fp16 : enable
...
Here is the last OCL kernel we tested (Result: Correctness score: 5 / 5 (compiles and correct), runtime in ms: 1.170):
```OCL
#pragma OPENCL EXTENSION cl_intel_subgroups : enable
#pragma OPENCL EXTENSION cl_intel_subgroup_matrix_multiply_accumulate : enable
#pragma OPENCL EXTENSION cl_khr_fp16 : enable
    ...
    const int N)
{
    ...
    for (int i = 0; i < 16; i++) {
        int eid = lid + i * 64;
        a_row[i] = eid >> 5;   // eid / 32
        a_col[i] = eid & 31;   // eid % 32
    ...
    // Load first A tile (k=0) into buffer 0
    {
        ...
        if (row_tile_valid) {
            ...
            #pragma unroll
            for (int i = 0; i < 16; i++)
                dst[a_slm_off[i]] = src[a_row[i] * K + a_col[i]];
        } else {
            #pragma unroll
            for (int i = 0; i < 16; i++) {
                int gr = row_base + a_row[i];
                dst[a_slm_off[i]] = (gr < M) ? A_us[gr * K + a_col[i]] : (ushort)0;
            }
        }
    }
    barrier(CLK_LOCAL_MEM_FENCE);
    ...
    for (int kp = 0; kp < num_k_pairs; kp++) {
        ...
        // ============ COMPUTE from buffer 0 (k0..k0+31) ============
        // k16 step 0: A cols [0..15], B rows [k0..k0+15]
        {
            __local const ushort* s = slm_A;

            // Interleave: load A rows 0-7 and start B load
            short8 a0, a1, a2, a3;
            #pragma unroll
            for (int r = 0; r < 8; r++)
                ((ushort*)&a0)[r] = intel_sub_group_block_read_us(s + r * SLM_STRIDE);
            ...
            int8 bv0;
            {
                ...
                for (int p = 0; p < 8; p++) {
                    ushort v0 = B_us[boff];
                    ushort v1 = B_us[boff + N];
                    ((int*)&bv0)[p] = as_int((ushort2)(v0, v1));
                    boff += N2;
                }
            }
            ...
            // k16 step 1: A cols [16..31], B rows [k0+16..k0+31]
            __local const ushort* s16 = s + 16;
            ...
            int8 bv1;
            {
                ...
                for (int p = 0; p < 8; p++) {
                    ushort v0 = B_us[boff];
                    ushort v1 = B_us[boff + N];
                    ((int*)&bv1)[p] = as_int((ushort2)(v0, v1));
                    boff += N2;
                }
            }
        ...
        barrier(CLK_LOCAL_MEM_FENCE);
        {
            ...
            if (row_tile_valid) {
                ...
                for (int i = 0; i < 16; i++) {
                    int gr = row_base + a_row[i];
                    dst[a_slm_off[i]] = (gr < M) ? A_us[gr * K + k1 + a_col[i]] : (ushort)0;
                }
            }
        }
        barrier(CLK_LOCAL_MEM_FENCE);

        // ============ COMPUTE from buffer 1 (k1..k1+31) ============
        {
            __local const ushort* s = slm_A + SLM_BUF_SIZE;
        ...
        if (kp + 1 < num_k_pairs) {
            ...
            barrier(CLK_LOCAL_MEM_FENCE);
            {
                ...
                if (row_tile_valid) {
                    ...
                    for (int i = 0; i < 16; i++) {
                        int gr = row_base + a_row[i];
                        dst[a_slm_off[i]] = (gr < M) ? A_us[gr * K + next_k0 + a_col[i]] : (ushort)0;
                    }
                }
            }
            barrier(CLK_LOCAL_MEM_FENCE);
