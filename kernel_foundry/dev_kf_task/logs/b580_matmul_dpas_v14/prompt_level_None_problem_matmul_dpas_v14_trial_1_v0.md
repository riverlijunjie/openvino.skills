...
// WG tile: 32 rows x 64 cols, K-step: 32, K-loop unrolled 2x (64 per iteration)
// 4 subgroups x 16 WIs = 64 WIs per WG
// Each subgroup: 4 vertical 8x16 DPAS tiles = 32 rows x 16 cols
// A cached in SLM (double-buffered with padding), B loaded directly from global/L2
// K=2048 divides evenly by 64 (2x unroll of 32), no remainder path needed.
...
#define SLM_TILE (TILE_M * SLM_STRIDE)
    ...
    const int N)
{
    ...
    // Double-buffered SLM for A with padding to avoid bank conflicts
    ...
    // Precompute per-WI A-load info (each WI loads 16 elements of the 32x32 tile = 1024 elements / 64 WIs)
    int a_local_off[16];
    int a_r[16];
    int a_c[16];
    #pragma unroll
    for (int i = 0; i < 16; i++) {
        ...
        a_local_off[i] = a_r[i] * SLM_STRIDE + a_c[i];  // padded stride
    }
    ...
    // 2x unrolled K-loop: process pairs of K-tiles
    ...
    for (int kp = 0; kp < num_k_pairs; kp++) {
        ...
        // ============ FIRST K-TILE (kt) ============
        {
            ...
            int8 b_val;
            {
                ...
                #pragma unroll
                for (int p = 0; p < 8; p++) {
                    ushort2 bv = vload2(0, &B_us[b_off]);
                    ((int*)&b_val)[p] = as_int(bv);
                    b_off += (N << 1);
                }
            }
        ...
        // ---- LOAD next A tile (kt+1) into other buffer ----
        {
            int next_buf = 1 - cur_buf;
            __local ushort* next_slm = slm_A + next_buf * SLM_TILE;
            barrier(CLK_LOCAL_MEM_FENCE);
        ...
        // ---- LOAD next A tile for next pair ----
        {
            const int next_kt2 = kt + 2;
            const bool has_next = (next_kt2 < num_k_tiles);
            if (has_next) {
                int next_buf = 1 - cur_buf;
                __local ushort* next_slm = slm_A + next_buf * SLM_TILE;
                barrier(CLK_LOCAL_MEM_FENCE);

                const int next_k = next_kt2 * TILE_K;
                if (row_tile_valid) {
    ...
    // Handle odd remaining K-tile if num_k_tiles is odd
    if (k_remainder) {
        const int kt = num_k_tiles - 1;
...
// K-loop unrolled 2x: process 2 K-tiles per iteration (64 K elements).
    ...
    const int N)
{
    ...
    // Each WI loads 16 elements of A tile (32x32 = 1024 elements / 64 WIs = 16 each)
    // Map lid to (row, col) in the 32x32 tile
    int a_local_off[16];
    int a_r[16];
    int a_c[16];
    #pragma unroll
    for (int i = 0; i < 16; i++) {
        ...
        a_r[i] = elem_id >> 5;   // elem_id / 32
        a_c[i] = elem_id & 31;   // elem_id % 32
        // Padded SLM layout: row * SLM_STRIDE + col
    ...
    // Main K-loop: unrolled 2x (process 2 K-tiles = 64 K elements per iteration)
    int kt = 0;
    for (; kt + 1 < num_k_tiles; kt += 2) {
        // ============ FIRST K-TILE (kt) ============
        {
            const int k = kt * TILE_K;
        ...
        // ============ SECOND K-TILE (kt+1) ============
        {
            ...
            // ---- LOAD next A tile (kt+2) if exists ----
            if (kt + 2 < num_k_tiles) {
                int next_buf = 1 - cur_buf;
                __local ushort* next_slm = slm_A + next_buf * SLM_TILE_PADDED;
                barrier(CLK_LOCAL_MEM_FENCE);

                const int next_k = (kt + 2) * TILE_K;
                if (row_tile_valid) {
                    __global const ushort* A_tile = A_us + a_row_base_K + next_k;
                    #pragma unroll
                    for (int i = 0; i < 16; i++)
    ...
    // Handle remaining single K-tile if num_k_tiles is odd
    if (kt < num_k_tiles) {
        const int k = kt * TILE_K;
        __local const ushort* cur_slm = slm_A + cur_buf * SLM_TILE_PADDED;
...
Here is the last OCL kernel we tested (Result: Correctness score: 5 / 5 (compiles and correct), runtime in ms: 1.210):
```OCL
#pragma OPENCL EXTENSION cl_intel_subgroups : enable
#pragma OPENCL EXTENSION cl_intel_subgroup_matrix_multiply_accumulate : enable
#pragma OPENCL EXTENSION cl_khr_fp16 : enable
