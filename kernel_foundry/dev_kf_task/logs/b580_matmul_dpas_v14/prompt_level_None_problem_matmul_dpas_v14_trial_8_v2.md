...
// GWS = (ceil(N/64)*64, ceil(M/32))  LWS = (64, 1)
// Optimizations:
//   - Prefetch next B tile during current DPAS
//   - Better interleaving of loads and compute
//   - Boustrophedon DPAS ordering
    ...
    const int N)
{
    ...
    for (int kt = 0; kt < num_k_tiles; kt++) {
        ...
        // ==== k16 step 0: A cols [0..15], B rows [k_off..k_off+15] ====
        {
            ...
            // Load B for k16 step 0 - use int pointer for paired reads
            int8 bv0;
            {
                __global const int* B_int = (__global const int*)(B_us + k_off * N + b_col);
                #pragma unroll
                for (int p = 0; p < 8; p++) {
                    ushort s0 = B_us[k_off * N + b_col + p * 2 * N];
                    ushort s1 = B_us[k_off * N + b_col + (p * 2 + 1) * N];
                    ((int*)&bv0)[p] = as_int((ushort2)(s0, s1));
                }
            }

            // Load A[8:16] overlapped with B load completion
            short8 a1;
            #pragma unroll
            for (int r = 0; r < 8; r++)
                ((ushort*)&a1)[r] = intel_sub_group_block_read_us(s + (8 + r) * SLM_STRIDE);

            // Start DPAS for rows 0-7 while loading A[16:24]
            acc0 = intel_sub_group_f16_f16_matrix_mad_k16(a0, bv0, acc0);
            ...
            int8 bv1;
            {
                int bbase = (k_off + 16) * N + b_col;
                #pragma unroll
                for (int p = 0; p < 8; p++) {
                    ushort s0 = B_us[bbase + p * 2 * N];
                    ushort s1 = B_us[bbase + (p * 2 + 1) * N];
                    ((int*)&bv1)[p] = as_int((ushort2)(s0, s1));
                }
            }
            ...
            // Boustrophedon: reverse order for step 1 tail
            acc3 = intel_sub_group_f16_f16_matrix_mad_k16(a3b, bv1, acc3);
            acc2 = intel_sub_group_f16_f16_matrix_mad_k16(a2b, bv1, acc2);
        }
...
Here is the last OCL kernel we tested (Result: Correctness score: 5 / 5 (compiles and correct), runtime in ms: 1.030):
```OCL
#pragma OPENCL EXTENSION cl_intel_subgroups : enable
#pragma OPENCL EXTENSION cl_intel_subgroup_matrix_multiply_accumulate : enable
#pragma OPENCL EXTENSION cl_khr_fp16 : enable
