...
### Version 1 (Result: Correctness score: 5 / 5 (compiles and correct), runtime in ms: 1.420):
```OCL
    ...
    const int N)
{
    ...
    // Main loop: iterate k_iters-1 times with double-buffering, then last iteration without load
    for (int ki = 0; ki < k_iters - 1; ki++) {
        ...
        // ===== Start loading next A tile (overlaps with remaining DPAS) =====
        {
    ...
    // ===== Last K iteration (no next A load needed) =====
    {
        int k = (k_iters - 1) * 32;
        __local const half* cur_slm = &slm_a[cur_buf * SLM_BUF_SIZE];
    ...
    // Store results using intel_sub_group_block_write_us for vectorized writes
    __global half* c_ptr = C + tile_row * N + tile_col;

    // Convert and store using subgroup block writes (16 halfs per write = one row of 16 cols)
    #pragma unroll
    for (int i = 0; i < 8; i++) {
        intel_sub_group_block_write_us((__global ushort*)(c_ptr + (0 + i) * N),
            as_ushort(convert_half(((float*)&acc0)[i])));
    }
    #pragma unroll
    for (int i = 0; i < 8; i++) {
        intel_sub_group_block_write_us((__global ushort*)(c_ptr + (8 + i) * N),
            as_ushort(convert_half(((float*)&acc1)[i])));
    }
    #pragma unroll
    for (int i = 0; i < 8; i++) {
        intel_sub_group_block_write_us((__global ushort*)(c_ptr + (16 + i) * N),
            as_ushort(convert_half(((float*)&acc2)[i])));
    }
    #pragma unroll
    for (int i = 0; i < 8; i++) {
        intel_sub_group_block_write_us((__global ushort*)(c_ptr + (24 + i) * N),
...
This is the code of the best kernel we have tested so far (Result: Correctness score: 5 / 5 (compiles and correct), runtime in ms: 1.400):
```OCL
// GEMM: C[M,N] = A[M,K] x B[K,N], all half, f32 accumulation
// Launch: GWS = ((N/64)*16, (M/32)*4, 1), LWS = (16, 4, 1)
// 4 subgroups of 16 WIs per WG. Each WG computes 32x64 tile.
