...
### Version 1 (Result: Correctness score: 5 / 5 (compiles and correct), runtime in ms: 0.985):
```OCL
    ...
    const int N)
{
    ...
    // Double-buffered SLM for A tile: 2 x 32 rows x 32 cols = 4KB
    __local ushort slm_A[2 * TILE_M * SLM_A_STRIDE];
    ...
    // Clamp col for B reads to avoid OOB; invalid cols produce garbage but are not stored
    const int b_col = col_valid ? col_idx : (N - 1);

    // Preload first A tile into buffer 0
    {
        __local ushort* slm_dst = slm_A;
        if (row_tile_valid) {
            __global const ushort* A_tile = A_us + row_base * K;
            #pragma unroll
            for (int i = 0; i < 16; i++) {
                int elem_id = lid + i * 64;
                int r = elem_id >> 5;
                int c = elem_id & 31;
                slm_dst[r * SLM_A_STRIDE + c] = A_tile[r * K + c];
            }
        } else {
            #pragma unroll
            for (int i = 0; i < 16; i++) {
                ...
                int c = elem_id & 31;
                int gr = row_base + r;
                slm_dst[r * SLM_A_STRIDE + c] = (gr < M) ? A_us[gr * K + c] : (ushort)0;
            }
        }
    }
    ...
    for (int kt = 0; kt < num_k_tiles; kt++) {
        ...
        // Prefetch next A tile into alternate buffer (if not last tile)
        if (kt + 1 < num_k_tiles) {
            const int k_next = (kt + 1) * TILE_K;
            __local ushort* slm_dst = slm_A + nxt_buf * TILE_M * SLM_A_STRIDE;
            if (row_tile_valid) {
                __global const ushort* A_tile = A_us + row_base * K + k_next;
                #pragma unroll
                for (int i = 0; i < 16; i++) {
                    int elem_id = lid + i * 64;
                    int r = elem_id >> 5;
                ...
                for (int i = 0; i < 16; i++) {
                    ...
                    int c = elem_id & 31;
                    int gr = row_base + r;
                    slm_dst[r * SLM_A_STRIDE + c] = (gr < M) ? A_us[gr * K + k_next + c] : (ushort)0;
                }
            }
        }

        // Compute on current buffer
        __local const ushort* slm_cur = slm_A + cur_buf * TILE_M * SLM_A_STRIDE;

        #pragma unroll
        for (int kk = 0; kk < TILE_K; kk += 16) {
            short8 a0, a1, a2, a3;
            __local const ushort* slm_base = slm_cur + kk;
            ...
            // Load B from global — use clamped b_col to avoid branch divergence
            int gk_base = k + kk;
            int8 b_val;
            #pragma unroll
            for (int p = 0; p < 8; p++) {
                int k_row0 = gk_base + 2 * p;
                ushort s0 = B_us[k_row0 * N + b_col];
                ushort s1 = B_us[(k_row0 + 1) * N + b_col];
...
### Version 2 (Result: Correctness score: 5 / 5 (compiles and correct), runtime in ms: 1.000):
```OCL
#pragma OPENCL EXTENSION cl_intel_subgroups : enable
#pragma OPENCL EXTENSION cl_intel_subgroup_matrix_multiply_accumulate : enable
#pragma OPENCL EXTENSION cl_khr_fp16 : enable
    ...
    const int N)
{
    ...
    // Precompute B column offset - clamp for OOB columns to avoid segfault
    const int b_col = col_valid ? col_idx : 0;
    ...
    for (int kt = 0; kt < num_k_tiles; kt++) {
        ...
        for (int kk = 0; kk < TILE_K; kk += 16) {
            ...
            // Load B from global - precompute base offset to reduce arithmetic
            int gk_base = k + kk;
            int b_base_off = gk_base * N + b_col;
            int8 b_val;

            // Merge paired B reads into packed int for DPAS
            #pragma unroll
            for (int p = 0; p < 8; p++) {
                int off0 = b_base_off + (2 * p) * N;
                ushort s0 = B_us[off0];
                ushort s1 = B_us[off0 + N];
                ((int*)&b_val)[p] = as_int((ushort2)(s0, s1));
            }

            // Zero out B for invalid columns
            if (!col_valid)
                b_val = (int8)(0);
...
Here is the last OCL kernel we tested (Result: Correctness score: 5 / 5 (compiles and correct), runtime in ms: 0.970):
```OCL
#pragma OPENCL EXTENSION cl_intel_subgroups : enable
#pragma OPENCL EXTENSION cl_intel_subgroup_matrix_multiply_accumulate : enable
#pragma OPENCL EXTENSION cl_khr_fp16 : enable
