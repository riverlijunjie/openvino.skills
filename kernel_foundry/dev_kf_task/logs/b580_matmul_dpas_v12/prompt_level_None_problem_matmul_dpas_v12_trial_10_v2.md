...
Here is the last OCL kernel we tested (Result: Correctness score: 5 / 5 (compiles and correct), runtime in ms: 2.110):
```OCL
#pragma OPENCL EXTENSION cl_intel_subgroups : enable
#pragma OPENCL EXTENSION cl_intel_subgroup_matrix_multiply_accumulate : enable
#pragma OPENCL EXTENSION cl_khr_fp16 : enable
    ...
    const int N)
{
    ...
    const int col_idx = col_base + sg_lid;
    const bool col_valid = col_idx < N;
    const int num_k_tiles = K >> 5;  // K / 32, guaranteed integer
    ...
    // Precompute per-WI A-load decomposition (constant across K-tiles)
    int a_local_off[16];
    int a_r_K[16];  // a_r[i] * K — precompute the multiply
    int a_c[16];
    int a_gr_valid[16]; // for boundary case
    #pragma unroll
    for (int i = 0; i < 16; i++) {
        int elem_id = lid + i * 64;
        int r = elem_id >> 5;
    ...
    // ---- Load first A tile into buffer 0 ----
    {
        ...
        if (row_tile_valid) {
            ...
            #pragma unroll
            for (int i = 0; i < 16; i++)
                dst[a_local_off[i]] = A_base[a_r_K[i] + a_c[i]];
        } else {
            #pragma unroll
            for (int i = 0; i < 16; i++)
                dst[a_local_off[i]] = a_gr_valid[i] ? A_us[(row_base + (a_local_off[i] >> 5)) * K + a_c[i]] : (ushort)0;
        }
    }
    barrier(CLK_LOCAL_MEM_FENCE);
    ...
    // Main loop: process K-tiles. Last iteration handled separately to avoid extra barrier.
    for (int kt = 0; kt < num_k_tiles - 1; kt++) {
        ...
        if (row_tile_valid) {
            ...
            #pragma unroll
            for (int i = 0; i < 16; i++)
                next_slm[a_local_off[i]] = A_tile[a_r_K[i] + a_c[i]];
        } else {
            #pragma unroll
            for (int i = 0; i < 16; i++) {
