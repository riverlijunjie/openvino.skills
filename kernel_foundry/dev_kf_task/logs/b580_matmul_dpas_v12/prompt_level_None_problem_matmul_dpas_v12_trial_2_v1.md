...
### Version 1 (Result: Correctness score: 5 / 5 (compiles and correct), runtime in ms: 1.230):
```OCL
...
Here is the last OCL kernel we tested (Result: Correctness score: 5 / 5 (compiles and correct), runtime in ms: 1.150):
```OCL
#pragma OPENCL EXTENSION cl_intel_subgroups : enable
#pragma OPENCL EXTENSION cl_intel_subgroup_matrix_multiply_accumulate : enable
#pragma OPENCL EXTENSION cl_khr_fp16 : enable
...
// 4 subgroups x 16 WIs = 64 WIs per WG
// Each subgroup: 4 vertical 8x16 DPAS tiles = 32 rows x 16 cols
// A cached in SLM (ushort), B loaded directly from global/L2
// GWS = (ceil(N/64)*64, ceil(M/32))  LWS = (64, 1)
// Subgroup size: 16
    ...
    const int N)
{
    ...
    const bool col_valid = col_idx < N;
    const bool row_tile_valid = (row_base + TILE_M <= M);
    // K=2048, TILE_K=32, so num_k_tiles=64 (even), no remainder needed
    const int num_k_tiles = K / TILE_K;

    // Clamp col_idx for B reads to avoid OOB (result will be discarded for invalid cols)
    const int b_col = col_valid ? col_idx : (N - 1);

    __global const ushort* A_us = (__global const ushort*)A;
    __global const ushort* B_us = (__global const ushort*)B;

    // Main loop over K tiles - unrolled 2x, no remainder (num_k_tiles is even)
    for (int kt = 0; kt < num_k_tiles; kt += 2) {
        ...
        // ---- First K tile ----
        {
            ...
            if (row_tile_valid) {
                ...
                for (int i = 0; i < 16; i++) {
                    ...
                    int r = elem_id >> 5;
                    int c = elem_id & 31;
                    slm_A[elem_id] = A_us[(row_base + r) * K + k + c];
                }
            } else {
                #pragma unroll
                for (int i = 0; i < 16; i++) {
            ...
            #pragma unroll
            for (int kk = 0; kk < TILE_K; kk += 16) {
                // Load A from SLM - scalar reads, naturally coalesced in subgroup
                short8 a0, a1, a2, a3;
                #pragma unroll
                for (int r = 0; r < 8; r++)
                    ((ushort*)&a0)[r] = slm_A[r * TILE_K + kk + sg_lid];
                #pragma unroll
                for (int r = 0; r < 8; r++)
                    ((ushort*)&a1)[r] = slm_A[(8 + r) * TILE_K + kk + sg_lid];
                #pragma unroll
                for (int r = 0; r < 8; r++)
                    ((ushort*)&a2)[r] = slm_A[(16 + r) * TILE_K + kk + sg_lid];
                #pragma unroll
                for (int r = 0; r < 8; r++)
                    ((ushort*)&a3)[r] = slm_A[(24 + r) * TILE_K + kk + sg_lid];
