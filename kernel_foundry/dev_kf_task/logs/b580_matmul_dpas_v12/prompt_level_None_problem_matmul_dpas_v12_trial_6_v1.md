...
This is the code of the best kernel we have tested so far (Result: Correctness score: 5 / 5 (compiles and correct), runtime in ms: 0.959):
```OCL
#pragma OPENCL EXTENSION cl_intel_subgroups : enable
#pragma OPENCL EXTENSION cl_intel_subgroup_matrix_multiply_accumulate : enable
#pragma OPENCL EXTENSION cl_khr_fp16 : enable
...
// Each subgroup: 4 vertical 8x16 DPAS tiles = 32 rows x 16 cols
// A cached in SLM (2KB), B loaded directly from global/L2
// K=2048 divides by 32, no remainder path.
// B reads use intel_sub_group_block_read_us for vectorized global access
// C stores use intel_sub_group_block_write_us for vectorized writes
// GWS = (ceil(N/64)*64, ceil(M/32))  LWS = (64, 1)
    ...
    const int N)
{
    ...
    // Precompute whether B subgroup block reads are safe (col_base + 16 <= N)
    const bool b_block_read_safe = (col_base + 16 <= N);
    ...
    for (int kt = 0; kt < num_k_tiles; kt++) {
        ...
        for (int kk = 0; kk < TILE_K; kk += 16) {
            ...
            // Load B from global — use block reads when safe, scalar pairs otherwise
            int8 b_val;

            if (b_block_read_safe) {
                // col_base is 16-aligned within the tile, subgroup reads 16 contiguous halves
                __global const ushort* B_row = B_us + (k + kk) * N + col_base;
                #pragma unroll
                for (int p = 0; p < 8; p++) {
                    ushort s0 = intel_sub_group_block_read_us(B_row);
                    ushort s1 = intel_sub_group_block_read_us(B_row + N);
                    ((int*)&b_val)[p] = as_int((ushort2)(s0, s1));
                    B_row += N2;
                }
            } else {
