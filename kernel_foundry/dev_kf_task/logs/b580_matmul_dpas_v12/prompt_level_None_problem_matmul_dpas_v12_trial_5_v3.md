...
### Version 1 (Result: Correctness score: 5 / 5 (compiles and correct), runtime in ms: 0.969):
```OCL
...
This is the code of the best kernel we have tested so far (Result: Correctness score: 5 / 5 (compiles and correct), runtime in ms: 0.968):
```OCL
#pragma OPENCL EXTENSION cl_intel_subgroups : enable
#pragma OPENCL EXTENSION cl_intel_subgroup_matrix_multiply_accumulate : enable
#pragma OPENCL EXTENSION cl_khr_fp16 : enable
...
Here is the last OCL kernel we tested (Result: Correctness score: 5 / 5 (compiles and correct), runtime in ms: 0.986):
```OCL
#pragma OPENCL EXTENSION cl_intel_subgroups : enable
#pragma OPENCL EXTENSION cl_intel_subgroup_matrix_multiply_accumulate : enable
#pragma OPENCL EXTENSION cl_khr_fp16 : enable
...
// 4 subgroups x 16 WIs = 64 WIs per WG
// Each subgroup: 4 vertical 8x16 DPAS tiles = 32 rows x 16 cols
// A cached in SLM (2KB per buffer, 4KB total for double buffer), B from global/L2
// GWS = (ceil(N/64)*64, ceil(M/32))  LWS = (64, 1)
// K=2048 divisible by 64, so we unroll K-loop 2x (process 2 k32 tiles per iteration)
    ...
    const int N)
{
    ...
    // Double-buffered SLM for A: 2 x 32x32 = 4KB
    __local ushort slm_A[2 * TILE_M * TILE_K];
    ...
    // Main K-loop: iterate over pairs of k32 tiles when possible
    // Process with double buffering: compute on current buffer while loading next
    for (int kt = 0; kt < num_k_tiles; kt++) {
        const int k = kt * TILE_K;
        const int cur_buf = kt & 1;
        const int nxt_buf = 1 - cur_buf;
        __local const ushort* slm_cur = slm_A + cur_buf * (TILE_M * TILE_K);

        // Start loading next A tile into alternate buffer (overlaps with compute below)
        // We issue the loads first, then do the compute, then barrier
        if (kt + 1 < num_k_tiles) {
            const int k_next = k + TILE_K;
            __local ushort* slm_dst = slm_A + nxt_buf * (TILE_M * TILE_K);
            if (row_tile_valid) {
                __global const ushort* A_tile = A_us + a_row_base_K + k_next;
                #pragma unroll
                for (int i = 0; i < 16; i++) {
                    int elem_id = lid + i * 64;
                    int r = elem_id >> 5;
                    int c = elem_id & 31;
                    slm_dst[elem_id] = A_tile[r * K + c];
                }
            } else {
                #pragma unroll
                for (int i = 0; i < 16; i++) {
                    ...
                    int c = elem_id & 31;
                    int gr = row_base + r;
                    slm_dst[elem_id] = (gr < M) ? A_us[gr * K + k_next + c] : (ushort)0;
                }
            }
        }

        // Compute on current A tile buffer — two k16 DPAS steps
        #pragma unroll
        for (int kk = 0; kk < TILE_K; kk += 16) {
            // Load A from SLM using vectorized subgroup block reads
            short8 a0, a1, a2, a3;
            ...
            // Load B from global — paired scalar reads packed into int for DPAS
            int b_base_offset = (k + kk) * N + b_col;
            int8 b_val;
