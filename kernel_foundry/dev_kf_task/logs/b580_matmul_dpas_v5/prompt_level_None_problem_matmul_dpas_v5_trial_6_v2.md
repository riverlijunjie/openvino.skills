...
1. `const int src_row = (lane < 8) ? lane : 0; ... intel_sub_group_f16_f16_matrix_mad_k16(a_vec, b_vec, acc_lo);` and  
   `const int src_row = (lane < 8) ? (lane + 8) : 8; ... intel_sub_group_f16_f16_matrix_mad_k16(a_vec, b_vec, acc_hi);`:  
   You’re under-utilizing DPAS lanes. Lanes 8..15 reuse row 0 / row 8, so half the subgroup is doing duplicated work and not producing independent outputs. That limits throughput and wastes execution slots.  
   **Improve it by remapping work so every lane contributes unique output elements per DPAS call** (e.g., each lane computes a small 2D micro-tile such as 2x2 or 1x4, and pack A/B fragments to match DPAS fragment layout). In practice: use subgroup block-read + DPAS layout where all 16 lanes provide distinct A fragments instead of broadcasting from 8 rows. This is usually the single biggest speedup opportunity here.

2. `for (int r = 0; r < TM; ++r) { ... Asub[r][lane] = ... }` and  
   `for (int kk = 0; kk < TK; ++kk) { ... Bsub[kk][lane] = ... }` with surrounding `barrier(CLK_LOCAL_MEM_FENCE);`:  
   The kernel is paying high SLM traffic + full barriers every K-block, even though each lane ultimately consumes one B column and repeatedly reads contiguous A rows. This creates synchronization overhead and local-memory bandwidth pressure.  
   **Improve it by reducing/removing SLM staging for at least one operand**: keep `B` directly in registers per lane (`vload16` from global for contiguous `B[k*N + col]` pattern), and only stage `A` (or vice versa depending on measured cache behavior). Also apply double-buffered SLM prefetch (ping-pong two tiles) to overlap load and compute and cut barrier stall time. You should see lower barrier wait and better EU occupancy.

3. `for (int k = k_full_end; k < K; ++k) { v += ... }` inside each output-row store loop (executed separately for lo and hi):  
   Tail-K handling is on the critical path and repeated per output element, causing scalar rework and extra global reads after the main loop. For small/medium K remainders this overhead is noticeable.  
   **Improve it by folding tail handling into the main tiled loop**: run `kb` up to `ceil_div(K,16)*16`, zero-pad out-of-range A/B loads (you already have bounds checks), and remove the separate scalar tail loops entirely. That eliminates two extra per-element remainder loops and improves instruction/cache locality at store time.
