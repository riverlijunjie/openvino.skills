...
1. `float8 d_acc[THREAD_M][THREAD_N];` and `float v = d_acc[mi][nj].s0;` : You’re computing/storing 8 lanes per output but only consuming lane 0 at the end. That is a huge register and compute waste (64 outputs × float8 per thread), which will hurt occupancy and instruction throughput.  
   **Improve it:** map each thread/subgroup so every DPAS lane is used for real outputs, or switch to a scalar accumulator layout (`float d_acc[THREAD_M][THREAD_N]`) with explicit dot/FMA if you cannot remap DPAS outputs correctly. On Battlemage, the best path is to reorganize output ownership to match DPAS fragment semantics, then store all produced lanes instead of discarding `.s1..s7`.

2. `if (next_kb < k_full_end && tile_idx + 1 < k_tiles) { ... Aslm[next_buf]... BslmT[next_buf]... }` followed by compute on current buffer: this is not true async overlap; it is synchronous global→SLM loading done by the same threads before compute, so memory latency is still on the critical path.  
   **Improve it:** use real async copy primitives (`async_work_group_copy`/prefetch builtins if available on your OpenCL stack) with event wait right before buffer use, or pipeline at subgroup level (some subgroups load next tile while others compute) with double-buffered barriers. Right now your “double buffering” mostly just alternates buffers without hiding load latency.

3. `for (int k = k_full_end; k < K; ++k) { v = mad(convert_float(A[r * K + k]), convert_float(B[k * N + c]), v); }` : tail-K cleanup is done per output element from global memory, causing redundant reloads of A/B and heavy scalar work when `K % TILE_K != 0`.  
   **Improve it:** handle the tail inside the tiled path by zero-padding SLM loads for the last partial K-tile and running the same DPAS/FMA compute path once. That removes this per-element global-memory loop and keeps memory access coalesced/reused. If you must keep a tail path, cache `A[r*K+k]` once per `mi` and `B[k*N+c]` once per `nj` tile-wise in registers/SLM, not per `(mi,nj)` scalar output.
