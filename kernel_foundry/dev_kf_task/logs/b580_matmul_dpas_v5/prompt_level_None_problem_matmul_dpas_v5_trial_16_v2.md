...
// Launch metadata:
//   LWS = {16, 2, 1}  (2 subgroups of 16 lanes = 32 threads)
//   GWS = {ceil_div(N,16)*16, ceil_div(M,16)*2, 1}
//   Each WG computes 16x16 C tile (2 subgroups, each 8x16)
//   Subgroup size: 16
    ...
    const int N)
{
    ...
    const int my_col = tile_col + lane;

    // SLM: A stored row-major [16][16+1], B transposed to [16][16+1] (col-major)
    // B transposed: Bt[col][k] so lane can do contiguous vload16
    __local half Asub[TILE_M][TILE_K + 1];
    __local half Bt[TILE_N][TILE_K + 1];

    float8 acc = (float8)(0.0f);

    const int k_full_end = (K / TILE_K) * TILE_K;
    const int tid = sg_id * 16 + lane;  // 0..31
    ...
    for (int kb = 0; kb < k_full_end; kb += TILE_K) {
        ...
        for (int i = 0; i < 8; ++i) {
            int idx = tid + i * 32;
            int ar = idx / TILE_K;   // row 0..15
            int ak = idx % TILE_K;   // col 0..15
            int gr = tile_row + ar;
            int gk = kb + ak;
            Asub[ar][ak] = (gr < M && gk < K) ? A[gr * K + gk] : (half)0.0h;
        }

        // Cooperative load B[16][16] transposed into Bt[col][k]: 256 elems, 32 threads -> 8 each
        #pragma unroll
        for (int i = 0; i < 8; ++i) {
            int idx = tid + i * 32;
            int bk = idx / TILE_N;   // k-row 0..15
            int bc = idx % TILE_N;   // col 0..15
            int gk = kb + bk;
            int gc = tile_col + bc;
            // Store transposed: Bt[bc][bk]
            Bt[bc][bk] = (gk < K && gc < N) ? B[gk * N + gc] : (half)0.0h;
        }

        barrier(CLK_LOCAL_MEM_FENCE);

        // DPAS: lane i in subgroup loads A row (my_row_base + min(lane,7))
        // For correct DPAS semantics: lane i provides row i's A data (for i<8)
        // The DPAS intrinsic internally broadcasts across subgroup
        half16 a_vec;
        int a_row_idx = sg_id * 8 + (lane < 8 ? lane : 0);
        a_vec = vload16(0, &Asub[a_row_idx][0]);

        // B: each lane loads its column's k-vector contiguously from Bt
        half16 b_vec = vload16(0, &Bt[lane][0]);

        // DPAS: 8 rows x 16 cols, k=16
    ...
    if (my_col < N) {
        float acc_arr[8];
        acc_arr[0] = acc.s0; acc_arr[1] = acc.s1;
        acc_arr[2] = acc.s2; acc_arr[3] = acc.s3;
        acc_arr[4] = acc.s4; acc_arr[5] = acc.s5;
        acc_arr[6] = acc.s6; acc_arr[7] = acc.s7;
...
1. `half16 a_vec; int a_row_idx = sg_id * 8 + (lane < 8 ? lane : 0); a_vec = vload16(0, &Asub[a_row_idx][0]);`  
   You are making **all 16 lanes load `a_vec`**, but only 8 unique rows exist per subgroup. Lanes 8..15 redundantly load row 0, which wastes local-memory bandwidth and register bandwidth every `kb` step.  
   **Improve it by splitting lanes into producer/consumer roles for A-fragments**: only lanes 0..7 should load the 8 A rows, then use subgroup shuffles/broadcast to distribute fragments as needed for DPAS operand layout. If your DPAS variant expects per-lane A payload, pack A once per 8-lane half-subgroup and reuse. This removes redundant `vload16` traffic and usually gives a noticeable speedup.

2. `barrier(CLK_LOCAL_MEM_FENCE);` (the one after DPAS at end of each `kb` iteration)  
   You have two full workgroup barriers per K-tile, and the second one serializes progress before the next load phase. This is a major stall source.  
   **Improve with double-buffered SLM (ping-pong)**: allocate `Asub[2][...]` and `Bt[2][...]`, prefetch tile `kb+16` while computing tile `kb`, and keep only the synchronization needed to protect the specific buffer being written/read. This overlaps memory with compute and reduces barrier cost per useful FMAs.

3. `for (int k = k_full_end; k < K; ++k) { val += convert_float(A[gr * K + k]) * convert_float(B[k * N + my_col]); }`  
   Tail-K handling is done **inside the per-output-row store path**, so each lane executes scalar global loads and FP32 converts for up to 8 rows, causing poor memory coalescing and high instruction overhead when `K % 16 != 0`.  
   **Improve by folding tail handling into the tiled path**: run one extra padded K-block (load out-of-range elements as zero into SLM) and execute the same DPAS kernel path, then store once. This removes scalar tail loops and keeps execution uniform across lanes. If you must keep scalar tail, at least hoist `if (k_full_end < K)` outside and vectorize loads (`half4/half8`) where possible.
