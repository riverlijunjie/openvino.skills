...
1. `__attribute__((reqd_work_group_size(16, 16, 1)))` + `const int linear_id = ly * 16 + lx;` + both SLM load loops (`for (int t = 0; t < 2; ++t)` for A and `for (int t = 0; t < 16; ++t)` for B): you launch 256 threads, but your compute path is effectively placeholder/scalar and not using subgroup MMA, so most of the runtime is dominated by cooperative memory movement + barriers instead of math.  
   **Improve it by restructuring to a single-subgroup kernel (`LWS={16,1,1}` or very small number of subgroups) that maps directly to DPAS tiles**. Keep one subgroup responsible for a compact output tile (e.g., 8x16 or 16x16 in DPAS-friendly layout), and increase arithmetic intensity per loaded byte. Right now the thread geometry is built for a 16x128 tile, but the compute body does not exploit that shape, so occupancy and memory traffic are mismatched.

2. `barrier(CLK_LOCAL_MEM_FENCE);` (twice per `kb` iteration): two full workgroup barriers per K-block are expensive at this WG size, and they serialize all 256 threads every 32-K chunk.  
   **Improve it by double-buffering SLM tiles (`Asub0/Asub1`, `Bsub0/Bsub1`) and pipelining load/compute**, so you only synchronize when switching buffers, and overlap global->SLM copies with DPAS/FMA on the previous tile. If you keep 256 threads, barrier cost remains high; with a smaller subgroup-centric WG, barrier overhead drops sharply.

3.  
   - `float8 acc0 = (float8)(0.0f);`  
   - `float8 acc1 = (float8)(0.0f);`  
   - `float8 acc = (float8)(0.0f);`  
   - final store path with `switch(j)` extracting `acc.s0 ... acc.s7`  
   You allocate unused accumulators (`acc0`, `acc1`) and then use a scalarized switch-based extraction on store, which adds unnecessary register pressure and instruction overhead.  
   **Improve it by keeping only the actually used accumulator vector(s), and store vector-lane values without a switch** (e.g., vector store when contiguous and aligned, or direct lane indexing pattern the compiler can lower cleanly). Also replace scalar remainder-style extraction with a packed write path so the compiler emits fewer moves/selects. This helps both instruction count and register allocation, especially once you add real DPAS accumulators.
