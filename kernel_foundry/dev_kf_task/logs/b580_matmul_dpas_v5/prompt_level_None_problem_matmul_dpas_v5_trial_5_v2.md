...
1. `half16 a_vec = vload16(0, &Asub[ly][0]);` +  
   `for (int kk = 0; kk < 16; ++kk) { b_vec.s[kk] = Bsub[kk][lx]; }` +  
   `dpas_acc = intel_sub_group_f16_f16_matrix_mad_k16(a_vec, b_vec, dpas_acc);`  
   You’re rebuilding DPAS operands **per work-item** from SLM and doing a scalar gather for `b_vec`, which is expensive and often prevents optimal lowering to XMX instructions. The gather loop is especially costly (16 scalar reads + register packing each K-block).  
   **Improve it by mapping work to subgroups/fragments directly**: store/load B in a layout that allows contiguous vector loads per lane (or pre-transpose B tiles in SLM once per tile), then use subgroup block reads (`intel_sub_group_block_read*`) to form DPAS-ready fragments without scalar lane packing. Also keep `dpas_acc` live across K-tiles instead of reinitializing temporary vectors each iteration when possible.

2. `__local half Asub[TILE_M][TILE_K + 1];` and `__local half Bsub[TILE_K][TILE_N + 1];` with  
   `barrier(CLK_LOCAL_MEM_FENCE);` (twice inside each K-tile loop)  
   You pay two full work-group barriers per 16-wide K step, which is a major stall at small tile size. The `+1` padding helps bank conflicts, but synchronization overhead dominates.  
   **Improve it with double-buffered SLM (ping-pong) and overlapped load/compute**: preload tile `kb+16` while computing tile `kb`, then only one barrier is needed per stage transition. If your compiler/device supports it, use async copy to SLM and subgroup-level sync where legal. This typically gives a large gain by hiding memory latency and cutting barrier cost.

3. `float acc = 0.0f;` and later `C[row * N + col] = convert_half(acc);` (one output element per thread)  
   Each thread computes only one `C[row,col]`, so arithmetic intensity per loaded tile is low and you underuse registers/XMX throughput. You repeatedly fetch tile data for minimal per-thread output work.  
   **Improve by increasing per-thread output (register blocking)**: e.g., compute 2–4 columns (or rows) per thread (`acc0..acc3`) so each loaded A/B fragment is reused more before eviction. Combine this with a tile shape better matched to DPAS/Xe hardware (for example, larger N tile per subgroup) to increase FLOPs per byte and reduce writeback overhead.
