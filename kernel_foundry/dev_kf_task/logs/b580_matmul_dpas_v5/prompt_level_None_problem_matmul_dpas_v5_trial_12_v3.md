...
1. `__attribute__((reqd_work_group_size(16, 16, 1)))` and the SLM loads  
   `for (int t = 0; t < 16; ++t) { ... Bsub[bk][bc] = ... }`:  
   You’re using **256 threads/WG** to cooperatively load a very large B tile (4096 halfs per K-block), and each thread performs many scalar SLM/global transactions. This makes the kernel heavily **memory-traffic and barrier bound**, while DPAS compute is effectively absent.  
   **Improve it by reducing WG to one subgroup (e.g., `reqd_work_group_size(16,1,1)`) and mapping one subgroup to a compact DPAS tile (e.g., 8x16 or 16x16 micro-tile)**, then replicate subgroups across M/N with more groups instead of more threads inside one WG. This cuts synchronization overhead, improves occupancy, and aligns with Intel subgroup MMA usage.

2. `barrier(CLK_LOCAL_MEM_FENCE);` (twice inside each `kb` loop):  
   Two full-workgroup barriers per K-block are expensive, especially with 256 threads and large SLM footprint. You’re serializing progress between load/compute phases and paying high stall cost every `TILE_K=32`.  
   **Improve it with double-buffered SLM (ping-pong) and software pipelining**: while computing on buffer 0, preload next K-slice into buffer 1; then swap. This reduces effective synchronization cost and hides global-memory latency behind compute. Also, if you move to one-subgroup WG, you can replace some WG barriers with cheaper subgroup-synchronous behavior.
...
3. Store path:  
   ...
   ```  
   This per-element `switch` creates unnecessary control/data-move overhead and prevents efficient vectorized stores.  
   **Improve by vectorizing the epilogue**: form a `half8 out = convert_half8_rte(acc);` and use `vstore8` when `c_col_base+7 < N`; only use scalar tail handling for boundary columns. Also precompute `row_base = c_row * N` once. This reduces instruction count and improves memory write efficiency.
