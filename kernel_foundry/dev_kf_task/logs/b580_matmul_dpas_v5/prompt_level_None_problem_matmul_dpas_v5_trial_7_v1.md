...
1. `a_vec = vload16(0, &Asub[buf_idx][sg_id * 8][0]);` and `half16 a_vec = vload16(0, &Asub[buf_idx][sg_id * 8][16]);`:  
   You are loading only **one row’s K-slice** for the subgroup (`sg_id * 8`), but your accumulator is `float8` (8 rows). That means the DPAS op is not fed with a true 8-row A fragment each step; you’re effectively under-utilizing the matrix engine data path and relying on an implicit layout assumption that usually does not hold.  
   **Improve it** by packing A exactly in the DPAS-required subgroup layout (8x16 fragment) in SLM/registers and issuing DPAS on that packed fragment. In practice: each lane should contribute to building the 8-row fragment (or use block reads into a packed per-subgroup buffer), then call DPAS with that packed operand. This removes wasted/redundant loads and gives the hardware the fragment shape it expects.

2. `Bsub[0][bk][bc] = (bk < K && gc < N) ? B[bk * N + gc] : (half)0.0h;` (and same pattern in the loop):  
   B is staged to SLM as scalar stores and later read as `vload16(... &Bsub[..., ...][lx])`, which is a **strided gather across rows** for each lane. This is not truly “column-major vector friendly” for subgroup DPAS feeding; it causes extra address generation and poor cache/SLM access regularity.  
   **Improve it** by storing B in a layout that matches DPAS consumption directly (e.g., packed `k16 x n16` blocks with contiguous vectors per lane/subgroup), and use subgroup block loads (`intel_sub_group_block_read*`) or vectorized global loads to bring B tiles in. The goal is: one coalesced load pattern from global, one packed write/read pattern in SLM, minimal per-element scalar indexing.

3. `barrier(CLK_LOCAL_MEM_FENCE);` at tile boundaries with prefetch done before compute in the same iteration:  
   Your “double buffering” still serializes too much because both subgroups fully finish SLM writes, then both wait, then both compute. There is limited overlap of memory and compute at subgroup level.  
   **Improve it** by turning this into a true software pipeline: prefetch `next_kb` *after* issuing current DPAS chunk(s), and use ping-pong buffers with only one mandatory barrier when switching consumer buffer. Also unroll the K-loop by 2 BK-steps so loads for `kb+BK` are issued while arithmetic for `kb` is in flight. This reduces barrier stall percentage and raises XMX utilization.
