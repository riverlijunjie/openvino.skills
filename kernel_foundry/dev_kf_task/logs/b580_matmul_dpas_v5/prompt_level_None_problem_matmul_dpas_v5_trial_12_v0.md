...
1. `float acc = 0.0f;` (single scalar accumulator per thread):  
   You’re underutilizing DPAS throughput because each thread computes only one `C[row, col]` value. That gives low arithmetic intensity relative to SLM/global traffic and barrier overhead.  
   **Improve it by giving each thread a small register tile**, e.g. 2×2 or 4 outputs (multiple `col` or `row` lanes), with `float acc0, acc1, ...`. Reuse the same loaded `a_vec`/`b_vec` fragments across multiple accumulators before moving to next K-slice. This increases FLOPs per byte loaded and usually gives a clear speedup.

2. `for (int kk = 0; kk < 16; ++kk) b_vec.s[kk] = Bsub[kk][lx];` (and the analogous loop for `16 + kk`):  
   This scalar gather from SLM into `half16` is expensive and blocks vectorized load paths. You’re doing 16 scalar reads every DPAS step per thread.  
   **Improve data layout so both A and B fragments are loaded with vector operations** (`vload16`) instead of scalar lane assembly. For example, store `Bsub` in a transposed/vector-friendly layout during cooperative load (e.g. `Bsub_colmajor[tn][bk]` or packed half16 chunks), so each thread can do one vector load for the needed `k16` fragment.

3. `barrier(CLK_LOCAL_MEM_FENCE);` … compute … `barrier(CLK_LOCAL_MEM_FENCE);` inside every `kb` iteration:  
   Two full workgroup barriers per BK tile are costly, especially with small per-iteration compute.  
   **Improve with double-buffered/ping-pong SLM staging**: while computing on buffer 0, prefetch next `Asub/Bsub` tile into buffer 1, then swap. This removes one synchronization point from the steady-state loop and overlaps load+compute better. Keep one barrier for correctness at buffer handoff, but avoid the strict load-then-compute-then-barrier pattern every iteration.
