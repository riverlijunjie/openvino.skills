...
1. `half16 b_vec; for (int kk = 0; kk < 16; ++kk) { b_vec.s[kk] = Bsub[kk][lx]; }`  
   You’re doing a per-thread **column gather from local memory** every K-tile, which is high-latency and hard for the compiler to map cleanly to the DPAS data path. This is likely your biggest cost in the “fast path.”  
   **Improve it** by changing the mapping so each subgroup/lane loads B in a layout that can be consumed without scalar gather (e.g., pre-transpose/pack B tiles in SLM during the cooperative load so each lane can use `vload16`/block read). Concretely: store B tile as `BsubT[lx][ly]` (or packed chunks) and then read contiguous vectors per lane. That removes 16 scalar LDS reads + register insertions per inner iteration.

2. `barrier(CLK_LOCAL_MEM_FENCE); ... barrier(CLK_LOCAL_MEM_FENCE);` (inside every `kb` iteration)  
   Two full work-group barriers per K-slice serialize progress and reduce occupancy hiding, especially with 16x16 WG.  
   **Improve it** with **double-buffered SLM ping-pong**: while computing on buffer 0, preload next tile into buffer 1, then swap. This reduces synchronization impact and overlaps memory + compute. If your compiler/device supports it well, also consider subgroup-scoped coordination for the compute path to avoid full WG stalls where not required.

3. `Asub[ly][lx] = ...; Bsub[ly][lx] = ...;` with `float acc = 0.0f;` and `C[row * N + col] = convert_half(acc);`  
   Each thread computes only **one C element**, so arithmetic intensity per load/barrier is low and DPAS throughput is underutilized. You pay heavy tiling/sync overhead for tiny per-thread output.  
   **Improve it** by increasing per-thread work (register blocking), e.g., 2x2 or 4 outputs per thread (multiple `acc` registers), reusing loaded A/B fragments across several C values before moving to next `kb`. This typically gives a large speedup because SLM loads and barriers are amortized over more FMAs/DPAS operations.  
   Also, if numerically acceptable for your workload, emit vectorized stores (`half2/half4`) for contiguous columns to reduce store overhead.
