...
1. `float8 dpas_result = intel_sub_group_f16_f16_matrix_mad_k16(a_vec, b_vec, (float8)(acc)); acc = dpas_result.s0;`: you’re issuing a subgroup MMA that computes an 8-lane result, then throwing away 7 lanes every time. That is a huge compute-efficiency loss. Map work so each subgroup lane owns one output element (or a small vector of outputs), keep `float8` accumulators live across the K loop, and write all lanes back. In other words, redesign from “1 thread -> 1 C element” to “1 subgroup -> tile fragment of C” so DPAS output is fully consumed.

2. `if (tile + 1 < k_tiles && next_kb < K) { ... Asub[next_buf] ... BsubT[next_buf] ... }` followed by `barrier(CLK_LOCAL_MEM_FENCE);`: this is not real overlap; prefetch and compute are still serialized by the same threads and blocked by a full WG barrier each iteration. On Xe2, move to subgroup-scoped pipelining: dedicate some lanes/iterations to staged loads (or use block reads) and use double-buffered scheduling with minimal synchronization (subgroup barriers where legal). Also split interior and boundary kernels so the hot path removes these per-iteration conditionals entirely.

3. `__local half Asub[2][TM][BK + 2];` and `__local half BsubT[2][TN][BK + 2];` with `vload16(...)`: the SLM layout/padding is not aligned to guaranteed 16-half vector boundaries for every row base, so you can get suboptimal SLM transactions and bank conflicts. Use padding/alignment that preserves 32-byte (or wider) alignment for each `k_offset` vector load (e.g., stride padded to a multiple of 16 halves, not `+2`), and consider loading/storing as `half8/half16` vectors directly during tile fill so both global->SLM and SLM->register paths stay vectorized and aligned.
