...
1. `__attribute__((reqd_work_group_size(16, 16, 1)))` + `const int ly = get_local_id(1);` + the commented-out DPAS block (`_ = a_vec; // placeholder`): right now you launch **256 threads/WG**, but the compute path is effectively scalar/placeholder and not mapping to DPAS math. That means high scheduling/barrier overhead per useful FLOP.  
   **Improve it by restructuring to one subgroup per WG** (e.g., `reqd_work_group_size(16,1,1)`), and make each subgroup compute a native DPAS tile (e.g., 8x16 per call, 2 calls for 16 rows). This removes 16x over-subscription, reduces barrier cost, and lets each lane keep DPAS accumulators in registers. If you keep 256-thread WGs, you must prove each subgroup is doing independent DPAS work; currently it isn’t.

2. `barrier(CLK_LOCAL_MEM_FENCE);` (twice inside every `kb` iteration) together with full SLM staging  
   ```
   __local half Asub[TILE_M][TILE_K + 1];
   __local half Bsub[TILE_K][TILE_N + 1];
   ...
   ...
   barrier(CLK_LOCAL_MEM_FENCE);
   ```
   You are paying two full-WG syncs per K-block and moving a lot through SLM, which is expensive versus subgroup-shuffle/register-fed DPAS.  
   **Improve by double-buffering and reducing sync count to one barrier per K-step**, or better, keep A/B fragments in registers for the active subgroup and use subgroup block reads/shuffles for B. Also remove the `+1` padding unless you can show bank conflicts—on Intel GPUs this often just increases SLM footprint and can lower occupancy.

3. Store path is branch-heavy and scalarized:  
   ```
   for (int j = 0; j < 8; ++j) {
       ...
       float v;
   ...
   ```
   This creates unnecessary control flow and prevents efficient vector stores.  
   **Improve by vectorized conversion/store**: build `half8 out = convert_half8_rte(acc);` and use `vstore8(out, 0, &C[c_row * N + c_col_base])` on full tiles; keep a short masked tail path only when `c_col_base+7 >= N`. This cuts instruction count and improves global-store throughput.
