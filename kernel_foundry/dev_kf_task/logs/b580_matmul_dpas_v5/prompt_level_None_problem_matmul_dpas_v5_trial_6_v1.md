...
1. `acc0 += d0.s0;` and `acc1 += d1.s0;`: you’re only consuming **one lane** of the DPAS result (`s0`) after computing a full `float8`. That throws away most of the work and effectively turns DPAS into overhead.  
   **Improve it:** map outputs so each work-item owns multiple output elements and accumulates all DPAS lanes (e.g., 8 outputs per WI in N dimension), then store all of them. If you only need a scalar per WI, remove DPAS and use a pure FMA path; but for XMX speed, redesign the thread-tile mapping to match DPAS fragment shape and use the full `float8` accumulator.

2. `for (int kk = 0; kk < 16; ++kk) { b_vec0.s[kk] = Bsub[kk][(lx << 1)]; b_vec1.s[kk] = Bsub[kk][(lx << 1) + 1]; }`: per-iteration scalar gather from SLM is expensive and creates a lot of indexing/pack overhead before every DPAS call.  
   **Improve it:** change B tile layout in SLM so each WI can load a contiguous vector directly (e.g., store B as `[col][k]`-friendly for your access pattern), then use vector loads (`vload16`) instead of scalar assembly. Also consider prefetching next `Bsub` tile while computing current tile (double-buffer in SLM) to hide this latency.

3. `barrier(CLK_LOCAL_MEM_FENCE);` (both barriers inside the `kb` loop): two full-workgroup barriers per K-block are a major synchronization cost, especially with small `TK=16`.  
   **Improve it:** use ping-pong SLM buffers (`Asub0/Asub1`, `Bsub0/Bsub1`) and software pipeline the loop: load tile `kb+TK` while computing tile `kb`, so you keep one barrier per iteration boundary instead of two. Also increase K unroll/work per barrier (if register pressure allows) so barrier overhead is amortized better.
