...
1. `float8 d_acc[THREAD_M][THREAD_N];` and `float v = d_acc[mi][nj].s0;` (plus every `intel_sub_group_f16_f16_matrix_mad_k16(...)` update): you’re keeping **8×8×float8 accumulators per thread** but only using `.s0` at the end. That is a huge register footprint and likely the main occupancy limiter. It also means you compute many lanes you never consume.  
   **Improve it** by mapping one DPAS result vector to multiple output elements instead of scalarizing to `.s0` only. If your intended mapping is scalar outputs, switch to a DPAS form / accumulator shape that matches scalar accumulation (or reduce THREAD_M/THREAD_N so you don’t explode live registers). Practically: cut per-thread tile (e.g., 4×8 or 4×4) and keep fewer live `float8` accumulators, then increase workgroups in flight. This usually beats “max math per thread” when register pressure is high.

2. `if (next_kb < k_full_end && tile_idx + 1 < k_tiles) { ... Aslm[next_buf] = ...; BslmT[next_buf] = ...; }` inside the K loop before compute: this is not true async overlap; it is still synchronous loads issued by the same threads, so memory traffic and compute are serialized within the iteration.  
   **Improve it** by software-pipelining at finer granularity: split compute into k-steps (e.g., 2×k16 already exists) and prefetch the next block between DPAS steps with `intel_sub_group_block_read`/vectorized block loads into registers, then store to SLM, minimizing stall windows. Also separate producer/consumer phases with two barriers only where needed (after tile fill, before tile reuse). Right now the placement gives limited overlap benefit.

3. Tail handling in epilogue:  
   `for (int k = k_full_end; k < K; ++k) { v = mad(convert_float(A[r * K + k]), convert_float(B[k * N + c]), v); }`  
   This does scalar global-memory FMAs per output element, causing very poor locality and a lot of redundant loads when `K % TILE_K != 0`.  
   **Improve it** by handling partial K tile **inside the tiled path**: load the last partial tile into SLM with zero-padding (you already do bounds checks during tile load), then run the same DPAS/microkernel once more with masked/zeroed data. That removes per-element tail loops and keeps accesses coalesced. If you keep a tail loop, at least hoist `A[r*K + k]` out of `nj` loop (reuse across 8 columns) and vectorize B loads for contiguous `c_col0 + nj`.
