...
1. `half16 b_vec; ... for (int kk = 0; kk < 16; ++kk) { b_vec.s[kk] = Bsub[kk][lx]; }`: this per-thread gather from local memory is a major bottleneck. You’re doing 16 scalar LDS reads + lane assembly every K-tile, which burns cycles and blocks DPAS throughput.  
   **Improve it by changing B tile layout so each thread can `vload16` contiguous data**, same as A. For example, store B in transposed form in SLM during cooperative load (`BsubT[lx][ly] = ...`), then use `half16 b_vec = vload16(0, &BsubT[lx][0]);`. This removes the scalar gather loop and gives the compiler much better chance to emit optimal XMX/DPAS code.

2. `barrier(CLK_LOCAL_MEM_FENCE); ... barrier(CLK_LOCAL_MEM_FENCE);` inside every `kb` iteration: two full work-group barriers per 16-K step are expensive and limit overlap of memory + compute.  
   **Improve with double-buffered SLM ping-pong** (`Asub[2]`, `Bsub[2]`): preload tile `t+1` while computing tile `t`, then only one synchronization point is needed per stage transition. This is usually a big win for tiled GEMM because it hides local/global memory latency behind math.

3. `if (row < M && col < N) { ... dpas ... }` inside the main tiled loop: this branch is evaluated every K tile, causing control overhead and possible divergence in edge work-groups. Also, edge checks are already handled at load time by zero-padding.  
   **Improve by splitting kernels (or paths) into interior and boundary handling**:  
   - Fast kernel for full tiles (`M,N` multiples of 16 region) with **no per-iteration bounds checks** in compute loop.  
   - Cleanup kernel for boundary tiles/tail.  
   If you keep one kernel, at least hoist a flag once (`bool in_bounds = row < M && col < N;`) and keep compute path branchless for interior groups via group-level condition (`gx < N/16 && gy < M/16`). This reduces repeated branch cost and helps vectorization/scheduling.
