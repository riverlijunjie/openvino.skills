...
// Input/Output: half
// Accumulation: float (exactly as reference)
//
// Launch metadata (host-side):
//   LWS = {16, 16, 1}   // required by attribute
//   GWS = { round_up(N,16), round_up(M,16), 1 }
//   Subgroup = 16
//
// Mapping:
//   - One work-group computes one 16x16 tile of C.
//   - K is processed in BK=32 tiles (two DPAS k16 ops per tile).
//   - Cooperative global->SLM staging, then subgroup-friendly DPAS.
//
// Notes:
//   - Uses Intel DPAS intrinsic: intel_sub_group_f16_f16_matrix_mad_k16
//   - Exact OOB masking for arbitrary M/N/K
//   - Tail K (when K % 32 != 0) handled by scalar cleanup for exactness.
    ...
    const int N)
{
    ...
    // +1 padding helps reduce SLM bank conflicts
    ...
    for (int kb = 0; kb < k_full; kb += BK) {
        // 16x32 = 512 elems A tile, 256 threads => 2 elems/thread
        {
            const int t = ly * 16 + lx; // 0..255
            #pragma unroll
            for (int it = 0; it < 2; ++it) {
                const int idx = t + it * 256; // 0..511
                const int ar  = idx / BK;     // 0..15
                const int ak  = idx % BK;     // 0..31
        ...
        // 32x16 = 512 elems B tile, 256 threads => 2 elems/thread
        {
            const int t = ly * 16 + lx; // 0..255
            #pragma unroll
            for (int it = 0; it < 2; ++it) {
                const int idx = t + it * 256; // 0..511
                const int br  = idx / TN;     // 0..31
                const int bc  = idx % TN;     // 0..15
                const int gk  = kb + br;
                const int gc  = wg_x * TN + bc;
                Bsub[br][bc] = (gk < K && gc < N) ? B[gk * N + gc] : (half)0.0h;
            }
        }
        ...
        if (row < M && col < N) {
            // DPAS step 0: k = [0..15]
            half16 a0 = vload16(0, &Asub[ly][0]);  // vectorized SLM load
            half16 b0;
            #pragma unroll
            for (int kk = 0; kk < 16; ++kk) {
                b0.s[kk] = Bsub[kk][lx];
            }
            acc = intel_sub_group_f16_f16_matrix_mad_k16(a0, b0, acc);

            // DPAS step 1: k = [16..31]
            half16 a1 = vload16(0, &Asub[ly][16]); // vectorized SLM load
            half16 b1;
            #pragma unroll
            for (int kk = 0; kk < 16; ++kk) {
                b1.s[kk] = Bsub[16 + kk][lx];
            }
            acc = intel_sub_group_f16_f16_matrix_mad_k16(a1, b1, acc);
        }

        barrier(CLK_LOCAL_MEM_FENCE);
    }

    // Tail cleanup for remaining K (exact reference behavior)
    if (row < M && col < N) {
...
1. `if (row < M && col < N) { ... acc = intel_sub_group_f16_f16_matrix_mad_k16(...); ... }`: this per-iteration bounds check is in the hot loop and causes divergence on edge work-groups, so DPAS work is predicated every `kb`.  
   **Improve it:** split the kernel path into **interior tiles** and **boundary tiles**. For interior (`wg_y*16+15 < M && wg_x*16+15 < N`), run a no-check fast path (no `if` around DPAS and no zero-fill conditionals when loading SLM). Keep your current masked path only for boundary groups. This removes branch overhead from almost all tiles and lets the compiler schedule DPAS more aggressively.

2. 
   ```c
   half16 b0;
   for (int kk = 0; kk < 16; ++kk) { b0.s[kk] = Bsub[kk][lx]; }
   ...
   half16 b1;
   for (int kk = 0; kk < 16; ++kk) { b1.s[kk] = Bsub[16 + kk][lx]; }
   ```
   this is a strided column gather from SLM, creating many scalar loads and poor bank behavior; it also burns instructions packing `half16`.  
   **Improve it:** store `Bsub` transposed in SLM during staging so DPAS can consume contiguous vectors:
   - write as `BsubT[bc][br] = ...` (shape `[TN][BK(+pad)]`)
   - then load with `half16 b0 = vload16(0, &BsubT[lx][0]);` and `half16 b1 = vload16(0, &BsubT[lx][16]);`
   This turns 32 scalar SLM reads + insertions into 2 vector loads and usually gives a large speedup.
...
3. 
   ...
   barrier(CLK_LOCAL_MEM_FENCE);
   ```
   plus single-buffered SLM (`Asub`, `Bsub`) forces strict load/compute/load/compute serialization each `kb` tile.  
   **Improve it:** use **double-buffered SLM ping-pong** (`Asub[2]`, `Bsub[2]`) and prefetch `kb+BK` while computing `kb`. Pattern: preload tile 0 → barrier → loop: compute on buffer `p`; concurrently load next tile into `1-p`; barrier; swap. This overlaps global-memory latency with DPAS compute and reduces stall time.  
   Also, because `BK=32` is small, consider increasing K blocking (e.g., 64) only if register pressure stays acceptable; with ping-pong you can often hide latency better without hurting occupancy.
