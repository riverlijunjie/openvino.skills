    ...
    const int N)
{
    ...
    const int gx = get_group_id(0);  // N dimension
    const int gy = get_group_id(1);  // M dimension

    const int tile_row = gy * TILE_M;
    const int tile_col = gx * TILE_N;
    const int col = tile_col + lane;

    // Accumulators for rows 0-7 and 8-15 of the 16x16 tile
    float8 acc_lo = (float8)(0.0f);
    float8 acc_hi = (float8)(0.0f);

    // SLM for A tile [16][16] with padding to avoid bank conflicts
    __local half Asub[TILE_M][TILE_K + 1];

    const int k_full_end = (K / TILE_K) * TILE_K;

    for (int kb = 0; kb < k_full_end; kb += TILE_K) {
        // Load A tile: 16 rows x 16 cols, 16 lanes load 16 iterations each
        // Each lane loads one column (lane) across all 16 rows
        #pragma unroll
        ...
        // Build B vector: each lane loads its column across K=16 rows
        // b_vec[kk] = B[kb+kk, col]
        half16 b_vec;
        bool col_valid = (col < N);
        #pragma unroll
        for (int kk = 0; kk < 16; ++kk) {
            int gk = kb + kk;
            ((half*)&b_vec)[kk] = (col_valid && gk < K) ? B[gk * N + col] : (half)0.0h;
        }

        // DPAS for rows 0-7
        // For DPAS: lanes 0-7 provide A data, each provides half16 = one row of A
        // Lanes 8-15 A data is ignored by hardware for the 8-row output
        {
            half16 a_vec;
            if (lane < 8) {
                a_vec = vload16(0, &Asub[lane][0]);
            } else {
                a_vec = (half16)(0.0h);
            }
    ...
    // Store results and handle K tail
    if (col < N) {
        // Rows 0-7
        float acc_arr_lo[8] = {
...
1. `for (int r = 0; r < TILE_M; ++r) { ... Asub[r][lane] = ... }` + `barrier(CLK_LOCAL_MEM_FENCE);` (inside every `kb`):  
   You’re paying a lot for the SLM round-trip and synchronization each K-tile: load A from global → write local → barrier → read local with `vload16` → barrier again. For this 16x16 shape, that overhead is significant versus the 2 DPAS ops you issue.  
   **Improve it by removing SLM for A entirely**: have lanes 0..7 load their `half16 a_vec` rows directly from global (`A[(tile_row+lane)*K + kb ... kb+15]` and `A[(tile_row+8+lane)*K + kb ...]`) and feed DPAS immediately. Lanes 8..15 can skip A loads. This eliminates both barriers and all local-memory traffic for A per chunk.

2. `for (int kk = 0; kk < 16; ++kk) { ((half*)&b_vec)[kk] = ... B[gk * N + col] ... }`:  
   This scalar gather builds `b_vec` with 16 independent loads per lane per chunk, plus pointer-cast element stores. It’s instruction-heavy and hurts memory coalescing efficiency.  
   **Improve it by vectorizing B loads**: load B in packed form (e.g., `half8`/`half16` via `vload`) when contiguous access is possible, or split into two `half8` loads and assemble `half16`. If alignment/layout makes direct `vload16` hard, use blocked loads into registers with fewer operations (e.g., 4x `half4`) instead of 16 scalar inserts. Also hoist `col_valid`/bounds handling so the hot path is branch-free for full tiles.
...
3. Tail handling in store path:  
   ...
   ```
   (done twice for `acc_lo` and `acc_hi`)  
   You recompute the K-tail separately for every output row and lane in scalar code, which is expensive when `K % 16 != 0`.  
   **Improve it by folding tail K into the main loop**: run `kb` to `ceil_div(K,16)*16`, and for out-of-range `k` use zero-masked A/B loads before DPAS. Then delete the per-element tail loops entirely. This keeps compute in DPAS and removes a large scalar epilogue cost.
