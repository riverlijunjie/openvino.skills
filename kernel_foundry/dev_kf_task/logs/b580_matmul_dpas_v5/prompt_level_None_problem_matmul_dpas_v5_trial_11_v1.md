...
1. `__attribute__((reqd_work_group_size(16,16,1)))` together with `float acc = 0.0f;` and one final store per thread: you’re launching **256 threads to compute only one 16x16 tile (256 outputs) with 1 accumulator each**, so each thread does very little math per BK step and spends a lot of time in sync/load overhead.  
   **Improve it by increasing per-thread work (register blocking):** keep each thread computing multiple C elements (e.g., 2x2 or 4x1 outputs), with `float acc[2][2]` etc. Then a 16x16 WG can compute a larger tile (e.g., 32x32) or keep 16x16 but with fewer threads. This raises arithmetic intensity and reduces barrier cost per FLOP.

2. `for (int kk = 0; kk < 16; ++kk) b_vec.s[kk] = Bsub[kk][lx];` (and similarly for `Bsub[16 + kk][lx]`): this is a **scalar gather from SLM into a vector every DPAS call**, which adds instruction overhead and creates a strided access pattern for B that is not friendly to vectorized loads.  
   **Improve B layout/load to avoid per-lane packing:** store B tiles in SLM in a transposed/vector-friendly layout during cooperative load (e.g., `Bsub[col][k]` or packed half16 chunks), then build `b_vec` with `vload16` directly from contiguous memory. That removes the 16-iteration gather loop and cuts SLM read instructions significantly.

3. `barrier(CLK_LOCAL_MEM_FENCE);` (before and after compute in every `kb` iteration): you have **two full WG barriers per BK block**, and with BK=32 that synchronization overhead is substantial relative to the compute done between barriers.  
   **Improve with double-buffered/pipelined SLM tiles:** allocate two Asub/Bsub buffers (`ping/pong`), preload next BK while computing current BK, and keep only one barrier per stage transition. Also consider increasing `BK` (if SLM budget allows) so each synchronization protects more compute. This reduces stall time from synchronization and improves EU occupancy/throughput.
