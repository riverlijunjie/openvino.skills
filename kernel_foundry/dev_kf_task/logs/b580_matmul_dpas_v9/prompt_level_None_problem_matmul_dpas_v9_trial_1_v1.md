...
This is the code of the best kernel we have tested so far (Result: Correctness score: 5 / 5 (compiles and correct), runtime in ms: 53.400):
```OCL
...
Here is the last OCL kernel we tested (Result: Correctness score: 5 / 5 (compiles and correct), runtime in ms: 114.000):
```OCL
// DPAS-based FP16 matmul: C[M,N] = A[M,K] x B[K,N]
// TILE_M=32, TILE_N=64, TILE_K=16
// Workgroup: 4 subgroups x 16 work-items = 64 work-items
// Each subgroup computes 32x16 output (4 DPAS 8x16 blocks)
// GWS: (N/16 * 16, M/32), LWS: (64, 1)
// Subgroup size: 16
    ...
    const int N)
{
    ...
    // Workgroup and subgroup indices
    const int wg_id_n = get_group_id(0);  // which N-tile
    const int wg_id_m = get_group_id(1);  // which M-tile
    const int sg_id = get_sub_group_id(); // 0..3, each handles 16 columns
    const int sg_lid = get_sub_group_local_id(); // 0..15

    const int tile_row = wg_id_m * TILE_M;
    const int tile_col = wg_id_n * TILE_N + sg_id * 16;

    // SLM for A tile [TILE_M x (TILE_K+1)] and B tile [TILE_K x (TILE_N+1)]
    // Pad by 1 to avoid bank conflicts
    __local half slm_A[TILE_M * (TILE_K + 1)];
    __local half slm_B[TILE_K * (TILE_N + 1)];

    // Accumulators: 4 blocks of 8x16 each (covering 32 rows x 16 cols)
    ...
    for (int k = 0; k < K; k += TILE_K) {
        // === Load A[tile_row..+32, k..+16] into SLM ===
        // 64 work-items load 32*16=512 halves -> 8 halves per work-item
        for (int i = 0; i < 8; i++) {
            int idx = lid * 8 + i;
            int row = idx / TILE_K;
            int col = idx % TILE_K;
            int global_r = tile_row + row;
            int global_c = k + col;
            half val = (global_r < M && global_c < K) ? A[global_r * K + global_c] : (half)0.0f;
            slm_A[row * (TILE_K + 1) + col] = val;
        }

        // === Load B[k..+16, tile_col_base..+64] into SLM ===
        // 64 work-items load 16*64=1024 halves -> 16 halves per work-item
        int base_col = wg_id_n * TILE_N;
        for (int i = 0; i < 16; i++) {
            int idx = lid * 16 + i;
            int row = idx / TILE_N;
            int col = idx % TILE_N;
            int global_r = k + row;
            int global_c = base_col + col;
            half val = (global_r < K && global_c < N) ? B[global_r * N + global_c] : (half)0.0f;
            slm_B[row * (TILE_N + 1) + col] = val;
        }

        barrier(CLK_LOCAL_MEM_FENCE);

        // === DPAS computation ===
        // Load B sub-tile for this subgroup: B[0:16, sg_id*16 : sg_id*16+16]
        // For DPAS, B (second operand) is int8 = 8 x int = 8 x (2 halves packed)
        // B matrix for DPAS: 16x16 in packed format
        // Each int8 element holds a row-pair of B
        // intel_sub_group_f16_f16_matrix_mad_k16:
        //   a: short8 - 8 rows of A, each with K=16 halves (spread across subgroup)
        //   b: int8 - 16x16 B tile packed
        //   acc: float8 - 8 rows of output

        // Load B tile from SLM into registers
        // B in SLM: slm_B[row * (TILE_N+1) + sg_id*16 + lane]
        // For DPAS b operand (int8): each work-item holds 8 ints
        // Each int packs 2 consecutive half values from the same column
        // Layout: b[i] packs row 2i and row 2i+1 for this lane's column
        __local uint* slm_B_uint = (__local uint*)(slm_B);

        // B tile: 16 rows x 16 cols for this subgroup
        // Pack into int8: 8 ints, each containing 2 halves (consecutive K rows)
        int8 b_val;
        {
            // Each lane reads its column; pack pairs of rows
            __local half* b_base = slm_B + sg_id * 16;
            uint* bp = (uint*)&b_val;
            for (int p = 0; p < 8; p++) {
                half h0 = b_base[(2*p) * (TILE_N + 1) + sg_lid];
                half h1 = b_base[(2*p+1) * (TILE_N + 1) + sg_lid];
                half2 pair = (half2)(h0, h1);
                bp[p] = as_uint(pair);
            }
        }

        // For each of 4 row-blocks of 8 rows in A
        for (int blk = 0; blk < 4; blk++) {
            // Load A sub-tile: A[blk*8..blk*8+8, 0:16] from SLM
            // short8: each work-item reads 8 halves across K dim
            // A layout in SLM: slm_A[row * (TILE_K+1) + k_col]
            // For DPAS a operand: short8 where each short is a half
            // Each lane holds one column of K; we need 8 rows x 16 K
            // Actually: a is short8, representing 8 rows, each row's K=16 spread across 16 lanes
            // Lane i holds K-element i for each of the 8 rows
            short8 a_val;
            {
                __local half* a_base = slm_A + blk * 8 * (TILE_K + 1);
                short* ap = (short*)&a_val;
                for (int r = 0; r < 8; r++) {
                    ap[r] = as_short(a_base[r * (TILE_K + 1) + sg_lid]);
                }
            }

            float8 *acc_ptr;
            if (blk == 0) acc_ptr = &acc0;
            else if (blk == 1) acc_ptr = &acc1;
            else if (blk == 2) acc_ptr = &acc2;
            else acc_ptr = &acc3;
    ...
    for (int blk = 0; blk < 4; blk++) {
        float8 acc;
        if (blk == 0) acc = acc0;
        else if (blk == 1) acc = acc1;
        else if (blk == 2) acc = acc2;
        else acc = acc3;

        float* ap = (float*)&acc;
        for (int r = 0; r < 8; r++) {
            int global_r = tile_row + blk * 8 + r;
            int global_c = tile_col + sg_lid;
            if (global_r < M && global_c < N) {
                C[global_r * N + global_c] = convert_half(ap[r]);
            }
        }
    }
...
======================= 4 passed, 1 deselected in 1.16s ========================
