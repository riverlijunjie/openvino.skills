...
Here is the last OCL kernel we tested (Result: Correctness score: 5 / 5 (compiles and correct), runtime in ms: 44.100):
```OCL
// Optimized FP16 GEMM using Intel XMX DPAS instructions
// C[M,N] = A[M,K] x B[K,N], all half precision, float accumulation
//
...
//   - 8 subgroups along M (8*8=64 rows), 2 subgroups along N (2*16=32 cols)
//   Wait - let's do 4 along M, 4 along N, each SG computes 2x(8x16) = 16x16
//   4*16=64 M, 4*16=64 N
// Each subgroup computes 16x16 output via 2 DPAS per k16 step (register blocking along M)
//
// Launch config:
//   LWS = (16, 16) = 256 WIs
    ...
    const int N)
{
    ...
    // 4 along M (each handles 16 rows = 2 DPAS blocks), 4 along N (each handles 16 cols)
    const int sg_row = sg_id / 4;   // [0..3]
    const int sg_col = sg_id % 4;   // [0..3]

    // Each subgroup computes 2 blocks of 8x16: rows [sg_row*16..sg_row*16+15], cols [sg_col*16..sg_col*16+15]
    const int out_row_start = wg_m + sg_row * 16;
    const int out_col_start = wg_n + sg_col * 16;

    // Two accumulators for register blocking along M (2 x 8 rows)
    float8 acc0 = (float8)(0.0f);  // rows 0-7
    float8 acc1 = (float8)(0.0f);  // rows 8-15

    __local half slm_a[TILE_M * SLM_A_STRIDE];  // 64 x 34
    __local half slm_b[TILE_K * SLM_B_STRIDE];  // 32 x 66

    for (int k0 = 0; k0 < K; k0 += TILE_K) {
        // Load A[wg_m..+64, k0..+32]: 64*32=2048 elements, 256 WIs -> 8 each
        for (int i = lid; i < TILE_M * TILE_K; i += NUM_WIS) {
            int row = i / TILE_K;
            int col = i % TILE_K;
            int gm = wg_m + row;
        ...
        // Load B[k0..+32, wg_n..+64]: 32*64=2048 elements, 256 WIs -> 8 each
        for (int i = lid; i < TILE_K * TILE_N; i += NUM_WIS) {
            int row = i / TILE_N;
            int col = i % TILE_N;
            int gk = k0 + row;
        ...
        #pragma unroll
        for (int ki = 0; ki < TILE_K; ki += 16) {
            // Load A block 0: rows [sg_row*16 + 0..7] x 16 K
            ...
            // Load A block 1: rows [sg_row*16 + 8..15] x 16 K
            ...
            // 2 DPAS calls sharing same B tile (register blocking along M)
