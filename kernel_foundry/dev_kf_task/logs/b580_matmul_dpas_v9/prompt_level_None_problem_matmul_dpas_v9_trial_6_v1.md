...
This is the code of the best kernel we have tested so far (Result: Correctness score: 5 / 5 (compiles and correct), runtime in ms: 23.800):
```OCL
...
Here is the last OCL kernel we tested (Result: Correctness score: 5 / 5 (compiles and correct), runtime in ms: 90.800):
```OCL
// Optimized FP16 GEMM using Intel XMX DPAS with register blocking + double buffering
// C[M,N] = A[M,K] x B[K,N], all half precision, float accumulation
//
// Tiling: TILE_M=64, TILE_N=64, TILE_K=32
// Workgroup: 8 subgroups of 16 WIs = 128 WIs
//   - 2 subgroups along M (2*32=64 rows via 4x register blocking)
//   - 4 subgroups along N (4*16=64 cols)
// Each subgroup computes 32x16 output (4 DPAS blocks of 8x16) per K=16 step
    ...
    const int N)
{
    ...
    const int lid = get_local_id(1) * get_local_size(0) + get_local_id(0);
    const int sg_id = lid / SG_SIZE;   // 0..7
    const int sg_lid = lid % SG_SIZE;  // 0..15

    // 2 along M (each handles 32 rows), 4 along N (each handles 16 cols)
    const int sg_row = sg_id / 4;   // [0..1]
    const int sg_col = sg_id % 4;   // [0..3]

    const int out_row_base = wg_m + sg_row * 32;  // start of 32-row block
    const int out_col_start = wg_n + sg_col * 16;

    // 4 accumulators per subgroup: 4 blocks of 8 rows each = 32 rows
    ...
    // 64*32=2048 A elements, 32*64=2048 B elements, 128 WIs -> 16 each
    {
        int k0 = 0;
        for (int i = lid; i < TILE_M * TILE_K; i += NUM_WIS) {
            int row = i / TILE_K;
    ...
    for (int t = 0; t < num_k_tiles; t++) {
        ...
        // Compute on current buffer: 2 K=16 steps per TILE_K=32
        #pragma unroll
        for (int ki = 0; ki < TILE_K; ki += 16) {
            // Load B sub-tile once per K step (shared across 4 row-blocks)
            ...
            // 4 row blocks: rows 0-7, 8-15, 16-23, 24-31 relative to sg_row*32
            // Block 0: rows 0-7
            {
                __local half* a_base = slm_a_comp + (sg_row * 32 + 0) * SLM_A_STRIDE + ki;
                short8 a_val;
                a_val.s0 = as_short(a_base[0 * SLM_A_STRIDE + sg_lid]);
                a_val.s1 = as_short(a_base[1 * SLM_A_STRIDE + sg_lid]);
                a_val.s2 = as_short(a_base[2 * SLM_A_STRIDE + sg_lid]);
            ...
            // Block 1: rows 8-15
            {
                __local half* a_base = slm_a_comp + (sg_row * 32 + 8) * SLM_A_STRIDE + ki;
                short8 a_val;
                a_val.s0 = as_short(a_base[0 * SLM_A_STRIDE + sg_lid]);
                a_val.s1 = as_short(a_base[1 * SLM_A_STRIDE + sg_lid]);
                a_val.s2 = as_short(a_base[2 * SLM_A_STRIDE + sg_lid]);
            ...
            // Block 2: rows 16-23
            {
                __local half* a_base = slm_a_comp + (sg_row * 32 + 16) * SLM_A_STRIDE + ki;
                short8 a_val;
                a_val.s0 = as_short(a_base[0 * SLM_A_STRIDE + sg_lid]);
                a_val.s1 = as_short(a_base[1 * SLM_A_STRIDE + sg_lid]);
                a_val.s2 = as_short(a_base[2 * SLM_A_STRIDE + sg_lid]);
            ...
            // Block 3: rows 24-31
            {
                __local half* a_base = slm_a_comp + (sg_row * 32 + 24) * SLM_A_STRIDE + ki;
                short8 a_val;
                a_val.s0 = as_short(a_base[0 * SLM_A_STRIDE + sg_lid]);
                a_val.s1 = as_short(a_base[1 * SLM_A_STRIDE + sg_lid]);
                a_val.s2 = as_short(a_base[2 * SLM_A_STRIDE + sg_lid]);
    ...
    if (out_col < N) {
        ...
        for (int r = 0; r < 8; r++) {
            ...
            if (row < M) C[row * N + out_col] = convert_half(((float*)&acc0)[r]);
        }
        // Block 1: rows 8-15
        #pragma unroll
        for (int r = 0; r < 8; r++) {
            int row = out_row_base + 8 + r;
            if (row < M) C[row * N + out_col] = convert_half(((float*)&acc1)[r]);
        }
        // Block 2: rows 16-23
        #pragma unroll
        for (int r = 0; r < 8; r++) {
            int row = out_row_base + 16 + r;
            if (row < M) C[row * N + out_col] = convert_half(((float*)&acc2)[r]);
        }
        // Block 3: rows 24-31
        #pragma unroll
        for (int r = 0; r < 8; r++) {
            int row = out_row_base + 24 + r;
            if (row < M) C[row * N + out_col] = convert_half(((float*)&acc3)[r]);
        }
    }
}
```
...
================== 4 passed, 1 deselected, 1 warning in 1.04s ==================
