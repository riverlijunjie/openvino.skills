...
This is the code of the best kernel we have tested so far (Result: Correctness score: 5 / 5 (compiles and correct), runtime in ms: 3.870):
```OCL
#pragma OPENCL EXTENSION cl_intel_subgroups : enable
#pragma OPENCL EXTENSION cl_intel_subgroup_matrix_multiply_accumulate : enable
#pragma OPENCL EXTENSION cl_khr_fp16 : enable
...
Here is the last OCL kernel we tested (Result: Correctness score: 5 / 5 (compiles and correct), runtime in ms: 6.660):
```OCL
#pragma OPENCL EXTENSION cl_intel_subgroups : enable
#pragma OPENCL EXTENSION cl_intel_subgroup_matrix_multiply_accumulate : enable
#pragma OPENCL EXTENSION cl_khr_fp16 : enable
    ...
    const int N)
{
    ...
    // Double-buffered SLM for A: 2 x 32 rows x 66 cols
    ...
    {
        int k = 0;
        // 32 rows x 64 cols = 2048 elements, 64 WIs => 32 elements each
        #pragma unroll
        for (int i = 0; i < 32; i++) {
            int elem_id = lid + i * 64;
    ...
    for (int t = 0; t < k_tiles; t++) {
        ...
        int slm_offset_cur = buf_cur * TILE_M * SLM_A_STRIDE;
        int slm_offset_nxt = buf_nxt * TILE_M * SLM_A_STRIDE;

        // Process 4 k16 sub-tiles from current buffer
        #pragma unroll
        for (int kk = 0; kk < TILE_K; kk += 16) {
            short8 a0, a1, a2, a3;
            ...
            #pragma unroll
            for (int r = 0; r < 8; r++) {
                ((short*)&a0)[r] = as_short(slm_A[slm_offset_cur + r * SLM_A_STRIDE + kk + sg_lid]);
            }
            #pragma unroll
            for (int r = 0; r < 8; r++) {
                ((short*)&a1)[r] = as_short(slm_A[slm_offset_cur + (8 + r) * SLM_A_STRIDE + kk + sg_lid]);
            }
            #pragma unroll
            for (int r = 0; r < 8; r++) {
                ((short*)&a2)[r] = as_short(slm_A[slm_offset_cur + (16 + r) * SLM_A_STRIDE + kk + sg_lid]);
            }
            #pragma unroll
            for (int r = 0; r < 8; r++) {
                ((short*)&a3)[r] = as_short(slm_A[slm_offset_cur + (24 + r) * SLM_A_STRIDE + kk + sg_lid]);
            }
            ...
            for (int p = 0; p < 8; p++) {
                int k_row0 = k + kk + 2 * p;
                int k_row1 = k + kk + 2 * p + 1;
                half b0 = (k_row0 < K && col_valid) ? B[k_row0 * N + col_idx] : (half)0.0h;
                half b1 = (k_row1 < K && col_valid) ? B[k_row1 * N + col_idx] : (half)0.0h;
                ((int*)&b_val)[p] = as_int((short2)(as_short(b0), as_short(b1)));
            }
        ...
        // Prefetch next A tile into alternate buffer (overlapped with compute above ideally)
        if (t + 1 < k_tiles) {
            ...
            for (int i = 0; i < 32; i++) {
                ...
                int gk = k_next + c;
                half val = (gr < M && gk < K) ? A[gr * K + gk] : (half)0.0h;
                slm_A[slm_offset_nxt + r * SLM_A_STRIDE + c] = val;
            }
            barrier(CLK_LOCAL_MEM_FENCE);
        }
    }
