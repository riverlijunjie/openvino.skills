...
This is the code of the best kernel we have tested so far (Result: Correctness score: 5 / 5 (compiles and correct), runtime in ms: 1.280):
```OCL
#pragma OPENCL EXTENSION cl_intel_subgroups : enable
#pragma OPENCL EXTENSION cl_intel_subgroup_matrix_multiply_accumulate : enable
#pragma OPENCL EXTENSION cl_khr_fp16 : enable
...
// WG tile: 32 rows x 64 cols, K-step: 32
// Double-buffered SLM for A: overlap loading next tile with DPAS on current
// 4 subgroups x 16 WIs = 64 WIs per WG
// Each subgroup: 4 vertical 8x16 DPAS tiles = 32 rows x 16 cols
// GWS = (ceil(N/64)*64, ceil(M/32))  LWS = (64, 1)
    ...
    const int N)
{
    ...
    const bool col_valid = col_idx < N;
    const bool row_tile_valid = (row_base + TILE_M <= M);
    const bool k_aligned = (K % TILE_K == 0);
    ...
    // Preload first A tile into buffer 0
    {
        ...
        if (row_tile_valid && (k + TILE_K <= K)) {
            ...
            for (int i = 0; i < 16; i++) {
                ...
                int r = elem_id >> 5;
                int c = elem_id & 31;
                slm_A[r * SLM_A_STRIDE + c] = A[(row_base + r) * K + c];
            }
        } else {
            #pragma unroll
            for (int i = 0; i < 16; i++) {
    ...
    for (int kt = 0; kt < num_k_tiles; kt++) {
        ...
        __local half* cur_A = slm_A + cur_buf * SLM_A_SIZE;
        __local half* nxt_A = slm_A + nxt_buf * SLM_A_SIZE;

        const int next_k = k + TILE_K;
        const bool has_next = (next_k < K);

        // Start loading next A tile into nxt_buf (overlaps with compute below)
        // We do it before the compute, relying on the barrier after compute
        // Actually we need to be careful: we must finish reading cur_buf before overwriting
        // So: read cur_buf into registers first, then start loading next, then compute

        // ---- Read A from SLM for first k16 step ----
        ...
        // ---- Read A from SLM for second k16 step ----
        ...
        // Now cur_A is fully read into registers. Load next tile into nxt_buf.
        if (has_next) {
        ...
        // ---- Load B and DPAS for second k16 ----
        {
            int8 b_val;
            int gk_base = k + 16;
            if (col_valid && (gk_base + 16 <= K)) {
                #pragma unroll
                for (int p = 0; p < 8; p++) {
                    int k_row0 = gk_base + 2 * p;
                    int k_row1 = k_row0 + 1;
...
Here is the last OCL kernel we tested (Result: Correctness score: 5 / 5 (compiles and correct), runtime in ms: 2.870):
```OCL
#pragma OPENCL EXTENSION cl_intel_subgroups : enable
#pragma OPENCL EXTENSION cl_intel_subgroup_matrix_multiply_accumulate : enable
#pragma OPENCL EXTENSION cl_khr_fp16 : enable
    ...
    const int N)
{
    ...
    const int col_sg = col_base + sg_id * 16 + sg_lid;
    const bool row_tile_full = (row_base + TILE_M <= M);
    const bool col_tile_full = (col_base + TILE_N <= N);
    ...
    {
        const int k = 0;
        // Load A: 32x32 = 1024 elems, 64 WIs, 16 each
        if (row_tile_full && (k + TILE_K <= K)) {
            #pragma unroll
            for (int i = 0; i < 16; i++) {
                int elem_id = lid + i * 64;
                int r = elem_id >> 5;
                int c = elem_id & 31;
                slm_A[0][r * SLM_A_STRIDE + c] = A[(row_base + r) * K + k + c];
            }
        } else {
            #pragma unroll
            for (int i = 0; i < 16; i++) {
        ...
        // Load B: 32x64 = 2048 elems, 64 WIs, 32 each
        if (col_tile_full && (k + TILE_K <= K)) {
            #pragma unroll
            for (int i = 0; i < 32; i++) {
                int elem_id = lid + i * 64;
                int r = elem_id >> 6;   // / 64
                int c = elem_id & 63;   // % 64
                slm_B[0][r * SLM_B_STRIDE + c] = B[(k + r) * N + col_base + c];
            }
        } else {
            #pragma unroll
            for (int i = 0; i < 32; i++) {
                ...
                int gk = k + r;
                int gc = col_base + c;
                slm_B[0][r * SLM_B_STRIDE + c] = (gk < K && gc < N) ? B[gk * N + gc] : (half)0.0h;
            }
        }
    }
    ...
    for (int kt = 0; kt < num_k_tiles; kt++) {
        ...
        // Prefetch next tile into nxt_buf (if there is a next tile)
        if (kt + 1 < num_k_tiles) {
            int k_next = (kt + 1) * TILE_K;
            bool k_next_full = (k_next + TILE_K <= K);

            if (row_tile_full && k_next_full) {
                #pragma unroll
                for (int i = 0; i < 16; i++) {
                    int elem_id = lid + i * 64;
                    int r = elem_id >> 5;
                    int c = elem_id & 31;
                    slm_A[nxt_buf][r * SLM_A_STRIDE + c] = A[(row_base + r) * K + k_next + c];
                }
            } else {
                #pragma unroll
                for (int i = 0; i < 16; i++) {
            ...
            if (col_tile_full && k_next_full) {
                #pragma unroll
                for (int i = 0; i < 32; i++) {
                    int elem_id = lid + i * 64;
                    int r = elem_id >> 6;
                    int c = elem_id & 63;
                    slm_B[nxt_buf][r * SLM_B_STRIDE + c] = B[(k_next + r) * N + col_base + c];
                }
            } else {
                #pragma unroll
                for (int i = 0; i < 32; i++) {
                    ...
                    int gk = k_next + r;
                    int gc = col_base + c;
                    slm_B[nxt_buf][r * SLM_B_STRIDE + c] = (gk < K && gc < N) ? B[gk * N + gc] : (half)0.0h;
                }
            }
        }

        // Compute from cur_buf: two k16 DPAS steps
        #pragma unroll
        for (int kk = 0; kk < TILE_K; kk += 16) {
            // Load A from SLM
            short8 a0, a1, a2, a3;
            ...
            // Load B from SLM: pack pairs of halfs into int for DPAS
            int8 b_val;
            int b_col = sg_id * 16 + sg_lid;
            #pragma unroll
            for (int p = 0; p < 8; p++) {
                int k_row0 = kk + 2 * p;
                int k_row1 = k_row0 + 1;
                short s0 = as_short(slm_B[cur_buf][k_row0 * SLM_B_STRIDE + b_col]);
                short s1 = as_short(slm_B[cur_buf][k_row1 * SLM_B_STRIDE + b_col]);
                ((int*)&b_val)[p] = as_int((short2)(s0, s1));
            }
    ...
    // Store results
    const bool col_valid = col_sg < N;
    if (col_valid) {
        if (row_tile_full) {
            #pragma unroll
            for (int r = 0; r < 8; r++)
                C[(row_base + r) * N + col_sg] = convert_half(((float*)&acc0)[r]);
            #pragma unroll
            for (int r = 0; r < 8; r++)
                C[(row_base + 8 + r) * N + col_sg] = convert_half(((float*)&acc1)[r]);
            #pragma unroll
            for (int r = 0; r < 8; r++)
                C[(row_base + 16 + r) * N + col_sg] = convert_half(((float*)&acc2)[r]);
            #pragma unroll
            for (int r = 0; r < 8; r++)
                C[(row_base + 24 + r) * N + col_sg] = convert_half(((float*)&acc3)[r]);
        } else {
            #pragma unroll
            for (int r = 0; r < 8; r++) {
                if (row_base + r < M)
...
================== 4 passed, 1 deselected, 1 warning in 0.73s ==================
