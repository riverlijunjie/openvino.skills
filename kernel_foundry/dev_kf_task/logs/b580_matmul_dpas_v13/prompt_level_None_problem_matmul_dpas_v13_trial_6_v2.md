...
### Version 1 (Result: Correctness score: 5 / 5 (compiles and correct), runtime in ms: 1.180):
```OCL
...
Here is the last OCL kernel we tested (Result: Correctness score: 5 / 5 (compiles and correct), runtime in ms: 2.570):
```OCL
#pragma OPENCL EXTENSION cl_intel_subgroups : enable
#pragma OPENCL EXTENSION cl_intel_subgroup_matrix_multiply_accumulate : enable
#pragma OPENCL EXTENSION cl_khr_fp16 : enable
...
// WG tile: 64M x 128N x 16K
// 8 subgroups x 16 WIs = 128 WIs per WG
// Each subgroup: 64 rows x 16 cols = 8 DPAS(8x16) tiles per k-step
// A in SLM (double-buffered): 2 x 64 x 16 x 2B = 4 KB
// B loaded directly from global memory per subgroup
    ...
    const int N)
{
    ...
    // 8 accumulators: 8 blocks of 8 rows x 16 cols
    ...
    // A load: 64 x 16 = 1024 elements, 1024/128 = 8 per WI
    ...
    for (int kt = 0; kt < num_k_tiles; kt++) {
        ...
        // Load A from SLM - 8 blocks of 8 rows
        ...
        #pragma unroll
        for (int r = 0; r < 8; r++)
            ((ushort*)&a4)[r] = intel_sub_group_block_read_us(cur_a + (32 + r) * TILE_K);
        #pragma unroll
        for (int r = 0; r < 8; r++)
            ((ushort*)&a5)[r] = intel_sub_group_block_read_us(cur_a + (40 + r) * TILE_K);
        #pragma unroll
        for (int r = 0; r < 8; r++)
            ((ushort*)&a6)[r] = intel_sub_group_block_read_us(cur_a + (48 + r) * TILE_K);
        #pragma unroll
        for (int r = 0; r < 8; r++)
            ((ushort*)&a7)[r] = intel_sub_group_block_read_us(cur_a + (56 + r) * TILE_K);
        ...
        // 8 DPAS operations - B is reused across all 8 row blocks
        acc0 = intel_sub_group_f16_f16_matrix_mad_k16(a0, b_val, acc0);
        acc1 = intel_sub_group_f16_f16_matrix_mad_k16(a1, b_val, acc1);
        acc2 = intel_sub_group_f16_f16_matrix_mad_k16(a2, b_val, acc2);
        acc3 = intel_sub_group_f16_f16_matrix_mad_k16(a3, b_val, acc3);
