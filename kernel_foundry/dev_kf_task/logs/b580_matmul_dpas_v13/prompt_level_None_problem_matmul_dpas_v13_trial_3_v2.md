...
### Version 1 (Result: Correctness score: 5 / 5 (compiles and correct), runtime in ms: 4.730):
```OCL
...
Here is the last OCL kernel we tested (Result: Correctness score: 5 / 5 (compiles and correct), runtime in ms: 5.140):
```OCL
#pragma OPENCL EXTENSION cl_intel_subgroups : enable
#pragma OPENCL EXTENSION cl_intel_subgroup_matrix_multiply_accumulate : enable
#pragma OPENCL EXTENSION cl_khr_fp16 : enable
...
// 8 subgroups x 16 WIs = 128 WIs per WG
// Each subgroup: 4 vertical 8x16 DPAS tiles = 32 rows x 16 cols
// A(32x32) and B(32x128) cached in SLM, double-buffered
// SLM: 2 * (32*32 + 32*128) * 2 bytes = 20 KB per WG
// GWS = (ceil(N/128)*128, ceil(M/32))  LWS = (128, 1)
// Subgroup size = 16
    ...
    const int N)
{
    ...
    // --- Load first tile into buffer 0 ---
    // Use strided access pattern for coalesced global memory reads
    {
        __local ushort* slm_a = slm;
        __local ushort* slm_b = slm + SLM_A_TILE;

        // Load A: 1024 elements / 128 WIs = 8 per WI, strided
        #pragma unroll
        for (int i = 0; i < 8; i++) {
            int elem = lid + i * 128;
            int r = elem >> 5;   // / 32
        ...
        // Load B: 4096 elements / 128 WIs = 32 per WI, strided
        #pragma unroll
        for (int i = 0; i < 32; i++) {
            int elem = lid + i * 128;
            int r = elem >> 7;   // / 128
    ...
    for (int kt = 0; kt < num_k_tiles; kt++) {
        ...
        __local const ushort* cur_a = slm + cur_buf * SLM_TILE_TOTAL;
        __local const ushort* cur_b = slm + cur_buf * SLM_TILE_TOTAL + SLM_A_TILE;
        ...
        // ---- LOAD next tile (software pipelining) ----
        if (has_next) {
            barrier(CLK_LOCAL_MEM_FENCE);
            ...
            // Load A: strided access for coalescing
            #pragma unroll
            for (int i = 0; i < 8; i++) {
                int elem = lid + i * 128;
                int r = elem >> 5;
            ...
            // Load B: strided access for coalescing
            #pragma unroll
            for (int i = 0; i < 32; i++) {
                int elem = lid + i * 128;
                int r = elem >> 7;
