...
Here is the last OCL kernel we tested (Result: Correctness score: 5 / 5 (compiles and correct), runtime in ms: 3.130):
```OCL
#pragma OPENCL EXTENSION cl_intel_subgroups : enable
#pragma OPENCL EXTENSION cl_intel_subgroup_matrix_multiply_accumulate : enable
#pragma OPENCL EXTENSION cl_khr_fp16 : enable
...
// 16 subgroups x 16 WIs = 256 WIs per WG
// Each subgroup: 32 rows x 16 cols = 4 DPAS(8x16)
// Both A and B in SLM for maximum reuse:
//   A[32x16] reused 16x across N-subgroups = 16KB saved
//   B[16x256] reused 4x across M-rows (32/8)
// SLM per buffer: 32*16 + 16*256 = 512 + 4096 = 4608 ushorts = 9.2KB
// Double-buffered: ~18.4KB (fits in 128KB SLM)
// Cooperative load: 256 WIs load A(512 elts) + B(4096 elts) = 4608 elts, 18 per WI
// GWS = (ceil(N/256)*256, ceil(M/32))  LWS = (256, 1)
// Subgroup size = 16
...
#define TILE_N 256
#define TILE_K 16
#define SLM_A_ELTS (TILE_M * TILE_K)       // 512
#define SLM_B_ELTS (TILE_K * TILE_N)       // 4096
#define SLM_BUF_ELTS (SLM_A_ELTS + SLM_B_ELTS)  // 4608
#define WG_SIZE 256
    ...
    const int N)
{
    ...
    __local ushort slm[2 * SLM_BUF_ELTS];
    ...
    // Helper: load A[32x16] and B[16x256] into SLM buffer
    // A: 512 elts, B: 4096 elts, total: 4608 elts
    // 4608 / 256 = 18 elements per WI

    // Load first tile into buffer 0
    {
        __local ushort* dst_a = slm;
        __local ushort* dst_b = slm + SLM_A_ELTS;
        ...
        for (int i = 0; i < 16; i++) {
            ...
            int bc = eid % TILE_N;   // col in [0,255]
            int gn = n_base + bc;
            dst_b[eid] = (br < K && gn < N) ? B_us[br * N + gn] : (ushort)0;
        }
    }
    barrier(CLK_LOCAL_MEM_FENCE);
    ...
    for (int kt = 0; kt < num_k_tiles; kt++) {
        ...
        __local const ushort* ca = slm + cur_buf * SLM_BUF_ELTS;
        __local const ushort* cb = slm + cur_buf * SLM_BUF_ELTS + SLM_A_ELTS;

        // Read A from SLM: 4 blocks of 8 rows x k16
        // A is stored row-major [32][16], subgroup reads broadcast via block_read
        short8 a0, a1, a2, a3;
        #pragma unroll
        for (int r = 0; r < 8; r++)
            ((ushort*)&a0)[r] = intel_sub_group_block_read_us(ca + r * TILE_K);
        ...
        // Read B from SLM: B is [16][256], each SG needs cols [sg_id*16, sg_id*16+15]
        // Pack into int8 (pairs of k-rows)
        int8 b_val;
        __local const ushort* bp = cb + sg_id * 16;  // offset to this SG's 16 columns
        #pragma unroll
        for (int p = 0; p < 8; p++) {
        ...
        if (kt + 1 < num_k_tiles) {
            const int next_buf = 1 - cur_buf;
            const int next_k = (kt + 1) * TILE_K;
            __local ushort* dst_a = slm + next_buf * SLM_BUF_ELTS;
            __local ushort* dst_b = slm + next_buf * SLM_BUF_ELTS + SLM_A_ELTS;
        ...
        // DPAS compute - can overlap with SLM writes on Xe2
        acc0 = intel_sub_group_f16_f16_matrix_mad_k16(a0, b_val, acc0);
        acc1 = intel_sub_group_f16_f16_matrix_mad_k16(a1, b_val, acc1);
        acc2 = intel_sub_group_f16_f16_matrix_mad_k16(a2, b_val, acc2);
        acc3 = intel_sub_group_f16_f16_matrix_mad_k16(a3, b_val, acc3);
