...
Here is the last OCL kernel we tested (Result: Correctness score: 5 / 5 (compiles and correct), runtime in ms: 5.150):
```OCL
#pragma OPENCL EXTENSION cl_intel_subgroups : enable
#pragma OPENCL EXTENSION cl_intel_subgroup_matrix_multiply_accumulate : enable
#pragma OPENCL EXTENSION cl_khr_fp16 : enable
...
// WG tile: 32M x 128N, K-step=32 (two k16 DPAS rounds per SLM load)
// 8 subgroups x 16 WIs = 128 WIs per WG
// Each subgroup: 32 rows x 16 cols = 4 DPAS(8x16) per k16 step, 8 DPAS per k32 step
// Both A and B in SLM for maximum reuse:
//   A: 32x32 = 2KB per buffer, reused by 8 SGs (N-direction)
//   B: 32x128 = 8KB per buffer, reused by 1 SG group (M-direction)
//   Double-buffered: 2*(2KB+8KB) = 20KB << 128KB SLM
...
#define SLM_BUF_SIZE (SLM_A_SIZE + SLM_B_SIZE)  // 10KB per buffer
#define WG_SIZE 128
#define NUM_SG 8
    ...
    const int N)
{
    ...
    const int n_base   = get_group_id(0) * TILE_N;
    const int row_base = get_group_id(1) * TILE_M;
    // Each of 8 subgroups handles a 16-col slice
    const int col_base = n_base + sg_id * 16;
    ...
    // A: 32*32 = 1024, B: 32*128 = 4096, total = 5120
    // 5120 / 128 WIs = 40 elements per WI
    // Split: A = 1024/128 = 8 per WI, B = 4096/128 = 32 per WI

    // Helper macro for loading A and B into a given SLM buffer
    #define LOAD_TILE(buf_offset, k_start) \
    { \
        __local ushort* dst_a = slm + (buf_offset) * SLM_BUF_SIZE; \
        __local ushort* dst_b = dst_a + SLM_A_SIZE; \
        /* Load A: 1024 elements, 8 per WI */ \
        _Pragma("unroll") \
        for (int i = 0; i < 8; i++) { \
            int eid = lid + i * WG_SIZE; \
            int ar = eid / TILE_K; \
        ...
        /* Load B: 4096 elements, 32 per WI */ \
        _Pragma("unroll") \
        for (int i = 0; i < 32; i++) { \
            int eid = lid + i * WG_SIZE; \
            int br = eid / TILE_N; \
    ...
    for (int kt = 0; kt < num_k_tiles; kt++) {
        ...
        __local const ushort* cb = ca + SLM_A_SIZE;

        // B is stored row-major in SLM: B[k_local][n_local], stride = TILE_N
        // For this SG, col offset within tile = sg_id * 16
        const int b_col_off = sg_id * 16;
        ...
        // ======== K16 step 0 ========
        {
            ...
            #pragma unroll
            for (int p = 0; p < 8; p++) {
                ushort s0 = intel_sub_group_block_read_us(cb + (2 * p) * TILE_N + b_col_off);
                ushort s1 = intel_sub_group_block_read_us(cb + (2 * p + 1) * TILE_N + b_col_off);
                ((int*)&b_val)[p] = as_int((ushort2)(s0, s1));
            }
        ...
        // ======== K16 step 1 ========
        {
            ...
            #pragma unroll
            for (int p = 0; p < 8; p++) {
                ushort s0 = intel_sub_group_block_read_us(cb + (K16 + 2 * p) * TILE_N + b_col_off);
                ushort s1 = intel_sub_group_block_read_us(cb + (K16 + 2 * p + 1) * TILE_N + b_col_off);
                ((int*)&b_val)[p] = as_int((ushort2)(s0, s1));
            }
        ...
        // Prefetch next tile while DPAS results are being written back
        if (kt + 1 < num_k_tiles) {
            barrier(CLK_LOCAL_MEM_FENCE);
