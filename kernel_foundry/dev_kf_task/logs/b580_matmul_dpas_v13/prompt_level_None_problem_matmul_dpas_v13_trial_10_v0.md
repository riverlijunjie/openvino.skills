...
This is the code of the best kernel we have tested so far (Result: Correctness score: 5 / 5 (compiles and correct), runtime in ms: 1.160):
```OCL
#pragma OPENCL EXTENSION cl_intel_subgroups : enable
#pragma OPENCL EXTENSION cl_intel_subgroup_matrix_multiply_accumulate : enable
#pragma OPENCL EXTENSION cl_khr_fp16 : enable
...
Here is the last OCL kernel we tested (Result: Correctness score: 5 / 5 (compiles and correct), runtime in ms: 3.370):
```OCL
#pragma OPENCL EXTENSION cl_intel_subgroups : enable
#pragma OPENCL EXTENSION cl_intel_subgroup_matrix_multiply_accumulate : enable
#pragma OPENCL EXTENSION cl_khr_fp16 : enable
...
// Tile: 64M x 128N, K-step=32 (2x k16 DPAS rounds per SLM load)
// 8 subgroups x 16 WIs = 128 WIs per WG
// Each subgroup: 64 rows x 16 cols = 8 DPAS(8x16) per k16 step
// A in SLM (shared across 8 subgroups = 8x reuse), double-buffered
// B loaded directly from global (each SG loads its own 16x16 block)
// SLM usage: 2 * 64*32 * 2 bytes = 8 KB (well within 128KB limit)
// GWS = (ceil(N/128)*128, ceil(M/64))  LWS = (128, 1)
// Subgroup size = 16
// Compute per k32 step: 8 SG * 8 DPAS * 2 rounds * 8*16*16 = 262144 FLOPs
// Memory per k32 step: A = 64*32*2 = 4KB, B = 8*16*32*2 = 8KB => 12KB

#define TILE_M 64
#define TILE_N 128
#define TILE_K 32
#define K16 16
#define SLM_A_SIZE (TILE_M * TILE_K)  // 2048 ushorts = 4KB per buffer
#define WG_SIZE 128
#define NUM_SG 8
    ...
    const int N)
{
    ...
    // 8 accumulators: rows [0..7], [8..15], ..., [56..63] x 16 cols
    float8 acc0 = 0.0f;
    float8 acc1 = 0.0f;
    float8 acc2 = 0.0f;
    float8 acc3 = 0.0f;
    ...
    // Load first A tile into buffer 0
    // 64*32 = 2048 elements, 2048/128 = 16 per WI
    {
        __local ushort* dst = slm_a;
        #pragma unroll
        for (int i = 0; i < 16; i++) {
            int eid = lid + i * WG_SIZE;
            int ar = eid / TILE_K;  // row within tile [0..63]
            int ac = eid % TILE_K;  // col within tile [0..31]
            int gr = row_base + ar;
            dst[eid] = (gr < M && ac < K) ? A_us[gr * K + ac] : (ushort)0;
        }
    }
    ...
    for (int kt = 0; kt < num_k_tiles; kt++) {
        ...
        // ======== K16 step 0 (columns 0..15 of A tile) ========
        {
            ...
            #pragma unroll
            for (int r = 0; r < 8; r++)
                ((ushort*)&a4)[r] = intel_sub_group_block_read_us(ca + (32 + r) * TILE_K);
            #pragma unroll
            for (int r = 0; r < 8; r++)
                ((ushort*)&a5)[r] = intel_sub_group_block_read_us(ca + (40 + r) * TILE_K);
            #pragma unroll
            for (int r = 0; r < 8; r++)
                ((ushort*)&a6)[r] = intel_sub_group_block_read_us(ca + (48 + r) * TILE_K);
            #pragma unroll
            for (int r = 0; r < 8; r++)
                ((ushort*)&a7)[r] = intel_sub_group_block_read_us(ca + (56 + r) * TILE_K);
        ...
        // ======== K16 step 1 (columns 16..31 of A tile) ========
        {
            int k1 = k_offset + K16;
            ...
            #pragma unroll
            for (int r = 0; r < 8; r++)
                ((ushort*)&a4)[r] = intel_sub_group_block_read_us(ca + (32 + r) * TILE_K + K16);
            #pragma unroll
            for (int r = 0; r < 8; r++)
                ((ushort*)&a5)[r] = intel_sub_group_block_read_us(ca + (40 + r) * TILE_K + K16);
            #pragma unroll
            for (int r = 0; r < 8; r++)
                ((ushort*)&a6)[r] = intel_sub_group_block_read_us(ca + (48 + r) * TILE_K + K16);
            #pragma unroll
            for (int r = 0; r < 8; r++)
                ((ushort*)&a7)[r] = intel_sub_group_block_read_us(ca + (56 + r) * TILE_K + K16);
    ...
    if (col_idx < N) {
        ...
        for (int r = 0; r < 8; r++) {
            ...
            if (gr < M) C_us[gr * N + col_idx] = as_ushort(convert_half(((float*)&acc4)[r]));
        }
        #pragma unroll
        for (int r = 0; r < 8; r++) {
            int gr = row_base + 40 + r;
            if (gr < M) C_us[gr * N + col_idx] = as_ushort(convert_half(((float*)&acc5)[r]));
        }
        #pragma unroll
        for (int r = 0; r < 8; r++) {
            int gr = row_base + 48 + r;
            if (gr < M) C_us[gr * N + col_idx] = as_ushort(convert_half(((float*)&acc6)[r]));
        }
        #pragma unroll
        for (int r = 0; r < 8; r++) {
            int gr = row_base + 56 + r;
            if (gr < M) C_us[gr * N + col_idx] = as_ushort(convert_half(((float*)&acc7)[r]));
        }
    }
}
```
