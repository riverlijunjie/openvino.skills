...
### Version 1 (Result: Correctness score: 5 / 5 (compiles and correct), runtime in ms: 4.350):
```OCL
...
This is the code of the best kernel we have tested so far (Result: Correctness score: 5 / 5 (compiles and correct), runtime in ms: 1.720):
```OCL
#pragma OPENCL EXTENSION cl_intel_subgroups : enable
#pragma OPENCL EXTENSION cl_intel_subgroup_matrix_multiply_accumulate : enable
#pragma OPENCL EXTENSION cl_khr_fp16 : enable
...
Here is the last OCL kernel we tested (Result: Correctness score: 5 / 5 (compiles and correct), runtime in ms: 4.550):
```OCL
#pragma OPENCL EXTENSION cl_intel_subgroups : enable
#pragma OPENCL EXTENSION cl_intel_subgroup_matrix_multiply_accumulate : enable
#pragma OPENCL EXTENSION cl_khr_fp16 : enable
...
// Tile: 32M x 128N x 32K, double-buffered SLM
// 8 subgroups x 16 WIs = 128 WIs per WG
// Each subgroup: 4 DPAS tiles (32 rows x 16 cols), 2 k16 steps per K tile
// 8 DPAS per subgroup per K tile = good compute density
// SLM: 2 * (32*32 + 32*128) * 2 bytes = 20 KB
// GWS = (ceil(N/128)*128, ceil(M/32))  LWS = (128, 1)
    ...
    const int N)
{
    ...
    // Load tile into specified SLM buffer
    // A: 32x32 = 1024 elems, 1024/128 = 8 per WI
    // B: 32x128 = 4096 elems, 4096/128 = 32 per WI
    // Use vectorized loads where possible

    // --- Load first tile into buffer 0 ---
    {
        __local ushort* sa = slm;
        __local ushort* sb = slm + SLM_A;

        // Load A - use stride pattern for better coalescing
        // lid / 16 = subgroup (0..7), lid % 16 = lane
        // Map: each WI loads 8 elements
        #pragma unroll
        for (int i = 0; i < 8; i++) {
            int eid = lid * 8 + i;
            int ar = eid >> 5;  // / 32
            int ac = eid & 31;  // % 32
            int gr = row_base + ar;
            sa[eid] = (gr < M && ac < K) ? A_us[gr * K + ac] : (ushort)0;
        }

        // Load B - vectorized: each WI loads 32 elements
        #pragma unroll
        for (int i = 0; i < 32; i++) {
            int eid = lid * 32 + i;
            int br = eid >> 7;  // / 128
            int bc = eid & 127; // % 128
            int gc = n_base + bc;
            sb[eid] = (br < K && gc < N) ? B_us[br * N + gc] : (ushort)0;
        }
    }
    barrier(CLK_LOCAL_MEM_FENCE);
    ...
    for (int kt = 0; kt < num_k_tiles; kt++) {
        __local const ushort* ca = slm + cur_buf * SLM_BUF;
        __local const ushort* cb = ca + SLM_A + sg_id * 16;
        ...
        #pragma unroll
        for (int r = 0; r < 8; r++)
            ((ushort*)&a0)[r] = intel_sub_group_block_read_us(ca + r * TK);
        #pragma unroll
        for (int r = 0; r < 8; r++)
            ((ushort*)&a1)[r] = intel_sub_group_block_read_us(ca + (8+r) * TK);
        #pragma unroll
        for (int r = 0; r < 8; r++)
            ((ushort*)&a2)[r] = intel_sub_group_block_read_us(ca + (16+r) * TK);
        #pragma unroll
        for (int r = 0; r < 8; r++)
            ((ushort*)&a3)[r] = intel_sub_group_block_read_us(ca + (24+r) * TK);

        #pragma unroll
        for (int p = 0; p < 8; p++) {
            ushort s0 = intel_sub_group_block_read_us(cb + (2*p) * TN);
            ushort s1 = intel_sub_group_block_read_us(cb + (2*p+1) * TN);
            ((int*)&bv)[p] = as_int((ushort2)(s0, s1));
        }
        ...
        // ---- k16 step 1 ----
        __local const ushort* ca2 = ca + 16;
        __local const ushort* cb2 = cb + 16 * TN;

        #pragma unroll
        for (int r = 0; r < 8; r++)
            ((ushort*)&a0)[r] = intel_sub_group_block_read_us(ca2 + r * TK);
        #pragma unroll
        for (int r = 0; r < 8; r++)
            ((ushort*)&a1)[r] = intel_sub_group_block_read_us(ca2 + (8+r) * TK);
        #pragma unroll
        for (int r = 0; r < 8; r++)
            ((ushort*)&a2)[r] = intel_sub_group_block_read_us(ca2 + (16+r) * TK);
        #pragma unroll
        for (int r = 0; r < 8; r++)
            ((ushort*)&a3)[r] = intel_sub_group_block_read_us(ca2 + (24+r) * TK);

        #pragma unroll
        for (int p = 0; p < 8; p++) {
            ushort s0 = intel_sub_group_block_read_us(cb2 + (2*p) * TN);
            ushort s1 = intel_sub_group_block_read_us(cb2 + (2*p+1) * TN);
            ((int*)&bv)[p] = as_int((ushort2)(s0, s1));
        }
        ...
        if (kt + 1 < num_k_tiles) {
            int nb = 1 - cur_buf;
            __local ushort* na = slm + nb * SLM_BUF;
            __local ushort* nbs = na + SLM_A;
            int nk = (kt + 1) * TK;
            ...
            for (int i = 0; i < 32; i++) {
                ...
                int gk = nk + br;
                int gc = n_base + bc;
                nbs[eid] = (gk < K && gc < N) ? B_us[gk * N + gc] : (ushort)0;
            }
    ...
    // ---- Store ----
    const int ci = n_base + sg_id * 16 + sg_lid;
    if (ci < N) {
        __global half* cp = C + ci;
        #pragma unroll
        for (int r = 0; r < 8; r++) {
            int gr = row_base + r;
            if (gr < M) cp[gr * N] = convert_half(((float*)&acc0)[r]);
        }
        #pragma unroll
        for (int r = 0; r < 8; r++) {
            int gr = row_base + 8 + r;
            if (gr < M) cp[gr * N] = convert_half(((float*)&acc1)[r]);
        }
        #pragma unroll
        for (int r = 0; r < 8; r++) {
            int gr = row_base + 16 + r;
            if (gr < M) cp[gr * N] = convert_half(((float*)&acc2)[r]);
        }
        #pragma unroll
        for (int r = 0; r < 8; r++) {
            int gr = row_base + 24 + r;
            if (gr < M) cp[gr * N] = convert_half(((float*)&acc3)[r]);
        }
    }
}
```
