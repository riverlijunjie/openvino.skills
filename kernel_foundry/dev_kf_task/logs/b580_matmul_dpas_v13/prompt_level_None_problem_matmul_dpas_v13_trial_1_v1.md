...
// =============================================================================
// Optimized FP16 GEMM: C[M,N] = A[M,K] * B[K,N]  (row-major, half I/O, float acc)
//
// WG tile: 64 rows x 64 cols,  K-step: 32
// 8 subgroups x 16 WIs = 128 WIs per WG
// Subgroup layout: 2 rows x 4 cols of subgroups
//   - Each subgroup computes 32 rows x 16 cols (4 DPAS 8x16 blocks stacked)
//   - sg_row = sg_id / 4  (0..1), sg_col = sg_id % 4  (0..3)
//   - row offset: sg_row * 32, col offset: sg_col * 16
//
// SLM double-buffered: A tile (64x32 = 4KB) + B tile (32x64 = 4KB) = 8KB per buffer
//   Total SLM: 16KB (well within 128KB limit)
//
// A in SLM: 64 rows x 32 cols, stored row-major with 32-half stride
// B in SLM: 32 rows x 64 cols, stored row-major with 64-half stride
//
// DPAS: intel_sub_group_f16_f16_matrix_mad_k16(short8 a, int8 b, float8 acc)
//   a: 8 rows x 16 cols (each WI holds 8 halfs = short8, broadcast across subgroup)
//   b: 16 rows x 16 cols (each WI holds 16 halfs packed as int8)
//   acc: 8 rows x 16 cols (each WI holds 8 floats = float8)
//
...
// SLM sizes in ushort (half)
#define SLM_A_SIZE (TILE_M * TILE_K)   // 64*32 = 2048
#define SLM_B_SIZE (TILE_K * TILE_N)   // 32*64 = 2048
#define SLM_BUF_SIZE (SLM_A_SIZE + SLM_B_SIZE)  // 4096
    ...
    const int N)
{
    ...
    // Subgroup position within the WG tile
    const int sg_row = sg_id >> 2;   // 0 or 1
    const int sg_col = sg_id & 3;    // 0..3
    const int sg_m_off = sg_row * SG_TILE_M;  // 0 or 32
    const int sg_n_off = sg_col * SG_TILE_N;  // 0, 16, 32, 48

    // Double-buffered SLM
    __local ushort slm[2 * SLM_BUF_SIZE];
    // Buffer b: A at slm[b*SLM_BUF_SIZE], B at slm[b*SLM_BUF_SIZE + SLM_A_SIZE]

    // Early exit for fully out-of-bounds WGs
    if (row_base >= M && n_base >= N)
        return;

    // Accumulators: 4 blocks of 8x16 = 32 rows x 16 cols
    float8 acc0 = 0.0f;
    float8 acc1 = 0.0f;
    float8 acc2 = 0.0f;
    float8 acc3 = 0.0f;
    ...
    const int num_k_tiles = (K + TILE_K - 1) >> 5;

    // Cooperative load helpers:
    // 128 WIs load 2048 A elements: each WI loads 16 elements
    // 128 WIs load 2048 B elements: each WI loads 16 elements

    // Precompute A load indices (64 rows x 32 cols = 2048 elements, 128 WIs x 16 each)
    // Element id = lid + i*128, row = id/32, col = id%32
    // Precompute B load indices (32 rows x 64 cols = 2048 elements)
    // Element id = lid + i*128, row = id/64, col = id%64

    // ---- Load first tile into buffer 0 ----
    {
        __local ushort* a_slm = slm;
        __local ushort* b_slm = slm + SLM_A_SIZE;
        ...
        for (int i = 0; i < 16; i++) {
            ...
            int grow = row_base + ar;
            a_slm[eid] = (grow < M && ac < K) ? A_us[grow * K + ac] : (ushort)0;
        }
        ...
        for (int i = 0; i < 16; i++) {
            int eid = lid + i * WG_SIZE;
            int br = eid >> 6;       // eid / 64
            int bc = eid & 63;       // eid % 64
            int gcol = n_base + bc;
            b_slm[eid] = (br < K && gcol < N) ? B_us[br * N + gcol] : (ushort)0;
        }
    }
    barrier(CLK_LOCAL_MEM_FENCE);
    ...
    for (int kt = 0; kt < num_k_tiles; kt++) {
        ...
        // Pointers for this subgroup's A and B data in SLM
        __local const ushort* sg_a = cur_a + sg_m_off * TILE_K;  // start row for this sg
        __local const ushort* sg_b = cur_b + sg_n_off;           // start col for this sg

        // ---- k16 step 0 (columns 0..15 of K-tile) ----
        {
            short8 a0, a1, a2, a3;

            // Load A sub-tile: rows [sg_m_off..sg_m_off+31], cols [0..15]
            // A is stored row-major with stride TILE_K=32 in SLM
            // intel_sub_group_block_read_us reads 16 consecutive ushorts across subgroup
            // For A: we need column-wise broadcast. Each row has 32 halfs.
            // We read one half per row per WI (the WI's k-column), broadcast across subgroup.
            #pragma unroll
            for (int r = 0; r < 8; r++)
            ...
            // Load B sub-tile: rows [0..15], cols [sg_n_off..sg_n_off+15]
            // B in SLM is row-major with stride TILE_N=64
            // We need int8 b: 16 rows x 16 cols packed as pairs
            // Each int = 2 consecutive halfs from same column pair
            int8 b_val;
            #pragma unroll
            for (int p = 0; p < 8; p++) {
                ushort s0 = intel_sub_group_block_read_us(sg_b + (2 * p) * TILE_N);
                ushort s1 = intel_sub_group_block_read_us(sg_b + (2 * p + 1) * TILE_N);
                ((int*)&b_val)[p] = as_int((ushort2)(s0, s1));
            }
        ...
        // ---- k16 step 1 (columns 16..31 of K-tile) ----
        {
            short8 a0, a1, a2, a3;
            __local const ushort* sg_a2 = sg_a + 16;  // offset by 16 in K dimension
            __local const ushort* sg_b2 = cur_b + 16 * TILE_N + sg_n_off;  // B rows 16..31
            ...
            #pragma unroll
            for (int p = 0; p < 8; p++) {
                ushort s0 = intel_sub_group_block_read_us(sg_b2 + (2 * p) * TILE_N);
                ushort s1 = intel_sub_group_block_read_us(sg_b2 + (2 * p + 1) * TILE_N);
                ((int*)&b_val)[p] = as_int((ushort2)(s0, s1));
            }
        ...
        if (has_next) {
            ...
            for (int i = 0; i < 16; i++) {
                ...
                int grow = row_base + ar;
                int gk = next_k + ac;
                next_a[eid] = (grow < M && gk < K) ? A_us[grow * K + gk] : (ushort)0;
            }
            ...
            for (int i = 0; i < 16; i++) {
                ...
                int gk = next_k + br;
                int gcol = n_base + bc;
                next_b[eid] = (gk < K && gcol < N) ? B_us[gk * N + gcol] : (ushort)0;
            }
    ...
    // ---- Store results ----
    const int out_col = n_base + sg_n_off + sg_lid;
    const bool col_valid = (out_col < N);

    if (col_valid) {
        __global half* C_ptr = C + out_col;
        const int r_base = row_base + sg_m_off;

        #pragma unroll
        for (int r = 0; r < 8; r++) {
            int grow = r_base + r;
            if (grow < M) C_ptr[grow * N] = convert_half(((float*)&acc0)[r]);
        }
        #pragma unroll
        for (int r = 0; r < 8; r++) {
            int grow = r_base + 8 + r;
            if (grow < M) C_ptr[grow * N] = convert_half(((float*)&acc1)[r]);
        }
        #pragma unroll
        for (int r = 0; r < 8; r++) {
            int grow = r_base + 16 + r;
            if (grow < M) C_ptr[grow * N] = convert_half(((float*)&acc2)[r]);
        }
        #pragma unroll
        for (int r = 0; r < 8; r++) {
            int grow = r_base + 24 + r;
            if (grow < M) C_ptr[grow * N] = convert_half(((float*)&acc3)[r]);
        }
    }
}
```
