...
Here is the last OCL kernel we tested (Result: Correctness score: 5 / 5 (compiles and correct), runtime in ms: 1.160):
```OCL
#pragma OPENCL EXTENSION cl_intel_subgroups : enable
#pragma OPENCL EXTENSION cl_intel_subgroup_matrix_multiply_accumulate : enable
#pragma OPENCL EXTENSION cl_khr_fp16 : enable
...
// WG tile: 32M x 256N, K-step=32 (2 x k16 DPAS per SLM load)
// 16 subgroups x 16 WIs = 256 WIs per WG
// Each subgroup: 32 rows x 16 cols = 4 DPAS(8x16) per k16 step
// A in SLM double-buffered: 2 * 32*32 * 2 = 4 KB
// B loaded directly from global (each SG loads its own 16-wide strip)
// GWS = (ceil(N/256)*256, ceil(M/32))  LWS = (256, 1)
// Subgroup size = 16
...
#define WG_SIZE 256
#define NUM_SG 16
// Elements per WI for A load: 32*32/256 = 4
#define A_ELEMS_PER_WI 4
    ...
    const int N)
{
    ...
    // Load first A tile (k=0..31) into buffer 0
    {
        __local ushort* dst = slm_a;
        #pragma unroll
