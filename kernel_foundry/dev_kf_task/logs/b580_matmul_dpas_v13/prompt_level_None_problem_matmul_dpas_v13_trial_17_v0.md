...
### Version 1 (Result: Correctness score: 5 / 5 (compiles and correct), runtime in ms: 1.150):
```OCL
...
Here is the last OCL kernel we tested (Result: Correctness score: 5 / 5 (compiles and correct), runtime in ms: 4.700):
```OCL
#pragma OPENCL EXTENSION cl_intel_subgroups : enable
#pragma OPENCL EXTENSION cl_intel_subgroup_matrix_multiply_accumulate : enable
#pragma OPENCL EXTENSION cl_khr_fp16 : enable
...
// =============================================================================
// Optimized FP16 GEMM for Intel Xe2 (B580)
...
// A in SLM (32x32 = 2KB per buffer, double-buffered = 4KB)
// B in SLM (32x256 = 16KB per buffer, double-buffered = 32KB)
// Total SLM: 36KB (within 128KB limit)
// A reused 16x across subgroups, B reused implicitly (from SLM vs global)
//
// GWS = (ceil(N/256)*256, ceil(M/32))  LWS = (256, 1)
// Subgroup size = 16
// =============================================================================
...
#define SLM_BUF_SIZE (SLM_A_SIZE + SLM_B_SIZE) // 9216 ushorts = 18KB per buffer
#define WG_SIZE 256
#define NUM_SG 16
    ...
    const int N)
{
    ...
    // Layout: [buf0_A (1024)][buf0_B (8192)][buf1_A (1024)][buf1_B (8192)]
    ...
    // === Cooperative load of first tile into buffer 0 ===
    // A: 32 x 32 = 1024 elements, 1024/256 = 4 per WI
    // B: 32 x 256 = 8192 elements, 8192/256 = 32 per WI
    {
    ...
    for (int kt = 0; kt < num_k_tiles; kt++) {
        ...
        for (int ks = 0; ks < 2; ks++) {
            ...
            // Read A from SLM: 32 rows, k16 columns
            short8 a0, a1, a2, a3;
            #pragma unroll
            for (int r = 0; r < 8; r++)
                ((ushort*)&a0)[r] = intel_sub_group_block_read_us(cur_a + r * SLM_A_STRIDE + k16_off);
            #pragma unroll
            for (int r = 0; r < 8; r++)
                ((ushort*)&a1)[r] = intel_sub_group_block_read_us(cur_a + (8 + r) * SLM_A_STRIDE + k16_off);
            #pragma unroll
            for (int r = 0; r < 8; r++)
                ((ushort*)&a2)[r] = intel_sub_group_block_read_us(cur_a + (16 + r) * SLM_A_STRIDE + k16_off);
            #pragma unroll
            for (int r = 0; r < 8; r++)
                ((ushort*)&a3)[r] = intel_sub_group_block_read_us(cur_a + (24 + r) * SLM_A_STRIDE + k16_off);

            // Read B from SLM: 16 rows x 16 cols for this subgroup
            // B in SLM is laid out as [k_row * TILE_N + col]
            // Each SG reads cols [sg_id*16 .. sg_id*16+15]
            int8 b_val;
            int b_col_off = sg_id * 16;
            #pragma unroll
            for (int p = 0; p < 8; p++) {
                ushort s0 = intel_sub_group_block_read_us(cur_b + (k16_off + 2*p) * SLM_B_STRIDE + b_col_off);
                ushort s1 = intel_sub_group_block_read_us(cur_b + (k16_off + 2*p + 1) * SLM_B_STRIDE + b_col_off);
                ((int*)&b_val)[p] = as_int((ushort2)(s0, s1));
            }
        ...
        // Prefetch next tile into alternate buffer (overlapped with compute on Xe2)
        if (kt + 1 < num_k_tiles) {
            barrier(CLK_LOCAL_MEM_FENCE);
