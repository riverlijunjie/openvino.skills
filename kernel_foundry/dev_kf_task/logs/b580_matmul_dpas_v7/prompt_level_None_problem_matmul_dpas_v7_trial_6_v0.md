...
1. `short8 a_val0, a_val1; ... a_val0 = (short8)(A_short[a_base + 0], ..., A_short[a_base + 7]); ... a_val1 = (short8)(A_short[a_base + 8], ..., A_short[a_base + 15]);` and  
   `int b0  = ...; int b1 = ...; ... int b7 = ...;`  
   You’re doing many scalar global loads + manual packing every K-step, which creates a lot of instruction overhead and hurts memory throughput. Replace these with vectorized loads (`vload8` / `vload16` where legal) and reinterpret casts to packed integer vectors directly (e.g., load `short16` then `as_int8`). Do the same for B by loading packed vectors instead of building `b0..b7` one-by-one. This reduces ALU packing work and improves load coalescing.

2. `for (; k < K; k++) { ... for (int j = 0; j < 8; j++) { float bj = intel_sub_group_shuffle(b_lane, j); ... switch(j) { ... intel_sub_group_shuffle(...) ... }}}`  
   The tail path is very expensive: nested loop + switch + repeated shuffle calls per element. Even though it handles remainder K, this can still be a visible stall for non-multiple-of-16 sizes. Optimize by removing the inner `switch` entirely and using direct vector construction from one shuffle per lane index (or unrolled fixed 8 FMAs with one shuffle each). Also keep only one shuffle result per `j` (you currently compute `bj` but then ignore it and shuffle again). This cuts subgroup cross-lane traffic and control-flow overhead.

3. `if (a_row < M) { ... } else { ... }`, `if (b_col < N) { ... } else { ... }`, and boundary store branch `if (tile_n + 8 <= N) { vstore8... } else { for (...) switch(j) ... }` inside the main kernel flow  
   Frequent per-lane boundary branching causes divergence and prevents clean scheduling, especially for edge tiles. Split execution into two kernels (or two code paths selected at launch): a fast interior kernel assuming full tiles and `K % 16 == 0` (no bounds checks in loop, always vector store), and a separate boundary kernel for leftovers. Your current kernel is correct, but the interior path is paying edge-handling cost on every tile. Isolating boundaries typically gives a significant speedup on Xe by keeping the hot path branch-free.
