...
//
// Work-group tile: TM=32, TN=64, TK=32 (two DPAS k16 per SLM load)
// Subgroups: 4 along M x 8 along N = 32 subgroups, each 8 WIs => 256 WIs
// Each subgroup computes 8x8 output via DPAS
//
// SLM layout (double-buffered):
//   A stored pre-packed as uint: [2][32][16] (32 rows x 32 cols / 2 = 16 uint per row)
//   B stored in VNNI uint format: [2][16][64] (16 k-pairs x 64 cols)
//   Total: 2 * (32*16*4 + 16*64*4) = 2*(2048+4096) = 12288 bytes
//
// LWS = (64, 4, 1)
// GWS = (ceil(N/64)*64, ceil(M/32)*4, 1)
// Subgroup size = 8
    ...
    const int N)
{
    ...
    // Double-buffered SLM
    // A pre-packed as uint pairs: [TM][TK/2] = [32][16] uints per buffer
    // B in VNNI format: [TK/2][TN] = [16][64] uints per buffer
    __local uint slm_A_packed[2][32 * 16];   // 2 * 2048 bytes = 4096
    __local uint slm_B_vnni[2][16 * 64];     // 2 * 4096 bytes = 8192
    ...
    // Load A: 32 rows x TK_actual cols, pack pairs into uint
    // Load B: TK_actual rows x 64 cols, pack k-pairs into VNNI uint

    #define LOAD_TILE(buf, k_start, k_end) \
    { \
        int tk_actual = k_end - k_start; \
        int tk_pairs = (tk_actual + 1) / 2; \
        /* Load A: 32 * tk_pairs uints, 256 WIs */ \
        for (int idx = flat_lid; idx < 32 * 16; idx += 256) { \
            int r = idx / 16; \
            int p = idx % 16; /* pair index */ \
            int gr = wg_m + r; \
            int gc0 = k_start + p * 2; \
            int gc1 = gc0 + 1; \
            ushort v0 = (gr < M && gc0 < k_end) ? as_ushort(A[gr * K + gc0]) : (ushort)0; \
            ushort v1 = (gr < M && gc1 < k_end) ? as_ushort(A[gr * K + gc1]) : (ushort)0; \
            slm_A_packed[buf][r * 16 + p] = (uint)v0 | ((uint)v1 << 16); \
        } \
        /* Load B: tk_pairs * 64 uints */ \
        for (int idx = flat_lid; idx < 16 * 64; idx += 256) { \
            int kp = idx / 64; \
            int c = idx % 64; \
            int gk0 = k_start + kp * 2; \
            int gk1 = gk0 + 1; \
            int gc = wg_n + c; \
            ushort b0 = (gk0 < k_end && gc < N) ? as_ushort(B[gk0 * N + gc]) : (ushort)0; \
            ushort b1 = (gk1 < k_end && gc < N) ? as_ushort(B[gk1 * N + gc]) : (ushort)0; \
            slm_B_vnni[buf][kp * 64 + c] = (uint)b0 | ((uint)b1 << 16); \
        } \
    }
    ...
    for (int kt = 0; kt < num_k_tiles; kt++) {
        ...
        if (kt + 1 < num_k_tiles) {
            int k_start_next = (kt + 1) * TK;
            int k_end_next = min(k_start_next + TK, K);
            LOAD_TILE(next_buf, k_start_next, k_end_next);
        }

        // Compute on current buffer: two DPAS k=16 steps
        int a_row_local = sg_y * 8 + sg_local_id;
        int b_col_local = sg_x * 8 + sg_local_id;

        #pragma unroll
        for (int kk = 0; kk < 2; kk++) {
            // A operand: 8 ints from pre-packed SLM
            // Row a_row_local, pairs [kk*8 .. kk*8+7]
            int a_base = a_row_local * 16 + kk * 8;
            int8 a_packed = (int8)(
            ...
            // B operand: 8 ints from VNNI SLM
            // k-pairs [kk*8 .. kk*8+7], column b_col_local
            int b_base_kp = kk * 8;
            int8 b_packed = (int8)(
                (int)slm_B_vnni[buf][(b_base_kp + 0) * 64 + b_col_local],
                (int)slm_B_vnni[buf][(b_base_kp + 1) * 64 + b_col_local],
                (int)slm_B_vnni[buf][(b_base_kp + 2) * 64 + b_col_local],
                (int)slm_B_vnni[buf][(b_base_kp + 3) * 64 + b_col_local],
                (int)slm_B_vnni[buf][(b_base_kp + 4) * 64 + b_col_local],
                (int)slm_B_vnni[buf][(b_base_kp + 5) * 64 + b_col_local],
                (int)slm_B_vnni[buf][(b_base_kp + 6) * 64 + b_col_local],
                (int)slm_B_vnni[buf][(b_base_kp + 7) * 64 + b_col_local]
            );
...
1. `int8 a_packed = (int8)( ... slm_A_packed[buf][a_base + 0], ..., slm_A_packed[buf][a_base + 7] );` and  
   `int8 b_packed = (int8)( ... slm_B_vnni[buf][(b_base_kp + 0) * 64 + b_col_local], ..., slm_B_vnni[buf][(b_base_kp + 7) * 64 + b_col_local] );`:  
   You’re doing 16 scalar SLM loads per `kk` iteration per lane, which is a major load-instruction bottleneck and increases register pressure from all the temporary scalars.  
   **Improve it by vector-loading from SLM**:
   - For A, load contiguous 8 ints with `vload8` (or equivalent pointer cast): `a_packed = vload8(0, (__local int*)&slm_A_packed[buf][a_base]);`
   - For B, reorganize SLM layout so each lane can also read B as contiguous vectors (e.g., store B as `[64][16]` instead of `[16][64]`, or add a swizzled layout), then use `vload8`.  
   This usually reduces instruction count and improves DPAS feed rate significantly.

2. `barrier(CLK_LOCAL_MEM_FENCE);` (inside the `for (kt...)` loop after compute):  
   The current “prefetch next tile then compute current tile” still forces a full workgroup barrier each K-tile, so global-memory load latency is not really hidden; everyone waits at tile boundaries.  
   **Improve it by true pipelining with async copy + wait groups** (Intel OpenCL async block read / prefetch style):
   - Start async copies for tile `kt+1` into `next_buf` as early as possible.
   - Compute on `buf` while copies are in flight.
   - Use `wait_group_events` (or platform equivalent) right before consuming `next_buf`, then one barrier for SLM visibility.  
   This converts the hard synchronization into overlapped memory/compute, which is typically a top runtime win for GEMM tiles.

3. `for (int idx = flat_lid; idx < 32 * 16; idx += 256) { ... }` and  
   `for (int idx = flat_lid; idx < 16 * 64; idx += 256) { ... }` inside `LOAD_TILE(...)`:  
   Packing A/B into DPAS-friendly format is done every K-tile by the full WG, and for edge K-tiles you still iterate full fixed extents (`32*16`, `16*64`) with many masked zero-fill branches (`gc0 < k_end`, `gk0 < k_end`, etc.). This adds nontrivial overhead compared to pure math.  
   **Improve it in two concrete ways**:
   - Split main path vs tail path: run a branch-free fast path for full `TK=32` tiles (no per-element bounds checks), and a separate tail kernel/path only for the last partial K tile.
   - Increase per-thread transaction width when loading globals (e.g., read `ushort2/ushort4` then pack) so fewer instructions are spent on pack/load bookkeeping.  
   This reduces control-flow and packing overhead, especially when K is large and most tiles are full.
