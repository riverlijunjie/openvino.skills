...
1. `for (int j = 0; j < 8; j++) { ... ((float*)&a_packed)[j] = as_float((uint)lo | ((uint)hi << 16)); }` and  
   ...
   You are repacking A/B from `half` to DPAS format **inside every K-tile iteration**, which adds a lot of scalar ops, pointer aliasing, and register pressure around the math pipe. This often becomes the bottleneck once DPAS is active.  
   **Improve it by loading from SLM in DPAS-ready vector types directly** (e.g., `ushort8/uint8`) and avoiding per-element `as_ushort/as_float` in loops. Also unroll the packing fully (`#pragma unroll`) or, better, pre-arrange SLM layout during the cooperative load so the compute path does zero repacking. In practice: write B into SLM already in VNNI-interleaved `(k0,k1)` pair order, then each lane can do one vector read for `b_packed`.

2. `barrier(CLK_LOCAL_MEM_FENCE);` (the one after DPAS in each `k0` iteration):  
   You have two full workgroup barriers per K-step; the second one is mostly there to protect SLM reuse, but it serializes progress and reduces EU occupancy.  
   **Improve it with double-buffered SLM tiles** (`slm_A[2][...]`, `slm_B[2][...]`): while computing on buffer `buf`, preload next K-slice into `buf^1`. Then you only need one synchronization point between producer/consumer phases, and you overlap memory traffic with DPAS compute. This usually gives a clear runtime win on Xe2.

3. `slm_A[r * 16 + c] = ... A[gr * K + gc] ...;` and `slm_B[r * 32 + c] = ... B[gr * N + gc] ...;` (scalar cooperative loads):  
   Current global→SLM loads are scalar and index-heavy; each thread issues many small transactions and integer ops.  
   **Improve by vectorizing/coalescing loads and stores**: have each work-item load `half2/half4` (or `uint`-sized chunks) when in-bounds, and store vectorized into SLM. Also split edge handling: run a fast interior kernel path with no bounds checks (`gr<M/gc<N` guaranteed), and keep predication only for boundary tiles. This reduces control divergence and memory instruction count substantially.
