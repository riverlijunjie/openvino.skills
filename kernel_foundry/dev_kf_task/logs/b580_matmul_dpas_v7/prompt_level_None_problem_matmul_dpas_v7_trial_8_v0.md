...
1. `ushort s0  = as_ushort(a_ptr[0]); ... ushort s15 = as_ushort(a_ptr[15]);` and  
   `a_packed = (int8)( as_int((ushort2)(s0, s1)), ... );`:  
   You’re spending a lot of scalar instructions every K-tile to repack A from `half` into DPAS format. That packing overhead is on the critical path and can rival the DPAS cost for small `tK=16`.  
   **Improve it** by storing A in SLM already as packed 32-bit pairs (same idea you used for B VNNI), e.g. `__local uint slm_A_pack[2][32*8]`, and during preload write `slm_A_pack[buf][row*8+kp] = v0 | (v1<<16)`. Then in compute just load `int8` directly from SLM without the 16 scalar extracts/repack. This cuts ALU pressure and register pressure significantly.

2. `for (int i = flat_lid; i < 32 * 16; i += 128) { ... slm_A[...] = ... }` and  
   `for (int i = flat_lid; i < 8 * 32; i += 128) { ... slm_B_vnni[...] = ... }`:  
   These cooperative loads are scalar and strided per thread, so global memory transactions are not maximally coalesced/vectorized, and you also do many address recomputations/bounds checks per element.  
   **Improve it** by switching to vectorized block loads (`half4/half8` or `uint2/uint4` depending on alignment) and writing packed data directly to SLM. For B, load two K rows for a column block in vector form and pack in vector lanes. Also hoist invariant base pointers (`A + wg_row*K + k0`, `B + k0*N + wg_col`) and reduce per-iteration integer math. This usually gives a noticeable bandwidth/latency win.

3. `C[out_row * N + out_col] = convert_half(((float*)&acc)[i]);`:  
   This scalarized extract via pointer cast is inefficient and can force awkward register moves; plus each lane writes 8 separate scalar stores.  
   **Improve it** by keeping accumulator layout in native vector form and using explicit lane extraction intrinsics (`acc.s0 ... acc.s7`) or `vstore_half`/vectorized stores where legal. Also consider storing `half8` chunks per subgroup column when bounds allow (fast path for full tiles, masked scalar path only at edges). This reduces store instructions and improves write combining.
