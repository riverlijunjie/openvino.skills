...
//
// WG tile: 32 rows x 64 cols of C
// Subgroup size: 8, each subgroup computes 8x8 of C
// Layout: 8 subgroups along N (8*8=64), 4 along M (4*8=32) = 32 subgroups
// WG size: 256 work-items = 32 subgroups * 8 WIs
// LWS = (64, 4, 1), GWS = (ceil(N/64)*64, ceil(M/32)*4, 1)
//
// SLM per K-step (tK=16):
//   A tile: 32 x 16 halfs = 1024 bytes
//   B tile: 16 x 64 halfs = 2048 bytes
//   Total: 3072 bytes per WG
    ...
    const int N)
{
    ...
    const int sg_idx_x = lid_x / 8; // 0..7 (along N)
    const int sg_idx_y = lid_y;      // 0..3 (along M)

    // SLM tiles
    __local half slm_A[32 * 16]; // [TM][TK]
    __local half slm_B[16 * 64]; // [TK][TN]

    // Accumulator for 8x8 output per subgroup
    float8 acc = (float8)(0.0f);

    // Check if this WG is fully inside bounds (no partial tiles)
    const int wg_row_end = wg_row + TM;
    const int wg_col_end = wg_col + TN;
    const bool full_tile = (wg_row_end <= M) && (wg_col_end <= N);

    // Main K-loop
    for (int k0 = 0; k0 < K; k0 += TK) {
        const int k_remaining = K - k0;
        const int cur_tk = (k_remaining >= TK) ? TK : k_remaining;

        // === Cooperative load A[wg_row..+32][k0..+16] into slm_A ===
        // 256 WIs load 32*16=512 halfs => 2 halfs each
        // Use vectorized approach: each WI loads 2 consecutive elements
        {
            const int elems = TM * TK; // 512
            const int per_wi = (elems + 255) / 256; // 2
            for (int i = 0; i < per_wi; i++) {
                int idx = flat_lid + i * 256;
                if (idx < elems) {
                    int r = idx / TK;
                    int c = idx % TK;
                    int gr = wg_row + r;
                    int gc = k0 + c;
                    slm_A[idx] = (gr < M && gc < K) ? A[gr * K + gc] : (half)0.0f;
                }
            }
        }

        // === Cooperative load B[k0..+16][wg_col..+64] into slm_B ===
        // 256 WIs load 16*64=1024 halfs => 4 halfs each
        {
            const int elems = TK * TN; // 1024
            const int per_wi = (elems + 255) / 256; // 4
            for (int i = 0; i < per_wi; i++) {
                int idx = flat_lid + i * 256;
                if (idx < elems) {
                    int r = idx / TN;
                    ...
                    int gr = k0 + r;
                    int gc = wg_col + c;
                    slm_B[idx] = (gr < K && gc < N) ? B[gr * N + gc] : (half)0.0f;
                }
            }
        }
        ...
        // === DPAS: each subgroup computes 8x8 tile ===
        // A sub-tile: rows [sg_idx_y*8 .. +8], 16 cols from slm_A
        // B sub-tile: 16 rows, cols [sg_idx_x*8 .. +8] from slm_B

        // Pack A for DPAS: each WI (lane i) loads row (sg_idx_y*8 + i), 16 halfs
        // Pack into int8: each int = 2 packed halfs
        int a_slm_row = sg_idx_y * 8 + sg_local_id;
        __local const ushort* a_ptr = (__local const ushort*)&slm_A[a_slm_row * TK];

        int8 a_packed;
        a_packed.s0 = (int)a_ptr[0] | ((int)a_ptr[1] << 16);
        a_packed.s1 = (int)a_ptr[2] | ((int)a_ptr[3] << 16);
        a_packed.s2 = (int)a_ptr[4] | ((int)a_ptr[5] << 16);
        a_packed.s3 = (int)a_ptr[6] | ((int)a_ptr[7] << 16);
        a_packed.s4 = (int)a_ptr[8] | ((int)a_ptr[9] << 16);
        a_packed.s5 = (int)a_ptr[10] | ((int)a_ptr[11] << 16);
        a_packed.s6 = (int)a_ptr[12] | ((int)a_ptr[13] << 16);
        a_packed.s7 = (int)a_ptr[14] | ((int)a_ptr[15] << 16);

        // Pack B for DPAS: B[k_pair*2, col] and B[k_pair*2+1, col] into int
        int b_col_base = sg_idx_x * 8;
        __local const ushort* b_base = (__local const ushort*)slm_B;

        int8 b_packed;
        int bc = b_col_base + sg_local_id;
        b_packed.s0 = (int)b_base[0 * TN + bc] | ((int)b_base[1 * TN + bc] << 16);
        b_packed.s1 = (int)b_base[2 * TN + bc] | ((int)b_base[3 * TN + bc] << 16);
        b_packed.s2 = (int)b_base[4 * TN + bc] | ((int)b_base[5 * TN + bc] << 16);
        b_packed.s3 = (int)b_base[6 * TN + bc] | ((int)b_base[7 * TN + bc] << 16);
        b_packed.s4 = (int)b_base[8 * TN + bc] | ((int)b_base[9 * TN + bc] << 16);
        b_packed.s5 = (int)b_base[10 * TN + bc] | ((int)b_base[11 * TN + bc] << 16);
        b_packed.s6 = (int)b_base[12 * TN + bc] | ((int)b_base[13 * TN + bc] << 16);
        b_packed.s7 = (int)b_base[14 * TN + bc] | ((int)b_base[15 * TN + bc] << 16);
    ...
    // === Store 8x8 output tile ===
    int out_row = wg_row + sg_idx_y * 8 + sg_local_id;
    int out_col = wg_col + sg_idx_x * 8;
    ...
    if (out_row < M) {
        ...
        if (out_col + 8 <= N) {
            ...
            // Partial store for right boundary
            half* rp = (half*)&result;
            for (int j = 0; j < 8 && (out_col + j) < N; j++) {
                C[out_base + j] = rp[j];
            }
        }
    }
...
1. `for (int k0 = 0; k0 < K; k0 += TK) { ... barrier(CLK_LOCAL_MEM_FENCE); ... barrier(CLK_LOCAL_MEM_FENCE); }`: you have **two full-WG barriers per K-slice** and all compute waits for all loads. This creates a strict load→sync→compute→sync pipeline with no overlap.  
   **Improve it by double-buffering SLM** (`slm_A[2][TM*TK]`, `slm_B[2][TK*TN]`) and ping-ponging buffers across `k0`: while computing on buffer `buf`, cooperatively preload `buf^1` for next `k0`. Then you only need one synchronization point between producer/consumer phases per iteration and you hide part of global-memory latency behind DPAS work.
...
const bool full_tile = (wg_row_end <= M) && (wg_col_end <= N);
```
`full_tile` is computed but never used. So even interior tiles still pay per-element bounds checks in the hot load path.  
   **Improve it by splitting fast/slow paths**: if `full_tile && (k0 + TK <= K)`, do unconditional vectorized loads (no predicates); only boundary WGs/tail-K use guarded scalar path. This removes many branches and predicate ops from the dominant iterations and improves memory throughput.
...
b_packed.s7 = (int)b_base[14 * TN + bc] | ((int)b_base[15 * TN + bc] << 16);
```
Each K-step does lots of manual scalar packing from SLM (many address calcs + shifts/or’s), which adds non-trivial EU overhead around DPAS.  
   **Improve it by changing SLM layout to DPAS-friendly packed format at load time**: store A/B into SLM already as packed `uint` pairs (or `uint4/uint8` vectors) matching `intel_sub_group_f16_f16_matrix_mad_k16` input layout. Then in compute you mostly do vector loads of packed values, not repacking every iteration. Also prefer vectorized SLM/global loads (`vload4/vload8`) in the cooperative copy loops to reduce instruction count and improve memory coalescing.
