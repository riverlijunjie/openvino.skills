...
1. `b0 = (int8)(slm_B[0*64+bc0], ..., slm_B[7*64+bc0]);` (and same for `b1/b2/b3`): this is a major hot spot because each work-item does many scalar SLM reads and register inserts every K-tile (32 scalar loads total just to build B operands). You’re effectively spending a lot of cycles on operand-gather instead of DPAS.  
   **Improve it by changing SLM layout for B to be subgroup-friendly**, so each `b*` can be loaded with `vload8` (or two `vload4`) instead of scalar gathers. For example, store B in SLM transposed as `[col][kp]` for each 32-col subgroup slice, then load contiguous 8 ints per lane:
   - during cooperative load, write `slm_B_t[c * 8 + kp] = packed_pair;`
   - in compute, do `int8 b0 = vload8(0, &slm_B_t[bc0 * 8]);` etc.  
   This usually gives a large speedup on Xe by reducing SLM access instructions and register pressure.

2. `for (int i = flat_lid; i < 8 * 64; i += WG_SIZE) { ... slm_B[kp * 64 + c] = ... }`: your B load is bandwidth-heavy and does limited vectorization from global memory (two scalar half loads + pack per element). This underutilizes global memory transactions and adds integer packing overhead in the inner K loop.  
   **Improve it by vectorizing global loads and packing less in scalar code**. Have each WI load `half2`/`ushort2` (or wider vectors where aligned) directly from `B`, then store packed `int` to SLM. Also unroll this loop (fixed trip count per WI = 8) with `#pragma unroll` to reduce loop/control overhead. Same idea for A load loop (trip count 4). This reduces instruction count and improves LSC transaction efficiency.

3. `barrier(CLK_LOCAL_MEM_FENCE); ... barrier(CLK_LOCAL_MEM_FENCE);` inside every K tile: full workgroup barriers twice per tile are expensive and stall all 8 subgroups, especially when K is large.  
   **Improve it with double-buffered SLM (ping-pong) and software pipelining**:
   - allocate `slm_A[2][...]`, `slm_B[2][...]`
   - preload tile 0
   - for each `kt`: compute from buffer `cur` while cooperatively loading `kt+1` into `next`, then one barrier to swap.  
   This overlaps memory staging with DPAS and typically cuts synchronization cost significantly.  
   Also add `#pragma unroll` on the fixed-size DPAS group construction/store paths to help the compiler schedule DPAS + loads better.
