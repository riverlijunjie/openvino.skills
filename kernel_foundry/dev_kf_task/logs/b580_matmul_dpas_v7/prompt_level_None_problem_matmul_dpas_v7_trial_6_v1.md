...
1. `if (next_kt < num_k_tiles) { ... slm_A_packed[next_buf][i] = ...; ... slm_B_vnni[next_buf][i] = ...; }` inside the main loop: this “prefetch” is not actually overlapped with compute, because you execute all global→SLM loads first and only then issue DPAS, and you still end with `barrier(CLK_LOCAL_MEM_FENCE);`. So each iteration is still load+compute serialized.  
   **Improve it:** move to a true pipelined schedule:
   - preload tile 0,
   - for each `kt`: compute on `buf` first, while issuing **asynchronous block copy** for `next_buf` (e.g., Intel async copy builtins / block read path),  
   - then wait on async event before swapping buffers.  
   This removes a lot of exposed memory latency and is usually the biggest win for this pattern.

2. `int8 a_packed = (int8)( ... slm_A_packed[buf][a_base + 0], ..., slm_A_packed[buf][a_base + 7]);` and similarly for `b_packed`: you’re doing 16 scalar SLM loads per DPAS step per lane (8 for A + 8 for B), plus heavy register packing overhead. That burns issue slots and increases register pressure before every MAD.  
   **Improve it:** load these as vectors directly from SLM (block/vector load) instead of scalar gather-style construction, and keep the layout aligned so one vector read maps to the 8 packed dwords needed by DPAS. For B especially, reorganize SLM layout to make each lane’s `int8` contiguous in memory (lane-major chunk), so the compiler can emit efficient block loads.

3. `int a_slm_row = sg_row * 8 + sg_lid;` with `acc` as a single `float8`: each lane computes exactly one output row of one 8x8 tile. That gives low per-thread arithmetic intensity and makes loop/control/barrier overhead relatively large vs useful math.  
   **Improve it:** increase per-lane work (register blocking), e.g., compute 2 rows (or 2 tiles in N) per lane with two accumulators (`float8 acc0, acc1` or `float8 acc_col0, acc_col1`) while reusing the same B fragment loads. This increases DPAS reuse, amortizes SLM/global load cost, and typically improves EU occupancy/perf balance on Xe2 as long as register usage stays below the spill threshold.
