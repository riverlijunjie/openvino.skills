...
1. `for (int idx = flat_lid; idx < TM * TK; idx += 256) { ... slm_A[...] = (gr < M && gc < K) ? A[gr * K + gc] : (half)0.0f; }` and  
   `for (int idx = flat_lid; idx < TK * TN; idx += 256) { ... slm_B[...] = (gr < K && gc < N) ? B[gr * N + gc] : (half)0.0f; }`:  
   You’re doing scalar global loads with per-element bounds checks in the hot K-loop. That creates a lot of address arithmetic + branch overhead and poor memory transaction efficiency.  
   **Improve it by splitting fast-path and edge-path per workgroup tile:** if `(wg_m+TM<=M && wg_n+TN<=N)` then run a branch-free vectorized load path (e.g., `ushort4/ushort8` or `half8` loads/stores to SLM, fully coalesced). Keep your current predicated scalar path only for boundary tiles. This usually gives a big speedup because most tiles are interior.

2. `a_packed.s0 = ...; ... a_packed.s7 = ...;` and `for (int p = 0; p < 8; p++) { ((int*)&b_packed)[p] = ...; }`:  
   The manual packing to DPAS format is expensive and repeated every `kk` iteration. You’re paying a lot of ALU/instruction overhead just to prepare operands.  
   **Improve it by changing SLM layout to pre-packed format during the SLM write phase.**  
   - Store A in packed `int` units directly (`2x f16` per int) with layout matching DPAS consumption for each subgroup row.  
   - Store B already VNNI-packed (`[k_pair, n]`) so the DPAS loop can do near-direct `int8` loads instead of repacking.  
   This moves cost out of the innermost compute loop and reduces instruction pressure significantly.

3. `barrier(CLK_LOCAL_MEM_FENCE); ... DPAS ... barrier(CLK_LOCAL_MEM_FENCE);` with single SLM buffers `slm_A`/`slm_B`:  
   This is a classic load-then-compute-then-wait pipeline stall. Every K-tile forces full-workgroup synchronization before and after compute, and global memory latency is exposed.  
   **Improve it with double-buffered SLM ping-pong** (e.g., `slm_A[2][...]`, `slm_B[2][...]`): while computing on buffer `t`, cooperatively preload buffer `t^1` for the next `k0`. Then only one barrier per stage transition is needed, and memory latency overlaps compute. On Xe2, this often improves sustained throughput materially for medium/large K.
