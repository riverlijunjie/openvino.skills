...
1. `acc = intel_sub_group_f16_f16_matrix_mad_k16(a_packed, b_packed, acc);` (inside `for (int kk = 0; kk < 2; kk++)`) + surrounding scalar gathers for `a_packed`/`b_packed`:  
   You’re feeding DPAS with values assembled through many scalar SLM loads and vector construction each iteration:
   ```c
   int8 a_packed = (int8)(slm_A_packed[...] ...);
   int8 b_packed = (int8)(slm_B_vnni[...] ...);
   ```
   This creates a high load/pack overhead relative to math. Improve by **preloading operands into registers once per K-slice** and doing multiple DPAS from those registers, or by reshaping SLM layout so each lane can issue **vector loads (`vload8`)** instead of 8 scalar loads. Also unroll K more aggressively (e.g., TK=64 with 4 DPAS steps if occupancy allows) so DPAS issue rate dominates SLM gather cost.

2. `barrier(CLK_LOCAL_MEM_FENCE);` at end of every K-tile while also prefetching next tile before compute:  
   Current double-buffering still has a full workgroup barrier per tile and limited overlap. Because all WIs must finish both prefetch and compute before progressing, latency hiding is weaker than it looks. Split roles: dedicate a subset of lanes/subgroups to async SLM/global loads while others compute, then synchronize once at tile boundary (producer/consumer pattern). On Xe2, this usually reduces stall cycles versus “everyone loads, everyone computes” with full barriers.

3. `for (int idx = flat_lid; idx < 32 * 16; idx += 256)` and `for (int idx = flat_lid; idx < 16 * 64; idx += 256)` in `LOAD_TILE`:  
   The tail tile path always iterates over full fixed tile size (`32*16` and `16*64`) and checks bounds per element:
   ```c
   ushort v0 = (gr < M && gc0 < k_end) ? ... : 0;
   ushort b0 = (gk0 < k_end && gc < N) ? ... : 0;
   ```
   Those per-element predicates are expensive and hurt memory throughput. Use **separate kernels or specialized paths**: one fast path for full interior tiles (no bounds checks), one remainder path for edges. Also pre-pad K/N/M to tile multiples at dispatch time when possible so main kernel runs branch-free. This is often a major runtime win for GEMM kernels.
