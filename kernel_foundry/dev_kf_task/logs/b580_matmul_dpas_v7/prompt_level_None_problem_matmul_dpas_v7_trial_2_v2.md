...
1. `for (int j = 0; j < 8; j++) { ... ((float*)&a_packed)[j] = as_float((uint)lo | ((uint)hi << 16)); }` and  
   `for (int j = 0; j < 8; j++) { ... ((float*)&b_packed)[j] = as_float((uint)lo | ((uint)hi << 16)); }`:  
   You are doing per-element scalar pack/repack every `k0` iteration, which adds a lot of integer ops, pointer aliasing, and register pressure around the DPAS hot path. On Xe2 this often becomes a front-end bottleneck relative to the DPAS throughput.  
   **Improve it by vectorizing and prepacking at load time into SLM in DPAS-ready layout**:  
   - While cooperatively loading A/B, write them into `__local uint`/`uint4` buffers already packed as `(half0, half1)` pairs (VNNI-style for B).  
   - Then each subgroup reads packed `uint8` (or equivalent) directly, avoiding the two inner `j` loops entirely.  
   - At minimum, replace scalar loads/stores with `vload_half{2,4,8}` + bitcast pack to reduce instruction count.

2. `barrier(CLK_LOCAL_MEM_FENCE);` (both barriers inside the K loop):  
   You use full-workgroup barriers twice per `tK=16` step, so synchronization cost is paid very frequently. Since each subgroup computes independently after SLM fill, this is a significant stall source as K grows.  
   **Improve it by reducing barrier frequency via deeper K blocking and double-buffered SLM**:  
   - Increase `tK` (e.g., 32 if register/SLM budget allows) so you do more DPAS work per synchronization.  
   - Use ping-pong SLM buffers (`slm_A0/slm_A1`, `slm_B0/slm_B1`) and overlap load of next tile with compute on current tile; this can collapse effective sync overhead and hide memory latency.

3. `C[out_row * N + out_col] = convert_half(((float*)&acc)[i]);`:  
   Final store is scalar half write per row, per lane, and you recompute address each iteration. This is okay functionally but leaves bandwidth/coalescing on the table and adds scalar conversion/store overhead in epilogue.  
   **Improve it by vectorized epilogue stores**:  
   - Keep accumulator in a form that allows packed conversion (`float8 -> half8`) and write with vector stores when in-bounds contiguous regions exist.  
   - Hoist `base = out_col` and `row_base = (wg_row + sg_row*8) * N` to reduce repeated MADs in the loop.  
   - For full tiles (common case), use a fast path without bounds checks; keep the checked path only for edge tiles. This reduces branch/divergence cost in the epilogue.
