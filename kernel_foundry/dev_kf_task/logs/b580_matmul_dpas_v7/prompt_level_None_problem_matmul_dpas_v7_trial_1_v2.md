...
1. `short8 a_val0, a_val1; ... a_val0 = (short8)(A_short[a_base + 0], ...); a_val1 = (short8)(A_short[a_base + 8], ...);` and the analogous scalar packing for `b_val`  
   You are doing **many scalar global loads + manual repacking** every K-step, which is a major instruction and memory pipeline bottleneck. Replace these with vectorized/coalesced block loads and reinterpret casts:
   - Load A as two `half8` (or one `half16` if supported), then bitcast to `int8` directly (or to the exact DPAS operand type expected by your compiler builtin).
   - Load B with vector loads too (e.g., `ushort2/ushort4` patterns per lane), avoiding 8 separate dependent scalar fetches and OR/shift packs.
   - Best next step: stage A/B tiles in `__local` once per K-block (double-buffered), then feed DPAS from local memory to reduce repeated global traffic and improve cache hit behavior.  
   Right now the math unit is likely underfed by load/pack overhead.

2. `for (; k < K; k++) { ... for (int j = 0; j < 8; j++) { ... switch(j) { ... intel_sub_group_shuffle(...) ... }}}`  
   The remainder path is very expensive: nested loop + switch + repeated shuffles in each iteration. Even if `K % 16` is small, this tail still adds branch/instruction overhead and hurts occupancy. Improve by:
   - Replacing the switch with straight-line vector accumulation (`acc += a_val * b_vec`) where `b_vec` is assembled once from subgroup shuffles.
   - Computing each shuffled lane value once (not multiple times inside switch cases).
   - Better: pad K to 16 on input/prepack B (if allowed by calling path) so you can remove scalar tail entirely; if not, use a single masked DPAS-like microkernel for tail chunks (8 or 4) to reduce scalar control flow.

3. `half8 result = convert_half8(acc); ... vstore8(result, 0, C + out_base);`  
   You always convert/store FP32 accumulators to FP16 at the end, which is correct functionally but can serialize the epilogue and cost bandwidth on boundary tiles due to scalar stores:
   - Keep fast path fully vectorized: for boundary handling, avoid switch-based scalar stores and use predicate-masked vector store logic (or split kernel into interior/full-tile kernel + boundary kernel).
   - If your workload is dominated by full tiles, launch a no-boundary kernel for `M%32==0 && N%64==0` region and a tiny cleanup kernel for edges. This removes per-thread boundary branching from the hot path and improves SIMD efficiency.
