...
1. `for (int idx = flat_lid; idx < TM * TK; idx += 256) { ... slm_A[...] = ... }` and `for (int idx = flat_lid; idx < TK * TN; idx += 256) { ... slm_B[...] = ... }`:  
   You’re doing scalar half loads/stores into SLM with per-element index math and bounds checks. That creates a lot of instruction overhead and under-utilizes memory bandwidth.  
   **Improve it by vectorizing and specializing the main loop for full tiles**: in the `k0 + TK <= K` loop, you already know `k0..k0+31` is valid, so remove `gc < K` checks there. Also split interior vs boundary workgroups (`wg_m+TM<=M && wg_n+TN<=N`) so interior path has no predicates. Then load global→SLM using `ushort2/ushort4` (or `half2/half4`) and contiguous assignments per WI. This cuts address arithmetic, branches, and memory transactions.

2. `a_packed.s0 = ...; ... a_packed.s7 = ...;` and `for (int p = 0; p < 8; p++) { ((int*)&b_packed)[p] = ... }`:  
   Packing operands for DPAS every `kk` iteration is expensive and currently done via many scalar ops plus pointer-cast stores (`(int*)&...`), which can inhibit optimization and add register pressure.  
   **Improve it by prepacking B in SLM once per TK tile into DPAS-ready layout** (VNNI/interleaved) during cooperative load, so the compute loop only performs direct vector reads. For A, use vector loads (`ushort8`/`int4`) from SLM and avoid per-element repacking where possible. Also replace pointer-punning writes with native vector constructors/shuffles to help the compiler generate cleaner code.

3. `if (k0 < K) { ... barrier(...) ... int rem_aligned = ((rem + 15) / 16) * 16; ... }`:  
   The remainder path is heavy: extra SLM traffic, multiple loops, conditionals inside packing, and barriers. For many shapes, this tail handling becomes disproportionately costly.  
   **Improve it by splitting kernels (or at least split code paths) for `K % 32 == 0` fast path vs tail path**. Keep the hot kernel branch-free and barrier-minimal. In the tail path, avoid loading partial tiles through generic loops; use masked vector loads and pad once, then a single compact DPAS tail loop. This reduces control-flow overhead in the common case and improves average runtime significantly.
