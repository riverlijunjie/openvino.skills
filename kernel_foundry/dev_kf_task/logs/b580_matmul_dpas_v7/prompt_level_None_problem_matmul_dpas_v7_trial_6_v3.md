...
1. `acc = intel_sub_group_f16_f16_matrix_mad_k16(a_packed, b_packed, acc);` (inside `for (int kk = 0; kk < 2; kk++)`):  
   You always execute **two k16 DPAS ops per K-tile**, even for the last partial tile (`K % 32 != 0`). In that case, your loader zero-fills invalid pairs, so correctness is fine, but you still pay full DPAS cost on zeros.  
   **Improve it** by making the compute count depend on valid pairs in the current tile:
   - compute `tk_actual = min(TK, K - kt*TK)`, `valid_k16 = (tk_actual + 15) / 16`
   - run `kk < valid_k16` instead of fixed `kk < 2`  
   This removes wasted DPAS work on tail tiles and improves runtime for non-multiple-of-32 K.

2. `int8 a_packed = (int8)( (int)slm_A_packed[buf][a_base + 0], ... );` and `int8 b_packed = (int8)( (int)slm_B_vnni[buf][...], ... );`  
   This builds vectors from **8 scalar SLM loads each time** (16 scalar loads per DPAS step, per lane), creating high SLM load pressure and extra instruction overhead.  
   **Improve it** by loading vector blocks directly from contiguous SLM:
   - For A, do vector read from `&slm_A_packed[buf][a_base]` (contiguous 8 uints)
   - For B, change SLM layout so each lane’s 8 kp values are contiguous for the DPAS operand, then vector read those 8 uints too  
   Right now B is stored as `[kp][col]`, which is good for write coalescing during load, but bad for per-lane DPAS operand gather. Store B in a compute-friendly layout (or pre-transpose while loading) so DPAS reads are contiguous.

3. `if (kt + 1 < num_k_tiles) { ... LOAD_TILE(next_buf, ...); }` followed by compute, with only `barrier(CLK_LOCAL_MEM_FENCE);` at loop end:  
   This is only **software double-buffering in SLM**, but global→SLM loads are still synchronous work done by all threads before math can fully hide latency. You’re not using async copy/pipelined producer-consumer primitives, so overlap is limited.  
   **Improve it** by pipelining loads with async block copy to SLM (`async_work_group_copy`/Intel block read equivalents where supported), then `wait_group_events` right before using `next_buf`. This allows better overlap of memory traffic with DPAS compute. Also split loaders by role (some lanes mostly load A, others B) to reduce contention and improve scheduling.
