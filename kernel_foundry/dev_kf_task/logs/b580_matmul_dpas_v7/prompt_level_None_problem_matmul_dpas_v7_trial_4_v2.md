...
1. `barrier(CLK_LOCAL_MEM_FENCE);` (inside the K-loop, after compute): this barrier serializes every tile step and prevents true overlap of prefetch + compute. Right now, you prefetch `next_buf`, then immediately do compute, then force a full work-group barrier every iteration. On Xe2, this often shows up as a stall because all 256 threads must wait even though DPAS work is subgroup-local.  
   **Improve it:** keep the double buffer, but pipeline at subgroup granularity so loads for `next_buf` are issued as early as possible and only one barrier is used to protect buffer handoff. In practice: preload tile 0, barrier once; for each `kt`, compute on `buf` while a dedicated subset of lanes/work-items performs next-tile loads, then one barrier before swapping buffers. This reduces barrier pressure and improves EU occupancy.
...
a_packed.s7 = as_int(vload2(0, (__local ushort*)(a_ptr + 14)));
```
(and same pattern in step 2): you rebuild `a_packed` with 16 scalar-ish load/pack ops per iteration per work-item (8 for first k16 + 8 for second k16). This is instruction-heavy and increases register/move pressure before every DPAS call.  
   **Improve it:** vectorize these loads directly from SLM with wider types (`int4`/`int8` block loads where alignment allows), or pre-layout `slm_A` in a DPAS-ready packed format during cooperative load so compute phase uses near-zero repacking. The best win is to store A in packed 32-bit pair format in SLM (like you already do for B VNNI), so DPAS operands are fetched directly.

3.  
```c
for (int i = flat_lid; i < TM * TK; i += 256) { ... slm_A[...] = ... }
for (int i = flat_lid; i < (TK/2) * TN; i += 256) { ... slm_B_vnni[...] = ... }
```
with boundary checks inside each iteration:
```c
(gr < M && gc < K) ? ... : 0
(gk0 < K && gc < N) ? ... : 0
```
These per-element conditionals add control overhead and hurt memory throughput, especially for full interior tiles where checks are always true.  
   **Improve it:** split kernel work into **interior tiles** (no bounds checks, fully vectorized/coalesced loads) and **edge tiles** (current guarded path). For interior path, use wider global loads (`ushort2/ushort4` or `uint2`) and write packed data straight to SLM. This removes branches from the hot path and substantially improves load efficiency.
