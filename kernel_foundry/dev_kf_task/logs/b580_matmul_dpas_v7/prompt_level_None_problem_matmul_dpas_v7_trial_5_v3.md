...
// Subgroups: 4 along M x 4 along N = 16 subgroups, each 8 WIs => 128 WIs
// Each subgroup computes 8x16 output (two 8x8 DPAS, reusing A operand)
//
// SLM:
//   A: [32][16] halfs = 1024 bytes, stored as [32][8] uint (packed pairs)
//   B: [8][64] uint VNNI = 2048 bytes (8 k-pairs x 64 cols)
//   Total per buffer: 3072 bytes, double-buffered = 6144 bytes
//
// LWS = (32, 4, 1)  => 128 WIs
    ...
    const int N)
{
    ...
    // 4x4 grid of subgroups: 4 along M, 4 along N
    // Each subgroup handles 8 rows x 16 cols (two 8x8 DPAS outputs)
    const int sg_y = sg_id / 4;  // 0..3
    const int sg_x = sg_id % 4;  // 0..3

    // SLM: A stored as packed uint [32][8], B as VNNI uint [8][64]
    __local uint slm_A[32 * 8];      // 1024 bytes
    __local uint slm_B[8 * 64];      // 2048 bytes

    // Two accumulators per subgroup: cols [sg_x*16..sg_x*16+7] and [sg_x*16+8..sg_x*16+15]
    float8 acc0 = (float8)(0.0f);
    float8 acc1 = (float8)(0.0f);

    // Check if this is a full interior tile (no bounds needed)
    const int is_full_m = (wg_m + TM <= M);
    const int is_full_n = (wg_n + TN <= N);

    for (int k0 = 0; k0 < K; k0 += TK) {
        const int k_end = min(k0 + TK, K);
        const int is_full_k = (k0 + TK <= K);

        // --- Cooperative SLM load ---
        // Load A: 32 rows x 16 cols => 32*8 = 256 uint packed pairs
        // 128 WIs => 2 each
        if (is_full_m && is_full_k) {
            // Fast path: no bounds checks
            for (int idx = flat_lid; idx < 256; idx += 128) {
                int r = idx >> 3;    // row 0..31
                int p = idx & 7;     // pair 0..7
                int gr = wg_m + r;
                int gc = k0 + p * 2;
                ushort v0 = as_ushort(A[gr * K + gc]);
                ushort v1 = as_ushort(A[gr * K + gc + 1]);
                slm_A[r * 8 + p] = (uint)v0 | ((uint)v1 << 16);
            }
        } else {
            for (int idx = flat_lid; idx < 256; idx += 128) {
                int r = idx >> 3;
        ...
        // Load B: 8 k-pairs x 64 cols = 512 uints
        // 128 WIs => 4 each
        if (is_full_n && is_full_k) {
            for (int idx = flat_lid; idx < 512; idx += 128) {
                int kp = idx >> 6;   // k-pair 0..7
                int c = idx & 63;    // col 0..63
                int gk0 = k0 + kp * 2;
                int gc = wg_n + c;
                ushort b0 = as_ushort(B[gk0 * N + gc]);
                ushort b1 = as_ushort(B[(gk0 + 1) * N + gc]);
                slm_B[kp * 64 + c] = (uint)b0 | ((uint)b1 << 16);
            }
        } else {
            for (int idx = flat_lid; idx < 512; idx += 128) {
                int kp = idx >> 6;
        ...
        // --- Compute: each subgroup does two DPAS (8x8 each), reusing A ---
        // A operand: row = sg_y*8 + sg_lid, 8 uint pairs
        int a_base = (sg_y * 8 + sg_lid) * 8;

        // Use vload for contiguous A data
        int8 a_packed;
        {
            __local uint* ap = &slm_A[a_base];
            a_packed = (int8)(
        ...
        // B operand 0: cols [sg_x*16 + sg_lid] (first 8x8)
        int b_col0 = sg_x * 16 + sg_lid;
        int8 b_packed0 = (int8)(
            (int)slm_B[0 * 64 + b_col0],
            (int)slm_B[1 * 64 + b_col0],
            (int)slm_B[2 * 64 + b_col0],
            (int)slm_B[3 * 64 + b_col0],
            (int)slm_B[4 * 64 + b_col0],
            (int)slm_B[5 * 64 + b_col0],
            (int)slm_B[6 * 64 + b_col0],
            (int)slm_B[7 * 64 + b_col0]
        );

        // B operand 1: cols [sg_x*16 + 8 + sg_lid] (second 8x8)
        int b_col1 = sg_x * 16 + 8 + sg_lid;
        int8 b_packed1 = (int8)(
            (int)slm_B[0 * 64 + b_col1],
            (int)slm_B[1 * 64 + b_col1],
            (int)slm_B[2 * 64 + b_col1],
            (int)slm_B[3 * 64 + b_col1],
            (int)slm_B[4 * 64 + b_col1],
            (int)slm_B[5 * 64 + b_col1],
            (int)slm_B[6 * 64 + b_col1],
            (int)slm_B[7 * 64 + b_col1]
        );
    ...
    // --- Store results ---
    // acc0: rows [wg_m + sg_y*8 + sg_lid], cols [wg_n + sg_x*16 + 0..7]
    // acc1: rows [wg_m + sg_y*8 + sg_lid], cols [wg_n + sg_x*16 + 8..15]
    int out_row = wg_m + sg_y * 8 + sg_lid;

    if (out_row < M) {
        int out_col_base0 = wg_n + sg_x * 16;
        int out_col_base1 = out_col_base0 + 8;

        // Store first 8 columns
        {
            half8 result = convert_half8(acc0);
            int out_base = out_row * N + out_col_base0;
            if (out_col_base0 + 8 <= N) {
                vstore8(result, 0, C + out_base);
            } else {
                for (int j = 0; j < 8; j++) {
                    if (out_col_base0 + j < N)
                        C[out_base + j] = ((half*)&result)[j];
                }
            }
        }

        // Store second 8 columns
        {
            half8 result = convert_half8(acc1);
            int out_base = out_row * N + out_col_base1;
            if (out_col_base1 + 8 <= N) {
                vstore8(result, 0, C + out_base);
            } else {
                for (int j = 0; j < 8; j++) {
                    if (out_col_base1 + j < N)
                        C[out_base + j] = ((half*)&result)[j];
                }
            }
        }
...
1. `ushort v0 = as_ushort(A[gr * K + gc]); ... ushort b1 = as_ushort(B[(gk0 + 1) * N + gc]);` inside the SLM load loops: you’re doing scalar half loads + packing in registers for every element, which burns instructions and memory transactions.  
   **How to improve:** load packed data directly as 32-bit values whenever alignment allows. For A, reinterpret as `const __global uint*` and load one `uint` per pair instead of two `half` loads + shifts. For B, same idea for each `(k-pair, col)` packed pair. Keep a fallback path for tail/alignment, but make the hot full-tile path purely vector/packed loads. This reduces global load instruction count and integer packing overhead significantly.

2. `barrier(CLK_LOCAL_MEM_FENCE); ... barrier(CLK_LOCAL_MEM_FENCE);` every `k0` iteration with single-buffer SLM staging: the whole workgroup stalls twice per K-tile, and compute cannot overlap with memory fetch.  
   **How to improve:** actually implement the double-buffering you mention in comments (`6144 bytes`) using two SLM pages for A/B and ping-pong them:
   - prefetch tile `t+1` into buffer `next`
   - compute tile `t` from buffer `cur`
   - swap `cur/next`
   This lets you overlap global→SLM loads with DPAS compute and cuts barrier impact (typically one sync point per stage transition instead of two hard-stop phases).

3. `int8 b_packed0 = (int8)( (int)slm_B[0 * 64 + b_col0], ... );` and same for `b_packed1`: each subgroup lane gathers B from SLM with a 64-stride pattern every K-tile, which is costly on SLM bandwidth/latency.  
   **How to improve:** change SLM layout for B to be subgroup-friendly at write time, e.g. store as `[sg_x][kp][8]` (or generally transpose/blocked so each lane reads contiguous addresses for its DPAS fragment). Then load with contiguous vector reads (`vload8`/block read) instead of 8 scalar gathers per operand. This removes repeated strided SLM accesses and lowers instruction pressure in the inner compute path.
