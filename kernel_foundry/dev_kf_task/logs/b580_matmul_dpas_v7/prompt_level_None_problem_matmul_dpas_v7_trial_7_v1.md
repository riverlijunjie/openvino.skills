...
1. `int8 a_packed; { ushort s0 = as_ushort(a_ptr[0]); ... a_packed = (int8)(as_int((ushort2)(s0, s1)), ... ); }`: this scalar repacking of A is a major instruction bottleneck (16 scalar loads + many pack ops every K-tile). You’re spending ALU cycles just formatting data for DPAS.  
   **Improve it by storing A in packed/VNNI-ready form in SLM during the preload stage**, exactly like you already do for B. For example, make `slm_A_packed[2][32*8]` as `uint`, and when loading A tile do `vload_half2`/`as_uint` once per pair. Then in compute you load 8 `uint`s directly into `a_packed` with no per-iteration repack. This usually gives a noticeable speedup because DPAS feed becomes bandwidth-driven instead of instruction-driven.

2. `if (next_kt < num_k_tiles) { ... load next A/B tile ... } ... barrier(CLK_LOCAL_MEM_FENCE);`: your “double buffering” is not overlapping copy and compute; loads happen before DPAS, so latency is exposed. Also a full workgroup barrier every iteration is expensive.  
   **Improve it by true pipelining**: compute on `buf` first, and in parallel issue prefetch for `next_buf` (or at least move prefetch immediately after previous barrier and before next compute so compiler can schedule overlap). On Xe, use subgroup/block read intrinsics (`intel_sub_group_block_read*`) for SLM/global transfers to reduce per-element overhead and help hide memory latency. Keep exactly one barrier between producer/consumer phases, but avoid putting all load work on the critical path before `matrix_mad`.

3. `slm_A[...]=(gr < M && gc < K) ? ... : 0;` and `slm_B_vnni[...]=(gk0 < K && gc < N) ? ... : 0;` inside the inner tile loops, plus final stores with nested `if (out_col < N) { for (...) if (out_row < M) ... }`: boundary checks are executed every tile/element and create divergence and extra predicates.  
   **Improve it by splitting kernels (or paths) into interior vs edge tiles**:  
   - Fast path: for full tiles (`wg_row+31<M`, `wg_col+31<N`, and full K tile where applicable), remove bounds checks entirely and use straight vector/block loads/stores.  
   - Slow path: keep your current guarded logic only for boundary workgroups.  
   This reduces branch/predicate overhead in the dominant case and improves SIMD/subgroup efficiency significantly on large GEMMs.
