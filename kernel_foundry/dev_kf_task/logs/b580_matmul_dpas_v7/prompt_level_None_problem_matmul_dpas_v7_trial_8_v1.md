...
//
// Work-group tile: TM=32, TN=64, TK=32 (two DPAS k=16 steps per SLM load)
// Subgroup arrangement: 4 along M x 8 along N = 32 subgroups of 8 WIs = 256 WIs
// Each subgroup: 8x8 output tile via DPAS
//
// LWS = (64, 4, 1)   [64 = 8 sg_x * 8 lanes, 4 = sg_y]
// GWS = (ceil(N/64)*64, ceil(M/32)*4, 1)
// Subgroup size = 8
//
// SLM usage per K-step (TK=32):
//   A tile: 32 rows x 32 cols = 2048 bytes (half)
//   B tile: 32 rows x 64 cols = 4096 bytes (half)
//   Total: 6144 bytes
    ...
    const int N)
{
    ...
    // Subgroup position within work-group
    const int sg_local_id = get_sub_group_local_id(); // 0..7
    const int sg_x = lid0 / 8;  // 0..7 (along N)
    const int sg_y = lid1;       // 0..3 (along M)

    // Output tile origin for this subgroup
    const int tile_m = wg_m + sg_y * 8;
    const int tile_n = wg_n + sg_x * 8;

    // SLM: A[TM][TK+2] padded to avoid bank conflicts, B[TK][TN+2] padded
    // Padding by 2 halfs to shift bank alignment
    #define SLM_A_STRIDE (TK + 2)
    #define SLM_B_STRIDE (TN + 2)
    __local half slm_A[TM * SLM_A_STRIDE];  // 32 * 34 = 1088 halfs
    __local half slm_B[TK * SLM_B_STRIDE];  // 32 * 66 = 2112 halfs

    // Accumulators for 8x8 output tile
    float8 acc = (float8)(0.0f);

    // Pre-cast global pointers
    __global const ushort* A_us = (__global const ushort*)A;
    __global const ushort* B_us = (__global const ushort*)B;

    // Main K-loop in steps of TK=32
    int k0 = 0;
    for (; k0 + TK <= K; k0 += TK) {

        // === Cooperative load A[wg_m..wg_m+31][k0..k0+31] into slm_A ===
        // 256 WIs load 32*32 = 1024 halfs => 4 each
        for (int idx = flat_lid; idx < TM * TK; idx += 256) {
            int r = idx / TK;  // row within tile
            int c = idx % TK;  // col within tile
            int gr = wg_m + r;
            int gc = k0 + c;
            slm_A[r * SLM_A_STRIDE + c] = (gr < M && gc < K) ? A[gr * K + gc] : (half)0.0f;
        }

        // === Cooperative load B[k0..k0+31][wg_n..wg_n+63] into slm_B ===
        // 256 WIs load 32*64 = 2048 halfs => 8 each
        for (int idx = flat_lid; idx < TK * TN; idx += 256) {
            int r = idx / TN;
            int c = idx % TN;
            int gr = k0 + r;
            int gc = wg_n + c;
            slm_B[r * SLM_B_STRIDE + c] = (gr < K && gc < N) ? B[gr * N + gc] : (half)0.0f;
        }

        barrier(CLK_LOCAL_MEM_FENCE);

        // === Two DPAS k=16 steps within TK=32 ===
        #pragma unroll
        for (int kk = 0; kk < TK; kk += 16) {
            // Load A operand from SLM: 8 rows x 16 cols
            // Each lane sg_local_id loads row (sg_y*8 + sg_local_id)
            int a_row_local = sg_y * 8 + sg_local_id;
            __local const half* a_ptr = &slm_A[a_row_local * SLM_A_STRIDE + kk];

            // Pack 16 halfs into int8 (pairs of halfs -> int)
            int8 a_packed;
            __local const ushort* a_us = (__local const ushort*)a_ptr;
            ...
            // Load B operand from SLM: 16 rows x 8 cols
            // Pack pairs of k-rows into int (VNNI format)
            int b_col_local = sg_x * 8 + sg_local_id;
            int8 b_packed;
            ...
            for (int p = 0; p < 8; p++) {
                int kr0 = kk + p * 2;
                int kr1 = kk + p * 2 + 1;
                ushort v0 = as_ushort(slm_B[kr0 * SLM_B_STRIDE + b_col_local]);
                ushort v1 = as_ushort(slm_B[kr1 * SLM_B_STRIDE + b_col_local]);
                ((int*)&b_packed)[p] = (int)v0 | ((int)v1 << 16);
            }

            // DPAS: acc[8x8] += A[8x16] * B[16x8]
            acc = intel_sub_group_f16_f16_matrix_mad_k16(a_packed, b_packed, acc);
        }
    ...
    // === Handle remaining K (k0 to K) in steps of 16 with SLM ===
    if (k0 < K) {
        int rem = K - k0;

        // Load partial A tile
        for (int idx = flat_lid; idx < TM * rem; idx += 256) {
            int r = idx / rem;
            int c = idx % rem;
            int gr = wg_m + r;
            int gc = k0 + c;
            slm_A[r * SLM_A_STRIDE + c] = (gr < M) ? A[gr * K + gc] : (half)0.0f;
        }
        // Zero-pad to 16-aligned width
        for (int idx = flat_lid; idx < TM * (16 - rem % 16) % 16; idx += 256) {
            // Not needed if we just zero-initialize properly
        }

        // Load partial B tile
        for (int idx = flat_lid; idx < rem * TN; idx += 256) {
            int r = idx / TN;
            int c = idx % TN;
            int gr = k0 + r;
            int gc = wg_n + c;
            slm_B[r * SLM_B_STRIDE + c] = B[gr * N + gc];
        }

        barrier(CLK_LOCAL_MEM_FENCE);

        // Process in k=16 steps, padding with zeros for remainder
        int rem_aligned = ((rem + 15) / 16) * 16;
        for (int kk = 0; kk < rem_aligned; kk += 16) {
            int a_row_local = sg_y * 8 + sg_local_id;
            int8 a_packed = (int8)(0);
            __local const ushort* a_us = (__local const ushort*)&slm_A[a_row_local * SLM_A_STRIDE + kk];

            for (int p = 0; p < 8; p++) {
                int ci0 = kk + p * 2;
                int ci1 = kk + p * 2 + 1;
                ushort lo = (ci0 < rem) ? a_us[p * 2] : 0;
                ushort hi = (ci1 < rem) ? a_us[p * 2 + 1] : 0;
                ((int*)&a_packed)[p] = (int)lo | ((int)hi << 16);
            }

            int b_col_local = sg_x * 8 + sg_local_id;
            int8 b_packed = (int8)(0);

            for (int p = 0; p < 8; p++) {
                int kr0 = kk + p * 2;
                int kr1 = kk + p * 2 + 1;
                ushort v0 = (kr0 < rem) ? as_ushort(slm_B[kr0 * SLM_B_STRIDE + b_col_local]) : 0;
                ushort v1 = (kr1 < rem) ? as_ushort(slm_B[kr1 * SLM_B_STRIDE + b_col_local]) : 0;
                ((int*)&b_packed)[p] = (int)v0 | ((int)v1 << 16);
            }
...
1. `__local half slm_A[TM * SLM_A_STRIDE];` / `__local half slm_B[TK * SLM_B_STRIDE];` (inside kernel body): this forces **dynamic local memory allocation per work-group**, which increases register pressure and can reduce occupancy on Xe2. Also, your padding (`+2`) is static and not necessarily optimal for the bank layout.
   - Improve by moving SLM buffers to kernel arguments (`__local half* slm_A, __local half* slm_B`) with fixed host-provided sizes, and test `+0/+8/+16` padding variants. On Battlemage, reducing SLM footprint often improves active WG count and hides DPAS latency better.
   - Also remove the second `barrier(CLK_LOCAL_MEM_FENCE);` at the end of each K iteration unless you rely on cross-iteration reuse (you don’t). The first barrier after loads is required; the trailing one is usually redundant synchronization overhead.

2. `a_packed.s0 = (int)a_us[0] | ((int)a_us[1] << 16); ...` and `((int*)&b_packed)[p] = (int)v0 | ((int)v1 << 16);`: you are doing a lot of **scalar pack instructions** every inner step (`kk`), which burns ALU cycles and creates dependency chains around DPAS.
   - Improve by storing A/B into SLM already in DPAS-ready packed layout during the cooperative load phase (write `int` packed pairs directly to SLM, e.g. `__local int*`). Then the DPAS loop becomes mostly loads + DPAS, not repacking.
   - At minimum, use vectorized loads (`ushort2/ushort4`) and `as_int` reinterpret casts instead of scalar shifts in the hot path. This cuts instruction count in the innermost loop.

3. `for (int idx = flat_lid; idx < TK * TN; idx += 256) { ... slm_B[...] = ... }` and similarly for A: cooperative loads are currently **scalar half loads/stores with per-element bounds checks**, which is expensive and underutilizes memory bandwidth.
   - Improve by vectorizing global↔SLM copies (`vload8/vstore8` or `uint4`/`ulong` transactions) for the full-tile path (`k0 + TK <= K`, and fully in-bounds M/N tiles). Keep scalar + guarded path only for edge tiles.
   - Split kernel logic into fast path (no bounds checks for interior tiles) and tail path (current guarded code). Right now every tile pays branch/check overhead even when fully valid; separating paths usually gives a noticeable runtime drop.
