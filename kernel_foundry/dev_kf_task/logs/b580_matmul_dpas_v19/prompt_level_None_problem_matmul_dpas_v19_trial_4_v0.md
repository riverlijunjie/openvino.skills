...
### Version 2 (Result: Correctness score: 5 / 5 (compiles and correct), runtime in ms: 1.490):
```OCL
// GEMM: C[M,N] = A[M,K] x B[K,N], all row-major FP16, f32 accumulation
// Launch: GWS = (N, M/32), LWS = (64, 1)
//   - 64 WIs per WG = 4 subgroups of 16
...
#define SG_SIZE 16
#define NUM_SG 4
// SLM stride: 32 halfs + 4 padding = 36 halfs to avoid bank conflicts
// With stride 36, consecutive rows offset by 36*2=72 bytes, spreading across banks
    ...
    const int N)
{
    ...
    // Cooperative A load: 64 threads load 32×32 = 1024 halfs, 16 per thread
    const int a_load_row = local_id / 2;
    ...
    // B base for this subgroup's column
    ...
    // K=2048, TILE_K=32 -> 64 iterations. Unroll 2x -> 32 outer loops.
    // Each outer loop processes 2 K-tiles (64 K elements).
    const int k_outer = K / (TILE_K * 2);  // 32

    for (int ko = 0; ko < k_outer; ko++) {
        // Iteration A: ki = ko*2, buffer = 0 (even)
        {
            int k_base = ko * TILE_K * 2;
            __local half* a_slm = slm_a;  // buffer 0

            // Load next A tile into buffer 1 (overlapped with compute)
            {
            ...
            for (int kk = 0; kk < 2; kk++) {
                ...
                // Load B: 16 rows × 16 cols packed into int8 VNNI
                __global const half* b_ptr = b_base + k_off * N;
                int8 b_val;
                #pragma unroll
                for (int p = 0; p < 8; p++) {
                ...
                // DPAS blocks - interleave A loads with compute
                short8 a_val;
        ...
        // Iteration B: ki = ko*2+1, buffer = 1 (odd)
        {
            int k_base = ko * TILE_K * 2 + TILE_K;
            __local half* a_slm = slm_a + TILE_M * SLM_A_STRIDE;  // buffer 1

            // Load next A tile into buffer 0 (unless last outer iteration)
            if (ko < k_outer - 1) {
                int next_k = k_base + TILE_K;
                __global const half* a_src = a_row_ptr + next_k;
                __local half* a_dst = slm_a + a_load_row * SLM_A_STRIDE + a_load_col;
...
Here is the last OCL kernel we tested (Result: Correctness score: 5 / 5 (compiles and correct), runtime in ms: 1.340):
```OCL
// GEMM: C[M,N] = A[M,K] x B[K,N], all row-major FP16, f32 accumulation
// Launch: GWS = (N, M/32), LWS = (64, 1)
//   - 64 WIs per WG = 4 subgroups of 16
