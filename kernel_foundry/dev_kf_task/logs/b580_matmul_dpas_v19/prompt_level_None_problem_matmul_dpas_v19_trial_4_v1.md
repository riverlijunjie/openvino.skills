...
### Version 1 (Result: Correctness score: 5 / 5 (compiles and correct), runtime in ms: 1.550):
```OCL
...
#define SG_SIZE 16
#define NUM_SG 4
// SLM stride = 32 halfs (64 bytes). Subgroup block reads at offsets 0 and 16 are aligned.
    ...
    const int N)
{
    ...
    const int a_load_col = (local_id % 2) * 16;

    // B base for this subgroup
    __global const half* b_base = B + n_start + sg_n_offset;
    ...
    // Preload first A tile into buffer 0
    {
        ...
        // Use intel_sub_group_block_read_us for coalesced global load
        // Each subgroup of 16 threads: but we have 64 threads loading independently
        // Just use vload8 pairs for simplicity and correctness
    ...
    for (int ki = 0; ki < k_iters; ki++) {
        ...
        // Load next A tile (skip last iteration)
        // Since K=2048 and TILE_K=32, k_iters=64, we skip when ki==63
        if (ki < k_iters - 1) {
        ...
        // Two k16 steps per TILE_K=32
        #pragma unroll
        for (int kk = 0; kk < 2; kk++) {
            const int k_inner = kk * 16;
            const int k_global = k_base + k_inner;

            // Load B: use intel_sub_group_block_read_us for coalesced 16-wide reads
            // Each call reads 16 consecutive halfs (one per lane) from a row of B
            // B[k_row][n_start + sg_n_offset + 0..15]
            __global const ushort* b_row_base = (__global const ushort*)(b_base + k_global * N);

            int8 b_val;
            #pragma unroll
            for (int p = 0; p < 8; p++) {
                ushort lo = intel_sub_group_block_read_us(b_row_base + (2 * p) * N);
                ushort hi = intel_sub_group_block_read_us(b_row_base + (2 * p + 1) * N);
                ((int*)&b_val)[p] = (int)((uint)lo | ((uint)hi << 16));
            }

            // Load A from SLM using intel_sub_group_block_read_us for vectorized SLM access
            // Each block read gets 16 consecutive halfs from SLM (one per lane)
            __local const ushort* a_slm_us = (__local const ushort*)(a_slm + k_inner);
            ...
            // Block 0: rows 0-7
            {
                ...
                #pragma unroll
                for (int r = 0; r < 8; r++) {
                    a_val[r] = as_short(a_slm[(0 + r) * SLM_A_STRIDE + k_inner + sg_lid]);
                }
    ...
    // Write results using intel_sub_group_block_write_us for coalesced stores
    __global ushort* c_ptr = (__global ushort*)(C + m_start * N + n_start + sg_n_offset);

    #pragma unroll
    for (int r = 0; r < 8; r++) {
        intel_sub_group_block_write_us(c_ptr + (0 + r) * N, as_ushort(convert_half(acc0[r])));
    }
    #pragma unroll
    for (int r = 0; r < 8; r++) {
...
This is the code of the best kernel we have tested so far (Result: Correctness score: 5 / 5 (compiles and correct), runtime in ms: 1.340):
```OCL
// GEMM: C[M,N] = A[M,K] x B[K,N], all row-major FP16, f32 accumulation
// Launch: GWS = (N, M/32), LWS = (64, 1)
//   - 64 WIs per WG = 4 subgroups of 16
...
Here is the last OCL kernel we tested (Result: Correctness score: 5 / 5 (compiles and correct), runtime in ms: 1.490):
```OCL
// GEMM: C[M,N] = A[M,K] x B[K,N], all row-major FP16, f32 accumulation
// Launch: GWS = (N, M/32), LWS = (64, 1)
//   - 64 WIs per WG = 4 subgroups of 16
...
//   - TILE_M=32, TILE_N=64, TILE_K=32
// A loaded cooperatively into SLM (double-buffered), B from global/L2
// K-loop 2x unrolled: processes 2 K-tiles (64 k-elements) per iteration
...
#define SG_SIZE 16
#define NUM_SG 4
// SLM stride with padding to avoid bank conflicts (32 + 4 = 36 halfs)
    ...
    const int N)
{
    ...
    // Precompute A row pointer offset
    const int a_row_offset = (m_start + a_load_row) * K;

    // Load first A tile (k=0) into SLM buffer 0
    {
        __global const half* a_src = A + a_row_offset + a_load_col;
        __local half* a_dst = slm_a + a_load_row * SLM_STRIDE + a_load_col;
        half8 v0 = vload8(0, a_src);
        half8 v1 = vload8(1, a_src);
        vstore8(v0, 0, a_dst);
        vstore8(v1, 0, a_dst + 8);
    ...
    // K=2048, TILE_K=32 -> 64 iterations. Unroll 2x -> 32 outer iterations.
    const int k_iters = K / TILE_K;  // 64
    const int k_iters_half = k_iters / 2;  // 32

    // B column base for this subgroup
    const int b_col_base = n_start + sg_n_offset + sg_lid;
    ...
    for (int ki2 = 0; ki2 < k_iters_half; ki2++) {
        ...
        for (int sub = 0; sub < 2; sub++) {
            ...
            if (ki < k_iters - 1) {
                ...
                __global const half* a_src = A + a_row_offset + next_k + a_load_col;
                __local half* a_dst = slm_a + next_buf * TILE_M * SLM_STRIDE + a_load_row * SLM_STRIDE + a_load_col;
                half8 v0 = vload8(0, a_src);
                half8 v1 = vload8(1, a_src);
                vstore8(v0, 0, a_dst);
                vstore8(v1, 0, a_dst + 8);
            ...
            __local const half* a_base = slm_a + cur_buf * TILE_M * SLM_STRIDE;
            ...
            for (int kk = 0; kk < 2; kk++) {
                ...
                // Load B: 16 k-rows × 16 cols in VNNI format
                __global const half* b_ptr = B + k_global * N + b_col_base;
                ...
                for (int p = 0; p < 8; p++) {
                    ...
                    ((int*)&b_packed)[p] = (int)((uint)lo | ((uint)hi << 16));
                }
                ...
                // Block 0: rows 0-7
                {
                    ...
                    #pragma unroll
                    for (int r = 0; r < 8; r++) {
                        a_val[r] = as_short(a_base[r * SLM_STRIDE + k_inner + sg_lid]);
                    }
                ...
                // Block 1: rows 8-15
                {
                    ...
                    #pragma unroll
                    for (int r = 0; r < 8; r++) {
                        a_val[r] = as_short(a_base[(8 + r) * SLM_STRIDE + k_inner + sg_lid]);
                    }
                ...
                // Block 2: rows 16-23
                {
                    ...
                    #pragma unroll
                    for (int r = 0; r < 8; r++) {
                        a_val[r] = as_short(a_base[(16 + r) * SLM_STRIDE + k_inner + sg_lid]);
                    }
                ...
                // Block 3: rows 24-31
                {
                    ...
                    #pragma unroll
                    for (int r = 0; r < 8; r++) {
                        a_val[r] = as_short(a_base[(24 + r) * SLM_STRIDE + k_inner + sg_lid]);
                    }
    ...
    __global half* c_ptr = C + m_start * N + b_col_base;
...
======================= 4 passed, 1 deselected in 0.81s ========================
