...
### Version 2 (Result: Correctness score: 5 / 5 (compiles and correct), runtime in ms: 1.220):
```OCL
// GEMM: C[M,N] = A[M,K] x B[K,N], all row-major FP16, f32 accumulation
// Launch: GWS = (N, M/32), LWS = (64, 1)
//   - 64 WIs per WG = 4 subgroups of 16
...
//   - TILE_M=32, TILE_N=64, TILE_K=32
// A loaded cooperatively into SLM (double-buffered), B from global/L2
// K=2048 always divides by 64, K-loop unrolled 2x (process 2 TILE_K per iteration)
// Hardware: Intel B580, Xe2, exec_size=16, 96 TFLOPS FP16 XMX
...
#define SG_SIZE 16
#define NUM_SG 4
// Pad SLM stride to 34 halfs (68 bytes) to reduce bank conflicts
// Xe2 has 16 SLM banks × 4B each = 64B per bank cycle
// Stride=32 halfs=64B means rows 0,1,2... all start at same bank offset
// Stride=34 halfs=68B staggers by 4B per row, reducing conflicts
    ...
    const int N)
{
    ...
    // SLM double buffer for A: 2 × 32 rows × 34 halfs = 4352 bytes total
    __local half slm_a[2 * TILE_M * SLM_A_STRIDE];

    // Accumulators: 32 rows × 16 cols per subgroup (4 blocks of 8 rows)
    float8 acc0 = (float8)(0.0f);
    float8 acc1 = (float8)(0.0f);
    float8 acc2 = (float8)(0.0f);
    float8 acc3 = (float8)(0.0f);
    ...
    // K=2048, TILE_K=32 => 64 k-tiles
    // Unroll 2x: process pairs of k-tiles per loop iteration => 32 outer iterations
    // Double-buffering: while computing from buf[cur], load next A into buf[next]
    const int k_iters = K / TILE_K;  // 64
    const int k_pairs = k_iters / 2; // 32

    for (int kp = 0; kp < k_pairs - 1; kp++) {
        // Process two consecutive k-tiles: ki=2*kp and ki=2*kp+1
        // For first k-tile (ki=2*kp): data is in current buffer, load next into other buffer
        // For second k-tile (ki=2*kp+1): data is in other buffer, load next into current buffer

        const int ki0 = 2 * kp;
        const int ki1 = ki0 + 1;

        // === First k-tile (ki0): compute from buf[ki0&1], load buf[ki1&1] ===
        {
            const int cur_buf = ki0 & 1;
            const int next_buf = 1 - cur_buf;
            const int k_base = ki0 * TILE_K;

            // Load next A tile (for ki1) into next_buf
            {
        ...
        // === Second k-tile (ki1): compute from buf[ki1&1], load buf[(ki1+1)&1] ===
        {
            const int cur_buf = ki1 & 1;
            const int next_buf = 1 - cur_buf;
            const int k_base = ki1 * TILE_K;

            // Load next A tile (for ki1+1 = first tile of next pair) into next_buf
            {
                __global const half* a_src = a_row_ptr + (k_base + TILE_K);
                __local half* a_dst = slm_a + next_buf * TILE_M * SLM_A_STRIDE + a_load_row * SLM_A_STRIDE + a_load_col;
                half8 v0 = vload8(0, a_src);
    ...
    // Last pair (kp = k_pairs - 1): ki0 = k_iters-2, ki1 = k_iters-1
    {
        const int ki0 = k_iters - 2;
        const int ki1 = k_iters - 1;

        // First of last pair: compute ki0, load ki1
        {
            const int cur_buf = ki0 & 1;
            const int next_buf = 1 - cur_buf;
            const int k_base = ki0 * TILE_K;
        ...
        // Second of last pair (ki1): compute only, no next load
        {
            const int cur_buf = ki1 & 1;
            const int k_base = ki1 * TILE_K;
            __local const half* a_slm = slm_a + cur_buf * TILE_M * SLM_A_STRIDE;
