...
### Version 1 (Result: Correctness score: 5 / 5 (compiles and correct), runtime in ms: 1.340):
```OCL
...
### Version 2 (Result: Correctness score: 5 / 5 (compiles and correct), runtime in ms: 1.370):
```OCL
// GEMM: C[M,N] = A[M,K] x B[K,N], all row-major FP16, f32 accumulation
// Launch: GWS = (N, M/32), LWS = (64, 1)
//   - 64 WIs per WG = 4 subgroups of 16
    ...
    const int N)
{
    ...
    // Cooperative A load setup: 64 threads load 32×32 halfs
    // Thread i loads row i/2, columns (i%2)*16 .. (i%2)*16+15
    const int a_load_row = local_id / 2;
    const int a_load_col = (local_id & 1) * 16;

    // B base pointer for this subgroup (using block reads)
    __global const ushort* b_base = (__global const ushort*)(B + n_start + sg_n_offset);

    // A source pointer for cooperative loads
    __global const half* a_row_ptr = A + (m_start + a_load_row) * K + a_load_col;
    ...
    // K=2048, TILE_K=32 => 64 iterations
    // Main loop: iterations 0..62 (load next + compute current)
    // Last iteration: 63 (compute only)
    const int k_iters = K / TILE_K;  // 64
    ...
    for (int ki = 0; ki < k_iters - 1; ki++) {
        ...
        // Process 2 k16 steps per TILE_K=32
        // Use intel_sub_group_block_read_us for B loads (coalesced 16-wide)
        #pragma unroll
        for (int kk = 0; kk < 2; kk++) {
            const int k_inner = kk * 16;
            const int k_global = k_base + k_inner;

            // Load B tile: 16 rows × 16 cols, packed into VNNI int8 format
            // Each block_read_us reads 16 consecutive halfs from one B row
            __global const ushort* b_row = b_base + k_global * N;

            int8 b_val;
            #pragma unroll
            for (int p = 0; p < 8; p++) {
                ushort u0 = intel_sub_group_block_read_us(b_row + (2 * p) * N);
                ushort u1 = intel_sub_group_block_read_us(b_row + (2 * p + 1) * N);
                ((int*)&b_val)[p] = (int)((uint)u0 | ((uint)u1 << 16));
            }

            // Load A from SLM and compute DPAS
            // Interleave A loads with DPAS for better latency hiding
            {
                short8 a_val;
                __local const half* a_ptr = a_slm + k_inner + sg_lid;
                #pragma unroll
    ...
    // Last iteration (ki = k_iters - 1): compute only, no next A load
    {
        const int ki = k_iters - 1;
        const int cur_buf = ki & 1;
        const int k_base = ki * TILE_K;
    ...
    __global ushort* c_out = (__global ushort*)(C + m_start * N + n_start + sg_n_offset);

    #pragma unroll
    for (int r = 0; r < 8; r++)
        intel_sub_group_block_write_us(c_out + r * N, as_ushort(convert_half(acc0[r])));
    #pragma unroll
    for (int r = 0; r < 8; r++)
        intel_sub_group_block_write_us(c_out + (8 + r) * N, as_ushort(convert_half(acc1[r])));
    #pragma unroll
    for (int r = 0; r < 8; r++)
        intel_sub_group_block_write_us(c_out + (16 + r) * N, as_ushort(convert_half(acc2[r])));
    #pragma unroll
    for (int r = 0; r < 8; r++)
        intel_sub_group_block_write_us(c_out + (24 + r) * N, as_ushort(convert_half(acc3[r])));
}
```

### Previous kernel and evaluation:
Here is the last OCL kernel we tested (Result: Correctness score: 5 / 5 (compiles and correct), runtime in ms: 1.220):
```OCL
// GEMM: C[M,N] = A[M,K] x B[K,N], all row-major FP16, f32 accumulation
// Launch: GWS = (N, M/32), LWS = (64, 1)
//   - 64 WIs per WG = 4 subgroups of 16
...
1. Reduce Divergent Branches: Keep all work-items in a wavefront/warp on the same execution path. Move divergent code outside the kernel or restructure to process uniform batches. Group similar work together.
