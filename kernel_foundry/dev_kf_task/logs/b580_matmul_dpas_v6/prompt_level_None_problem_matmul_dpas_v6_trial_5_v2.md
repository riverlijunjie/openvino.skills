...
1. `float b0 = convert_float(B_slm[kk * TILE_N + lx * 2]);` / `float b1 = convert_float(B_slm[kk * TILE_N + lx * 2 + 1]);` and `float a_val = convert_float(A_slm[(ly * THREAD_M + r) * TILE_K + kk]);`: you are doing FP16→FP32 conversion inside the innermost MAC loop, so each FMA pays extra conversion latency and instruction pressure.  
   **Improve it:** preload a register fragment per `kk` once, then reuse:
   - load `a_frag[r]` (8 values) once per `kk` before the `r`-accumulation FMAs (or keep them in `half` vectors and convert in vector form),
   - load `b_frag` once per `kk` as a `half2`/`float2`,
   - then perform pure FMA on registers.  
   This reduces repeated scalar converts and improves ILP.

2. `if (gm < M && gk + 7 < K) { ... } else { ... }` (A load) and `if (gk < K && gn + 1 < N) { ... } else { ... }` (B load): boundary checks are inside every K-tile iteration for every work-item, causing control divergence and extra instructions even for full interior tiles.  
   **Improve it:** split into fast path + tail path:
   - main kernel path assumes full tiles (`wg_m + TILE_M <= M`, `wg_n + TILE_N <= N`, `k0 + TILE_K <= K`) and uses branch-free vector loads/stores,
   - separate cleanup handling for edge tiles (or a guarded epilogue loop only for last tile).  
   This is usually a major runtime win because most workgroups are interior.

3. `A_slm[base + x] = ...` / `B_slm[base] = ...` with `__local half A_slm[...]`, `__local half B_slm[...]`: current SLM access pattern risks bank conflicts and underutilizes vectorized local-memory transactions in compute phase (especially scalar `half` reads per `kk`).  
   **Improve it:** store/load in vectorized layout and pad stride:
   - use `__local half8` (or `half4`) for A and `__local half2/half4` for B where possible,
   - pad leading dimension (e.g., `A_slm[TILE_M][TILE_K+1]`, `B_slm[TILE_K][TILE_N+1]`) to break bank-aliasing on `kk`-major access,
   - in compute, fetch vector chunks from SLM into registers, then unroll FMAs.  
   This lowers SLM bank conflicts and reduces local-memory instruction count.
