...
1. `/* Load 4 B values directly from global memory */ ... _b0 = convert_float(B[(_gk0 + 0) * N + out_col]); ...` inside `COMPUTE_TILE`: this is your biggest bottleneck.  
   You reload B from global memory for every `_kk` step and every thread, so the same B tile is fetched repeatedly with little reuse. That makes the kernel bandwidth-limited.  
   **Improve it by tiling B into local memory too** (double-buffered like A), or at least cache a per-thread B strip in registers across multiple row FMAs. For your tile shape (128x16x64), one B tile is only `64*16=1024` halfs (~2 KB), so adding 2-way ping-pong for B costs ~4 KB SLM and can drastically cut global traffic. Then compute uses `B_slm` loads instead of global loads in the inner loop.

2. `acc0 = fma(convert_float(A_slm[buf][_aoff0+0]), _b0, acc0);` (and all similar `convert_float(A_slm...)` in the inner loop): repeated half→float conversion in the hottest loop is expensive.  
   You’re doing conversion per FMA operand, which adds extra instructions and pressure in the critical path.  
   **Improve by vectorizing/unrolling with packed loads**: load `half4` (or `ushort` packed + convert) from A and B, convert once to `float4`, then do 4 FMAs from registers. Also consider storing A in SLM in a layout friendly to vector loads (`half4` aligned) so each thread reads contiguous packed elements. This reduces conversion overhead and improves ILP.

3. `barrier(CLK_LOCAL_MEM_FENCE);` after `LOAD_A(nxt, next_k0);` in the main loop: your “double buffering” is not actually overlapping load and compute.  
   In the current order, all threads finish loading next A tile, then compute current tile, then synchronize. That is still mostly phased execution, so latency hiding is weak.  
   **Improve pipeline ordering**: compute on current tile first, while prefetching next tile as early as possible, and use a true ping-pong schedule with minimal full-WG barriers (one before consuming a freshly loaded tile). If your target supports async copy to local memory, use async prefetch for A/B tile + wait group before use. That gives real overlap between memory and math and usually lowers runtime materially.
