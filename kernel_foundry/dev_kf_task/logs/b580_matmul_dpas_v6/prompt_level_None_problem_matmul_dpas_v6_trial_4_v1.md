...
1. `if (next_k < K) { ... A_slm[next_buf][idx] = ...; B_slm[next_buf][idx] = ...; }` + later `barrier(CLK_LOCAL_MEM_FENCE);` inside the K-loop: your “double buffering” is logically correct, but it is not actually overlapping memory and compute, because prefetch and compute are done by the same wave before the barrier. You still pay a full-tile sync each iteration.  
   **Improve it:** switch to true async local-memory copies (`async_work_group_copy` + `event_t`/`wait_group_events`) so global→SLM transfer of tile `k+1` runs while ALU computes tile `k`. If async copy isn’t available/fast on your target, reduce barrier frequency by computing multiple `kk` chunks per sync (e.g., two 16-wide stages) and prefetch per stage. Right now the barrier cost is a major limiter.

2. `float b0 = convert_float(B_slm[buf][kk * TILE_N + col_base]);`  
   `float b1 = convert_float(B_slm[buf][kk * TILE_N + col_base + 1]);`  
   plus scalar SLM loads in the inner loop: you are doing scalar half loads and scalar converts every `kk`, which increases instruction count and stresses SLM/L1 path.  
   **Improve it:** vectorize these accesses. Load `half2` for B (`vload_half2`/reinterpret) and convert once to `float2`; similarly load A in packed form (`half2`/`half4`) and unroll FMA on vectors. Also pad B tile stride to avoid bank conflicts (e.g., `B_slm[2][TILE_K*(TILE_N+PAD)]`, `PAD=1..2`), since all threads read same `kk` row with contiguous columns—classic conflict pattern on many local-memory banks.

3. `float acc[THREAD_M][THREAD_N];` with `THREAD_M=8, THREAD_N=2` under `WG_X=16, WG_Y=16` (256 threads): this is a high per-thread register footprint (16 accumulators + temporaries), which can reduce occupancy and hide less latency. Combined with 20KB SLM/workgroup, you may be limiting active workgroups/CU.  
   **Improve it:** retune tile/thread shape for occupancy: e.g., reduce WG to 128 threads (like 16x8 or 8x16) and increase per-thread compute slightly (or keep compute and lower accumulator pressure with `THREAD_M=4..8`, `THREAD_N=2..4` depending on ISA). Profile register count and active waves; pick the shape that maximizes sustained throughput, not just arithmetic intensity. This usually gives a noticeable runtime drop versus a fixed 256-thread design.
