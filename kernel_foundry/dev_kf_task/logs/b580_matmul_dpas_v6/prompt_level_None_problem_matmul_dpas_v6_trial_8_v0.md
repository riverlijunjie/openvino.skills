...
1. `/* Load 4 B values directly from global memory */ ... _b0 = convert_float(B[(_gk0 + 0) * N + out_col]); ...` inside `COMPUTE_TILE`  
   You are reloading `B[k, out_col]` from global memory for every subgroup and every `THREAD_M` row accumulation step, so the kernel is heavily bandwidth-limited on B. Even though each thread reuses `_b0.._b3` across 8 FMAs, there is still no inter-thread reuse.  
   **Improve it by tiling B in local memory too (double-buffered like A)**: allocate `__local half B_slm[2][TILE_K * TILE_N]` (only 2*64*16 half = 4KB), have threads cooperatively load B tile once per K-tile, then read B from SLM in the compute loop. This turns many global scalar loads into shared loads and substantially reduces DRAM traffic. If SLM budget is tight, at least load B with vectorized global loads (`half4`/`half8`) per thread and keep a small register tile for B.

2. `A_slm[buf][_idx] = A[(wg_m + _r) * K + (k0) + _c];` and later `convert_float(A_slm[buf][_aoff+...])`  
   A is loaded and consumed as **scalar half**, and each multiply does an on-the-fly `half->float` conversion. That creates a lot of conversion instructions and misses vector datapath opportunities.  
   **Improve by vectorizing A movement and compute granularity**: load A as `half8`/`half4` (aligned path for interior tiles), and in compute use packed operations (`half8` dot-like accumulation pattern into float registers, or subgroup matrix ops if available on your target, e.g. Intel joint_matrix / DPAS equivalents). At minimum, unroll on K with vector loads and convert once per vector (`float4`) instead of four scalar converts.

3. `barrier(CLK_LOCAL_MEM_FENCE);` after `LOAD_A(0,0)` and once per K-tile in the main loop  
   The double-buffering is only partial: you prefetch A before compute, but due to barrier placement and synchronous workgroup progression, load/compute overlap is limited, and every iteration still pays full workgroup barrier cost.  
   **Improve pipeline depth and reduce synchronization stalls**: use true staged ping-pong for both A and B tiles where possible, and restructure loop so each iteration does: compute on `cur` while preloading `next` as early as possible, with exactly one barrier at tile handoff. Also split interior fast path from boundary path at kernel level (or early branch) so interior tiles avoid per-iteration bounds checks (`if (col_valid && _gk0 + 3 < K)`), reducing control divergence and helping compiler scheduling.
