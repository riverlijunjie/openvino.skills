...
Here is the last OCL kernel we tested (Result: Correctness score: 5 / 5 (compiles and correct), runtime in ms: 212.000):
```OCL
#pragma OPENCL EXTENSION cl_khr_fp16 : enable
...
// LWS = (16, 16, 1) = 256 work-items
//   16 threads along x cover 32 cols (stride 16, each does col lx and lx+16)
//   16 threads along y cover 128 rows (8 contiguous rows each)
// GWS = (ceil(N/32)*16, ceil(M/128)*16, 1)
// SLM: A_slm[128][16+1] padded, B_slm[16][32+1] padded to avoid bank conflicts
...
// Pad to avoid SLM bank conflicts
#define A_STRIDE (TILE_K + 1)
#define B_STRIDE (TILE_N + 1)
    ...
    const int N)
{
    ...
    // rows: ly*8 .. ly*8+7 (contiguous for A reuse)
    // cols: lx, lx+16 (stride 16 for B broadcast)
    ...
    __local half B_slm[TILE_K * B_STRIDE];  // 16 x 33

    for (int k0 = 0; k0 < K; k0 += TILE_K) {
        // Load A_slm[128][16] with padding stride 17
        // 128*16 = 2048 halfs, 256 threads => 8 each
        #pragma unroll
        for (int i = 0; i < 8; i++) {
            int idx = flat_id + i * NUM_THREADS;
            int r = idx >> 4;    // idx / 16
        ...
        // Load B_slm[16][32] with padding stride 33
        // 16*32 = 512 halfs, 256 threads => 2 each
        #pragma unroll
        for (int i = 0; i < 2; i++) {
            int idx = flat_id + i * NUM_THREADS;
            int r = idx >> 5;    // idx / 32
        ...
        #pragma unroll
        for (int kk = 0; kk < TILE_K; kk++) {
            // Load 2 B values for this thread's columns
            float b0 = convert_float(B_slm[kk * B_STRIDE + lx]);
            float b1 = convert_float(B_slm[kk * B_STRIDE + lx + 16]);

            // Load 8 A values and accumulate
            #pragma unroll
            for (int r = 0; r < THREAD_M; r++) {
                float a_val = convert_float(A_slm[(row_base + r) * A_STRIDE + kk]);
    ...
    for (int r = 0; r < THREAD_M; r++) {
        ...
        if (out_row < M) {
            int col0 = wg_n + lx;
            if (col0 < N)
                C[out_row * N + col0] = convert_half(acc[r][0]);
            int col1 = wg_n + lx + 16;
            if (col1 < N)
                C[out_row * N + col1] = convert_half(acc[r][1]);
        }
    }
}
```
...
1. `__local half A_slm[TILE_M * A_STRIDE];` and `__local half B_slm[TILE_K * B_STRIDE];`: you are spending a lot of local-memory bandwidth every `kk` step by reloading scalar halves from SLM (`A_slm[...]` and `B_slm[...]`) and converting each to float in the inner loop.  
   **Why it hurts:** inner-loop SLM traffic + repeated `convert_float` can become the limiter instead of FMA throughput.  
   **What to change:** keep B in registers for each thread across multiple rows and vectorize A loads. Concretely:
   - load `b0/b1` once per `kk` (you already do), but unroll `r` fully and load A as `half8`/`float8` chunks (or two `half4`) to reduce indexing and conversion overhead;
   - if your target supports it, use half-native math (`half`/`half2` accumulate in float via dot/fma intrinsics) so conversion happens in wider chunks, not scalar per element.
   This reduces instruction count and pressure on the local-memory path.
...
2. 
   ...
   A_slm[r * A_STRIDE + c] = (gm < M && gk < K) ? A[gm * K + gk] : (half)0.0h;
   ```
   and final store bounds:
   ```c
   if (out_row < M) { if (col0 < N) ...; if (col1 < N) ...; }
   ```
   **Why it hurts:** boundary checks are on hot paths for *every tile*, causing divergence and extra predicates even for fully interior workgroups.  
   **What to change:** split fast path and edge path:
   - fast kernel body for interior tiles where `wg_m + TILE_M <= M && wg_n + TILE_N <= N && k0 + TILE_K <= K`, with no per-element bounds checks;
   - keep your current guarded loads/stores only for boundary groups.
   This usually gives a noticeable speedup because most CTAs run branch-free.

3. `for (int k0 = 0; k0 < K; k0 += TILE_K) { ... barrier ... compute ... barrier ... }`: the loop is strictly “load tile, barrier, compute, barrier”, so compute and memory are serialized.  
   **Why it hurts:** no overlap between global->SLM copy and FMA work; latency of global loads is exposed each iteration.  
   **What to change:** add double-buffered local tiles (ping-pong):
   - allocate `A_slm[2][...]`, `B_slm[2][...]`;
   - preload tile 0, then in each iteration compute on buffer `p` while prefetching next tile into buffer `1-p`;
   - use barriers only when swapping buffers.
   This pipelines memory and compute and is one of the highest-impact changes for GEMM-style kernels.
