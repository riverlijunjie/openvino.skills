...
1. `/* Load 4 B values directly from global memory */ ... _b0 = convert_float(B[(_gk0 + 0) * N + out_col]); ...` inside `COMPUTE_TILE`  
   You’re reloading `B` from global memory in the innermost K loop for every thread, which is the dominant runtime bottleneck (high latency + poor reuse). In your current mapping, many threads consume overlapping `B` rows but still fetch independently.  
   **Improve it:** tile `B` into local memory too (double-buffer it like `A`), e.g., `B_slm[2][TILE_K * TILE_N]`, and have the 256 threads cooperatively load each `64x16` tile once per K-tile. Then compute from SLM for both operands. This usually gives a large speedup because each `B[k,n]` value is reused by 128 output rows in the workgroup instead of being fetched repeatedly from global memory.

2. `if (col_valid && _gk0 + 3 < K) { ... } else { ... ? ... : 0.0f; }` in every `_kk` iteration  
   This injects control flow and per-element bounds checks into the hot inner loop, causing branch overhead and inhibiting vectorized/load-combine codegen. Even when most tiles are interior, the compiler must preserve the boundary path.  
   **Improve it:** split compute into two paths outside the hot loop:  
   - **Fast interior kernel path** (or interior branch): when `wg_n + TILE_N <= N` and current K tile fully valid, run a branch-free loop with unconditional loads.  
   - **Remainder path** only for edge tiles.  
   This keeps the main FMA loop clean and usually improves IPC and register scheduling.

3. `A_slm[buf][_idx] = A[(wg_m + _r) * K + (k0) + _c];` and scalar loads for both A/B (`half` one-by-one)  
   You’re doing scalar half loads and scalar convert operations, which underutilize memory transactions and increases instruction count.  
   **Improve it:** vectorize loads/converts in the cooperative load and compute path (e.g., `half2/half4` or `ushort2/ushort4` + vector convert), and unroll in a way that consumes vectors per iteration. For example, load `half4` from `B` and `A` where alignment allows, then convert to `float4` and accumulate. Also consider changing load mapping so each thread reads contiguous chunks (`_idx` linear chunks already helps for A; apply same idea for B tile staging). This reduces memory instructions and conversion overhead in the tight loop.
