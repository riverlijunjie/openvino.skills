...
1. `float b_val = convert_float(B_slm[kk * TILE_N + lx]);` together with `const int out_col = wg_n + lx;` and `#define THREAD_N 1`: you are giving each thread only **1 output column**, so arithmetic intensity per loaded `A_slm` value is low, and each thread repeatedly reloads one `B` scalar per `kk`.  
   **Improve it by increasing per-thread N blocking** (e.g., `THREAD_N=2/4`) and making each thread compute multiple columns:
   - Keep `acc[THREAD_M][THREAD_N]`.
   - Load `THREAD_N` `b_val` values per `kk` (ideally vectorized, e.g. `half2/half4` where alignment allows).
   - Update all `THREAD_M * THREAD_N` FMAs.  
   This reuses `A` values across more outputs and usually gives a large speedup on GEMM-style kernels.

2. 
   - `A_slm[i] = (gm < M && gk < K) ? A[gm * K + gk] : (half)0.0h;`
   - `B_slm[i] = (gk < K && gn < N) ? B[gk * N + gn] : (half)0.0h;`  
   These boundary checks run in the hot K-loop on every tile load, creating extra control flow and instruction overhead for all workgroups, even interior ones.  
   **Improve by splitting interior vs edge kernels (or interior fast path):**
   - Fast path for full tiles (`wg_m + TILE_M <= M`, `wg_n + TILE_N <= N`, and `k0 + TILE_K <= K`) with **no per-element conditionals**.
   - Separate tail handling for boundary tiles only.  
   This removes branches from the dominant path and improves memory throughput.

3. 
   - `A_slm[i]` / `B_slm[i]` scalar cooperative loads
   - `float a_val = convert_float(A_slm[(ly * THREAD_M + r) * TILE_K + kk]);`
   - `float b_val = convert_float(B_slm[kk * TILE_N + lx]);`  
   You are doing mostly scalar half loads and scalar half→float converts in the inner loop. That underutilizes memory bandwidth and vector ALUs.  
   **Improve with vectorized loads + packed math path:**
   - During SLM fill, load global memory as `half2/half4` (or wider) and store vector chunks to local memory.
   - In compute, iterate `kk` by 2 (or 4), load packed `half2` from `A_slm/B_slm`, convert once to `float2`, and accumulate two FMAs per loop step.
   - Keep layout/alignment so these vector loads are naturally aligned.  
   This reduces load/conversion instruction count and typically improves both occupancy headroom and inner-loop throughput.
