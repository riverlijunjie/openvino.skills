...
Here is the last OCL kernel we tested (Result: Correctness score: 5 / 5 (compiles and correct), runtime in ms: 3.360):
```OCL
#pragma OPENCL EXTENSION cl_intel_subgroups : enable
#pragma OPENCL EXTENSION cl_intel_subgroup_matrix_multiply_accumulate : enable
#pragma OPENCL EXTENSION cl_khr_fp16 : enable
...
// Double-buffered SLM for A only; B read from global (L2 cached across subgroups)
// SLM A: 2 x 64 x 66 x 2 bytes = ~17 KB
// GWS = (ceil(N/128)*128, ceil(M/64))  LWS = (128, 1)
...
#define SLM_A_STRIDE (TILE_K + 2)
#define NUM_WI 128
#define A_ELEMS_PER_WI ((TILE_M * TILE_K) / NUM_WI)  // 64*64/128 = 32
    ...
    const int N)
{
    ...
    for (int kt = 0; kt < num_k_tiles; kt++) {
        ...
        #pragma unroll
        for (int kk = 0; kk < TILE_K; kk += 16) {
            // Load B from global: each WI in subgroup reads its column
            int8 b_val;
            ...
            // Load A blocks from SLM and compute DPAS
            short8 a_block;
