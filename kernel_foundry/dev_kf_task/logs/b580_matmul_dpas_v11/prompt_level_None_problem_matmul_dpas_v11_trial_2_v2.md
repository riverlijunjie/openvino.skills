...
This is the code of the best kernel we have tested so far (Result: Correctness score: 5 / 5 (compiles and correct), runtime in ms: 2.190):
```OCL
#pragma OPENCL EXTENSION cl_intel_subgroups : enable
#pragma OPENCL EXTENSION cl_intel_subgroup_matrix_multiply_accumulate : enable
#pragma OPENCL EXTENSION cl_khr_fp16 : enable
...
Here is the last OCL kernel we tested (Result: Correctness score: 5 / 5 (compiles and correct), runtime in ms: 5.050):
```OCL
#pragma OPENCL EXTENSION cl_intel_subgroups : enable
#pragma OPENCL EXTENSION cl_intel_subgroup_matrix_multiply_accumulate : enable
#pragma OPENCL EXTENSION cl_khr_fp16 : enable

// TILE_M=32, TILE_N=64, TILE_K=32, A in SLM (double-buffered), B from global
// 4 subgroups x 16 WIs = 64 WIs per WG
// Each subgroup: 4 row-blocks x 16 cols = 32x16 output tile
// Per K-tile: 2 k16 steps x 4 row-blocks = 8 DPAS
// Double buffering: load next A while computing current → 1 barrier between iterations
// SLM: 2 x 32 x 34 halves = ~4.4KB (tiny, fits easily)
// GWS = (ceil(N/64)*64, ceil(M/32))  LWS = (64, 1)
    ...
    const int N)
{
    ...
    // A tile: 32 x 32 = 1024 halves, 64 WIs => 16 each
    ...
    for (int kt = 0; kt < num_k_tiles; kt++) {
        ...
        // Compute on current buffer: 2 k16 steps x 4 row-blocks = 8 DPAS
        #pragma unroll
        for (int kk = 0; kk < TILE_K; kk += 16) {
            // Load A sub-tiles from SLM
            short8 a0, a1, a2, a3;
            #pragma unroll
            for (int r = 0; r < 8; r++)
        ...
        // Load next A tile into nxt_buf while compute is done
        if (has_next) {
            barrier(CLK_LOCAL_MEM_FENCE);
