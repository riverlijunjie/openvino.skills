...
Here is the last OCL kernel we tested (Result: Correctness score: 5 / 5 (compiles and correct), runtime in ms: 3.440):
```OCL
#pragma OPENCL EXTENSION cl_intel_subgroups : enable
#pragma OPENCL EXTENSION cl_intel_subgroup_matrix_multiply_accumulate : enable
#pragma OPENCL EXTENSION cl_khr_fp16 : enable
...
// 8 subgroups x 16 WIs = 128 WIs per WG
// Each subgroup handles 64 rows x 16 cols of output
// Per K-tile: 2 k16 steps x 8 row-blocks = 16 DPAS per subgroup per k-tile
// A in double-buffered SLM (~8.7 KB total), B from global (L2 cached)
// Low SLM = high occupancy = better latency hiding
// GWS = (ceil(N/128)*128, ceil(M/64))  LWS = (128, 1)
...
#define SLM_A_STRIDE (TILE_K + 2)
#define NUM_WI 128
#define A_ELEMS_PER_WI 16  // 64*32/128
    ...
    const int N)
{
    ...
    // Double-buffered A in SLM: 2 x 64 x 34 halves = 8704 bytes
    __local half slm_A[2 * TILE_M * SLM_A_STRIDE];
    ...
    // Precompute A load indices (same for every tile, only k_offset changes)
    int a_r[A_ELEMS_PER_WI], a_c[A_ELEMS_PER_WI];
    int a_slm_off[A_ELEMS_PER_WI];
    #pragma unroll
    for (int i = 0; i < A_ELEMS_PER_WI; i++) {
        int elem_id = lid + i * NUM_WI;
        a_r[i] = elem_id / TILE_K;
        a_c[i] = elem_id % TILE_K;
        a_slm_off[i] = a_r[i] * SLM_A_STRIDE + a_c[i];
    }
    ...
    // Load first A tile into buffer 0
    {
        ...
        if (row_tile_full && TILE_K <= K) {
            #pragma unroll
            for (int i = 0; i < A_ELEMS_PER_WI; i++) {
                dst[a_slm_off[i]] = A[(row_base + a_r[i]) * K + a_c[i]];
            }
        } else {
            #pragma unroll
            for (int i = 0; i < A_ELEMS_PER_WI; i++) {
                int gr = row_base + a_r[i];
                dst[a_slm_off[i]] = (gr < M && a_c[i] < K) ? A[gr * K + a_c[i]] : (half)0.0h;
            }
        }
    }
    barrier(CLK_LOCAL_MEM_FENCE);
    ...
    for (int kt = 0; kt < num_k_tiles; kt++) {
        ...
        // ---- Compute: 2 k16 steps x 8 row-blocks = 16 DPAS ----
        // Pre-load all A blocks into registers, then fire DPAS back-to-back
        #pragma unroll
        for (int kk = 0; kk < TILE_K; kk += 16) {
            // Load all 8 A blocks into registers first
            short8 a0, a1, a2, a3, a4, a5, a6, a7;
            ...
            // Load B from global: each subgroup lane reads its column
            int8 b_val;
            if (col_valid) {
                int gk = k_cur + kk;
                #pragma unroll
            ...
            // Fire all 8 DPAS back-to-back for maximum XMX utilization
            acc0 = intel_sub_group_f16_f16_matrix_mad_k16(a0, b_val, acc0);
            acc1 = intel_sub_group_f16_f16_matrix_mad_k16(a1, b_val, acc1);
            acc2 = intel_sub_group_f16_f16_matrix_mad_k16(a2, b_val, acc2);
            acc3 = intel_sub_group_f16_f16_matrix_mad_k16(a3, b_val, acc3);
        ...
        // ---- Load next A tile into nxt_buf (overlapped with compute via pipelining) ----
        if (has_next) {
            barrier(CLK_LOCAL_MEM_FENCE);
            __local half* nxt_A = slm_A + nxt_buf * TILE_M * SLM_A_STRIDE;
            if (row_tile_full && k_next + TILE_K <= K) {
                #pragma unroll
                for (int i = 0; i < A_ELEMS_PER_WI; i++) {
                    nxt_A[a_slm_off[i]] = A[(row_base + a_r[i]) * K + k_next + a_c[i]];
                }
            } else {
                #pragma unroll
                for (int i = 0; i < A_ELEMS_PER_WI; i++) {
                    int gr = row_base + a_r[i];
                    int gk = k_next + a_c[i];
                    nxt_A[a_slm_off[i]] = (gr < M && gk < K) ? A[gr * K + gk] : (half)0.0h;
                }
            }
            barrier(CLK_LOCAL_MEM_FENCE);
        }
