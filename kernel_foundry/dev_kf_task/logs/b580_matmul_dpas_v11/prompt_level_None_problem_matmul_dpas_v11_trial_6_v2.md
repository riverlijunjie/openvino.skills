...
This is the code of the best kernel we have tested so far (Result: Correctness score: 5 / 5 (compiles and correct), runtime in ms: 2.040):
```OCL
#pragma OPENCL EXTENSION cl_intel_subgroups : enable
#pragma OPENCL EXTENSION cl_intel_subgroup_matrix_multiply_accumulate : enable
#pragma OPENCL EXTENSION cl_khr_fp16 : enable
...
Here is the last OCL kernel we tested (Result: Correctness score: 5 / 5 (compiles and correct), runtime in ms: 4.830):
```OCL
#pragma OPENCL EXTENSION cl_intel_subgroups : enable
#pragma OPENCL EXTENSION cl_intel_subgroup_matrix_multiply_accumulate : enable
#pragma OPENCL EXTENSION cl_khr_fp16 : enable
...
// SLM A: 2 x 64 x 33 x 2 = ~8.4 KB
// SLM B: 2 x 32 x 129 x 2 = ~16.5 KB  Total ~25 KB
// GWS = (ceil(N/128)*128, ceil(M/64))  LWS = (128, 1)
...
#define NUM_WI 128
#define A_ELEMS (TILE_M * TILE_K)
#define B_ELEMS (TILE_K * TILE_N)
#define A_PER_WI (A_ELEMS / NUM_WI)
#define B_PER_WI (B_ELEMS / NUM_WI)
    ...
    const int N)
{
    ...
    // Precompute load indices for A (16 elements per WI)
    // elem_id = lid + i * 128, r = elem_id / 32, c = elem_id % 32
    // Precompute load indices for B (32 elements per WI)
    // elem_id = lid + i * 128, r = elem_id / 128, c = elem_id % 128
    ...
    // --- Load first tile into buffer 0 ---
    {
        ...
        // Load A: 64x32 = 2048 halves, 128 WIs, 16 each
        #pragma unroll
        for (int i = 0; i < A_PER_WI; i++) {
            int elem_id = lid + i * NUM_WI;
            int r = elem_id >> 5;  // / 32
            int c = elem_id & 31;  // % 32
            int gr = row_base + r;
            int gk = k_off + c;
            a_dst[r * SLM_A_STRIDE + c] = (gr < M && gk < K) ? A[gr * K + gk] : (half)0.0h;
        }

        // Load B: 32x128 = 4096 halves, 128 WIs, 32 each
        #pragma unroll
        for (int i = 0; i < B_PER_WI; i++) {
            int elem_id = lid + i * NUM_WI;
            int r = elem_id >> 7;  // / 128
            int c = elem_id & 127; // % 128
            int gk = k_off + r;
            int gn = n_base + c;
            b_dst[r * SLM_B_STRIDE + c] = (gk < K && gn < N) ? B[gk * N + gn] : (half)0.0h;
        }
    }
    barrier(CLK_LOCAL_MEM_FENCE);
    ...
    for (int kt = 0; kt < num_k_tiles; kt++) {
        ...
        // ---- Compute: 2 k16 steps x 8 row-blocks = 16 DPAS per step ----
        int sg_col_off = sg_id * 16;

        #pragma unroll
        for (int kk = 0; kk < TILE_K; kk += 16) {
            // Load B tile from SLM: pack into int8
            int8 b_val;
            #pragma unroll
            for (int p = 0; p < 8; p++) {
                int k_row0 = kk + 2 * p;
                int k_row1 = k_row0 + 1;
                short s0 = as_short(cur_B[k_row0 * SLM_B_STRIDE + sg_col_off + sg_lid]);
                short s1 = as_short(cur_B[k_row1 * SLM_B_STRIDE + sg_col_off + sg_lid]);
                ((int*)&b_val)[p] = as_int((short2)(s0, s1));
            }

            // Load A and DPAS for each of 8 row-blocks
            short8 a_block;
        ...
        // ---- Load next tile into nxt_buf (overlapped after compute) ----
        if (has_next) {
            barrier(CLK_LOCAL_MEM_FENCE);

            __local half* a_nxt = slm_A + nxt_buf * TILE_M * SLM_A_STRIDE;
            __local half* b_nxt = slm_B + nxt_buf * TILE_K * SLM_B_STRIDE;
            ...
            for (int i = 0; i < A_PER_WI; i++) {
                ...
                int gr = row_base + r;
                int gk = k_next + c;
                a_nxt[r * SLM_A_STRIDE + c] = (gr < M && gk < K) ? A[gr * K + gk] : (half)0.0h;
            }
            ...
            for (int i = 0; i < B_PER_WI; i++) {
                ...
                int gk = k_next + r;
                int gn = n_base + c;
                b_nxt[r * SLM_B_STRIDE + c] = (gk < K && gn < N) ? B[gk * N + gn] : (half)0.0h;
            }
    ...
    // ---- Store results ----
    if (col_idx < N) {
        #define STORE8(off, acc) \
            if (row_base + (off) + 8 <= M) { \
                _Pragma("unroll") \
                for (int r = 0; r < 8; r++) \
                    C[(row_base + (off) + r) * N + col_idx] = convert_half(((float*)&(acc))[r]); \
            } else { \
                _Pragma("unroll") \
                for (int r = 0; r < 8; r++) \
                    if (row_base + (off) + r < M) \
