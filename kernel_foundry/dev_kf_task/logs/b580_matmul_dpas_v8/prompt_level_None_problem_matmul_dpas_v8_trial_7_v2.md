...
### Version 2 (Result: Correctness score: 5 / 5 (compiles and correct), runtime in ms: 44.400):
```OCL
#pragma OPENCL EXTENSION cl_intel_subgroups : enable
#pragma OPENCL EXTENSION cl_intel_subgroups_short : enable
#pragma OPENCL EXTENSION cl_intel_subgroup_matrix_multiply_accumulate : enable
    ...
    const int N)
{
    ...
    // SLM layout:
    // A: 32 rows x 32 cols as ushort = 1024 ushorts (2KB)
    // B_vnni: 2 blocks x 8 pairs x 64 cols as uint = 1024 uints (4KB)
    // Total: 6KB - fits well in SLM
    ...
    // Check if this WG tile is fully within bounds (common case optimization)
    const int wg_m_end = wg_m + TILE_M;
    const int wg_n_end = wg_n + TILE_N;
    const int full_m = (wg_m_end <= M);
    const int full_n = (wg_n_end <= N);

    for (int k0 = 0; k0 < K; k0 += TILE_K) {
        const int full_k = ((k0 + TILE_K) <= K);
        ...
        // 1024 uints, 64 threads => 16 per thread
        if (full_n && full_k) {
            // Fast path
            #pragma unroll
            for (int i = 0; i < 16; i++) {
                ...
                slm_b_vnni[block * 512 + pair * TILE_N + col] = ((uint)v1 << 16) | (uint)v0;
            }
        } else {
            #pragma unroll
            for (int i = 0; i < 16; i++) {
        ...
        for (int kk = 0; kk < 2; kk++) {
            ...
            __local uint* b_base = slm_b_vnni + kk * 512;
...
Here is the last OCL kernel we tested (Result: Correctness score: 5 / 5 (compiles and correct), runtime in ms: 44.300):
```OCL
#pragma OPENCL EXTENSION cl_intel_subgroups : enable
#pragma OPENCL EXTENSION cl_intel_subgroups_short : enable
#pragma OPENCL EXTENSION cl_intel_subgroup_matrix_multiply_accumulate : enable
