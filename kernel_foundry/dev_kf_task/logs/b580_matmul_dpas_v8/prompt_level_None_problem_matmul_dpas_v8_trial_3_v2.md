...
### Version 2 (Result: Correctness score: 5 / 5 (compiles and correct), runtime in ms: 77.700):
```OCL
#pragma OPENCL EXTENSION cl_intel_subgroups : enable
#pragma OPENCL EXTENSION cl_intel_subgroups_short : enable
#pragma OPENCL EXTENSION cl_intel_subgroup_matrix_multiply_accumulate : enable
    ...
    const int N)
{
    ...
    // SLM for A[32][32] and B_vnni[16][64] (two K-steps worth of B in VNNI)
    // A: 32*32 = 1024 halfs = 2048 bytes
    // B_vnni: 2 * 8*64 = 1024 uints = 4096 bytes
    __local ushort slm_a[TILE_M * TILE_K]; // 32*32 = 1024
    __local uint slm_b_vnni[2 * 8 * TILE_N]; // 2 * 512 = 1024

    int flat_id = get_local_id(1) * 16 + get_local_id(0); // 0..63

    for (int k0 = 0; k0 < K; k0 += TILE_K) {
        // === Cooperative load A[32][32] ===
        // 64 WIs, 1024 elements => 16 elements per WI
        #pragma unroll
        for (int i = 0; i < 16; i++) {
            int idx = flat_id * 16 + i;
            int r = idx >> 5;  // idx / 32
            int c = idx & 31;  // idx % 32
            int gr = wg_m + r;
            int gc = k0 + c;
        ...
        // Block 0: k0..k0+15, Block 1: k0+16..k0+31
        // Each block: 8 pairs x 64 cols = 512 uints
        // Total: 1024 uints, 64 WIs => 16 per WI
        #pragma unroll
        for (int i = 0; i < 16; i++) {
            int idx = flat_id * 16 + i;
            int block = idx >> 9;     // idx / 512 => 0 or 1
            int rem = idx & 511;      // idx % 512
            int pair = rem >> 6;      // rem / 64 => k-pair 0..7
            int col = rem & 63;       // rem % 64
...
Here is the last OCL kernel we tested (Result: Correctness score: 5 / 5 (compiles and correct), runtime in ms: 57.300):
```OCL
#pragma OPENCL EXTENSION cl_intel_subgroups : enable
#pragma OPENCL EXTENSION cl_intel_subgroups_short : enable
#pragma OPENCL EXTENSION cl_intel_subgroup_matrix_multiply_accumulate : enable
...
================== 4 passed, 1 deselected, 1 warning in 0.94s ==================
