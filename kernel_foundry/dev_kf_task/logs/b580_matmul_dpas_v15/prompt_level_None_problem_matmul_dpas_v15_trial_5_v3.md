...
Here is the last OCL kernel we tested (Result: Correctness score: 5 / 5 (compiles and correct), runtime in ms: 1.570):
```OCL
// v32: True software-pipelined K-loop with cross-iteration overlap
// Architecture: 32 SGs (4x8), each SG: 32x32 tile, WG tile: 128x256
// K-step: 128 (4x32), 2D block reads for A and B
// Key improvement: Load iter[i+1] block 0 during iter[i] block 3 compute
// Launch: GWS = (ceil(M/128)*512, ceil(N/256), 1), LWS = (512, 1, 1)
// Subgroup size: 16
...
// Macro: compute one 32x32x16 DPAS block (4 row-groups x 2 col-groups)
// Interleave A-row pairs with B to reduce register pressure peaks
#define COMPUTE_BLOCK(a_arr, a_off, b_arr, b_off) { \
    int8 b0_ = as_int8(*(__private uint8*)&(b_arr)[b_off]); \
    int8 b1_ = as_int8(*(__private uint8*)&(b_arr)[(b_off)+8]); \
    short8 a0_ = as_short8(*(__private ushort8*)&(a_arr)[(a_off)]); \
    ...
    int M, int N, int K)
{
    ...
    // Load A[0] and B[0] for the very first K-block
    ushort a_cur[64];
    intel_sub_group_2d_block_read_16b_32r16x2c(
    ...
    // K=2048 divides evenly by 32 → 64 K-blocks total
    // Process in groups: each iteration handles one 32-wide K-block
    // Data for current block is already loaded; we load next block during compute

    const int num_k_blocks = K >> 5; // K / 32

    for (int kb = 0; kb < num_k_blocks - 1; kb++) {
        int k_next = (kb + 1) << 5; // (kb+1) * 32

        // Start loading A for next K-block
        ushort a_next[64];
        intel_sub_group_2d_block_read_16b_32r16x2c(
            (__global void*)A, A_wb, M, A_pb, (int2)(k_next, tile_m), a_next);

        // Compute first half of current block (k16=0..15) while A_next loads
        COMPUTE_BLOCK(a_cur, 0, b0_cur, 0);

        // Load B for next K-block (first half)
        uint b0_next[16];
        intel_sub_group_2d_block_read_transform_16b_16r16x2c(
            (__global void*)B, B_wb, K, B_pb, (int2)(tile_n, k_next), b0_next);

        // Compute second half of current block (k16=16..31)
        COMPUTE_BLOCK(a_cur, 32, b1_cur, 0);

        // Load B for next K-block (second half)
        uint b1_next[16];
        intel_sub_group_2d_block_read_transform_16b_16r16x2c(
        ...
        #pragma unroll
        for (int i = 0; i < 16; i++) b0_cur[i] = b0_next[i];
        #pragma unroll
        for (int i = 0; i < 16; i++) b1_cur[i] = b1_next[i];
    }

    // Epilogue: compute last K-block (no next block to load)
    COMPUTE_BLOCK(a_cur, 0, b0_cur, 0);
    COMPUTE_BLOCK(a_cur, 32, b1_cur, 0);
