...
Here is the last OCL kernel we tested (Result: Correctness score: 5 / 5 (compiles and correct), runtime in ms: 4.390):
```OCL
// v35: 128x128 WG tile (4x4 SGs) for better occupancy, 16 WGs for 512x512
// Architecture: 16 SGs (4x4), each SG: 32x32 tile, WG tile: 128x128
// K-step: 128 (4x32), 2D block reads for A and B
// Launch: GWS = (ceil(M/128)*256, ceil(N/128), 1), LWS = (256, 1, 1)
// Subgroup size: 16
// For M=N=K=512: grid = (4,4) = 16 WGs, better filling 20 Xe cores
    ...
    int M, int N, int K)
{
    ...
    // Software-pipelined K-loop with cross-iteration prefetch
    // Load first K-block outside loop
    ushort a_cur[64];
    ...
    for (int k0 = 0; k0 < K; k0 += 128) {
        // ---- K-block 0: k0..k0+31 ----
        // a_cur and b0_cur/b1_cur already loaded (from prefetch or prolog)

        // Start loading next A block
        ushort a_next[64];
        intel_sub_group_2d_block_read_16b_32r16x2c(
        ...
        // ---- K-block 3: k0+96..k0+127 ----
        // Prefetch next iteration's first A and B blocks
        int next_k = k0 + 128;
        if (next_k < K) {
            intel_sub_group_2d_block_read_16b_32r16x2c(
                (__global void*)A, A_wb, M, A_pb, (int2)(next_k, tile_m), a_cur);
        }
        ...
        if (next_k < K) {
            intel_sub_group_2d_block_read_transform_16b_16r16x2c(
                (__global void*)B, B_wb, K, B_pb, (int2)(tile_n, next_k), b0_cur);
        }
        ...
        if (next_k < K) {
            intel_sub_group_2d_block_read_transform_16b_16r16x2c(
                (__global void*)B, B_wb, K, B_pb, (int2)(tile_n, next_k + 16), b1_cur);
        }
    }
