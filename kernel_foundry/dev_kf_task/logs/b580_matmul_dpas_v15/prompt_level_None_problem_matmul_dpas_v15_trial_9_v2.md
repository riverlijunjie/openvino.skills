...
This is the code of the best kernel we have tested so far (Result: Correctness score: 5 / 5 (compiles and correct), runtime in ms: 0.274):
```OCL
...
Here is the last OCL kernel we tested (Result: Correctness score: 5 / 5 (compiles and correct), runtime in ms: 4.880):
```OCL
// v35: 16 SGs (4x4), each SG: 32x64 tile, WG tile: 128x256
// K-step: 64 (2x32), 2D block reads for A and B, 8x K-unroll per iteration
// Launch: GWS = (ceil(N/256)*256, ceil(M/128), 1), LWS = (256, 1, 1)
// Subgroup size: 16
...
// Compute 32x64x16 DPAS block: 4 row-groups x 4 col-groups
// a_arr: 32 ushort (32 rows x 16 K, packed), b_arr: 4 sets of 8 uint (16K x 64N)
#define COMPUTE_BLOCK(a_arr, a_off, b0, b1, b2, b3) { \
    int8 bi0_ = as_int8(*(__private uint8*)&(b0)[0]); \
    int8 bi1_ = as_int8(*(__private uint8*)&(b1)[0]); \
    int8 bi2_ = as_int8(*(__private uint8*)&(b2)[0]); \
    int8 bi3_ = as_int8(*(__private uint8*)&(b3)[0]); \
    short8 a0_ = as_short8(*(__private ushort8*)&(a_arr)[(a_off)]); \
    short8 a1_ = as_short8(*(__private ushort8*)&(a_arr)[(a_off)+8]); \
    short8 a2_ = as_short8(*(__private ushort8*)&(a_arr)[(a_off)+16]); \
    short8 a3_ = as_short8(*(__private ushort8*)&(a_arr)[(a_off)+24]); \
    acc00 = intel_sub_group_f16_f16_matrix_mad_k16(a0_, bi0_, acc00); \
    acc01 = intel_sub_group_f16_f16_matrix_mad_k16(a0_, bi1_, acc01); \
    acc02 = intel_sub_group_f16_f16_matrix_mad_k16(a0_, bi2_, acc02); \
    acc03 = intel_sub_group_f16_f16_matrix_mad_k16(a0_, bi3_, acc03); \
    acc10 = intel_sub_group_f16_f16_matrix_mad_k16(a1_, bi0_, acc10); \
    acc11 = intel_sub_group_f16_f16_matrix_mad_k16(a1_, bi1_, acc11); \
    acc12 = intel_sub_group_f16_f16_matrix_mad_k16(a1_, bi2_, acc12); \
    acc13 = intel_sub_group_f16_f16_matrix_mad_k16(a1_, bi3_, acc13); \
    acc20 = intel_sub_group_f16_f16_matrix_mad_k16(a2_, bi0_, acc20); \
    acc21 = intel_sub_group_f16_f16_matrix_mad_k16(a2_, bi1_, acc21); \
    acc22 = intel_sub_group_f16_f16_matrix_mad_k16(a2_, bi2_, acc22); \
    acc23 = intel_sub_group_f16_f16_matrix_mad_k16(a2_, bi3_, acc23); \
    acc30 = intel_sub_group_f16_f16_matrix_mad_k16(a3_, bi0_, acc30); \
    acc31 = intel_sub_group_f16_f16_matrix_mad_k16(a3_, bi1_, acc31); \
    acc32 = intel_sub_group_f16_f16_matrix_mad_k16(a3_, bi2_, acc32); \
    acc33 = intel_sub_group_f16_f16_matrix_mad_k16(a3_, bi3_, acc33); \
}
    ...
    int M, int N, int K)
{
    ...
    // 16 accumulators: 4 row-groups x 4 col-groups
    float8 acc00 = 0, acc01 = 0, acc02 = 0, acc03 = 0;
    float8 acc10 = 0, acc11 = 0, acc12 = 0, acc13 = 0;
    float8 acc20 = 0, acc21 = 0, acc22 = 0, acc23 = 0;
    float8 acc30 = 0, acc31 = 0, acc32 = 0, acc33 = 0;

    const int A_wb = K * 2, A_pb = K * 2;
    const int B_wb = N * 2, B_pb = N * 2;

    // Main loop: 4x K-unroll (128 K per iteration), 16 iters for K=2048
    for (int k0 = 0; k0 < K; k0 += 128) {
        // ---- K-block 0: k0..k0+31 ----
        ushort a_data0[64];
        intel_sub_group_2d_block_read_16b_32r16x2c(
        ...
        // B: 4 column groups of 16 each, 16 rows for first K half
        uint b_c0_k0[8], b_c1_k0[8], b_c2_k0[8], b_c3_k0[8];
        intel_sub_group_2d_block_read_transform_16b_16r16x1c(
            (__global void*)B, B_wb, K, B_pb, (int2)(tile_n, k0), b_c0_k0);
        intel_sub_group_2d_block_read_transform_16b_16r16x1c(
            (__global void*)B, B_wb, K, B_pb, (int2)(tile_n + 16, k0), b_c1_k0);
        intel_sub_group_2d_block_read_transform_16b_16r16x1c(
            (__global void*)B, B_wb, K, B_pb, (int2)(tile_n + 32, k0), b_c2_k0);
        intel_sub_group_2d_block_read_transform_16b_16r16x1c(
            (__global void*)B, B_wb, K, B_pb, (int2)(tile_n + 48, k0), b_c3_k0);
        ...
        // Compute k0..k0+15
        COMPUTE_BLOCK(a_data0, 0, b_c0_k0, b_c1_k0, b_c2_k0, b_c3_k0);

        // B for k0+16..k0+31
        uint b_c0_k1[8], b_c1_k1[8], b_c2_k1[8], b_c3_k1[8];
        intel_sub_group_2d_block_read_transform_16b_16r16x1c(
            (__global void*)B, B_wb, K, B_pb, (int2)(tile_n, k0 + 16), b_c0_k1);
        intel_sub_group_2d_block_read_transform_16b_16r16x1c(
            (__global void*)B, B_wb, K, B_pb, (int2)(tile_n + 16, k0 + 16), b_c1_k1);
        intel_sub_group_2d_block_read_transform_16b_16r16x1c(
            (__global void*)B, B_wb, K, B_pb, (int2)(tile_n + 32, k0 + 16), b_c2_k1);
        intel_sub_group_2d_block_read_transform_16b_16r16x1c(
            (__global void*)B, B_wb, K, B_pb, (int2)(tile_n + 48, k0 + 16), b_c3_k1);

        // Compute k0+16..k0+31
        COMPUTE_BLOCK(a_data0, 32, b_c0_k1, b_c1_k1, b_c2_k1, b_c3_k1);

        // ---- K-block 1: k0+32..k0+63 ----
        uint b_c0_k2[8], b_c1_k2[8], b_c2_k2[8], b_c3_k2[8];
        intel_sub_group_2d_block_read_transform_16b_16r16x1c(
            (__global void*)B, B_wb, K, B_pb, (int2)(tile_n, k0 + 32), b_c0_k2);
        intel_sub_group_2d_block_read_transform_16b_16r16x1c(
            (__global void*)B, B_wb, K, B_pb, (int2)(tile_n + 16, k0 + 32), b_c1_k2);
        intel_sub_group_2d_block_read_transform_16b_16r16x1c(
            (__global void*)B, B_wb, K, B_pb, (int2)(tile_n + 32, k0 + 32), b_c2_k2);
        intel_sub_group_2d_block_read_transform_16b_16r16x1c(
            (__global void*)B, B_wb, K, B_pb, (int2)(tile_n + 48, k0 + 32), b_c3_k2);

        ushort a_data2[64];
        intel_sub_group_2d_block_read_16b_32r16x2c(
            (__global void*)A, A_wb, M, A_pb, (int2)(k0 + 64, tile_m), a_data2);

        COMPUTE_BLOCK(a_data1, 0, b_c0_k2, b_c1_k2, b_c2_k2, b_c3_k2);

        uint b_c0_k3[8], b_c1_k3[8], b_c2_k3[8], b_c3_k3[8];
        intel_sub_group_2d_block_read_transform_16b_16r16x1c(
            (__global void*)B, B_wb, K, B_pb, (int2)(tile_n, k0 + 48), b_c0_k3);
        intel_sub_group_2d_block_read_transform_16b_16r16x1c(
            (__global void*)B, B_wb, K, B_pb, (int2)(tile_n + 16, k0 + 48), b_c1_k3);
        intel_sub_group_2d_block_read_transform_16b_16r16x1c(
            (__global void*)B, B_wb, K, B_pb, (int2)(tile_n + 32, k0 + 48), b_c2_k3);
        intel_sub_group_2d_block_read_transform_16b_16r16x1c(
            (__global void*)B, B_wb, K, B_pb, (int2)(tile_n + 48, k0 + 48), b_c3_k3);

        COMPUTE_BLOCK(a_data1, 32, b_c0_k3, b_c1_k3, b_c2_k3, b_c3_k3);

        // ---- K-block 2: k0+64..k0+95 ----
        uint b_c0_k4[8], b_c1_k4[8], b_c2_k4[8], b_c3_k4[8];
        intel_sub_group_2d_block_read_transform_16b_16r16x1c(
            (__global void*)B, B_wb, K, B_pb, (int2)(tile_n, k0 + 64), b_c0_k4);
        intel_sub_group_2d_block_read_transform_16b_16r16x1c(
            (__global void*)B, B_wb, K, B_pb, (int2)(tile_n + 16, k0 + 64), b_c1_k4);
        intel_sub_group_2d_block_read_transform_16b_16r16x1c(
            (__global void*)B, B_wb, K, B_pb, (int2)(tile_n + 32, k0 + 64), b_c2_k4);
        intel_sub_group_2d_block_read_transform_16b_16r16x1c(
            (__global void*)B, B_wb, K, B_pb, (int2)(tile_n + 48, k0 + 64), b_c3_k4);

        ushort a_data3[64];
        intel_sub_group_2d_block_read_16b_32r16x2c(
            (__global void*)A, A_wb, M, A_pb, (int2)(k0 + 96, tile_m), a_data3);

        COMPUTE_BLOCK(a_data2, 0, b_c0_k4, b_c1_k4, b_c2_k4, b_c3_k4);

        uint b_c0_k5[8], b_c1_k5[8], b_c2_k5[8], b_c3_k5[8];
        intel_sub_group_2d_block_read_transform_16b_16r16x1c(
            (__global void*)B, B_wb, K, B_pb, (int2)(tile_n, k0 + 80), b_c0_k5);
        intel_sub_group_2d_block_read_transform_16b_16r16x1c(
            (__global void*)B, B_wb, K, B_pb, (int2)(tile_n + 16, k0 + 80), b_c1_k5);
        intel_sub_group_2d_block_read_transform_16b_16r16x1c(
            (__global void*)B, B_wb, K, B_pb, (int2)(tile_n + 32, k0 + 80), b_c2_k5);
        intel_sub_group_2d_block_read_transform_16b_16r16x1c(
            (__global void*)B, B_wb, K, B_pb, (int2)(tile_n + 48, k0 + 80), b_c3_k5);

        COMPUTE_BLOCK(a_data2, 32, b_c0_k5, b_c1_k5, b_c2_k5, b_c3_k5);

        // ---- K-block 3: k0+96..k0+127 ----
        uint b_c0_k6[8], b_c1_k6[8], b_c2_k6[8], b_c3_k6[8];
        intel_sub_group_2d_block_read_transform_16b_16r16x1c(
            (__global void*)B, B_wb, K, B_pb, (int2)(tile_n, k0 + 96), b_c0_k6);
        intel_sub_group_2d_block_read_transform_16b_16r16x1c(
            (__global void*)B, B_wb, K, B_pb, (int2)(tile_n + 16, k0 + 96), b_c1_k6);
        intel_sub_group_2d_block_read_transform_16b_16r16x1c(
            (__global void*)B, B_wb, K, B_pb, (int2)(tile_n + 32, k0 + 96), b_c2_k6);
        intel_sub_group_2d_block_read_transform_16b_16r16x1c(
            (__global void*)B, B_wb, K, B_pb, (int2)(tile_n + 48, k0 + 96), b_c3_k6);

        COMPUTE_BLOCK(a_data3, 0, b_c0_k6, b_c1_k6, b_c2_k6, b_c3_k6);

        uint b_c0_k7[8], b_c1_k7[8], b_c2_k7[8], b_c3_k7[8];
        intel_sub_group_2d_block_read_transform_16b_16r16x1c(
            (__global void*)B, B_wb, K, B_pb, (int2)(tile_n, k0 + 112), b_c0_k7);
        intel_sub_group_2d_block_read_transform_16b_16r16x1c(
            (__global void*)B, B_wb, K, B_pb, (int2)(tile_n + 16, k0 + 112), b_c1_k7);
        intel_sub_group_2d_block_read_transform_16b_16r16x1c(
            (__global void*)B, B_wb, K, B_pb, (int2)(tile_n + 32, k0 + 112), b_c2_k7);
        intel_sub_group_2d_block_read_transform_16b_16r16x1c(
            (__global void*)B, B_wb, K, B_pb, (int2)(tile_n + 48, k0 + 112), b_c3_k7);

        COMPUTE_BLOCK(a_data3, 32, b_c0_k7, b_c1_k7, b_c2_k7, b_c3_k7);
    }

    // Store C tiles: 4 row-groups x 4 col-groups
    #define STORE_BLOCK(acc, row_off, col_off) { \
        ushort c_data[8]; \
        c_data[0] = as_ushort((half)acc.s0); c_data[1] = as_ushort((half)acc.s1); \
        c_data[2] = as_ushort((half)acc.s2); c_data[3] = as_ushort((half)acc.s3); \
    ...
    STORE_BLOCK(acc01, 0, 16);  STORE_BLOCK(acc11, 8, 16);
    STORE_BLOCK(acc21, 16, 16); STORE_BLOCK(acc31, 24, 16);
    STORE_BLOCK(acc02, 0, 32);  STORE_BLOCK(acc12, 8, 32);
    STORE_BLOCK(acc22, 16, 32); STORE_BLOCK(acc32, 24, 32);
    STORE_BLOCK(acc03, 0, 48);  STORE_BLOCK(acc13, 8, 48);
    STORE_BLOCK(acc23, 16, 48); STORE_BLOCK(acc33, 24, 48);
    #undef STORE_BLOCK
}
```
