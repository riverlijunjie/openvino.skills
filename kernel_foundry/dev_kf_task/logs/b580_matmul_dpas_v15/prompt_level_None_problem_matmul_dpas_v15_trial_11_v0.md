...
Here is the last OCL kernel we tested (Result: Correctness score: 5 / 5 (compiles and correct), runtime in ms: 1.380):
```OCL
// v30: 4x K-unroll, no remainder, software-pipelined loads/compute
// Architecture: 32 SGs (4x8), each SG: 32x32 tile, WG tile: 128x256
// K-step: 128 (4x32), 2D block reads for A and B
// Launch: GWS = (ceil(M/128)*512, ceil(N/256), 1), LWS = (512, 1, 1)
// Subgroup size: 16
...
// Macro for one 32-wide K-step compute (16+16)
#define COMPUTE_K32(a_arr, b0_arr, b1_arr) \
{ \
    int8 b0 = as_int8(*(__private uint8*)&b0_arr[0]); \
    int8 b1 = as_int8(*(__private uint8*)&b0_arr[8]); \
    short8 a0 = as_short8(*(__private ushort8*)&a_arr[0]); \
    short8 a1 = as_short8(*(__private ushort8*)&a_arr[8]); \
    short8 a2 = as_short8(*(__private ushort8*)&a_arr[16]); \
    short8 a3 = as_short8(*(__private ushort8*)&a_arr[24]); \
...
{ \
    int8 b0 = as_int8(*(__private uint8*)&b1_arr[0]); \
    int8 b1 = as_int8(*(__private uint8*)&b1_arr[8]); \
    short8 a0 = as_short8(*(__private ushort8*)&a_arr[32]); \
    short8 a1 = as_short8(*(__private ushort8*)&a_arr[40]); \
    short8 a2 = as_short8(*(__private ushort8*)&a_arr[48]); \
    short8 a3 = as_short8(*(__private ushort8*)&a_arr[56]); \
    acc00 = intel_sub_group_f16_f16_matrix_mad_k16(a0, b0, acc00); \
    acc01 = intel_sub_group_f16_f16_matrix_mad_k16(a0, b1, acc01); \
    acc10 = intel_sub_group_f16_f16_matrix_mad_k16(a1, b0, acc10); \
    acc11 = intel_sub_group_f16_f16_matrix_mad_k16(a1, b1, acc11); \
    ...
    int M, int N, int K)
{
    ...
    // K is guaranteed divisible by 32. Process in chunks of 128 (4x32).
    // For K=2048, this gives 16 iterations with no remainder.
    const int k_main = K & ~127; // round down to multiple of 128

    int k0 = 0;
    for (; k0 < k_main; k0 += 128) {
        // Load all 4 K-blocks upfront to maximize load/compute overlap
        // Block 0: k0..k0+31
        ushort a_data0[64];
        intel_sub_group_2d_block_read_16b_32r16x2c(
            (__global void*)A, A_wb, M, A_pb, (int2)(k0, tile_m), a_data0);
        uint b0_d0[16], b1_d0[16];
        intel_sub_group_2d_block_read_transform_16b_16r16x2c(
            (__global void*)B, B_wb, K, B_pb, (int2)(tile_n, k0), b0_d0);
        intel_sub_group_2d_block_read_transform_16b_16r16x2c(
            (__global void*)B, B_wb, K, B_pb, (int2)(tile_n, k0 + 16), b1_d0);

        // Block 1: k0+32..k0+63
        ushort a_data1[64];
        intel_sub_group_2d_block_read_16b_32r16x2c(
            (__global void*)A, A_wb, M, A_pb, (int2)(k0 + 32, tile_m), a_data1);
        uint b0_d1[16], b1_d1[16];
        intel_sub_group_2d_block_read_transform_16b_16r16x2c(
            (__global void*)B, B_wb, K, B_pb, (int2)(tile_n, k0 + 32), b0_d1);
        intel_sub_group_2d_block_read_transform_16b_16r16x2c(
            (__global void*)B, B_wb, K, B_pb, (int2)(tile_n, k0 + 48), b1_d1);

        // Block 2: k0+64..k0+95
        ushort a_data2[64];
        intel_sub_group_2d_block_read_16b_32r16x2c(
            (__global void*)A, A_wb, M, A_pb, (int2)(k0 + 64, tile_m), a_data2);
        uint b0_d2[16], b1_d2[16];
        intel_sub_group_2d_block_read_transform_16b_16r16x2c(
            (__global void*)B, B_wb, K, B_pb, (int2)(tile_n, k0 + 64), b0_d2);
        intel_sub_group_2d_block_read_transform_16b_16r16x2c(
            (__global void*)B, B_wb, K, B_pb, (int2)(tile_n, k0 + 80), b1_d2);

        // Block 3: k0+96..k0+127
        ushort a_data3[64];
        intel_sub_group_2d_block_read_16b_32r16x2c(
            (__global void*)A, A_wb, M, A_pb, (int2)(k0 + 96, tile_m), a_data3);
        uint b0_d3[16], b1_d3[16];
        intel_sub_group_2d_block_read_transform_16b_16r16x2c(
            (__global void*)B, B_wb, K, B_pb, (int2)(tile_n, k0 + 96), b0_d3);
        intel_sub_group_2d_block_read_transform_16b_16r16x2c(
            (__global void*)B, B_wb, K, B_pb, (int2)(tile_n, k0 + 112), b1_d3);

        // Compute all 4 blocks
        COMPUTE_K32(a_data0, b0_d0, b1_d0);
        COMPUTE_K32(a_data1, b0_d1, b1_d1);
        COMPUTE_K32(a_data2, b0_d2, b1_d2);
        COMPUTE_K32(a_data3, b0_d3, b1_d3);
    }

    // Handle remaining K in 32-element chunks (for K not divisible by 128)
    for (; k0 < K; k0 += 32) {
        ushort a_data[64];
        intel_sub_group_2d_block_read_16b_32r16x2c(
        ...
        COMPUTE_K32(a_data, b0_data, b1_data);
    }
