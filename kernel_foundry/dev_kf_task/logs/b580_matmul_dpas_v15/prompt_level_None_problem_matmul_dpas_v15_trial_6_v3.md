...
```OCL
// v32: True software pipeline across loop iterations
// Architecture: 32 SGs (4x8), each SG: 32x32 tile, WG tile: 128x256
// K-step: 32, software-pipelined: load[i+1] overlapped with compute[i]
// Launch: GWS = (ceil(M/128)*512, ceil(N/256), 1), LWS = (512, 1, 1)
// Subgroup size: 16
    ...
    int M, int N, int K)
{
    ...
    // Main software-pipelined loop: compute[k] while loading[k+32]
    // K=2048 divides by 32 → 64 iterations, last one has no next load
    for (int k0 = 0; k0 < K - 32; k0 += 32) {
        // Kick off next A load early
        ushort a_nxt[64];
        intel_sub_group_2d_block_read_16b_32r16x2c(
            (__global void*)A, A_wb, M, A_pb, (int2)(k0 + 32, tile_m), a_nxt);

        // Compute first half k0..k0+15 with current data
        {
            int8 b0_ = as_int8(*(__private uint8*)&blo_cur[0]);
            int8 b1_ = as_int8(*(__private uint8*)&blo_cur[8]);
            short8 a0_ = as_short8(*(__private ushort8*)&a_cur[0]);
            short8 a1_ = as_short8(*(__private ushort8*)&a_cur[8]);
            short8 a2_ = as_short8(*(__private ushort8*)&a_cur[16]);
            short8 a3_ = as_short8(*(__private ushort8*)&a_cur[24]);
            acc00 = intel_sub_group_f16_f16_matrix_mad_k16(a0_, b0_, acc00);
            acc01 = intel_sub_group_f16_f16_matrix_mad_k16(a0_, b1_, acc01);
            acc10 = intel_sub_group_f16_f16_matrix_mad_k16(a1_, b0_, acc10);
            acc11 = intel_sub_group_f16_f16_matrix_mad_k16(a1_, b1_, acc11);
        ...
        // Load next B lo while compute runs
        uint blo_nxt[16];
        intel_sub_group_2d_block_read_transform_16b_16r16x2c(
            (__global void*)B, B_wb, K, B_pb, (int2)(tile_n, k0 + 32), blo_nxt);

        // Compute second half k0+16..k0+31
        {
            int8 b0_ = as_int8(*(__private uint8*)&bhi_cur[0]);
            int8 b1_ = as_int8(*(__private uint8*)&bhi_cur[8]);
            short8 a0_ = as_short8(*(__private ushort8*)&a_cur[32]);
            short8 a1_ = as_short8(*(__private ushort8*)&a_cur[40]);
            short8 a2_ = as_short8(*(__private ushort8*)&a_cur[48]);
            short8 a3_ = as_short8(*(__private ushort8*)&a_cur[56]);
            acc30 = intel_sub_group_f16_f16_matrix_mad_k16(a3_, b0_, acc30);
            acc31 = intel_sub_group_f16_f16_matrix_mad_k16(a3_, b1_, acc31);
            acc20 = intel_sub_group_f16_f16_matrix_mad_k16(a2_, b0_, acc20);
            acc21 = intel_sub_group_f16_f16_matrix_mad_k16(a2_, b1_, acc21);
        ...
        uint bhi_nxt[16];
        intel_sub_group_2d_block_read_transform_16b_16r16x2c(
            (__global void*)B, B_wb, K, B_pb, (int2)(tile_n, k0 + 48), bhi_nxt);
    ...
    // Epilogue: compute last K-block (k=K-32..K-1), no next load needed
    {
        int8 b0_ = as_int8(*(__private uint8*)&blo_cur[0]);
        int8 b1_ = as_int8(*(__private uint8*)&blo_cur[8]);
        short8 a0_ = as_short8(*(__private ushort8*)&a_cur[0]);
