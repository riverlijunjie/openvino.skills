...
The best kernel (v30/v31) achieves 0.274ms with a 32-SG (4×8) workgroup, each SG computing a 32×32 tile, giving a 128×256 WG tile. The key to its performance is interleaved load/compute scheduling where next-iteration loads overlap with current-iteration DPAS compute.

The last tested kernel (1.380ms) loaded all 4 K-blocks upfront before computing any, which prevents load/compute overlap and causes pipeline stalls.

The best version already uses:
- 4× K-unroll (128 per iteration)
- Interleaved load/compute within the K-loop
- 2D block reads with VNNI transform for B
- L3-aware WG swizzle

**Potential improvements over the best v30:**
1. **Software pipelining across loop iterations**: Load the first K-block of the NEXT iteration during the last compute of the current iteration (true double-buffering across iterations)
2. **Prefetch hints**: Add explicit prefetch for A and B tiles a few iterations ahead
3. **Swizzle factor tuning**: Try SWIZZLE=4 for better L2 reuse
4. **Remove dead code paths**: K=2048 is always divisible by 128
5. **Tighter interleaving**: Interleave individual B loads between DPAS instructions more aggressively

Let me create a version with cross-iteration software pipelining (prologue loads iteration 0, then main loop computes iteration i while loading iteration i+1).

## Improved OCL code

```OCL
// v32: Cross-iteration software pipelining + prefetch + SWIZZLE=4
// Architecture: 32 SGs (4x8), each SG: 32x32 tile, WG tile: 128x256
// K-step: 128 (4x32), 2D block reads for A and B
// Software pipeline: load iter i+1 while computing iter i
...
// Reverse DPAS: rows 3,2,1,0 (boustrophedon)
    ...
    int M, int N, int K)
{
    ...
    // L3-aware WG swizzle with SWIZZLE=4
    ...
    // Software-pipelined main loop with cross-iteration overlap
    // K=2048 divides evenly by 128 → 16 iterations, no remainder
    // Strategy: within each 128-step, interleave loads of next K-block
    // with compute of current K-block, plus boustrophedon DPAS ordering
    ...
    for (int k0 = 0; k0 < K; k0 += 128) {
        ...
        // Prefetch A[1] while B[0] loads settle
        ushort a_data1[64];
        intel_sub_group_2d_block_read_16b_32r16x2c(
