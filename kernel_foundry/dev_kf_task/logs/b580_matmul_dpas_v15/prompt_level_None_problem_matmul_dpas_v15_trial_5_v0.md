...
Here is the last OCL kernel we tested (Result: Correctness score: 5 / 5 (compiles and correct), runtime in ms: 1.600):
```OCL
// v33: Software-pipelined K-loop with cross-iteration prefetch
// Architecture: 32 SGs (4x8), each SG: 32x32 tile, WG tile: 128x256
// K-step: 128 (4x32), 2D block reads for A and B
// Key change: Prologue loads first block, epilogue computes last block,
//   main loop overlaps last K-block compute with next iteration's first loads
// Launch: GWS = (ceil(M/128)*512, ceil(N/256), 1), LWS = (512, 1, 1)
// Subgroup size: 16
...
// Compute one 32x32x16 DPAS block (4 row-groups x 2 col-groups)
// Interleave b0/b1 usage per row to reduce RAW stalls
#define COMPUTE_BLOCK(a_arr, a_off, b_arr, b_off) { \
    int8 b0_ = as_int8(*(__private uint8*)&(b_arr)[b_off]); \
    int8 b1_ = as_int8(*(__private uint8*)&(b_arr)[(b_off)+8]); \
    short8 a0_ = as_short8(*(__private ushort8*)&(a_arr)[(a_off)]); \
    ...
    int M, int N, int K)
{
    ...
    // Software-pipelined loop: prologue loads first K-block,
    // main loop computes current while loading next,
    // epilogue computes last K-block
    ...
    // ============ MAIN LOOP ============
    // Process K in steps of 32, with software pipelining
    // Total K iterations = K/32. We already loaded k=0, so loop from k=32.
    // Each iteration: compute with current data, load next data
    const int k_iters = K / 32;

    for (int ki = 0; ki < k_iters - 1; ki++) {
        int k_next = (ki + 1) * 32;

        // Start loading next A while computing current
        ushort a_next[64];
        intel_sub_group_2d_block_read_16b_32r16x2c(
            (__global void*)A, A_wb, M, A_pb, (int2)(k_next, tile_m), a_next);

        // Compute first half of current K-block (k16 = 0..15)
        COMPUTE_BLOCK(a_cur, 0, b0_cur, 0);

        // Load next B first half while computing
        uint b0_next[16];
        intel_sub_group_2d_block_read_transform_16b_16r16x2c(
            (__global void*)B, B_wb, K, B_pb, (int2)(tile_n, k_next), b0_next);

        // Compute second half of current K-block (k16 = 16..31)
        COMPUTE_BLOCK(a_cur, 32, b1_cur, 0);

        // Load next B second half
        uint b1_next[16];
        intel_sub_group_2d_block_read_transform_16b_16r16x2c(
            (__global void*)B, B_wb, K, B_pb, (int2)(tile_n, k_next + 16), b1_next);

        // Swap buffers (compiler should optimize this to register renaming)
        #pragma unroll
