...
### Version 2 (Result: Correctness score: 5 / 5 (compiles and correct), runtime in ms: 0.277):
```OCL
// v31: True software-pipelined 2x K-unroll with double-buffered load/compute
// Architecture: 32 SGs (4x8), each SG: 32x32 tile, WG tile: 128x256
...
// Macro: 8 DPAS ops for one 32x32x16 block
    ...
    int M, int N, int K)
{
    ...
    // L3-aware WG swizzle (boustrophedon)
    const int grid_m = get_num_groups(0);
    const int grid_n = get_num_groups(1);
    const int gid0 = get_group_id(0);
    const int gid1 = get_group_id(1);
    ...
    // K=2048 divides by 128, so 16 iterations of 128 each
    // 4x unroll with aggressive interleaving
    for (int k0 = 0; k0 < K; k0 += 128) {
        // === Block 0: k0 .. k0+31 ===
        // Load A0, B0
        ushort a0[64];
        intel_sub_group_2d_block_read_16b_32r16x2c(
            (__global void*)A, A_wb, M, A_pb, (int2)(k0, tile_m), a0);
        uint b0lo[16], b0hi[16];
        intel_sub_group_2d_block_read_transform_16b_16r16x2c(
            (__global void*)B, B_wb, K, B_pb, (int2)(tile_n, k0), b0lo);
        intel_sub_group_2d_block_read_transform_16b_16r16x2c(
            (__global void*)B, B_wb, K, B_pb, (int2)(tile_n, k0 + 16), b0hi);

        // Start loading A1 (overlaps with block 0 compute)
        ushort a1[64];
        intel_sub_group_2d_block_read_16b_32r16x2c(
            (__global void*)A, A_wb, M, A_pb, (int2)(k0 + 32, tile_m), a1);

        // Compute block 0
        DPAS8(a0, 0, b0lo, 0);

        // Load B1 (overlaps with second half of block 0 compute)
        uint b1lo[16], b1hi[16];
        intel_sub_group_2d_block_read_transform_16b_16r16x2c(
            (__global void*)B, B_wb, K, B_pb, (int2)(tile_n, k0 + 32), b1lo);

        DPAS8(a0, 32, b0hi, 0);

        intel_sub_group_2d_block_read_transform_16b_16r16x2c(
            (__global void*)B, B_wb, K, B_pb, (int2)(tile_n, k0 + 48), b1hi);

        // === Block 1: k0+32 .. k0+63 ===
        // Start loading A2
        ushort a2[64];
        intel_sub_group_2d_block_read_16b_32r16x2c(
            (__global void*)A, A_wb, M, A_pb, (int2)(k0 + 64, tile_m), a2);

        DPAS8(a1, 0, b1lo, 0);

        uint b2lo[16], b2hi[16];
        intel_sub_group_2d_block_read_transform_16b_16r16x2c(
            (__global void*)B, B_wb, K, B_pb, (int2)(tile_n, k0 + 64), b2lo);

        DPAS8(a1, 32, b1hi, 0);

        intel_sub_group_2d_block_read_transform_16b_16r16x2c(
            (__global void*)B, B_wb, K, B_pb, (int2)(tile_n, k0 + 80), b2hi);

        // === Block 2: k0+64 .. k0+95 ===
        ushort a3[64];
        intel_sub_group_2d_block_read_16b_32r16x2c(
            (__global void*)A, A_wb, M, A_pb, (int2)(k0 + 96, tile_m), a3);

        DPAS8(a2, 0, b2lo, 0);

        uint b3lo[16], b3hi[16];
        intel_sub_group_2d_block_read_transform_16b_16r16x2c(
            (__global void*)B, B_wb, K, B_pb, (int2)(tile_n, k0 + 96), b3lo);

        DPAS8(a2, 32, b2hi, 0);

        intel_sub_group_2d_block_read_transform_16b_16r16x2c(
            (__global void*)B, B_wb, K, B_pb, (int2)(tile_n, k0 + 112), b3hi);
