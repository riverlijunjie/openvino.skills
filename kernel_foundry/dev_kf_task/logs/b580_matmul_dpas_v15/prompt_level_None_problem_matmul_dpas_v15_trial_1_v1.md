...
### Version 1 (Result: Correctness score: 5 / 5 (compiles and correct), runtime in ms: 0.650):
```OCL
// v29: K-unroll 4x (64 per iter) + B prefetch + interleaved load/compute
// Launch: GWS = (ceil(N/WG_TILE_N)*WG_SIZE, ceil(M/WG_TILE_M)), LWS = (WG_SIZE, 1)
// WG_TILE_M = 128, WG_TILE_N = 256, WG_SIZE = 512, SG_SIZE = 16
    ...
    int M, int N, int K)
{
    ...
    intel_sub_group_2d_block_prefetch_16b_16r16x2c(
        (__global void*)A, A_wb, M, A_pb, (int2)(0, tile_m));
    intel_sub_group_2d_block_prefetch_16b_16r16x2c(
        (__global void*)A, A_wb, M, A_pb, (int2)(0, tile_m + 16));
    intel_sub_group_2d_block_prefetch_16b_16r16x2c(
        (__global void*)B, B_wb, K, B_pb, (int2)(tile_n, 0));
    intel_sub_group_2d_block_prefetch_16b_16r16x2c(
        (__global void*)B, B_wb, K, B_pb, (int2)(tile_n, 16));

    const int k_iters = K / (K_STEP * K_UNROLL);

    for (int ki = 0; ki < k_iters; ki++) {
        int k0 = ki * (K_STEP * K_UNROLL);

        // ---- First K_STEP (k0 .. k0+31) ----
        ushort a_data0[64];
        ...
        int k_next = k0 + K_STEP;
        intel_sub_group_2d_block_prefetch_16b_16r16x2c(
            (__global void*)A, A_wb, M, A_pb, (int2)(k_next, tile_m));
        intel_sub_group_2d_block_prefetch_16b_16r16x2c(
            (__global void*)A, A_wb, M, A_pb, (int2)(k_next, tile_m + 16));
        ...
        // Prefetch B for second half
        intel_sub_group_2d_block_prefetch_16b_16r16x2c(
            (__global void*)B, B_wb, K, B_pb, (int2)(tile_n, k_next));
        intel_sub_group_2d_block_prefetch_16b_16r16x2c(
            (__global void*)B, B_wb, K, B_pb, (int2)(tile_n, k_next + 16));
        ...
        // ---- Second K_STEP (k0+32 .. k0+63) ----
        ushort a_data1[64];
        intel_sub_group_2d_block_read_16b_32r16x2c(
            (__global void*)A, A_wb, M, A_pb, (int2)(k_next, tile_m), a_data1);

        uint b0_data1[16], b1_data1[16];
        intel_sub_group_2d_block_read_transform_16b_16r16x2c(
            (__global void*)B, B_wb, K, B_pb, (int2)(tile_n, k_next), b0_data1);
        intel_sub_group_2d_block_read_transform_16b_16r16x2c(
            (__global void*)B, B_wb, K, B_pb, (int2)(tile_n, k_next + 16), b1_data1);

        // Prefetch next iteration's A and B
        int k_prefetch = k0 + K_STEP * K_UNROLL;
        if (ki + 1 < k_iters) {
            intel_sub_group_2d_block_prefetch_16b_16r16x2c(
                (__global void*)A, A_wb, M, A_pb, (int2)(k_prefetch, tile_m));
            intel_sub_group_2d_block_prefetch_16b_16r16x2c(
                (__global void*)A, A_wb, M, A_pb, (int2)(k_prefetch, tile_m + 16));
            intel_sub_group_2d_block_prefetch_16b_16r16x2c(
                (__global void*)B, B_wb, K, B_pb, (int2)(tile_n, k_prefetch));
            intel_sub_group_2d_block_prefetch_16b_16r16x2c(
                (__global void*)B, B_wb, K, B_pb, (int2)(tile_n, k_prefetch + 16));
        }

        // First 16 columns of K (second step)
        {
        ...
        // Second 16 columns of K (second step)
        {
...
### Version 2 (Result: Correctness score: 5 / 5 (compiles and correct), runtime in ms: 0.869):
```OCL
// v29: K-loop 2x unroll + B prefetch + load/compute interleaving
// Architecture: 32 SGs (4x8), each SG computes 32x32, WG tile = 128x256
// Launch: GWS = (ceil(M/128)*512, ceil(N/256)), LWS = (512, 1)
// Subgroup size: 16
    ...
    int M, int N, int K)
{
    ...
    // 2x K-loop unroll: process 64 K elements per iteration
    int k0 = 0;
    for (; k0 + 64 <= K; k0 += 64) {
        // === Iteration 0: k0 ===
        ushort a_data0[64];
        ...
        intel_sub_group_2d_block_prefetch_16b_16r16x2c(
            (__global void*)B, B_wb, K, B_pb, (int2)(tile_n, k0 + 32));
        intel_sub_group_2d_block_prefetch_16b_16r16x2c(
            (__global void*)B, B_wb, K, B_pb, (int2)(tile_n, k0 + 48));
        ...
        // Prefetch next outer iteration's B (k0+64)
        if (k0 + 64 < K) {
            intel_sub_group_2d_block_prefetch_16b_16r16x2c(
                (__global void*)B, B_wb, K, B_pb, (int2)(tile_n, k0 + 64));
            intel_sub_group_2d_block_prefetch_16b_16r16x2c(
                (__global void*)B, B_wb, K, B_pb, (int2)(tile_n, k0 + 80));
        }
    ...
    // Handle remaining 32 elements (if K not divisible by 64)
    for (; k0 < K; k0 += 32) {
        ushort a_data[64];
        intel_sub_group_2d_block_read_16b_32r16x2c(
...
Here is the last OCL kernel we tested (Result: Correctness score: 5 / 5 (compiles and correct), runtime in ms: 0.417):
```OCL
