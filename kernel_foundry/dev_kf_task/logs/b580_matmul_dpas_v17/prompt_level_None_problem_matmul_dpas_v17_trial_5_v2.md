...
### Version 1 (Result: Correctness score: 5 / 5 (compiles and correct), runtime in ms: 0.717):
```OCL
...
// K-loop step=32, double-buffered A in SLM
// Optimizations: stride=32, interleaved A prefetch with DPAS, B prefetch for next k16,
//   no remainder path (K=2048 divides by 32), boustrophedon DPAS order
    ...
    const int N)
{
    ...
    // SLM for A: double-buffered, 32 rows x 32 cols, stride=32 (no bank conflict padding needed
    // since 32 halfs = 64 bytes = 4 banks of 16 bytes, and sg_lid indexes within 16 elements)
    ...
    for (int ki = 0; ki < k_iters - 1; ki++) {
        ...
        // === DPAS first k16 - interleave with next A tile load and B prefetch ===
        acc0 = intel_sub_group_f16_f16_matrix_mad_k16(a00, b0, acc0);

        // Start loading next A tile into alternate SLM buffer (overlaps with DPAS)
        ...
        *(__local half16*)a_next_dst = *(__global const half16*)a_next_src;
        ...
        // === Load B for second k16 (k_base+16..k_base+31) - overlapped with first k16 DPAS completion ===
        int8 b1;
        {
        ...
        // === DPAS second k16 - boustrophedon order (reverse) for register locality ===
    ...
    // === Last iteration (ki = k_iters - 1): no next A load needed ===
    {
        ...
        // Boustrophedon: reverse order for last k16
        acc3 = intel_sub_group_f16_f16_matrix_mad_k16(a13, b1, acc3);
        acc2 = intel_sub_group_f16_f16_matrix_mad_k16(a12, b1, acc2);
        acc1 = intel_sub_group_f16_f16_matrix_mad_k16(a11, b1, acc1);
        acc0 = intel_sub_group_f16_f16_matrix_mad_k16(a10, b1, acc0);
...
### Version 2 (Result: Correctness score: 5 / 5 (compiles and correct), runtime in ms: 0.720):
```OCL
// GEMM: C[M,N] = A[M,K] x B[K,N], row-major, FP16 in/out, FP32 accumulation
// Architecture: 64 WIs (4 SGs of 16), TILE 32x64x32, A in SLM (double-buffered), B from global
// Launch: GWS = (N/64 * 64, M/32), LWS = (64, 1)
...
// Optimizations over best (0.721ms):
//   - Better interleaving: B loads for k16_1 start during k16_0 DPAS
//   - Boustrophedon DPAS ordering (forward/reverse) for register locality
//   - Tighter code in last iteration
    ...
    const int N)
{
    ...
    for (int ki = 0; ki < k_iters - 1; ki++) {
        ...
        // === DPAS first k16 - interleave with next A load ===
        acc0 = intel_sub_group_f16_f16_matrix_mad_k16(a00, b0, acc0);

        // Start loading next A tile into alternate buffer (overlaps with DPAS)
        __global const half* a_next_src = A + a_gm_row * K + (k_base + 32) + a_load_col;
        __local half* a_next_dst = slm_A + next_buf * A_SLM_SIZE + a_load_row * A_SLM_STRIDE + a_load_col;
        ...
        *(__local half16*)a_next_dst = *(__global const half16*)a_next_src;
        ...
        // === Load B for second k16 (overlaps with tail of first k16 DPAS) ===
        int8 b1;
        {
        ...
        // === DPAS second k16 (boustrophedon: reverse order) ===
        acc3 = intel_sub_group_f16_f16_matrix_mad_k16(a13, b1, acc3);
        acc2 = intel_sub_group_f16_f16_matrix_mad_k16(a12, b1, acc2);
        acc1 = intel_sub_group_f16_f16_matrix_mad_k16(a11, b1, acc1);
        acc0 = intel_sub_group_f16_f16_matrix_mad_k16(a10, b1, acc0);
...
Here is the last OCL kernel we tested (Result: Correctness score: 5 / 5 (compiles and correct), runtime in ms: 0.716):
```OCL
// GEMM: C[M,N] = A[M,K] x B[K,N], row-major, FP16 in/out, FP32 accumulation
// Architecture: 64 WIs (4 SGs of 16), TILE 32x64x32, A in SLM (double-buffered), B from global
// Launch: GWS = (N/64 * 64, M/32), LWS = (64, 1)
