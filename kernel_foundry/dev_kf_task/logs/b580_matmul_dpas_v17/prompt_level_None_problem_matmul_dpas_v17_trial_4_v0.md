...
This is the code of the best kernel we have tested so far (Result: Correctness score: 5 / 5 (compiles and correct), runtime in ms: 0.721):
```OCL
// GEMM: C[M,N] = A[M,K] x B[K,N], row-major, FP16 in/out, FP32 accumulation
// Architecture: 64 WIs (4 SGs of 16), TILE 32x64x32, A in SLM (double-buffered), B from global
// Launch: GWS = (N/64 * 64, M/32), LWS = (64, 1)
...
// Each WG computes a 32(M) x 64(N) output tile
// 4 subgroups, each handles 32 rows x 16 cols
// K-loop unrolled 2x (processes 64 K-elements per unrolled iteration)
    ...
    const int N)
{
    ...
    // Cooperative A load: 64 WIs load 32x32 = 1024 halfs = 16 halfs per WI
    ...
    // Main K-loop, unrolled 2x (process two k-tiles per iteration)
    const int k_iters_2x = k_iters / 2;  // 32 unrolled iterations

    for (int ki2 = 0; ki2 < k_iters_2x; ki2++) {
        // ==== First k-tile of the pair ====
        {
            const int k_base = ki2 * 64;
            __local const half* a_slm = slm_A + cur_buf * A_SLM_SIZE;
        ...
        // ==== Second k-tile of the pair ====
        {
            ...
            // Load next A tile (for second k-tile)
            int next_buf = 1 - cur_buf;
            if (ki2 < k_iters_2x - 1 || 1) {
                // Always load next - last iteration loads beyond but we handle it
                // Actually for last pair's second tile, we don't need next load
                // But since k_iters is even (64), and we do k_iters/2=32 pairs,
                // the last pair processes k=62*32..63*32. After that no more loads needed.
                // We guard with the condition below.
            }
            {
                int next_k = k_base + 32;
                if (next_k < K) {
...
================== 4 passed, 1 deselected, 1 warning in 0.96s ==================
