...
Here is the last OCL kernel we tested (Result: Correctness score: 5 / 5 (compiles and correct), runtime in ms: 0.727):
```OCL
// GEMM: C[M,N] = A[M,K] x B[K,N], row-major, FP16 in/out, FP32 accumulation
// Architecture: 64 WIs (4 SGs of 16), TILE 32x64x32, A in SLM (double-buffered), B from global
// Launch: GWS = (N/64 * 64, M/32), LWS = (64, 1)
// Each WG computes a 32(M) x 64(N) output tile
// 4 subgroups, each handles 32 rows x 16 cols
// K-loop 2x unrolled: 32 pairs for K=2048, only 31 barriers in main loop
// Optimizations:
//   - SLM stride=34 to avoid bank conflicts (32 halfs data + 2 padding)
//   - Boustrophedon DPAS ordering for register locality
//   - Interleaved A prefetch with DPAS execution
//   - B1 load overlapped with first DPAS batch
    ...
    const int N)
{
    ...
    // SLM stride=34 to avoid bank conflicts when reading columns
    // 32 halfs of data per row + 2 halfs padding
    ...
    // K=2048, k_iters=64, 2x unroll: 32 pairs, 31 barriers in main loop + 0 in last pair
    const int k_pairs = K / 64;  // 32
    int cur_buf = 0;

    // Macro for DPAS-friendly A load from SLM (8 rows)
    #define LOAD_A_SLM8(a_var, base_ptr) \
        (a_var).s0 = as_short((base_ptr)[0 * A_SLM_STRIDE]); \
        (a_var).s1 = as_short((base_ptr)[1 * A_SLM_STRIDE]); \
        (a_var).s2 = as_short((base_ptr)[2 * A_SLM_STRIDE]); \
        (a_var).s3 = as_short((base_ptr)[3 * A_SLM_STRIDE]); \
        (a_var).s4 = as_short((base_ptr)[4 * A_SLM_STRIDE]); \
        (a_var).s5 = as_short((base_ptr)[5 * A_SLM_STRIDE]); \
        (a_var).s6 = as_short((base_ptr)[6 * A_SLM_STRIDE]); \
        (a_var).s7 = as_short((base_ptr)[7 * A_SLM_STRIDE]);

    // Macro for loading B tile (16 rows x 16 cols, one element per WI per row)
    #define LOAD_B_TILE(b_var, b_base) \
    { \
    ...
    // Main loop: process K in pairs of 32 (64 elements per iteration)
    for (int ki = 0; ki < k_pairs - 1; ki++) {
        const int k_base_0 = ki * 64;
        const int k_base_1 = k_base_0 + 32;
        const int next_buf = 1 - cur_buf;

        // ============ FIRST K-STEP (k_base_0, uses cur_buf) ============
        {
            __local const half* a_slm = slm_A + cur_buf * A_SLM_SIZE;

            // Load B for first k16
            int8 b0;
            LOAD_B_TILE(b0, B + k_base_0 * N + baseN + sg_lid);

            // Load A from SLM for first k16 (columns 0..15)
            short8 a00, a01, a02, a03;
            __local const half* a_k0 = a_slm + sg_lid;
            LOAD_A_SLM8(a00, a_k0);
            LOAD_A_SLM8(a01, a_k0 + 8 * A_SLM_STRIDE);
            LOAD_A_SLM8(a02, a_k0 + 16 * A_SLM_STRIDE);
            LOAD_A_SLM8(a03, a_k0 + 24 * A_SLM_STRIDE);

            // DPAS first k16 - forward, interleave A prefetch for step 1
            acc0 = intel_sub_group_f16_f16_matrix_mad_k16(a00, b0, acc0);

            // Prefetch next A tile (for step 1) into alternate buffer
            __global const half* a_next_src = A + a_gm_row * K + k_base_1 + a_load_col;
            __local half* a_next_dst = slm_A + next_buf * A_SLM_SIZE + a_load_row * A_SLM_STRIDE + a_load_col;
            *(__local half16*)a_next_dst = *(__global const half16*)a_next_src;
            ...
            int8 b1;
            LOAD_B_TILE(b1, B + (k_base_0 + 16) * N + baseN + sg_lid);

            // Load A from SLM for second k16 (columns 16..31)
            short8 a10, a11, a12, a13;
            __local const half* a_k1 = a_slm + 16 + sg_lid;
            LOAD_A_SLM8(a10, a_k1);
            LOAD_A_SLM8(a11, a_k1 + 8 * A_SLM_STRIDE);
            LOAD_A_SLM8(a12, a_k1 + 16 * A_SLM_STRIDE);
            LOAD_A_SLM8(a13, a_k1 + 24 * A_SLM_STRIDE);
        ...
        // ============ SECOND K-STEP (k_base_1, uses next_buf) ============
        {
            __local const half* a_slm = slm_A + next_buf * A_SLM_SIZE;

            // Load B for first k16
            int8 b0;
            LOAD_B_TILE(b0, B + k_base_1 * N + baseN + sg_lid);
            ...
            // DPAS first k16 - forward, interleave next A prefetch
            acc0 = intel_sub_group_f16_f16_matrix_mad_k16(a00, b0, acc0);

            // Prefetch A for next pair's step 0 into cur_buf
            __global const half* a_next_src = A + a_gm_row * K + (k_base_1 + 32) + a_load_col;
            __local half* a_next_dst = slm_A + cur_buf * A_SLM_SIZE + a_load_row * A_SLM_STRIDE + a_load_col;
            *(__local half16*)a_next_dst = *(__global const half16*)a_next_src;
            ...
            // Load B for second k16
            int8 b1;
            LOAD_B_TILE(b1, B + (k_base_1 + 16) * N + baseN + sg_lid);
            ...
            // DPAS second k16 - forward (boustrophedon alternates per step)
            acc0 = intel_sub_group_f16_f16_matrix_mad_k16(a10, b1, acc0);
            acc1 = intel_sub_group_f16_f16_matrix_mad_k16(a11, b1, acc1);
            acc2 = intel_sub_group_f16_f16_matrix_mad_k16(a12, b1, acc2);
            acc3 = intel_sub_group_f16_f16_matrix_mad_k16(a13, b1, acc3);
    ...
    // ============ LAST PAIR (ki = k_pairs - 1): no prefetch beyond K ============
    {
        const int k_base_0 = (k_pairs - 1) * 64;
        const int k_base_1 = k_base_0 + 32;
        const int next_buf = 1 - cur_buf;
...
======================= 4 passed, 1 deselected in 0.96s ========================
