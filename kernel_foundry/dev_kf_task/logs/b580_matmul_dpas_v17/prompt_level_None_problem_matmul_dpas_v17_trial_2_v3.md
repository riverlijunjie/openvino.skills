...
Here is the last OCL kernel we tested (Result: Correctness score: 2 / 5 (compiled but incorrect), runtime in ms: -1.000):
```OCL
// GEMM: C[M,N] = A[M,K] x B[K,N], row-major, FP16 in/out, FP32 accumulation
// Architecture: 64 WIs (4 SGs of 16), TILE 32x64x32, A in SLM, B from global/L2
// Launch: GWS = (N/4, M/32) * LWS, LWS = (16, 4, 1)
//   i.e. GWS[0] = (N/64)*16, GWS[1] = (M/32)*4
// Subgroup size = 16
// Hardware: Intel B580 (Xe2), 20 Xe-cores, 96 TFLOPS FP16 XMX
    ...
    const int N)
{
    ...
    // Base coordinates for this WG's output tile
    const int baseM = wg_m * 32;
    const int baseN = wg_n * 64;

    // Each subgroup handles 16 columns
    const int sgColBase = baseN + sg_id * 16;

    // SLM for A: double-buffered, 32 rows x 32 cols
    // Stride = 34 halfs to reduce bank conflicts
    #define SLM_A_STRIDE 34
    __local half slm_A[2 * 32 * SLM_A_STRIDE];
    ...
    for (int ki = 0; ki < k_iters; ki++) {
        ...
        // ---- Load next A tile into other buffer (overlapped with compute) ----
        // We load it after reading current buffer but before the barrier
        const int next_buf = 1 - buf;

        // ---- Read A from current SLM buffer ----
        __local const half* cur_slm = slm_A + buf * 32 * SLM_A_STRIDE;

        // ---- Load B from global memory ----
        // B is [K, N] row-major. Each WI reads its column (sgColBase + sg_lid)
        // For k16 DPAS: int8 b where each int packs 2 consecutive k-values
        __global const half* b_col = B + k_base * N + sgColBase + sg_lid;

        // ==== K-step 0: k_base + 0..15 ====
        // Load B for k=0..15
        int8 b_val0;
        {
            ushort bk0  = as_ushort(b_col[0 * N]);
            ushort bk1  = as_ushort(b_col[1 * N]);
            ushort bk2  = as_ushort(b_col[2 * N]);
            ushort bk3  = as_ushort(b_col[3 * N]);
            ushort bk4  = as_ushort(b_col[4 * N]);
            ushort bk5  = as_ushort(b_col[5 * N]);
            ushort bk6  = as_ushort(b_col[6 * N]);
            ushort bk7  = as_ushort(b_col[7 * N]);
            ushort bk8  = as_ushort(b_col[8 * N]);
            ushort bk9  = as_ushort(b_col[9 * N]);
            ushort bk10 = as_ushort(b_col[10 * N]);
            ushort bk11 = as_ushort(b_col[11 * N]);
            ushort bk12 = as_ushort(b_col[12 * N]);
            ushort bk13 = as_ushort(b_col[13 * N]);
            ushort bk14 = as_ushort(b_col[14 * N]);
            ushort bk15 = as_ushort(b_col[15 * N]);

            b_val0.s0 = as_int((ushort2)(bk0, bk1));
            b_val0.s1 = as_int((ushort2)(bk2, bk3));
            b_val0.s2 = as_int((ushort2)(bk4, bk5));
            b_val0.s3 = as_int((ushort2)(bk6, bk7));
            b_val0.s4 = as_int((ushort2)(bk8, bk9));
            b_val0.s5 = as_int((ushort2)(bk10, bk11));
            b_val0.s6 = as_int((ushort2)(bk12, bk13));
            b_val0.s7 = as_int((ushort2)(bk14, bk15));
        }

        // Load A from SLM for k-step 0 (columns 0..15)
        // For intel_sub_group_f16_f16_matrix_mad_k16:
        //   short8 a: WI[w] holds a.sR = A[row_base+R][k_base+w] as short (= f16 bits)
        //   This represents 8 rows × 16 k-elements distributed across 16 WIs
        short8 a00, a01, a02, a03;
        {
            __local const half* a_ptr = cur_slm + sg_lid;  // column = sg_lid (k-index within tile)
            a00.s0 = as_short(a_ptr[0 * SLM_A_STRIDE]);
            a00.s1 = as_short(a_ptr[1 * SLM_A_STRIDE]);
            a00.s2 = as_short(a_ptr[2 * SLM_A_STRIDE]);
            a00.s3 = as_short(a_ptr[3 * SLM_A_STRIDE]);
            a00.s4 = as_short(a_ptr[4 * SLM_A_STRIDE]);
            a00.s5 = as_short(a_ptr[5 * SLM_A_STRIDE]);
            a00.s6 = as_short(a_ptr[6 * SLM_A_STRIDE]);
            a00.s7 = as_short(a_ptr[7 * SLM_A_STRIDE]);

            a01.s0 = as_short(a_ptr[8 * SLM_A_STRIDE]);
            a01.s1 = as_short(a_ptr[9 * SLM_A_STRIDE]);
            a01.s2 = as_short(a_ptr[10 * SLM_A_STRIDE]);
            a01.s3 = as_short(a_ptr[11 * SLM_A_STRIDE]);
            a01.s4 = as_short(a_ptr[12 * SLM_A_STRIDE]);
            a01.s5 = as_short(a_ptr[13 * SLM_A_STRIDE]);
            a01.s6 = as_short(a_ptr[14 * SLM_A_STRIDE]);
            a01.s7 = as_short(a_ptr[15 * SLM_A_STRIDE]);

            a02.s0 = as_short(a_ptr[16 * SLM_A_STRIDE]);
            a02.s1 = as_short(a_ptr[17 * SLM_A_STRIDE]);
            a02.s2 = as_short(a_ptr[18 * SLM_A_STRIDE]);
            a02.s3 = as_short(a_ptr[19 * SLM_A_STRIDE]);
            a02.s4 = as_short(a_ptr[20 * SLM_A_STRIDE]);
            a02.s5 = as_short(a_ptr[21 * SLM_A_STRIDE]);
            a02.s6 = as_short(a_ptr[22 * SLM_A_STRIDE]);
            a02.s7 = as_short(a_ptr[23 * SLM_A_STRIDE]);

            a03.s0 = as_short(a_ptr[24 * SLM_A_STRIDE]);
            a03.s1 = as_short(a_ptr[25 * SLM_A_STRIDE]);
            a03.s2 = as_short(a_ptr[26 * SLM_A_STRIDE]);
            a03.s3 = as_short(a_ptr[27 * SLM_A_STRIDE]);
            a03.s4 = as_short(a_ptr[28 * SLM_A_STRIDE]);
            a03.s5 = as_short(a_ptr[29 * SLM_A_STRIDE]);
            a03.s6 = as_short(a_ptr[30 * SLM_A_STRIDE]);
            a03.s7 = as_short(a_ptr[31 * SLM_A_STRIDE]);
        }
        ...
        // ==== K-step 1: k_base + 16..31 ====
        __global const half* b_col1 = b_col + 16 * N;
        int8 b_val1;
        {
            ushort bk0  = as_ushort(b_col1[0 * N]);
            ushort bk1  = as_ushort(b_col1[1 * N]);
            ushort bk2  = as_ushort(b_col1[2 * N]);
            ushort bk3  = as_ushort(b_col1[3 * N]);
            ushort bk4  = as_ushort(b_col1[4 * N]);
            ushort bk5  = as_ushort(b_col1[5 * N]);
            ushort bk6  = as_ushort(b_col1[6 * N]);
            ushort bk7  = as_ushort(b_col1[7 * N]);
            ushort bk8  = as_ushort(b_col1[8 * N]);
            ushort bk9  = as_ushort(b_col1[9 * N]);
            ushort bk10 = as_ushort(b_col1[10 * N]);
            ushort bk11 = as_ushort(b_col1[11 * N]);
            ushort bk12 = as_ushort(b_col1[12 * N]);
            ushort bk13 = as_ushort(b_col1[13 * N]);
            ushort bk14 = as_ushort(b_col1[14 * N]);
            ushort bk15 = as_ushort(b_col1[15 * N]);

            b_val1.s0 = as_int((ushort2)(bk0, bk1));
            b_val1.s1 = as_int((ushort2)(bk2, bk3));
            b_val1.s2 = as_int((ushort2)(bk4, bk5));
            b_val1.s3 = as_int((ushort2)(bk6, bk7));
            b_val1.s4 = as_int((ushort2)(bk8, bk9));
            b_val1.s5 = as_int((ushort2)(bk10, bk11));
            b_val1.s6 = as_int((ushort2)(bk12, bk13));
            b_val1.s7 = as_int((ushort2)(bk14, bk15));
        }

        // Load A from SLM for k-step 1 (columns 16..31)
        short8 a10, a11, a12, a13;
        {
            __local const half* a_ptr = cur_slm + 16 + sg_lid;  // k offset 16 + lane
            a10.s0 = as_short(a_ptr[0 * SLM_A_STRIDE]);
            a10.s1 = as_short(a_ptr[1 * SLM_A_STRIDE]);
            a10.s2 = as_short(a_ptr[2 * SLM_A_STRIDE]);
            a10.s3 = as_short(a_ptr[3 * SLM_A_STRIDE]);
            a10.s4 = as_short(a_ptr[4 * SLM_A_STRIDE]);
            a10.s5 = as_short(a_ptr[5 * SLM_A_STRIDE]);
            a10.s6 = as_short(a_ptr[6 * SLM_A_STRIDE]);
            a10.s7 = as_short(a_ptr[7 * SLM_A_STRIDE]);

            a11.s0 = as_short(a_ptr[8 * SLM_A_STRIDE]);
            a11.s1 = as_short(a_ptr[9 * SLM_A_STRIDE]);
            a11.s2 = as_short(a_ptr[10 * SLM_A_STRIDE]);
            a11.s3 = as_short(a_ptr[11 * SLM_A_STRIDE]);
            a11.s4 = as_short(a_ptr[12 * SLM_A_STRIDE]);
            a11.s5 = as_short(a_ptr[13 * SLM_A_STRIDE]);
            a11.s6 = as_short(a_ptr[14 * SLM_A_STRIDE]);
            a11.s7 = as_short(a_ptr[15 * SLM_A_STRIDE]);

            a12.s0 = as_short(a_ptr[16 * SLM_A_STRIDE]);
            a12.s1 = as_short(a_ptr[17 * SLM_A_STRIDE]);
            a12.s2 = as_short(a_ptr[18 * SLM_A_STRIDE]);
            a12.s3 = as_short(a_ptr[19 * SLM_A_STRIDE]);
            a12.s4 = as_short(a_ptr[20 * SLM_A_STRIDE]);
            a12.s5 = as_short(a_ptr[21 * SLM_A_STRIDE]);
            a12.s6 = as_short(a_ptr[22 * SLM_A_STRIDE]);
            a12.s7 = as_short(a_ptr[23 * SLM_A_STRIDE]);

            a13.s0 = as_short(a_ptr[24 * SLM_A_STRIDE]);
            a13.s1 = as_short(a_ptr[25 * SLM_A_STRIDE]);
            a13.s2 = as_short(a_ptr[26 * SLM_A_STRIDE]);
            a13.s3 = as_short(a_ptr[27 * SLM_A_STRIDE]);
            a13.s4 = as_short(a_ptr[28 * SLM_A_STRIDE]);
            a13.s5 = as_short(a_ptr[29 * SLM_A_STRIDE]);
            a13.s6 = as_short(a_ptr[30 * SLM_A_STRIDE]);
            a13.s7 = as_short(a_ptr[31 * SLM_A_STRIDE]);
        }
    ...
    // ---- Store C: 32 rows x 16 cols per subgroup ----
    __global half* c_out = C + baseM * N + sgColBase + sg_lid;

    c_out[0 * N] = convert_half(acc0.s0);
    c_out[1 * N] = convert_half(acc0.s1);
    c_out[2 * N] = convert_half(acc0.s2);
    c_out[3 * N] = convert_half(acc0.s3);
    c_out[4 * N] = convert_half(acc0.s4);
    c_out[5 * N] = convert_half(acc0.s5);
    c_out[6 * N] = convert_half(acc0.s6);
    c_out[7 * N] = convert_half(acc0.s7);

    c_out[8 * N]  = convert_half(acc1.s0);
    c_out[9 * N]  = convert_half(acc1.s1);
    c_out[10 * N] = convert_half(acc1.s2);
    c_out[11 * N] = convert_half(acc1.s3);
    c_out[12 * N] = convert_half(acc1.s4);
    c_out[13 * N] = convert_half(acc1.s5);
    c_out[14 * N] = convert_half(acc1.s6);
    c_out[15 * N] = convert_half(acc1.s7);

    c_out[16 * N] = convert_half(acc2.s0);
    c_out[17 * N] = convert_half(acc2.s1);
    c_out[18 * N] = convert_half(acc2.s2);
    c_out[19 * N] = convert_half(acc2.s3);
    c_out[20 * N] = convert_half(acc2.s4);
    c_out[21 * N] = convert_half(acc2.s5);
    c_out[22 * N] = convert_half(acc2.s6);
    c_out[23 * N] = convert_half(acc2.s7);

    c_out[24 * N] = convert_half(acc3.s0);
    c_out[25 * N] = convert_half(acc3.s1);
    c_out[26 * N] = convert_half(acc3.s2);
    c_out[27 * N] = convert_half(acc3.s3);
    c_out[28 * N] = convert_half(acc3.s4);
    c_out[29 * N] = convert_half(acc3.s5);
    c_out[30 * N] = convert_half(acc3.s6);
    c_out[31 * N] = convert_half(acc3.s7);
}
```
...
Process killed due to inactivity
