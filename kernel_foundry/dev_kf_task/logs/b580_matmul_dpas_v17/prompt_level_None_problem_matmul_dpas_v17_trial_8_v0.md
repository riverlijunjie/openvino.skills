...
Here is the last OCL kernel we tested (Result: Correctness score: 5 / 5 (compiles and correct), runtime in ms: 0.732):
```OCL
// GEMM: C[M,N] = A[M,K] x B[K,N], row-major, FP16 in/out, FP32 accumulation
// Architecture: 64 WIs (4 SGs of 16), TILE 32x64x32, A in SLM (double-buffered), B from global
// Launch: GWS = (N/64 * 64, M/32), LWS = (64, 1)
// Each WG computes a 32(M) x 64(N) output tile
// 4 subgroups, each handles 32 rows x 16 cols
// K-loop 2x unrolled: 32 pairs for K=2048, only 31 barriers
// SLM stride = 34 to avoid bank conflicts (breaks power-of-2 alignment)
// Boustrophedon DPAS ordering for register locality
    ...
    const int N)
{
    ...
    // SLM stride = 34 to avoid bank conflicts (32 would cause all 16 WIs in a SG
    // to hit the same bank when reading columns with stride 32)
    ...
    // Preload first A tile (k=0..31) into SLM buffer 0
    {
        __global const half* a_src = A + a_gm_row * K + a_load_col;
        __local half* a_dst = slm_A + a_load_row * A_SLM_STRIDE + a_load_col;
        half16 tmp = *(__global const half16*)a_src;
        *(__local half16*)a_dst = tmp;
    }
    barrier(CLK_LOCAL_MEM_FENCE);

    const int k_pairs = K / 64;  // 32 pairs for K=2048
    int cur_buf = 0;

    // Macro for loading B tile (16 rows x 16 cols, each WI loads one column)
    #define LOAD_B_TILE(b_var, b_base_ptr) \
    { \
    ...
    { \
        (a_var).s0 = as_short((a_ptr)[0 * A_SLM_STRIDE]); \
        (a_var).s1 = as_short((a_ptr)[1 * A_SLM_STRIDE]); \
        (a_var).s2 = as_short((a_ptr)[2 * A_SLM_STRIDE]); \
        (a_var).s3 = as_short((a_ptr)[3 * A_SLM_STRIDE]); \
        (a_var).s4 = as_short((a_ptr)[4 * A_SLM_STRIDE]); \
        (a_var).s5 = as_short((a_ptr)[5 * A_SLM_STRIDE]); \
        (a_var).s6 = as_short((a_ptr)[6 * A_SLM_STRIDE]); \
        (a_var).s7 = as_short((a_ptr)[7 * A_SLM_STRIDE]); \
    }

    // B base pointer for this subgroup's column
    __global const half* b_base = B + baseN + sg_lid;

    // Main loop: process pairs of k-steps (each pair = 64 K elements)
    for (int ki = 0; ki < k_pairs - 1; ki++) {
        ...
        // ============ FIRST K-STEP (k_base_0, uses cur_buf) ============
        {
            ...
            int8 b0;
            LOAD_B_TILE(b0, b_base + k_base_0 * N);

            // Load A from SLM for first k16
            short8 a00, a01, a02, a03;
            __local const half* a_k0 = a_slm + sg_lid;
            LOAD_A8(a00, a_k0);
            LOAD_A8(a01, a_k0 + 8 * A_SLM_STRIDE);
            LOAD_A8(a02, a_k0 + 16 * A_SLM_STRIDE);
            LOAD_A8(a03, a_k0 + 24 * A_SLM_STRIDE);

            // DPAS first k16 - forward
            acc0 = intel_sub_group_f16_f16_matrix_mad_k16(a00, b0, acc0);

            // Prefetch next A tile (k_base_1) into next_buf
            {
                __global const half* a_src = A + a_gm_row * K + k_base_1 + a_load_col;
            ...
            int8 b1;
            LOAD_B_TILE(b1, b_base + (k_base_0 + 16) * N);

            // Load A from SLM for second k16
            short8 a10, a11, a12, a13;
            __local const half* a_k1 = a_slm + 16 + sg_lid;
            LOAD_A8(a10, a_k1);
            LOAD_A8(a11, a_k1 + 8 * A_SLM_STRIDE);
            LOAD_A8(a12, a_k1 + 16 * A_SLM_STRIDE);
            LOAD_A8(a13, a_k1 + 24 * A_SLM_STRIDE);
        ...
        // ============ SECOND K-STEP (k_base_1, uses next_buf) ============
        {
            ...
            // Load B for first k16
            int8 b0;
            LOAD_B_TILE(b0, b_base + k_base_1 * N);
            ...
            // Prefetch next pair's first A tile (k_base_1 + 32) into cur_buf
            {
                __global const half* a_src = A + a_gm_row * K + (k_base_1 + 32) + a_load_col;
                __local half* a_dst = slm_A + cur_buf * A_SLM_SIZE + a_load_row * A_SLM_STRIDE + a_load_col;
                *(__local half16*)a_dst = *(__global const half16*)a_src;
            }
            ...
            // Load B for second k16
            int8 b1;
            LOAD_B_TILE(b1, b_base + (k_base_1 + 16) * N);
...
======================= 4 passed, 1 deselected in 0.88s ========================
