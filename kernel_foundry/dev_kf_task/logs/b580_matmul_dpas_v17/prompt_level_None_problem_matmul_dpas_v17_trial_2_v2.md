...
### Version 2 (Result: Correctness score: 5 / 5 (compiles and correct), runtime in ms: 1.320):
```OCL
// GEMM: C[M,N] = A[M,K] x B[K,N], row-major, FP16 in/out, FP32 accumulation
// Architecture: 64 WIs (4 SGs of 16), TILE 32x64x32, A in SLM (double-buffered), B from global
    ...
    const int N)
{
    ...
    // Cooperative A load mapping: 64 WIs load 32x32 = 1024 halfs = 16 halfs/WI
    // Each WI loads one half-row (16 halfs)
    // lid 0..63: row = lid/2 (0..31), col_half = lid%2 (0 or 1) -> col_start = col_half*16
    const int a_load_row = lid / 2;
    const int a_load_col = (lid & 1) * 16;

    // Preload first A tile (k=0..31) into SLM buffer 0
    {
        int gm_row = baseM + a_load_row;
        __global const half* a_src = A + gm_row * K + a_load_col;
    ...
    // K-loop: K is guaranteed divisible by 32
    ...
    for (int ki = 0; ki < k_iters; ki++) {
        ...
        // Load next A tile into next_buf (overlapped with compute below)
        // We do the load AFTER compute to ensure current buffer is consumed first
        // Actually, let's load next A first (it goes to different buffer), then compute current

        // Prefetch next A tile into next_buf
        if (ki + 1 < k_iters) {
            int next_k = k_base + 32;
            int gm_row = baseM + a_load_row;
            __global const half* a_src = A + gm_row * K + next_k + a_load_col;
            __local half* a_dst = slm_A + next_buf * 32 * SLM_A_STRIDE + a_load_row * SLM_A_STRIDE + a_load_col;
        ...
        __local const half* a_slm = slm_A + cur_buf * 32 * SLM_A_STRIDE;

        // Process two k16 steps per 32-wide K tile
        #pragma unroll
        for (int kk = 0; kk < 2; kk++) {
            int k_off = kk * 16;

            // Load A from SLM: short8 per WI for each 8-row block
            // For intel_sub_group_f16_f16_matrix_mad_k16:
            //   a (short8): WI[w] holds A[row_base+r][k_base+w] for r=0..7
            //   Each short = one f16 value
            short8 a0, a1, a2, a3;

            #pragma unroll
            for (int r = 0; r < 8; r++) {
                ((short*)&a0)[r] = as_short(a_slm[(0 + r) * SLM_A_STRIDE + k_off + sg_lid]);
            }
            #pragma unroll
            for (int r = 0; r < 8; r++) {
                ((short*)&a1)[r] = as_short(a_slm[(8 + r) * SLM_A_STRIDE + k_off + sg_lid]);
            }
            #pragma unroll
            for (int r = 0; r < 8; r++) {
                ((short*)&a2)[r] = as_short(a_slm[(16 + r) * SLM_A_STRIDE + k_off + sg_lid]);
            }
            #pragma unroll
            for (int r = 0; r < 8; r++) {
                ((short*)&a3)[r] = as_short(a_slm[(24 + r) * SLM_A_STRIDE + k_off + sg_lid]);
            }

            // Load B from global: int8 per WI
            // For DPAS b (int8): WI[w] holds pairs of B values for column (baseN + w)
            // b.si = pack(B[k_base + k_off + 2*i, col], B[k_base + k_off + 2*i+1, col])
            int8 b_data;
            __global const half* b_col = B + (k_base + k_off) * N + baseN + sg_lid;

            #pragma unroll
            for (int i = 0; i < 8; i++) {
                half val0 = b_col[(2 * i) * N];
                half val1 = b_col[(2 * i + 1) * N];
                ushort u0 = as_ushort(val0);
                ushort u1 = as_ushort(val1);
                ((int*)&b_data)[i] = ((int)u1 << 16) | (int)u0;
            }
...
Here is the last OCL kernel we tested (Result: Correctness score: 5 / 5 (compiles and correct), runtime in ms: 1.120):
```OCL
// GEMM: C[M,N] = A[M,K] x B[K,N], row-major, FP16 in/out, FP32 accumulation
// Architecture: 64 WIs (4 SGs of 16), TILE 32x64x32, A in SLM (double-buffered), B from global
// Launch: GWS = (N/64 * 64, M/32), LWS = (64, 1)
...
======================= 4 passed, 1 deselected in 0.94s ========================
