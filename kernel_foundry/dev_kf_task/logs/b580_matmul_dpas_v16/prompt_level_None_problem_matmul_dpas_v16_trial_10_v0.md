...
// A loaded cooperatively into SLM, B read directly from global via block reads
// Double-buffered SLM for A, K-loop unrolled 2x
    ...
    const int N)
{
    ...
    const int sg_n_offset = sg_id * 16;

    // SLM for A: double-buffered, 32 rows x 32 cols of half
    // Use stride of 34 (32+2) to reduce bank conflicts
    #define SLM_A_STRIDE 34
    __local half slm_A[2 * TILE_M * SLM_A_STRIDE];  // 2 buffers

    // Accumulators: 4 blocks of 8 rows x 16 cols = float8 x 4
    float8 acc0 = (float8)(0.0f);
    float8 acc1 = (float8)(0.0f);
    float8 acc2 = (float8)(0.0f);
    float8 acc3 = (float8)(0.0f);

    // Cooperative A load: 64 WIs load 32x32 halfs = 1024 halfs
    // Each WI loads 1024/64 = 16 halfs
    #define A_LOADS_PER_WI 16

    // Preload first A tile into SLM buffer 0
    {
        __global const half* A_ptr = A + base_m * K;  // start of A tile rows
        // Each WI loads 16 consecutive halfs
        // Total elements: 32*32 = 1024, 64 WIs, 16 each
        // Map: lid -> (row, col_start): lid*16 / 32 = row offset, lid*16 % 32 = col offset
        // But better: treat as linear and map to 2D
        int linear_base = lid * A_LOADS_PER_WI;
        int a_row = linear_base / TILE_K;
        int a_col = linear_base % TILE_K;

        __global const half* src = A_ptr + a_row * K + a_col;
        __local half* dst = slm_A + a_row * SLM_A_STRIDE + a_col;

        // Load 16 halfs (could be split across rows if a_col + 16 > 32)
        // Since 16 <= 32 and a_col is multiple of 16 (lid*16%32 = 0 or 16), no row split
        half8 v0 = vload8(0, src);
        half8 v1 = vload8(1, src);
        vstore8(v0, 0, dst);
        vstore8(v1, 1, dst);
    }
    barrier(CLK_LOCAL_MEM_FENCE);
    ...
    // Main K-loop: step by TILE_K=32
    for (int k = 0; k < K; k += TILE_K) {
        // Prefetch next A tile into other buffer (if not last iteration)
        int next_k = k + TILE_K;
        int next_buf = 1 - buf;

        if (next_k < K) {
            __global const half* A_ptr = A + base_m * K + next_k;
            int linear_base = lid * A_LOADS_PER_WI;
            int a_row = linear_base / TILE_K;
            int a_col = linear_base % TILE_K;

            __global const half* src = A_ptr + a_row * K + a_col;
            __local half* dst = slm_A + next_buf * (TILE_M * SLM_A_STRIDE) + a_row * SLM_A_STRIDE + a_col;
        ...
        // Compute: read A from current SLM buffer, B from global
        __local half* cur_A = slm_A + buf * (TILE_M * SLM_A_STRIDE);
        __global const half* B_base = B + k * N + base_n + sg_n_offset;

        // K-step split into two k16 sub-steps
        #pragma unroll
        for (int kk = 0; kk < TILE_K; kk += 16) {
            // Load B: 16 k-rows x 16 cols for this subgroup
            // B is row-major: B[k+kk+i][base_n + sg_n_offset + j]
            // For DPAS b operand (int8 per WI): need 16k x 16n packed as VNNI
            // int8 b: each int holds 2 halfs (k-pair), 8 ints = 16 k-values for one column
            // Layout: b[i] = pack(B[k+2i][col], B[k+2i+1][col]) where col = sg_lid
            int8 b_val;
            __global const half* B_ptr = B_base + kk * N;

            #pragma unroll
            for (int i = 0; i < 8; i++) {
                half b0 = B_ptr[(2*i) * N + sg_lid];
                half b1 = B_ptr[(2*i+1) * N + sg_lid];
                b_val[i] = as_int((half2)(b0, b1));
            }

            // Load A from SLM and compute DPAS for 4 row-blocks
            // A operand (short8 per WI): 8 rows x 16 k-values
            // For row block r (rows r*8..r*8+7):
            // a[i] = pack(A[r*8+row][kk+2*sg_lid], A[r*8+row][kk+2*sg_lid+1])
            // where row = i (0..7), packed as short = 2 halfs in k-dimension

            #pragma unroll
            for (int r = 0; r < 4; r++) {
                short8 a_val;
                __local half* A_row_base = cur_A + (r * 8) * SLM_A_STRIDE + kk;

                #pragma unroll
                for (int i = 0; i < 8; i++) {
                    half h0 = A_row_base[i * SLM_A_STRIDE + 2 * sg_lid];
                    half h1 = A_row_base[i * SLM_A_STRIDE + 2 * sg_lid + 1];
                    a_val[i] = as_short((half2)(h0, h1));
                }

                if (r == 0) acc0 = intel_sub_group_f16_f16_matrix_mad_k16(a_val, b_val, acc0);
                else if (r == 1) acc1 = intel_sub_group_f16_f16_matrix_mad_k16(a_val, b_val, acc1);
                else if (r == 2) acc2 = intel_sub_group_f16_f16_matrix_mad_k16(a_val, b_val, acc2);
                else acc3 = intel_sub_group_f16_f16_matrix_mad_k16(a_val, b_val, acc3);
            }
        }
    ...
    // Write C: each SG writes 32 rows x 16 cols
    // DPAS output float8: acc[r][i] = C[base_m + r*8 + i][base_n + sg_n_offset + sg_lid]
    int c_col = base_n + sg_n_offset + sg_lid;

    if (c_col < N) {
        #pragma unroll
        for (int i = 0; i < 8; i++) {
            int row = base_m + i;
            if (row < M) C[row * N + c_col] = convert_half(acc0[i]);
        }
        #pragma unroll
        for (int i = 0; i < 8; i++) {
            int row = base_m + 8 + i;
            if (row < M) C[row * N + c_col] = convert_half(acc1[i]);
        }
        #pragma unroll
        for (int i = 0; i < 8; i++) {
            int row = base_m + 16 + i;
            if (row < M) C[row * N + c_col] = convert_half(acc2[i]);
        }
        #pragma unroll
        for (int i = 0; i < 8; i++) {
            int row = base_m + 24 + i;
            if (row < M) C[row * N + c_col] = convert_half(acc3[i]);
        }
    }
}
```
...
======================= 4 passed, 1 deselected in 1.07s ========================
