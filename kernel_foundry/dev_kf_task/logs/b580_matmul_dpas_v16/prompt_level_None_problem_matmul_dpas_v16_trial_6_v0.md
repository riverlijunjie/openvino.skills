    ...
    const int N)
{
    ...
    const int a_row = lid >> 1;            // 0..31
    const int a_col_base = (lid & 1) << 4; // 0 or 16
    const int a_row_stride = K;            // row stride in global A

    // Precompute A base pointer for this WI's cooperative load responsibility
    __global const half* a_load_base = A + (base_row + a_row) * K + a_col_base;

    // Preload first A tile (k=0) into buffer 0
    {
        __local half* a_dst = slm_A + a_row * SLM_STRIDE + a_col_base;
        half8 v0 = vload8(0, a_load_base);
        half8 v1 = vload8(1, a_load_base);
        vstore8(v0, 0, a_dst);
        vstore8(v1, 0, a_dst + 8);
    }
    barrier(CLK_LOCAL_MEM_FENCE);
    ...
    __global const half* b_col_base = B + sg_col + sg_lid;
    ...
    for (int k = 0; k < 2048; k += 64) {
        ...
        {
            __local const half* slm_base = slm_A;
            __global const half* b_ptr = b_col_base + k * N;

            // Load A from SLM k-step 0 (cols 0..15) and B simultaneously
            short8 a00, a10, a20, a30;
            int8 b0;
            {
                __local const half* ap = slm_base + sg_lid;
            ...
            // Start loading A(k+32) into buffer 1 while DPAS is in flight
            __global const half* a_next = a_load_base + (k + 32);
            __local half* a_dst = slm_A + BUF_SIZE + a_row * SLM_STRIDE + a_col_base;
            half8 pf0 = vload8(0, a_next);
            half8 pf1 = vload8(1, a_next);

            acc2 = intel_sub_group_f16_f16_matrix_mad_k16(a20, b0, acc2);
            acc3 = intel_sub_group_f16_f16_matrix_mad_k16(a30, b0, acc3);

            // Load A from SLM k-step 1 (cols 16..31) and B k-step 1
            short8 a01, a11, a21, a31;
            int8 b1;
            {
                __local const half* ap = slm_base + 16 + sg_lid;
            ...
            // Write prefetched A to SLM (after reads from buffer 0 are done)
            vstore8(pf0, 0, a_dst);
            vstore8(pf1, 0, a_dst + 8);
        ...
        // ===== Phase 1: Compute from buffer 1, prefetch A(k+64) into buffer 0 =====
        {
            ...
            __global const half* b_ptr = b_col_base + (k+32) * N;

            // Load A from SLM k-step 0 and B
            short8 a00, a10, a20, a30;
            int8 b0;
            {
                __local const half* ap = slm_base + sg_lid;
            ...
            // Start loading A(k+64) into buffer 0 while DPAS in flight
            // On last iteration (k=1984), k+64=2048=K, loads OOB but we never use it
            __global const half* a_next = a_load_base + (k + 64);
            __local half* a_dst = slm_A + a_row * SLM_STRIDE + a_col_base;
            half8 pf0 = vload8(0, a_next);
            half8 pf1 = vload8(1, a_next);
            ...
            // Load A from SLM k-step 1 and B k-step 1
            short8 a01, a11, a21, a31;
            int8 b1;
            {
                __local const half* ap = slm_base + 16 + sg_lid;
...
self = <task.TestMatmulOCL object at 0x7d51fe3e7b30>
kernel = <function initialize_matmul_kernel.<locals>.run at 0x7d51986e51c0>
ocl_queue = <pyopencl._cl.CommandQueue object at 0x7d51a6536020>, _run = 0
...
E       assert False
E        +  where False = <function allclose at 0x7d51fed58730>(array([[  6.1679688, -69.375    ,   8.421875 , ..., -36.9375   ,\n        -37.5      ,  40.8125   ],\n       [-63.       ,   6.4492188,   3.9394531, ...,  31.0625   ,\n         35.34375  ,  29.65625  ],\n       [ -9.6484375, -32.46875  ,  -7.1132812, ...,  17.671875 ,\n         31.5625   ,  -1.0107422],\n       ...,\n       [-40.5      , -43.71875  ,  76.4375   , ..., 109.4375   ,\n          7.3554688,   6.921875 ],\n       [  9.9140625,  19.078125 ,  -3.8613281, ..., -23.703125 ,\n         37.71875  ,  29.0625   ],\n       [ 52.28125  , -31.640625 ,  33.3125   , ..., -24.28125  ,\n        -10.8515625,  34.40625  ]], shape=(2048, 2048), dtype=float32), array([[-12.434087 , -85.22102  ,  45.86866  , ..., -67.074715 ,\n        -64.52674  ,  37.798523 ],\n       [-82.95244  ,  28.332115 ,   4.3084497, ...,  37.17192  ,\n         48.87541  ,  55.1519   ],\n       [ 31.096529 , -51.77693  ,  -9.3054905, ...,   8.124319 ,\n         61.21928  ,   4.7092314],\n       ...,\n       [-65.29967  , -27.73106  ,  74.195465 , ..., 122.09403  ,\n        -41.569603 ,  10.711429 ],\n       [ 44.6838   ,   2.3142765,  22.61605  , ..., -35.807106 ,\n         42.793472 ,  52.60636  ],\n       [ 50.399834 ,  -3.015791 ,  21.545517 , ..., -21.399685 ,\n        -36.035267 ,  49.01544  ]], shape=(2048, 2048), dtype=float32), rtol=0.02, atol=0.02)
E        +    where <function allclose at 0x7d51fed58730> = np.allclose

task.py:244: AssertionError
________________ TestMatmulOCL.test_correctness_wrt_pytorch[1] _________________

self = <task.TestMatmulOCL object at 0x7d51a64dac00>
kernel = <function initialize_matmul_kernel.<locals>.run at 0x7d51986e51c0>
ocl_queue = <pyopencl._cl.CommandQueue object at 0x7d51a6536020>, _run = 1
...
E       AssertionError: matmul result mismatch vs pytorch/numpy
E       assert False
E        +  where False = <function allclose at 0x7d51fed58730>(array([[ 58.25      ,  -0.97265625,  56.9375    , ...,   6.3359375 ,\n        -71.25      , -61.0625    ],\n       [ 52.84375   ,  27.375     ,  -5.2382812 , ..., -64.75      ,\n         13.390625  ,  21.28125   ],\n       [  1.4306641 ,  25.09375   , -28.640625  , ...,  41.75      ,\n        -39.84375   ,  11.015625  ],\n       ...,\n       [-51.15625   ,   4.7226562 ,  -8.9375    , ...,  23.515625  ,\n         24.296875  , -15.4375    ],\n       [-57.125     ,  19.8125    ,  31.125     , ...,  -1.4296875 ,\n         18.953125  ,  30.65625   ],\n       [-53.625     ,  52.40625   ,  12.796875  , ..., -12.1328125 ,\n        -47.5625    ,  46.59375   ]], shape=(2048, 2048), dtype=float32), array([[  63.220627 ,   13.663691 ,   62.708282 , ...,   26.950535 ,\n        -100.14888  ,  -76.10468  ],\n       [  30.338015 ,   -9.576593 ,  -15.848044 , ...,  -86.66203  ,\n           6.3691177,    9.569207 ],\n       [  -9.825886 ,   26.83852  ,  -39.88768  , ...,   94.32298  ,\n         -40.437588 ,   13.349518 ],\n       ...,\n       [ -50.946926 ,  -10.7210655,  -18.652342 , ...,   -4.0612535,\n         -29.112085 ,   -2.7683525],\n       [ -41.46417  ,   -5.034666 ,   35.500336 , ...,    3.5289268,\n          14.26104  ,   55.58531  ],\n       [ -33.896618 ,   51.45737  ,   13.108513 , ...,   11.92079  ,\n         -64.022385 ,   63.048595 ]], shape=(2048, 2048), dtype=float32), rtol=0.02, atol=0.02)
E        +    where <function allclose at 0x7d51fed58730> = np.allclose

task.py:244: AssertionError
_______________ TestMatmulOCL.test_correctness_wrt_reference[0] ________________

self = <task.TestMatmulOCL object at 0x7d51a6523560>
kernel = <function initialize_matmul_kernel.<locals>.run at 0x7d51986e51c0>
ocl_queue = <pyopencl._cl.CommandQueue object at 0x7d51a6536020>, _run = 0
...
E       AssertionError: matmul result mismatch vs reference
E       assert False
E        +  where False = <function allclose at 0x7d51fed58730>(array([[  15.2265625,  -17.15625  ,  -16.953125 , ...,  -24.9375   ,\n         -16.234375 , -100.3125   ],\n       [ -22.984375 ,  -25.59375  ,    1.4238281, ...,   40.6875   ,\n          23.28125  ,  -48.28125  ],\n       [  17.28125  ,   -4.8984375,  -48.40625  , ...,  -23.9375   ,\n         -27.375    ,   33.40625  ],\n       ...,\n       [ -24.875    ,  -72.375    ,  -53.       , ...,   23.546875 ,\n           1.6230469,  -36.3125   ],\n       [  21.875    ,   69.       ,  -27.453125 , ...,    7.3710938,\n         -46.71875  ,   12.9375   ],\n       [ -23.09375  ,  -48.625    ,   81.75     , ...,  -60.375    ,\n           9.96875  ,   23.53125  ]], shape=(2048, 2048), dtype=float32), array([[  46.4375   ,  -21.75     ,  -36.71875  , ...,  -39.96875  ,\n           3.125    , -147.25     ],\n       [ -13.859375 ,  -17.453125 ,   17.375    , ...,   57.90625  ,\n          25.859375 ,  -68.0625   ],\n       [  34.1875   ,   -9.421875 ,  -49.       , ...,  -46.71875  ,\n         -64.5625   ,   37.6875   ],\n       ...,\n       [   3.3261719,  -60.03125  ,  -64.       , ...,   18.328125 ,\n         -17.234375 ,  -30.90625  ],\n       [  -0.9145508,   97.25     ,  -18.109375 , ...,   35.75     ,\n         -45.0625   ,   21.78125  ],\n       [ -36.5      ,  -63.8125   ,   68.125    , ...,  -48.09375  ,\n         -29.515625 ,   11.1640625]], shape=(2048, 2048), dtype=float32), rtol=0.02, atol=0.02)
E        +    where <function allclose at 0x7d51fed58730> = np.allclose
