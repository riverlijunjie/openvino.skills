...
Here is the last OCL kernel we tested (Result: Correctness score: 5 / 5 (compiles and correct), runtime in ms: 1.610):
```OCL
// GEMM: C[M,N] = A[M,K] * B[K,N], all half, f32 accumulation
// Launch: GWS = (N/64 * 64, M/32), LWS = (64, 1)
// 4 subgroups of 16 WIs. Tile: 32x64x32. A in SLM (double-buffered), B from global.
// K=2048, M=2048, N=2048 - all divisible by 64.
// Optimizations: 2x K-unroll, interleaved B loads with DPAS, padded SLM stride,
//                early A prefetch overlapping with compute, no branch in main loop.

#pragma OPENCL EXTENSION cl_intel_subgroups : enable
#pragma OPENCL EXTENSION cl_intel_subgroups_short : enable
#pragma OPENCL EXTENSION cl_intel_subgroup_matrix_multiply_accumulate : enable

// Padded SLM stride to reduce bank conflicts (34 halfs = 68 bytes)
// 32 banks * 4 bytes = 128 bytes per bank cycle
// Stride of 68 bytes means consecutive rows offset by 68%128=68 bytes = 17 banks
// This ensures no bank conflicts when 16 WIs read from consecutive rows at same col
    ...
    const int N)
{
    ...
    // Since K=2048 divides by 64, no remainder or bounds check needed
    for (int k = 0; k < K; k += 64) {
        ...
        // ===== Phase 0: Compute from buffer 0, load A(k+32) into buffer 1 =====
        {
            ...
            // --- K-step 0: k..k+15 ---
            // Load A from SLM (cols 0..15) and B simultaneously
            short8 a00, a10, a20, a30;
            int8 b0;
            {
                __local const half* ap = slm_base + sg_lid;
                ...
                // Interleave A reads and B reads for better scheduling
                a00.s0 = as_short(ap[0*SLM_STRIDE]);
                b0.s0 = as_int((half2)(bp[0], bp[N]));
                a00.s1 = as_short(ap[1*SLM_STRIDE]);
                b0.s1 = as_int((half2)(bp[2*N], bp[3*N]));
                a00.s2 = as_short(ap[2*SLM_STRIDE]);
                b0.s2 = as_int((half2)(bp[4*N], bp[5*N]));
                a00.s3 = as_short(ap[3*SLM_STRIDE]);
                b0.s3 = as_int((half2)(bp[6*N], bp[7*N]));
                a00.s4 = as_short(ap[4*SLM_STRIDE]);
                b0.s4 = as_int((half2)(bp[8*N], bp[9*N]));
                a00.s5 = as_short(ap[5*SLM_STRIDE]);
                b0.s5 = as_int((half2)(bp[10*N], bp[11*N]));
                a00.s6 = as_short(ap[6*SLM_STRIDE]);
                b0.s6 = as_int((half2)(bp[12*N], bp[13*N]));
                a00.s7 = as_short(ap[7*SLM_STRIDE]);
                b0.s7 = as_int((half2)(bp[14*N], bp[15*N]));
            ...
            // DPAS k-step 0 - interleave with B1 prefetch
            acc0 = intel_sub_group_f16_f16_matrix_mad_k16(a00, b0, acc0);
            acc1 = intel_sub_group_f16_f16_matrix_mad_k16(a10, b0, acc1);

            // Start loading B for k-step 1 while DPAS is in flight
            int8 b1;
            {
                __global const half* bp = b_base + 16*N;
                b1.s0 = as_int((half2)(bp[0], bp[N]));
            ...
            // DPAS k-step 1 - interleave with A prefetch for next tile
            acc0 = intel_sub_group_f16_f16_matrix_mad_k16(a01, b1, acc0);
            acc1 = intel_sub_group_f16_f16_matrix_mad_k16(a11, b1, acc1);
        ...
        // ===== Phase 1: Compute from buffer 1, prefetch A(k+64) into buffer 0 =====
        {
            ...
            // Load A(k+64) into buffer 0 - no bounds check needed since K=2048 divides by 64
            // On the last iteration (k=1984), k+64=2048=K, but we still write to SLM
            // (it won't be consumed). This avoids a branch.
            // Actually we need to guard against reading past A. Use select-style:
            // Since K=2048 and loop goes k=0,64,...,1984, k+64 can be 2048 which is OOB.
            // We'll just not load on the last iteration using a simple trick:
            // Load from a valid address (clamp) - the data won't be used anyway.
            {
                int next_k = k + 64;
                // Clamp to avoid OOB: if next_k >= K, load from k (already valid, won't be used)
                int load_k = (next_k < K) ? next_k : k;
                __global const half* a_src = A + (base_row + a_row) * K + load_k + a_col_base;
                __local half* a_dst = slm_A + a_row * SLM_STRIDE + a_col_base;
                half8 v0 = vload8(0, a_src);
                half8 v1 = vload8(1, a_src);
                vstore8(v0, 0, a_dst);
    ...
    // Write C: each WI writes 32 rows to its column
    // Use vstore where possible for better write coalescing
    __global half* c_ptr = C + base_row * N + sg_col + sg_lid;
