...
Here is the last OCL kernel we tested (Result: Correctness score: 5 / 5 (compiles and correct), runtime in ms: 1.550):
```OCL
// GEMM: C[M,N] = A[M,K] * B[K,N], all half, f32 accumulation
// Launch: GWS = (N/64 * 64, M/32), LWS = (64, 1)
// 4 subgroups of 16 WIs. Tile: 32x64x32. A in SLM (double-buffered), B from global.
...
// Padded stride to avoid SLM bank conflicts: 32 + 2 = 34
    ...
    const int N)
{
    ...
    // Accumulators: 32 rows x 16 cols per subgroup = 4 x float8
    float8 acc0 = 0.0f;
    float8 acc1 = 0.0f;
    float8 acc2 = 0.0f;
    float8 acc3 = 0.0f;
    ...
    // Macro to load A from SLM: 8 rows starting at slm_ptr + sg_lid
    #define LOAD_A_8ROWS(dst, slm_ptr) \
    { \
        __local const half* _ap = (slm_ptr) + sg_lid; \
        (dst).s0 = as_short(_ap[0*SLM_STRIDE]); \
        (dst).s1 = as_short(_ap[1*SLM_STRIDE]); \
        (dst).s2 = as_short(_ap[2*SLM_STRIDE]); \
        (dst).s3 = as_short(_ap[3*SLM_STRIDE]); \
        (dst).s4 = as_short(_ap[4*SLM_STRIDE]); \
        (dst).s5 = as_short(_ap[5*SLM_STRIDE]); \
        (dst).s6 = as_short(_ap[6*SLM_STRIDE]); \
        (dst).s7 = as_short(_ap[7*SLM_STRIDE]); \
    }

    // Macro to load B: 16 k-rows packed into int8
    #define LOAD_B_16ROWS(dst, bp) \
    { \
        __global const half* _bp = (bp); \
        (dst).s0 = as_int((half2)(_bp[0],    _bp[N])); \
        (dst).s1 = as_int((half2)(_bp[2*N],  _bp[3*N])); \
        (dst).s2 = as_int((half2)(_bp[4*N],  _bp[5*N])); \
        (dst).s3 = as_int((half2)(_bp[6*N],  _bp[7*N])); \
        (dst).s4 = as_int((half2)(_bp[8*N],  _bp[9*N])); \
        (dst).s5 = as_int((half2)(_bp[10*N], _bp[11*N])); \
        (dst).s6 = as_int((half2)(_bp[12*N], _bp[13*N])); \
        (dst).s7 = as_int((half2)(_bp[14*N], _bp[15*N])); \
    }

    // Main K-loop: step by 64 (2x unrolled), K=2048 divides evenly by 64
    // Loop processes k and k+32 per iteration, total 32 iterations
    for (int k = 0; k < K; k += 64) {

        // ===== Phase 0: Compute from buffer 0 =====
        {
            __local const half* slm_base = slm_A;
            __global const half* b_ptr = B + k * N + sg_col + sg_lid;

            // Load A k-step 0 (cols 0..15) and B k-step 0 simultaneously
            short8 a00, a10, a20, a30;
            int8 b0;
            LOAD_A_8ROWS(a00, slm_base);
            LOAD_A_8ROWS(a10, slm_base + 8*SLM_STRIDE);
            LOAD_B_16ROWS(b0, b_ptr);
            LOAD_A_8ROWS(a20, slm_base + 16*SLM_STRIDE);
            LOAD_A_8ROWS(a30, slm_base + 24*SLM_STRIDE);
            ...
            // Load A k-step 1 (cols 16..31) and B k-step 1
            short8 a01, a11, a21, a31;
            int8 b1;
            LOAD_A_8ROWS(a01, slm_base + 16);
            LOAD_A_8ROWS(a11, slm_base + 8*SLM_STRIDE + 16);
            LOAD_B_16ROWS(b1, b_ptr + 16*N);
            LOAD_A_8ROWS(a21, slm_base + 16*SLM_STRIDE + 16);
            LOAD_A_8ROWS(a31, slm_base + 24*SLM_STRIDE + 16);
        ...
        // ===== Phase 1: Compute from buffer 1 =====
        {
            ...
            // Load A k-step 1 and B k-step 1
            short8 a01, a11, a21, a31;
            int8 b1;
            LOAD_A_8ROWS(a01, slm_base + 16);
            LOAD_A_8ROWS(a11, slm_base + 8*SLM_STRIDE + 16);
            ...
            // Load next A (k+64) into buffer 0 - skip on last iteration
            if (k + 64 < K) {
                __global const half* a_src = A + (base_row + a_row) * K + (k + 64) + a_col_base;
                __local half* a_dst = slm_A + a_row * SLM_STRIDE + a_col_base;
                half8 v0 = vload8(0, a_src);
    ...
    #pragma unroll
    for (int i = 0; i < 8; i++) c_ptr[i * N] = convert_half(acc0[i]);
    c_ptr += 8 * N;
    #pragma unroll
    for (int i = 0; i < 8; i++) c_ptr[i * N] = convert_half(acc1[i]);
    c_ptr += 8 * N;
    #pragma unroll
    for (int i = 0; i < 8; i++) c_ptr[i * N] = convert_half(acc2[i]);
    c_ptr += 8 * N;
    #pragma unroll
    for (int i = 0; i < 8; i++) c_ptr[i * N] = convert_half(acc3[i]);
...
================== 4 passed, 1 deselected, 1 warning in 0.98s ==================
