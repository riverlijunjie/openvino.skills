...
// 4 subgroups of 16 WIs. Tile: 32x64x32. A in SLM (double-buffered), B from global.
// K=2048, M=2048, N=2048 - all divisible by 64.
// Optimization: SLM stride padded to 36 to reduce bank conflicts.
// K-loop unrolled 2x (step 64), no remainder handling needed.

#pragma OPENCL EXTENSION cl_intel_subgroups : enable
#pragma OPENCL EXTENSION cl_intel_subgroups_short : enable
#pragma OPENCL EXTENSION cl_intel_subgroup_matrix_multiply_accumulate : enable

// SLM_STRIDE = 36 to avoid bank conflicts (32 would alias every row to same banks)
    ...
    const int N)
{
    ...
    // SLM double buffer for A: 2 * 32 rows * 36 cols (padded) = 2304 halfs = 4608 bytes
    ...
    for (int k = 0; k < K; k += 64) {
        ...
        // ===== Phase 0: Compute from buffer 0, load A(k+32) into buffer 1 =====
        {
            ...
            // Load A from SLM k-step 0 (cols 0..15) - read before B to give B loads latency
            short8 a00, a10, a20, a30;
            {
                __local const half* ap = slm_base + sg_lid;
                a00.s0 = as_short(ap[0*SLM_STRIDE]); a00.s1 = as_short(ap[1*SLM_STRIDE]);
            ...
            // Load B k-step 0 (k..k+15): 16 rows, each WI reads its column
            int8 b0;
            {
            ...
            // Start loading A(k+32) into buffer 1 (overlapped with DPAS below)
            __global const half* a_next = A + (base_row + a_row) * K + (k + 32) + a_col_base;
            __local half* a_dst1 = slm_A + TILE_M * SLM_STRIDE + a_row * SLM_STRIDE + a_col_base;
            half8 av0 = vload8(0, a_next);
            half8 av1 = vload8(1, a_next);
            ...
            // Store A to SLM buffer 1
            vstore8(av0, 0, a_dst1);
            vstore8(av1, 0, a_dst1 + 8);
        }
        barrier(CLK_LOCAL_MEM_FENCE);
        ...
        // ===== Phase 1: Compute from buffer 1, load A(k+64) into buffer 0 =====
        {
            ...
            // Load A(k+64) into buffer 0 - start before DPAS
            // On last iteration (k=1984), k+64=2048=K, we skip (but read is harmless if we guard)
            half8 av0, av1;
            __local half* a_dst0 = slm_A + a_row * SLM_STRIDE + a_col_base;
            if (k + 64 < K) {
                __global const half* a_next = A + (base_row + a_row) * K + (k + 64) + a_col_base;
                av0 = vload8(0, a_next);
                av1 = vload8(1, a_next);
            }
            ...
            // Store A to SLM buffer 0
            if (k + 64 < K) {
                vstore8(av0, 0, a_dst0);
                vstore8(av1, 0, a_dst0 + 8);
            }
        }
        barrier(CLK_LOCAL_MEM_FENCE);
    }
