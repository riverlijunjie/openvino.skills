...
### Version 2 (Result: Correctness score: 5 / 5 (compiles and correct), runtime in ms: 1.380):
```OCL
// GEMM: C[M,N] = A[M,K] * B[K,N], all half, f32 accumulation
// Launch: GWS = (N/64 * 64, M/32), LWS = (64, 1)
// 4 subgroups of 16 WIs. Tile: 32x64x32. A in SLM (double-buffered), B from global.
    ...
    const int N)
{
    ...
    {
        __global const half* a_src = A + (base_row + a_load_row) * K + a_load_col;
        __local half* a_dst = slm_A + a_load_row * SLM_A_STRIDE + a_load_col;
        half16 v = vload16(0, a_src);
        vstore16(v, 0, a_dst);
    }
    barrier(CLK_LOCAL_MEM_FENCE);

    // Main K-loop: step by 64 (2x unrolled), K guaranteed divisible by 64
    for (int k = 0; k < K; k += 64) {
        // ===== Phase 0: Compute from buffer 0, load next A into buffer 1 =====
        {
            __local const half* slm_src = slm_A;
            __global const half* b_base = B + k * N + base_col + sg_col_offset + sg_lid;
            ...
            // Load A from SLM for k-step 0: each WI reads column sg_lid (one half per row)
            short8 a00, a10, a20, a30;
            {
                __local const half* abase = slm_src + sg_lid;
                a00.s0 = as_short(abase[0*SLM_A_STRIDE]);
                a00.s1 = as_short(abase[1*SLM_A_STRIDE]);
                a00.s2 = as_short(abase[2*SLM_A_STRIDE]);
                a00.s3 = as_short(abase[3*SLM_A_STRIDE]);
                a00.s4 = as_short(abase[4*SLM_A_STRIDE]);
                a00.s5 = as_short(abase[5*SLM_A_STRIDE]);
                a00.s6 = as_short(abase[6*SLM_A_STRIDE]);
                a00.s7 = as_short(abase[7*SLM_A_STRIDE]);

                abase += 8 * SLM_A_STRIDE;
                a10.s0 = as_short(abase[0*SLM_A_STRIDE]);
                a10.s1 = as_short(abase[1*SLM_A_STRIDE]);
                a10.s2 = as_short(abase[2*SLM_A_STRIDE]);
                a10.s3 = as_short(abase[3*SLM_A_STRIDE]);
                a10.s4 = as_short(abase[4*SLM_A_STRIDE]);
                a10.s5 = as_short(abase[5*SLM_A_STRIDE]);
                a10.s6 = as_short(abase[6*SLM_A_STRIDE]);
                a10.s7 = as_short(abase[7*SLM_A_STRIDE]);

                abase += 8 * SLM_A_STRIDE;
                a20.s0 = as_short(abase[0*SLM_A_STRIDE]);
                a20.s1 = as_short(abase[1*SLM_A_STRIDE]);
                a20.s2 = as_short(abase[2*SLM_A_STRIDE]);
                a20.s3 = as_short(abase[3*SLM_A_STRIDE]);
                a20.s4 = as_short(abase[4*SLM_A_STRIDE]);
                a20.s5 = as_short(abase[5*SLM_A_STRIDE]);
                a20.s6 = as_short(abase[6*SLM_A_STRIDE]);
                a20.s7 = as_short(abase[7*SLM_A_STRIDE]);

                abase += 8 * SLM_A_STRIDE;
                a30.s0 = as_short(abase[0*SLM_A_STRIDE]);
                a30.s1 = as_short(abase[1*SLM_A_STRIDE]);
                a30.s2 = as_short(abase[2*SLM_A_STRIDE]);
                a30.s3 = as_short(abase[3*SLM_A_STRIDE]);
                a30.s4 = as_short(abase[4*SLM_A_STRIDE]);
                a30.s5 = as_short(abase[5*SLM_A_STRIDE]);
                a30.s6 = as_short(abase[6*SLM_A_STRIDE]);
                a30.s7 = as_short(abase[7*SLM_A_STRIDE]);
            }
            ...
            {
                __local const half* abase = slm_src + 16 + sg_lid;
                a01.s0 = as_short(abase[0*SLM_A_STRIDE]);
                a01.s1 = as_short(abase[1*SLM_A_STRIDE]);
                a01.s2 = as_short(abase[2*SLM_A_STRIDE]);
                a01.s3 = as_short(abase[3*SLM_A_STRIDE]);
                a01.s4 = as_short(abase[4*SLM_A_STRIDE]);
                a01.s5 = as_short(abase[5*SLM_A_STRIDE]);
                a01.s6 = as_short(abase[6*SLM_A_STRIDE]);
                a01.s7 = as_short(abase[7*SLM_A_STRIDE]);

                abase += 8 * SLM_A_STRIDE;
                a11.s0 = as_short(abase[0*SLM_A_STRIDE]);
                a11.s1 = as_short(abase[1*SLM_A_STRIDE]);
                a11.s2 = as_short(abase[2*SLM_A_STRIDE]);
                a11.s3 = as_short(abase[3*SLM_A_STRIDE]);
                a11.s4 = as_short(abase[4*SLM_A_STRIDE]);
                a11.s5 = as_short(abase[5*SLM_A_STRIDE]);
                a11.s6 = as_short(abase[6*SLM_A_STRIDE]);
                a11.s7 = as_short(abase[7*SLM_A_STRIDE]);

                abase += 8 * SLM_A_STRIDE;
                a21.s0 = as_short(abase[0*SLM_A_STRIDE]);
                a21.s1 = as_short(abase[1*SLM_A_STRIDE]);
                a21.s2 = as_short(abase[2*SLM_A_STRIDE]);
                a21.s3 = as_short(abase[3*SLM_A_STRIDE]);
                a21.s4 = as_short(abase[4*SLM_A_STRIDE]);
                a21.s5 = as_short(abase[5*SLM_A_STRIDE]);
                a21.s6 = as_short(abase[6*SLM_A_STRIDE]);
                a21.s7 = as_short(abase[7*SLM_A_STRIDE]);

                abase += 8 * SLM_A_STRIDE;
                a31.s0 = as_short(abase[0*SLM_A_STRIDE]);
                a31.s1 = as_short(abase[1*SLM_A_STRIDE]);
                a31.s2 = as_short(abase[2*SLM_A_STRIDE]);
                a31.s3 = as_short(abase[3*SLM_A_STRIDE]);
                a31.s4 = as_short(abase[4*SLM_A_STRIDE]);
                a31.s5 = as_short(abase[5*SLM_A_STRIDE]);
                a31.s6 = as_short(abase[6*SLM_A_STRIDE]);
                a31.s7 = as_short(abase[7*SLM_A_STRIDE]);
            }
            ...
            // Load next A (k+32) into buffer 1
            {
                __global const half* a_src = A + (base_row + a_load_row) * K + (k + 32) + a_load_col;
                __local half* a_dst = slm_A + TILE_M * SLM_A_STRIDE + a_load_row * SLM_A_STRIDE + a_load_col;
                half16 v = vload16(0, a_src);
                vstore16(v, 0, a_dst);
            }
        }
        ...
        {
            __local const half* slm_src = slm_A + TILE_M * SLM_A_STRIDE;
            __global const half* b_base = B + (k + 32) * N + base_col + sg_col_offset + sg_lid;

            // Load B for k-step 0 (k+32..k+47)
            int8 b0;
            {
                __global const half* bp = b_base;
                b0.s0 = as_int((half2)(bp[0], bp[N]));
            ...
            // Load next A (k+64) into buffer 0 - always valid since K%64==0 and loop condition handles it
            if (k + 64 < K) {
                __global const half* a_src = A + (base_row + a_load_row) * K + (k + 64) + a_load_col;
                __local half* a_dst = slm_A + a_load_row * SLM_A_STRIDE + a_load_col;
                half16 v = vload16(0, a_src);
                vstore16(v, 0, a_dst);
            }
    ...
    // Write C: each WI writes 32 values (one per row) to its column
    const int out_col = base_col + sg_col_offset + sg_lid;
    __global half* c_ptr = C + base_row * N + out_col;
...
Here is the last OCL kernel we tested (Result: Correctness score: 5 / 5 (compiles and correct), runtime in ms: 1.300):
```OCL
// GEMM: C[M,N] = A[M,K] * B[K,N], all half, f32 accumulation
// Launch: GWS = (N/64 * 64, M/32), LWS = (64, 1)
// 4 subgroups of 16 WIs. Tile: 32x64x32. A in SLM (double-buffered), B from global.
...
================== 4 passed, 1 deselected, 1 warning in 0.91s ==================
