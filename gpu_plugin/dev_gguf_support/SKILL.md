...
2. **Local repos**:
    - OpenVINO repo: /home/ov2022/workspace/openvino
    - OpenVINO GenAI repo: /home/ov2022/workspace/openvino.genai

3. **Implementation**:
    - Implement the GGUF support feature in the local openvino and openvino.genai repos.
...
4. **Remote build and validation**:
    ...
    - Remote openvino genai directory: /mnt/river/moe/openvino.genai
    - Remote copy modified files to remote machine's openvino and openvino.genai directory.
    - Remote build command:
          openvino: cd /mnt/river/moe/openvino && source build_release.sh
          openvino.genai: cd /mnt/river/moe/openvino.genai && source build.sh
    - Remote unit test command: cd /mnt/river/moe/openvino/bin/intel64/Release && ./ov_gpu_unit_tests --gtest_filter=*gguf*
...
6. **Profile performance and optimize OpenCL kernel**:
    ...
    - Optimize the bottleneck OpenCL kernel to improve performance.
        ...
        - apply dev_kf_distill skill to optimize the kernel and test should run on remote machine, make sure the optimized kernel can be correctly executed and reach to 80% roofline performance based on the profiling results.
