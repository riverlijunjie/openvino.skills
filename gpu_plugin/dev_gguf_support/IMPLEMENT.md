...
- ⚠️ **性能**：当前 GEMV 内核 `local={1,1,1}`、未做 SLM/subgroup 优化，是**正确性优先**实现；8B 上
  ~30s/token，长文本会很慢（属已知优化点，见 §5；不影响正确性）。
...
- **`M > 阈值`（prefill / 长 prompt）**：`execute_transcode_plus_onednn_woq()`：
  1. **scratchpad** 经 `get_internal_buffer_descs` 声明两块内部 buffer（框架按形状自动分配/复用，
     大小只随静态 K,N，不随 M）：低比特权重 `layout([N,K], i4|i8)` + `f16` scale `layout([K/group, N])`。
