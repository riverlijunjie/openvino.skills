"""
A/B: reduce SLM usage via SLM_JBLK to raise thread occupancy (user request #1).
SLM = SLM_SLOTS(2) * SLM_JBLK * 2 * 512.  JBLK8=16KB, JBLK4=8KB, JBLK2=4KB,
JBLK1=2KB.  Smaller SLM may lift the occupancy cliff (the pad A/B showed 16->17KB
already hurts), at the cost of more barriers (SLM_JPS = 8/JBLK phases/K-block).
...
CONFIGS = [
    ("jblk8 16KB", ""),
    ("jblk4 8KB",  "-DSLM_JBLK=4"),
    ("jblk2 4KB",  "-DSLM_JBLK=2"),
    ("jblk1 2KB",  "-DSLM_JBLK=1"),
]
...
def main():
    ...
    hdr = f"{'shape':<26}" + "".join(f"{l+' ms':>15}" for l in labels)
    hdr += "".join(f"{'x'+labels[i].split()[0]:>10}" for i in range(1, len(labels))) + "  ok"
    ...
    for quant, K, N, tl in shapes:
        ...
        row = f"{quant+' K'+str(K)+' N'+str(N)+' t'+str(tl):<26}" + "".join(f"{v:>15.3f}" for v in mn)
