...
Correctness + benchmark + roofline harness for dense GEMV CM (C-for-Metal)
kernels:
    gemv_q4k_sg.cm : gemv_q4k_sg
    gemv_q5k_sg.cm : gemv_q5k_sg
    gemv_q6k_sg.cm : gemv_q6k_sg

Converted from the SYCL ESIMD kernels in sycl_gguf_kernel/gemv_q{4,5,6}k_L1*.cpp.
Unlike the OpenCL sub-group port (see /mnt/river/ovmx/ocl_gguf_kernel/), these
CM kernels have NO sub-group/lane dimension: ONE CM thread computes all
OPG=16 output rows of a row-group directly via vector<float,16> SIMD
arithmetic. The on-device weight layout (SG-transposed) is unchanged from the
OpenCL port, since it already stores 16 contiguous per-lane words -- exactly
what CM's cm_ptr_load<T,16> wants (no shuffle needed). Input is fp16; output
is fp32.

Dispatch (CM): global (threads) = (output_len/OPG, KSPLIT, token_len)
               local  (threads) = (1, KSPLIT, 1)
CM kernels are built with the '-cmc' OpenCL build option (Intel NEO's CM
front-end); pyopencl Buffers bind directly to CM `T* [[type("svmptr_t")]]`
kernel parameters exactly like plain OpenCL buffers.

Typical shapes (single decoder layer):
    token_len=1,  K=4096, N=4096   (dense attention/FFN)
    token_len=1,  K=2048, N=2048   (smaller model)

Usage:
    python test_dense_gemv.py [--device 0] [--iters 50] [--warmup 5]
...
OPG = 16  # output rows per row-group (fixed by the weight packing)
...
# ---------------------------------------------------------------------------
# Inline cache-flush kernel, written in CM (built with '-cmc' like the rest
# of this file's kernels) so the whole test harness stays OpenCL-C-free.
# ---------------------------------------------------------------------------
_FLUSH_SRC = r"""
#include <cm/cm.h>
#define FLUSH_VEC 16
...
class HW:
    ...
    def roofline(self, flops, bytes_moved, seconds):
        ...
        # CM kernels use FP16 operands/arithmetic.  BMG peak FP16 throughput
        # is 116 TFLOPS = 116000 GFLOPS.
...
class Gpu:
    ...
    def setup_flush(self, mib=256):
        ...
        prog = self._build_src(_FLUSH_SRC, "_flush", build_opts="-cmc")
        self._flush_krn = cl.Kernel(prog, "cache_flush")
        self.queue.finish()
...
# ---------------------------------------------------------------------------
# Shuffle: GGUF raw -> flat separated-section layout (matches SYCL kernel)
# ---------------------------------------------------------------------------
def shuffle_q4k(raw, K, N):
    ...
    rw[:, :, 0:32] = low; rw[:, :, 32:64] = high
    rw = rw.reshape(NB, 256)
...
def shuffle_q5k(raw, K, N):
    ...
    d_b, dm_b = blk[:, 0:2], blk[:, 2:4]
    sc = blk[:, 4:16]; qh_r = blk[:, 16:48]; qs_r = blk[:, 48:176].reshape(NB, 4, 32)
    g_arr = np.arange(8, dtype=np.uint8)
    rh = ((qh_r[:, None, :] >> g_arr[None, :, None]) & 1).astype(np.uint8)
    low = qs_r & 0x0F; high = (qs_r >> 4) & 0x0F
    rw = np.empty((NB, 8, 32), dtype=np.uint8)
    rw[:, 0::2, :] = low; rw[:, 1::2, :] = high
    (sc0,sc1,sc2,sc3,sc4,sc5,sc6,sc7,sc8,sc9,sc10,sc11) = (sc[:,i] for i in range(12))
    rs = np.empty((NB, 8), dtype=np.uint8); rm = np.empty((NB, 8), dtype=np.uint8)
    rs[:,0]=sc0&63; rs[:,1]=sc1&63; rs[:,2]=sc2&63; rs[:,3]=sc3&63
    rs[:,4]=(sc8&0x0F)|((sc0>>2)&0x30); rs[:,5]=(sc9&0x0F)|((sc1>>2)&0x30)
    ...
    for s in range(8):
        opqh |= (rh[:, :, s*4:s*4+4] << s).astype(np.uint8)
...
def deq_q4k(pqs, psl, K, N):
    ...
    j_arr = np.arange(8, dtype=np.uint32)
    sq = ((sli[:,None]>>(j_arr*4))&0xF)|(((shi[:,None]>>(j_arr*2))&3)<<4)
    mq = ((mli[:,None]>>(j_arr*4))&0xF)|(((mhi[:,None]>>(j_arr*2))&3)<<4)
...
def deq_q5k(pqs, pqh, psl, K, N):
    ...
    w0 = (qs_b&0x0F).astype(np.float32)+lo_b*16.0
    w1 = ((qs_b>>4)&0x0F).astype(np.float32)+hi_b*16.0
    W = np.empty((NB, 8, 32), dtype=np.float32)
...
def deq_q6k(pql, pqh, ps_u8, pd_buf, K, N):
    ...
    w0 = (ql_b&0x0F).astype(np.float32)
    w0 = w0 + (((hlo[:,:,None]>>SH[None,None,:])&3).astype(np.float32)*16.0)
    w0 = (w0-32.0)*clo[:,:,None]
    w1 = ((ql_b>>4)&0x0F).astype(np.float32)
    w1 = w1 + (((hhi[:,:,None]>>SH[None,None,:])&3).astype(np.float32)*16.0)
    w1 = (w1-32.0)*chi[:,:,None]
    W = np.empty((NB, 8, 32), dtype=np.float32)
    W[:,:,0:16]=w0; W[:,:,16:32]=w1
    return W.reshape(N, nprow, 256).reshape(N, K)


# ---------------------------------------------------------------------------
# SG packing helpers (flat separated-section -> SG transposed layout)
# ---------------------------------------------------------------------------
...
def pack_qk_pqs_sg(pqs, K, N, opg=OPG):
    ...
    src = pqs.reshape(nrg, opg, nbpr, 8, 4, 4)
...
def pack_q5k_pqh_sg(pqh, K, N, opg=OPG):
    nbpr = K // 256; nrg = N // opg
    src = pqh.reshape(nrg, opg, nbpr, 8, 4)
    return _ascon(src.transpose(0, 2, 3, 1, 4)).reshape(-1).astype(np.uint8)


def pack_q6k_pqh_sg(pqh, K, N, opg=OPG):
    nbpr = K // 256; nrg = N // opg
    src = pqh.reshape(nrg, opg, nbpr, 8, 2, 4)
    return _ascon(src.transpose(0, 2, 3, 4, 1, 5)).reshape(-1).astype(np.uint8)


def pack_q6k_ps_sg(ps, K, N, opg=OPG):
    nbpr = K // 256; nrg = N // opg
    src = ps.reshape(nrg, opg, nbpr, 16)
    return _ascon(src.transpose(0, 2, 3, 1)).reshape(-1).astype(np.uint8)


def pack_q6k_pd_sg(pd, K, N, opg=OPG):
    nbpr = K // 256; nrg = N // opg
    src = pd.reshape(nrg, opg, nbpr, 2)
    return _ascon(src.transpose(0, 2, 1, 3)).reshape(-1).astype(np.uint8)


# ---------------------------------------------------------------------------
# Build packed weight buffers
# ---------------------------------------------------------------------------
def build_q4k_weight(K, N, seed=1234):
    nbpr = N * K // 256
    raw = gen_q4k_blocks(nbpr, seed)
    pqs, psl = shuffle_q4k(raw, K, N)
    ref_W = deq_q4k(pqs, psl, K, N)
    pqs_T = pack_qk_pqs_sg(pqs, K, N)
    psl_T = pack_qk_psl_sg(psl, K, N)
    flat = np.concatenate([pqs_T, psl_T])
    weight_bytes = nbpr * 144
    return flat, ref_W, weight_bytes


def build_q5k_weight(K, N, seed=1234):
    nbpr = N * K // 256
    raw = gen_q5k_blocks(nbpr, seed)
    pqs, pqh, psl = shuffle_q5k(raw, K, N)
    ref_W = deq_q5k(pqs, pqh, psl, K, N)
    pqs_T = pack_qk_pqs_sg(pqs, K, N)
    pqh_T = pack_q5k_pqh_sg(pqh, K, N)
    psl_T = pack_qk_psl_sg(psl, K, N)
    flat = np.concatenate([pqs_T, pqh_T, psl_T])
    weight_bytes = nbpr * 176
    return flat, ref_W, weight_bytes


def build_q6k_weight(K, N, seed=1234):
    nbpr = N * K // 256
    raw = gen_q6k_blocks(nbpr, seed)
    pql, pqh, ps, pd = shuffle_q6k(raw, K, N)
    ref_W = deq_q6k(pql, pqh, ps, pd, K, N)
    pql_T = pack_qk_pqs_sg(pql, K, N)       # pql uses same pqs packing
    pqh_T = pack_q6k_pqh_sg(pqh, K, N)
    ps_T  = pack_q6k_ps_sg(ps, K, N)
    pd_T  = pack_q6k_pd_sg(pd, K, N)
    flat = np.concatenate([pql_T, pqh_T, ps_T, pd_T])
    weight_bytes = nbpr * 210
    return flat, ref_W, weight_bytes
...
def choose_ksplit(output_len, K, token_len, target=2048, maxk=8):
    base_groups = (output_len // OPG) * token_len
    if base_groups >= 512:
        return 1
    ks = max(1, round(target / max(base_groups, 1)))
    ks = min(ks, K // 256, maxk)
    return _pow2_floor(ks)


# ===========================================================================
# Test runner: dense GEMV (single weight matrix, no gate/up fusion)
# ===========================================================================
def test_gemv(gpu, hw, quant,
              K=4096, N=4096, token_len=1,
              seed=42, verbose=False,
              iters=50, warmup=5, do_bench=True):
    """
    Test gemv_q{4,5,6}k_sg CM kernel.
    Kernel: y = W * x  (W: [N,K] quantized, x: [token_len, K] fp16)
    Output: [token_len, N] fp32
    """
    label = f"gemv_{quant}_sg  N={N} K={K} token_len={token_len}"
    print(f"\n  [{label}]")
    assert N % OPG == 0, "N must be a multiple of OPG=16"
    assert K % 256 == 0, "K must be a multiple of 256"

    if quant == "q4k":
        flat, ref_W, wbytes = build_q4k_weight(K, N, seed)
        cm_file = "gemv_q4k_sg.cm"
        kernel_name = "gemv_q4k_sg"
    elif quant == "q5k":
        flat, ref_W, wbytes = build_q5k_weight(K, N, seed)
        cm_file = "gemv_q5k_sg.cm"
        kernel_name = "gemv_q5k_sg"
    elif quant == "q6k":
        flat, ref_W, wbytes = build_q6k_weight(K, N, seed)
        cm_file = "gemv_q6k_sg.cm"
        kernel_name = "gemv_q6k_sg"
    else:
        raise ValueError(f"Unknown quant: {quant}")

    rng = np.random.default_rng(seed + 1)
    x_fp16 = rng.standard_normal((token_len, K)).astype(np.float16)
    x_f32 = x_fp16.astype(np.float32)

    # Reference: numpy matmul (fp64 for precision)
    ref = (ref_W.astype(np.float64) @ x_f32.T).astype(np.float32).T  # [token_len, N]

    # Build & run GPU kernel (CM front-end: '-cmc')
    ksplit = choose_ksplit(N, K, token_len)
    opts = f"-cmc -DKSPLIT={ksplit}"
    prog = gpu.build(cl_src(cm_file), opts)
    krn = cl.Kernel(prog, kernel_name)

    x_b = _cbuf(gpu.ctx, x_fp16)
    w_b = _cbuf(gpu.ctx, flat)
    ...
    gsize = (N // OPG, ksplit, token_len)
    lsize = (1, ksplit, 1)
    print(f"    KSPLIT={ksplit}  threads={(N//OPG)*ksplit*token_len}  gsize={gsize}  lsize={lsize}")
    ...
    if do_bench:
        ...
        moved = wbytes + token_len * K * 2 + token_len * N * 4
...
def main():
    ...
    SEP = "=" * 80
    print(SEP)
    print("  Dense GEMV CM kernels: gemv_q4k_sg / gemv_q5k_sg / gemv_q6k_sg")
    print(SEP)

    results = []
    # Typical LLM shapes: small (Qwen3-8B) and large (dense 35B equivalent)
    shapes = [
        ("q4k", 2048, 2048, 1),
        ("q4k", 4096, 4096, 1),
        ("q4k", 4096, 4096, 4),
    ...
    for quant, K, N, token_len in shapes:
        ok, timing, rl = test_gemv(gpu, hw, quant, K=K, N=N,
    ...
    for quant, K, N, token_len, ok in results:
        ...
        print(f"    [{status}] gemv_{quant}_sg  N={N} K={K} token_len={token_len}")
