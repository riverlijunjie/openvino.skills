...
REM Llama-3.1-8B-Instruct (dense decoder) roofline sweep on PTL (Arc B390 iGPU).
REM NL=32 H=4096 I=14336 NH=32 NKV=8 HD=128 vocab=128256
REM QKV concat = (NH+2*NKV)*HD = 6144;  O = NH*HD x H = 4096x4096
REM Quick-validation grid: S/kv in {1024, 2048} only.
...
set LOGS=D:\river\moe\roofline_results\llama3_1_8b_instruct\ptl
...
call :do fc_gate_decode_M1            "%BUILD%\fc_bench.exe" 1     4096  14336  128 2500 150 8 u4 64
call :do fc_up_decode_M1              "%BUILD%\fc_bench.exe" 1     4096  14336  128 2500 150 8 u4 64
call :do fc_down_decode_M1            "%BUILD%\fc_bench.exe" 1    14336   4096  128 2500 150 8 u4 64
call :do lm_head_decode_M1            "%BUILD%\fc_bench.exe" 1     4096 128256  128  500  50 8 u8 64
...
for %%S in (1024 2048) do (
  ...
  call :do fc_gate_prefill_S%%S       "%BUILD%\fc_bench.exe" %%S  4096  14336  128 20 5 4 u4 64
  call :do fc_up_prefill_S%%S         "%BUILD%\fc_bench.exe" %%S  4096  14336  128 20 5 4 u4 64
  call :do fc_down_prefill_S%%S       "%BUILD%\fc_bench.exe" %%S 14336   4096  128 20 5 4 u4 64
)
...
REM ---------- small ops (Llama-3.1-8B shapes; no q_norm/k_norm) ----------
...
call :do so_swish_decode              "%BUILD%\small_ops_bench.exe" swish     1 14336   --iters 30000 --warmup 300 --bufs 8
call :do so_multiply_decode           "%BUILD%\small_ops_bench.exe" multiply  1 14336   --iters 30000 --warmup 300 --bufs 8
