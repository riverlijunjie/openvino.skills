...
REM
REM   *** INT4 group_size=64 variant ***  (body FC + MoE experts = INT4 g64).
REM   LM_Head = INT8 per-token / per-output-channel (group_size = K = 2048).
REM   Everything else identical to run_qwen3_6_ptl.bat (g128).
REM
REM Architecture = qwen3_5_moe + attention output gate (attn_output_gate=true):
REM   - full-attn QKV+gate fused FC width = 2*16*256 + 2*2*256 = 9216
REM   - MoE: H=2048 I=512 NE=256 TK=8 + shared_I=512 (always-on shared expert)
REM   - LM_Head: vocab=248320 (INT8 per-token); body FC INT4 g64; KV cache INT8
REM   - PA: NH=16 NKV=2 HD=256 (GQA=8)
REM   - GatedDeltaNet linear-attention bench (qk_heads=16, v_heads=32, K=V=128)
REM
REM DECODE-512 WINDOW: PA decode swept at KV = P, P+256, P+512 for prompt P in
REM {1024,2048,4096,8192}. Only PA grows with KV; FC/MoE/GDN/small are M=1 const.
REM ============================================================================
...
REM ---------- MoE fused (with shared expert), INT4 g64 (7th arg = group_size) ----------
...
REM ---------- fc_bench: full-attn QKV+gate (9216), O (4096->2048), LM_Head ----------
REM (4th arg = weight group_size; INT4 body = 64; LM_Head INT8 per-token = 2048)
...
REM ---------- Linear-attention input projection (in_proj_qkv 8192 + in_proj_z 4096 = 12288) ----------
...
REM qk_heads=16, v_heads=32 (GQA group=2), head_dim=128. cache_interval=0.
