...
def summarize_log(path: Path):
    ...
    # Determine expected iterations-per-bench using the MODE (most common call
    # count) across GPU kernels.  A kernel may launch more than once per
    # inference (e.g. moe_prefill's grouped_micro_gemm is invoked 3 times for
    # gate/up/down, while all other moe_* kernels run once), so taking the max
    # over-estimates iters and under-reports per-iter time.  Mode picks the
    # canonical "1 launch per iteration" count, and grouped/multi-launch
    # kernels get their full launches × per-launch ns counted in per_iter_ns.
    ...
    if counts:
        # mode of non-trivial counts; if tied, prefer the smaller value
        cnt = Counter(counts)
    ...
    for name, stat in gpu_kernels.items():
        # Skip kernels that ran fewer than `iters` times — they are one-off
        # init kernels (e.g. reorder_data for weight layout) and shouldn't be
        # counted per-iter. Kernels that ran MORE than `iters` (multi-launch
        # per iter) get their total proportionally counted.
        if stat["calls"] < iters:
            continue
        per_iter_ns = stat["total_ns"] / iters if iters > 0 else 0
