...
REM ============================================================================
REM Qwen3.6-35B-A3B roofline sweep on PTL 12Xe -- group_size=64 weight variant.
REM C:-drive paths (D: is full): build + logs live under river_roofline on C:.
REM Reuses the existing MSVC build (no rebuild: group_size is a runtime arg).
REM All compressed weights g64: INT4 body FC + MoE experts + INT8 LM_Head/Embed.
REM GatedDeltaNet head_dim=128 and PagedAttention KV lengths are NOT group sizes
REM and stay unchanged. Only fc_bench/moe_bench group_size arg is 128 -> 64.
REM ============================================================================
...
set LOGS=C:\Users\Local_Admin\river_roofline\roofline_results\qwen3_6\ptl_g64
...
REM ---------- gdn_bench: linear attention (head_dim=128 is NOT a group size) ----------
