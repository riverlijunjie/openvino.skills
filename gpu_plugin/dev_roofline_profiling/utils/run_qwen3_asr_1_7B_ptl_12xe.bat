...
set LOGS=D:\river\moe\roofline_results\qwen3_asr_1_7B\ptl_12xe
set PATH=%OV_BIN%;%TBB%;%PATH%

if not exist "%LOGS%" mkdir "%LOGS%"
echo === START %date% %time% > "%LOGS%\_index.txt"

REM ===================== Text decoder PA (GQA 16:8, HD=128, INT8 KV) ============
set PA_NH=16
set PA_NKV=8
set PA_HD=128

REM pa_bench positional args: <mode> <S_q> <S_kv> <iters> <warmup> <bufs> <kv_dtype:f16|i8> <impl:ocl|cm>
REM PA decode (S_q=1, S_kv = ctx). INT8 KV, ocl impl.
...
REM PA prefill (S_q=S, S_kv=0). INT8 KV, ocl impl.
...
"%CLI%" -d "%BUILD%\fc_bench.exe" 1 2048   4096 128 5000 200 8 f16 > "%LOGS%\fc_decode_qkv.log"     2>&1
"%CLI%" -d "%BUILD%\fc_bench.exe" 1 2048   2048 128 8000 200 8 f16 > "%LOGS%\fc_decode_o.log"       2>&1
"%CLI%" -d "%BUILD%\fc_bench.exe" 1 2048   6144 128 4000 150 8 f16 > "%LOGS%\fc_decode_gate.log"    2>&1
"%CLI%" -d "%BUILD%\fc_bench.exe" 1 2048   6144 128 4000 150 8 f16 > "%LOGS%\fc_decode_up.log"      2>&1
"%CLI%" -d "%BUILD%\fc_bench.exe" 1 6144   2048 128 4000 150 8 f16 > "%LOGS%\fc_decode_down.log"    2>&1
"%CLI%" -d "%BUILD%\fc_bench.exe" 1 2048 151936 128  200  20 4 f16 > "%LOGS%\fc_decode_lm_head.log" 2>&1
...
for %%S in (512 1024 4096 8192) do (
  ...
  "%CLI%" -d "%BUILD%\fc_bench.exe" %%S 2048   4096 128 !IT! 10 4 f16 > "%LOGS%\fc_prefill_qkv_S%%S.log"     2>&1
  "%CLI%" -d "%BUILD%\fc_bench.exe" %%S 2048   2048 128 !IT! 10 4 f16 > "%LOGS%\fc_prefill_o_S%%S.log"       2>&1
  "%CLI%" -d "%BUILD%\fc_bench.exe" %%S 2048   6144 128 !IT! 10 4 f16 > "%LOGS%\fc_prefill_gate_S%%S.log"    2>&1
  "%CLI%" -d "%BUILD%\fc_bench.exe" %%S 2048   6144 128 !IT! 10 4 f16 > "%LOGS%\fc_prefill_up_S%%S.log"      2>&1
  "%CLI%" -d "%BUILD%\fc_bench.exe" %%S 6144   2048 128 !IT! 10 4 f16 > "%LOGS%\fc_prefill_down_S%%S.log"    2>&1
  endlocal
)
"%CLI%" -d "%BUILD%\fc_bench.exe" 1 2048 151936 128 200 20 4 f16 > "%LOGS%\fc_prefill_lm_head.log" 2>&1

REM ===================== Text decoder small ops (decode, M=1) ===================
"%CLI%" -d "%BUILD%\small_ops_bench.exe" rmsnorm   1 2048    --iters 8000 --warmup 200 > "%LOGS%\small_decode_rmsnorm.log"      2>&1
...
"%CLI%" -d "%BUILD%\small_ops_bench.exe" add       1 2048    --iters 8000 --warmup 200 > "%LOGS%\small_decode_add.log"          2>&1
...
for %%S in (512 1024 4096 8192) do (
  ...
  setlocal EnableDelayedExpansion
  "%CLI%" -d "%BUILD%\small_ops_bench.exe" rmsnorm   %%S 2048    --iters !ITS! --warmup 30 > "%LOGS%\small_prefill_rmsnorm_S%%S.log"      2>&1
  ...
  "%CLI%" -d "%BUILD%\small_ops_bench.exe" add       %%S 2048    --iters !ITS! --warmup 30 > "%LOGS%\small_prefill_add_S%%S.log"          2>&1
  endlocal
)

REM ===================== Audio encoder FC (M=1500 fixed, FP16) ==================
REM Encoder runs once per inference. Numbers below feed the fixed-overhead column
REM in the SUMMARY. K=1024 is divisible by 128 so group_size=128 works naturally.
"%CLI%" -d "%BUILD%\fc_bench.exe" 1500 1024 3072 128 200 10 4 f16 > "%LOGS%\fc_enc_qkv_S1500.log" 2>&1
"%CLI%" -d "%BUILD%\fc_bench.exe" 1500 1024 1024 128 200 10 4 f16 > "%LOGS%\fc_enc_o_S1500.log"   2>&1
"%CLI%" -d "%BUILD%\fc_bench.exe" 1500 1024 4096 128 200 10 4 f16 > "%LOGS%\fc_enc_fc1_S1500.log" 2>&1
"%CLI%" -d "%BUILD%\fc_bench.exe" 1500 4096 1024 128 200 10 4 f16 > "%LOGS%\fc_enc_fc2_S1500.log" 2>&1
"%CLI%" -d "%BUILD%\fc_bench.exe" 1500 1024 2048 128 200 10 4 f16 > "%LOGS%\fc_enc_outproj_S1500.log" 2>&1
...
REM Encoder uses FP16 KV (no cache anyway) and NH=NKV=16 HD=64.
set PA_NH=16
set PA_NKV=16
set PA_HD=64
