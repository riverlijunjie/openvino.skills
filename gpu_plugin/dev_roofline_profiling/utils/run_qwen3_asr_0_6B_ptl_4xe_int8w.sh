#!/usr/bin/env bash
# ============================================================================
# Qwen3-ASR-0.6B roofline sweep on PTL 4Xe Linux  (INT8 weights / INT8 KV)
# Target: intel@10.239.152.140 — Intel PTL 4Xe iGPU, 32 EUs (4 Xe×8), 2450 MHz, 110 GB/s
#
# Text decoder (Qwen3, dense): hidden=1024, layers=28, GQA NH=16 NKV=8 HD=128,
# intermediate=3072, vocab=151936, tie_word_embeddings=true.
# Audio encoder (Whisper-style): d_model=896, layers=18, NH=14 HD=64, FFN=3584, S=1500.
#
# All FC/Embedding-style benches: precision=u8 (group size 128) to model INT8 weight compression.
# Text-decoder PA: INT8 KV cache (i8), opencl + micro_kernel.
# Audio encoder SDPA: FP16 (bidirectional, no KV cache).
# ============================================================================
...
LOGS=$HOME/river/roofline_results/qwen3_asr_0_6B/ptl_4xe_int8w
mkdir -p "$LOGS"
...
# ===================== Text decoder FC (decode, M=1, INT8 weights) =====================
# fc_bench: M K N gs iters warmup bufs precision flush_mb
run fc_decode_qkv      "$BUILD/fc_bench" 1 1024   4096 128 6000 200 8 u8
run fc_decode_o        "$BUILD/fc_bench" 1 2048   1024 128 8000 200 8 u8
run fc_decode_gate     "$BUILD/fc_bench" 1 1024   3072 128 6000 200 8 u8
run fc_decode_up       "$BUILD/fc_bench" 1 1024   3072 128 6000 200 8 u8
run fc_decode_down     "$BUILD/fc_bench" 1 3072   1024 128 6000 200 8 u8
run fc_decode_lm_head  "$BUILD/fc_bench" 1 1024 151936 128  300  20 4 u8

# ===================== Text decoder FC (prefill, M=S, INT8 weights) =====================
for S in 512 1024 4096 8192; do
  ...
  run fc_prefill_qkv_S$S    "$BUILD/fc_bench" $S 1024   4096 128 $IT 10 4 u8
  run fc_prefill_o_S$S      "$BUILD/fc_bench" $S 2048   1024 128 $IT 10 4 u8
  run fc_prefill_gate_S$S   "$BUILD/fc_bench" $S 1024   3072 128 $IT 10 4 u8
  run fc_prefill_up_S$S     "$BUILD/fc_bench" $S 1024   3072 128 $IT 10 4 u8
  run fc_prefill_down_S$S   "$BUILD/fc_bench" $S 3072   1024 128 $IT 10 4 u8
done
run fc_prefill_lm_head      "$BUILD/fc_bench" 1 1024 151936 128 300 20 4 u8
...
# ===================== Audio encoder FC (M=1500 fixed, INT8 weights) =====================
run fc_enc_qkv_S1500     "$BUILD/fc_bench" 1500  896 2688 128 150 10 4 u8
run fc_enc_o_S1500       "$BUILD/fc_bench" 1500  896  896 128 150 10 4 u8
run fc_enc_fc1_S1500     "$BUILD/fc_bench" 1500  896 3584 128 150 10 4 u8
run fc_enc_fc2_S1500     "$BUILD/fc_bench" 1500 3584  896 128 150 10 4 u8
run fc_enc_outproj_S1500 "$BUILD/fc_bench" 1500  896 1024 128 150 10 4 u8
