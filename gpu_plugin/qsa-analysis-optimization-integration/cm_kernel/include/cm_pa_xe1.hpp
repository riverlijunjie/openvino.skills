...
#        pragma unroll
            ...
            rQ[ri] = 0;
            auto gathered = cm_load<uint, VectorSize::N1, DataSize::U32, CacheHint::Cached, CacheHint::Cached>(q_gather, gather_offsets, gather_pred);
...
#        if KV_CACHE_COMPRESSION == 1
            uint32_t k_dscale_offset = cur_block_id * k_quan_blk_stride + CMPA_BLOCK_SZ * head_size * sizeof(uint8_t) + kv_pos % CMPA_BLOCK_SZ * sizeof(half);
...
#        endif
            uint32_t v_dscale_offset = cur_block_id * v_quan_blk_stride + CMPA_BLOCK_SZ * head_size * sizeof(uint8_t) + kv_pos % CMPA_BLOCK_SZ * sizeof(half);
...
#        endif
                ...
                for (int k = REG_K * wg_local_id; k < head_size; k += REG_K * (local_size / 2)) {
                    auto k_base =
                        reinterpret_cast<svmptr_t>((int8_t*)k_cache_base + cur_block_id * k_quan_blk_stride + (kv_pos % CMPA_BLOCK_SZ) * kv_pitch + k);
#        if KV_CACHE_COMPRESSION == 2
...
#        pragma unroll
                    for (int r = 0; r < REG_K; r++) {
                        uint elem_off_bytes = cur_block_id * v_quan_blk_stride + (kv_pos % CMPA_BLOCK_SZ) * kv_pitch + r * kv_pitch + k;
...
void pa_kernel_lsc_prefetch_f16(int wg_local_id,
                                int q_start,
                                int kv_stop,           //
...
#            pragma unroll
            for (int Vr = 0; Vr < REG_K; Vr++) {
                uint elem_offset = cur_block_id * blk_stride + (kv_pos % CMPA_BLOCK_SZ) * head_size + Vr * head_size + k;
...
#            pragma unroll
                for (int tile = 0; tile < VALUE_TILE_NUM; tile++) {
                    ...
                    rO_lo[ri + tile] =
                        cm_dpas<CM_PRECISION_HF, CM_PRECISION_HF, SystolicDepth, RepeatCount, float>(0, Vtile.format<int32_t>(), P_vec.format<int32_t>());
                }
            } else {
#            pragma unroll
                for (int tile = 0; tile < VALUE_TILE_NUM; tile++) {
...
#            pragma unroll
                for (int tile = 0; tile < VALUE_TILE_NUM; tile++) {
                    auto Vtile = Vmat.format<half, REG_K / 2, REG_N * 2 * VALUE_TILE_NUM>().select<REG_K / 2, 1, REG_N * 2, 1>(0, REG_N * 2 * tile);
                    rO_lo[ri + tile] = cm_dpas<CM_PRECISION_HF, CM_PRECISION_HF, SystolicDepth, RepeatCount>(rO_lo[ri + tile].format<float>(),
...
#            pragma unroll
                for (int tile = 0; tile < VALUE_TILE_NUM; tile++) {
                    auto Vtile = Vmat.format<half, REG_K / 2, REG_N * 2 * VALUE_TILE_NUM>().select<REG_K / 2, 1, REG_N * 2, 1>(0, REG_N * 2 * tile);
                    rO_hi[ri + tile] = cm_dpas<CM_PRECISION_HF, CM_PRECISION_HF, SystolicDepth, RepeatCount>(rO_hi[ri + tile].format<float>(),
                                                                                                             Vtile.format<int32_t>(),
                                                                                                             P_vec.format<int32_t>());
                }
            }
...
#            pragma unroll
                for (int tile = 0; tile < VALUE_TILE_NUM; tile++) {
                    auto Vtile = Vmat.format<half, REG_K / 2, REG_N * 2 * VALUE_TILE_NUM>().select<REG_K / 2, 1, REG_N * 2, 1>(0, REG_N * 2 * tile);
                    rO[ri + tile] = cm_dpas<CM_PRECISION_HF, CM_PRECISION_HF, SystolicDepth, RepeatCount>(rO[ri + tile].format<float>(),
                                                                                                          Vtile.format<int32_t>(),
                                                                                                          P_vec.format<int32_t>());
                }
            }
