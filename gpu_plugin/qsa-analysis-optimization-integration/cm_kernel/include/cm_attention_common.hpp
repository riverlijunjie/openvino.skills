...
#pragma unroll
        for (int k = 0; k < num_K; k++) {
            St2.row(k) =
                cm_dpas<CM_PRECISION_HF, CM_PRECISION_HF, SystolicDepth, RepeatCount, float>(St2.row(k), Qt[ri].format<int32_t>(), Kmat[k].format<int32_t>());
        }
    }
    return St;
}
...
#pragma unroll
        for (int p = 0; p < num_P_tiles; p++) {
            rO[ri + p] = cm_dpas<CM_PRECISION_HF, CM_PRECISION_HF, SystolicDepth, RepeatCount, float>(0, Vmat.format<int32_t>(), P2.row(p).format<int32_t>());
        }
    }
}

template <int num_P_tiles = REG_N / REG_M, int num_rO_tiles>
inline void ugemm_PV1(uint slm_V,
                      matrix_ref<half, REG_N, REG_K> P,
                      vector_ref<float, REG_N> max_comp,
                      matrix_ref<float, num_rO_tiles, REG_M * REG_N> rO,
                      uint slm_offset = 0) {
    constexpr int _head_size = num_rO_tiles * REG_N / num_P_tiles;
    auto P2 = P.format<half, num_P_tiles, REG_M * REG_K>();
#pragma unroll
...
template <typename T, int rows, int cols>
CM_INLINE vector<float, cols> online_softmax_update_tree(matrix_ref<T, rows, cols> St, vector_ref<T, cols> cur_max, vector_ref<T, cols> cur_sum) {
...
#pragma unroll
        for (int r = 0; r < rows / 2; r++)
            t.row(r) = cm_max<float>(St[r], St[r + rows / 2]);
#pragma unroll
...
#pragma unroll
        for (int r = 0; r < rows / 2; r++)
            t.row(r) = cm_add<float>(St[r], St[r + rows / 2]);
#pragma unroll
        for (int stride = rows / 4; stride > 0; stride >>= 1)
#pragma unroll
            for (int r = 0; r < stride; r++)
