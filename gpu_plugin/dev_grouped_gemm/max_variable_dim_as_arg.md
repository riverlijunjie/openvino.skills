...
This generates a much higher number of workers than necessary, since each expert need to processCollapse commentComment on line R17densamoilov commented on Apr 3, 2026 densamoilovon Apr 3, 2026ContributorMore actionsIs there a way to manage it without relying on a hint or are we likely to continue using the hint going forward?Reactmzhukova replied on Apr 3, 2026 mzhukovaon Apr 3, 2026ContributorAuthorMore actionsAFAIK eventually there is a plan to try out different strategies for launching workers, however all of it would come with a price for traversing the offsets array in one way or another, so we would need to carefully analyze where this would work
this one seem like a natural and quite cheap way to get the improvementReact👍React with 👍1densamoilovWrite a replyResolve comment
